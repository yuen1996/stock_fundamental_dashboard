...
_SUM_KEYS = {
    ...
    "incometax",
    # Banking – Cash Flow
    "cfo (cash flow from ops, rm)", "cfo (cash flow from ops, rm)",
...
_LAST_KEYS = {
    ...
    "gross loans", "deposits", "demand deposits", "savings deposits",
...
def _ttm_agg_for(category: str, key: str, label: str) -> str:
    """
    Decide how to aggregate last-4 quarters for a given field in this config.
    Returns one of: "sum", "mean", "last".
    """
    cat = (category or "").strip()
    k_norm = _norm_key(key)
    l_norm = (label or key or "").strip().lower()

    # 1) Explicit means win first
    if k_norm in _MEAN_KEYS or l_norm in _MEAN_KEYS:
        return "mean"

    # 2) Explicit sums win next
    if k_norm in _SUM_KEYS or l_norm in _SUM_KEYS:
        return "sum"

    # 3) Category defaults (strong)
    if cat in ("Income Statement", "Cash Flow"):
        # Per-share payouts (DPS/DPU/EPS typed as IS/Other) already captured in _SUM_KEYS.
        return "sum"
    if cat == "Balance Sheet":
        return "last"

    # 4) Heuristics: ratios/rates/percentages → last
    if _looks_like_ratio_or_rate(l_norm) or _looks_like_ratio_or_rate(k_norm):
        return "last"

    # 5) Fallback to explicit last list (levels, counts, prices)
    ...
    "Banking": {
        "Income Statement": [
            ...
            _f("Net Interest Income (incl. Islamic, RM)", "NII (incl Islamic)", unit="RM",
               help="Optional. If left blank, engine will compute: (Interest Income - Interest Expense) + Net Islamic Income"),
        ...
        "Other": [
            ...
            # Three-proportion (bank) optional raw overrides:
