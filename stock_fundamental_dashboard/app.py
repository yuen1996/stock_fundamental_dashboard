...
with st.container():
    ...
    if not res.get("ok"):
        ...
        if last_dt is None:
            st.markdown(
                ...
                <div class='card lg warn'>
                  ...
                  <div class='muted'>Total backups listed: {cnt}</div>
                </div>
                """,
                unsafe_allow_html=True,
            )
        ...
        # Optional link card to DO console
        if do_url:
            st.markdown(
                f"""
                <div class='card lg'>
        ...
        # 2) Recent backups table (last 5)
        backups = (res.get("raw") or {}).get("backups", [])
        if backups:
            backups_sorted = sorted(
                backups, key=lambda b: b.get("created_at", ""), reverse=True
            )[:5]
            ...
            st.markdown(
                ...
                <div class='card lg'>
                  ...
                  <div class='muted' style='margin-top:6px'>Showing up to the 5 most recent backups.</div>
                </div>
                """,
                unsafe_allow_html=True,
            )
