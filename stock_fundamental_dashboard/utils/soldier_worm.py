...
def _safe_period_label(value: object, *, placeholder: str = "Unknown FY") -> str:
    """Return a friendly string label for an annual period.

    Soldier Worm historically stored FY labels as ints; however, upstream data can
    contain blanks, strings, or other non-integer tokens.  This helper preserves the
    old behaviour when the value cleanly coerces to an int, but gracefully falls
    back to the original text (or a placeholder) when coercion fails.  The output is
    always a string so downstream filtering and display continue to work.
    """
    ...
    try:
        # Using float() first handles values like "2023.0" while still rejecting
        # labels such as "FY2023" (which fall through to the string fallback).
        coerced = int(float(raw_text))
                    ...
                    if paths:
                        ...
                        if miss:
                            rows.append({
                                "Category": cat,
                                "Metric": metric,
                                "Period": _safe_period_label(r.get("Year")),
...
def build_soldier_worm_ttm_kpis(
    ...
    *,
    ...
    for lab in (labels or []):
        ...
        canon = _canon_metric(lab, bucket) or lab
