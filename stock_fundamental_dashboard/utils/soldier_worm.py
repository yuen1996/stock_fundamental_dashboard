...
try:
    ...
    from utils.calculations import (
        RAW_ALIASES, Q_ALIASES, BUCKET_CALCS,
        _pick, _to_num, _safe_div, _avg2, _sum, _last, _ctx_price,
        _canon_metric, _lookup_calc,
        ttm_raw_row_from_quarters, enrich_three_fees_inplace, enrich_auto_raw_row_inplace,
        _infer_bucket_for_rows, cagr_from_summary, price_per_share_from_summary,
    )
except Exception:
...
def _canon_from_syn(label: str, bucket: str) -> Optional[str]:
    """
    Convert a UI/synonym label to its canonical summary label using the
    synonym index defined in config.summary_syn_index(bucket).
    Tolerant to (%) and (x)/(×) variations.
    """
    try:
        idx = config.summary_syn_index(bucket)  # provided in config.py
    except Exception:
        return None
...
def build_soldier_worm_report(
    ...
    *,
    ...
    if config is not None and hasattr(config, "INDUSTRY_SUMMARY_RATIOS_CATEGORIES"):
        ...
        include_extras = bool(getattr(config, "SOLDIER_WORM_INCLUDE_EXTRAS", True))
    ...
    # ---- requirements backfill: add any metric we know paths for ----
    try:
        exclude = set(getattr(config, "SOLDIER_WORM_EXCLUDE", []))
    except Exception:
        exclude = set()

    already = {m for _, m in ordered_metrics}
    for m in _requirements_catalog(bucket).keys():
        if m not in already and m not in exclude:
            ordered_metrics.append(("Other", m))
            already.add(m)
