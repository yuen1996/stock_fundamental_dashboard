...
def build_soldier_worm_report(
    ...
    *,
    ...
    base_cols = ["Category","Metric","Period","Missing Inputs"]
    for c in base_cols:
        if c not in out.columns:
            out[c] = "" if c != "Period" else None
    out = out[base_cols]
    if out.empty:
        return pd.DataFrame(columns=base_cols)

    # Optional extras (safe for UI use)
    def _count_missing(s):
        if s is None or (isinstance(s, float) and np.isnan(s)):
            return 0
        toks = [t.strip() for t in str(s).split(",") if t.strip()]
        return len(toks)

    out["Missing Count"] = out["Missing Inputs"].apply(_count_missing)
    out["Fix Tip"] = out["Missing Inputs"].apply(
        lambda s: (f"Fill these fields in Add/Edit / source data: {s}" if str(s).strip() else "")
    )

    # Stable ordering based on config-driven Category/Metric order + FY/TTM
    cat_order = {}
    met_order = {}
    for c, m in ordered_metrics:
    ...
    out["__p_order"] = out["Period"].apply(lambda p: int(p) if isinstance(p, (int, np.integer)) else 999999)

    out = out.sort_values(["__cat_order","__met_order","__p_order","Category","Metric"]).drop(
        columns=["__cat_order","__met_order","__p_order"]
    )

    # put base cols first
    cols = base_cols + [c for c in out.columns if c not in base_cols]
    out = out[cols]
