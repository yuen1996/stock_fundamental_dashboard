...
def _backfill_ttm_stocks_from_prev(raw_ttm: dict, prev_row: dict | None) -> None:
    """
    For TTM rows, if stock items are missing/zero (because the quarterly sheet lacks them),
    mirror the latest FY values so ratios remain computable until quarterlies exist.
    """
    if not raw_ttm or not prev_row:
        return

    def fill(label_key: str, canon_key: str):
        v = _to_num(raw_ttm.get(label_key))
        if v in (None, 0):
            pv = _pick(prev_row, canon_key)  # alias-aware from calculations.py
            if pv is not None:
                raw_ttm[label_key] = pv

    # Core equity / leverage / liquidity stocks
    fill("Shares", "shares")
    fill("Equity", "equity")
    fill("Total Borrowings", "borrowings")
    fill("Cash & Cash Equivalents", "cash")
    fill("Total Assets", "total_assets")
    fill("Current Assets", "current_assets")
    fill("Current Liabilities", "current_liabilities")

    # Useful extras by bucket (harmless if absent)
    fill("Orderbook", "orderbook")          # construction/industrial
    fill("Gross Loans", "gross_loans")      # banking
    fill("Deposits", "deposits")            # banking
    ...
    *,
    ...
    if include_ttm and not quarterly_df.empty:
        ...
        if raw_ttm:
            ...
            # NEW: backfill missing TTM stock items from latest FY
            _backfill_ttm_stocks_from_prev(raw_ttm, prev)
            ctx = {"price_fallback": price_fallback}
            for cat, metric in ordered_metrics:
                canon = _canon_metric(metric, bucket) or metric
                fn = _lookup_calc(calcs, canon) or _lookup_calc(calcs, metric)
...
def build_soldier_worm_cashflow_kpis(
    ...
    *,
    ...
    # NEW: if TTM basis, mirror stock items from latest FY for yield/ratios
    if basis == "TTM" and annual_df is not None and not annual_df.empty:
        prev = annual_df.dropna(subset=["Year"]).sort_values("Year").iloc[-1].to_dict()
        _backfill_ttm_stocks_from_prev(raw, prev)
    ...
    *,
    ...
    if include_ttm and not quarterly_df.empty:
        ...
        if raw_ttm:
            ...
            # NEW: mirror missing stock items in TTM from latest FY
            _backfill_ttm_stocks_from_prev(raw_ttm, prev_row)
            _eval_row(raw_ttm, prev_row, "TTM", rows_out)
...
def build_soldier_worm_ttm_kpis_trace(
    ...
    *,
    ...
    _backfill_ttm_stocks_from_prev(ttm, prev)
...
def build_soldier_worm_cashflow_kpis_trace(
    ...
    *,
    ...
    # NEW
    prev = (annual_df.dropna(subset=["Year"]).sort_values("Year").iloc[-1].to_dict()
            if (annual_df is not None and not annual_df.empty) else None)
    _backfill_ttm_stocks_from_prev(raw, prev)
