...
# (robust: support both utils.calculations and top-level calculations)
try:
...
def _canon_label(tok: str) -> str:
    mapping = {
        ...
        "revenue_prev": "Revenue (prev FY)",        # <-- add this
...
def _requirements_catalog(bucket: str) -> dict[str, list[list[str]]]:
    ...
    common = {
        ...
        "Revenue YoY (%)": [["revenue", "revenue_prev"]],
        "Sales YoY (%)":   [["revenue", "revenue_prev"]],  # alias
...
def build_soldier_worm_report(
    ...
    *,
    ...
    if not annual_df.empty and "Year" in annual_df.columns:
        # keep only rows with a real FY
        A = annual_df.copy()
        A["Year"] = pd.to_numeric(A["Year"], errors="coerce")
        A = A[A["Year"].notna()]
        A["Year"] = A["Year"].astype(int)
        A = A.drop_duplicates(subset=["Year"], keep="last").sort_values("Year")
...
def build_soldier_worm_calc_trace(
    ...
    *,
    ...
    # Normalize annuals: coerce Year → numeric, drop blanks, dedupe, sort
    if not annual_df.empty and "Year" in annual_df.columns:
        A = annual_df.copy()
        A["Year"] = pd.to_numeric(A["Year"], errors="coerce")
        A = A[A["Year"].notna()]
    ...
    for _, r in A.iterrows():
        cur = r.to_dict()
        lab = str(int(r["Year"]))      # always a real FY now
    ...
    if include_ttm and not quarterly_df.empty:
        ...
        if raw_ttm:
            enrich_three_fees_inplace(raw_ttm)
            _eval_row(raw_ttm, (A.iloc[-1].to_dict() if not A.empty else None), "TTM", rows_out)
...
def build_soldier_worm_ttm_kpis_trace(
    ...
    *,
    ...
    # Clean annuals same way so prev FY is a real year
    if annual_df is not None and not annual_df.empty and "Year" in annual_df.columns:
        A = annual_df.copy()
        A["Year"] = pd.to_numeric(A["Year"], errors="coerce")
        A = A[A["Year"].notna()]
        A["Year"] = A["Year"].astype(int)
    ...
    prev = A.iloc[-1].to_dict() if not A.empty else None
    ...
    for lbl in (labels or []):
        ...
        if _num(val) is None:
            ...
            # --- EXTRA explicit formulas ----------------------------------
                ...
                # Revenue YoY (%) = ((Revenue_TTM / Revenue_prevFY) - 1) × 100
                rev_ttm = _num(ttm.get("Revenue") or ttm.get("Total Revenue") or ttm.get("TotalRevenue") or ttm.get("Sales"))
                # prev FY Revenue from the last annual row
                rev_prev = None
                if isinstance(A, pd.DataFrame) and not A.empty:
                    prev_row = A.iloc[-1].to_dict()
                    rev_prev = _num(prev_row.get("Revenue") or prev_row.get("Total Revenue")
                                    or prev_row.get("TotalRevenue") or prev_row.get("Sales"))

                val, note = None, ""
                if rev_ttm is not None and rev_prev not in (None, 0):
                    val = ((float(rev_ttm) / float(rev_prev)) - 1.0) * 100.0
                elif rev_prev == 0:
                    note = "Not meaningful (prev Revenue = 0)"
                else:
                    note = "Missing Revenue_TTM or prev FY Revenue"
                ...
                rows.append({
                    ...
                    "Path Used": "revenue_ttm | revenue_prevFY",
                    "Formula": "(Revenue_TTM / Revenue_prevFY) − 1 × 100",
                    "Inputs":  _fmt_inputs({"Revenue_TTM": rev_ttm, "Revenue_prevFY": rev_prev}),
                    "Flags":   "",
                    "Note":    note,
                })
                continue
...
# ========================= Soldier Worm vNext: "Fix list" + Form-field mapping =========================

def _sw_norm(s: str) -> str:
    """Normalize strings for fuzzy matching (keys/labels)."""
    s = (s or "").strip().lower()
    # kill common punctuation/units
    for ch in ["(", ")", "[", "]", "{", "}", ",", ".", ":", ";", "/", "\\", "%", "×", "x"]:
        s = s.replace(ch, " ")
    s = " ".join(s.split())
    return s

def _build_form_field_index(bucket: str) -> dict[str, list[tuple[str, str, str]]]:
    """
    Build index: norm_token -> [(section, label, key), ...]
    Uses config.INDUSTRY_FORM_CATEGORIES when available.
    """
    idx: dict[str, list[tuple[str, str, str]]] = {}
    if config is None or not hasattr(config, "INDUSTRY_FORM_CATEGORIES"):
        return idx

    cats = (config.INDUSTRY_FORM_CATEGORIES.get(bucket)
            or config.INDUSTRY_FORM_CATEGORIES.get("General")
            or {})
    # cats: {section: [field_dict,...]}
    for section, fields in (cats or {}).items():
        for f in (fields or []):
            try:
            ...
            if not key and not label:
                continue
            tup = (section, label, key)
            for token in filter(None, {_sw_norm(label), _sw_norm(key)}):
                idx.setdefault(token, []).append(tup)
...
def _locate_form_fields_for_missing_label(label: str, bucket: str) -> list[tuple[str, str, str]]:
    """
    Try to map a missing label (e.g. 'Revenue', 'Total Assets') to form fields.
    Returns list of (section, label, key).
    """
    base = (label or "").split("—")[0].strip()  # strip TTM hints
    if not base:
        return []
    idx = _build_form_field_index(bucket)
    norm = _sw_norm(base)
    ...
    # small heuristics (aliases / abbreviations)
    heur = {
        "cash and cash equivalents": ["cash equivalents", "cash & equivalents", "cash & cash equivalents"],
        "total assets": ["assets"],
        "total liabilities": ["liabilities"],
        "equity": ["equity book value", "equity (book value)"],
        "cost of sales": ["cogs", "costofsales"],
        "gross profit": ["grossprofit"],
        "operating profit": ["operatingprofit", "operating income"],
        "net profit": ["npat", "profit after tax"],
        "depreciation and amortization": ["depamort", "depreciation amortization"],
        "interest expense": ["finance costs", "borrowing costs"],
        "accounts receivable": ["receivables"],
        "accounts payable": ["payables"],
        "total borrowings": ["borrowings", "debt"],
        "shares": ["shares outstanding", "number of shares"],
        "price": ["annual price per share"],
        "dps": ["dividend per share"],
        # Plantation specific
        "cpo asp": ["average cpo selling price"],
        "oer": ["average oil extraction rate", "oil extraction rate"],
        "ffb input": ["ffb processed"],
        "ffb harvested": ["ffb yield", "fresh fruit bunches harvested"],
        "tonnes sold": ["ffb sales volume", "sales volume"],
    }
    for canon, variants in heur.items():
        if norm == _sw_norm(canon) or norm in {_sw_norm(v) for v in variants}:
            for cand in [canon] + variants:
                n2 = _sw_norm(cand)
                if n2 in idx:
                    return idx[n2]
...
def build_soldier_worm_fixlist(
    ...
    *,
    ...
    """
    Turn the Missing-Inputs table into an actionable "what to fill first" list.

    Output columns:
      Input | Where to Fill | Form Keys | Blocked Metrics | Periods | Count
    """
    if warn_df is None or warn_df.empty:
        return pd.DataFrame(columns=["Input","Where to Fill","Form Keys","Blocked Metrics","Periods","Count"])

    records: list[dict] = []
    for _, r in warn_df.iterrows():
        metric  = str(r.get("Metric","")).strip()
        period  = r.get("Period","")
        mis_str = str(r.get("Missing Inputs","") or "")
        if not mis_str.strip():
            continue
        parts = [p.strip() for p in mis_str.split(",") if p.strip()]
        for p in parts:
            base = p.split("—")[0].strip()
            hint = ("—" + p.split("—", 1)[1].strip()) if "—" in p else ""
            records.append({
                "Input": base,
                "Hint": hint,
                "Metric": metric,
    ...
    grouped = []
    for inp, g in df.groupby("Input", dropna=True):
        metrics = list(dict.fromkeys(g["Metric"].tolist()))
        periods = list(dict.fromkeys(g["Period"].tolist()))
        cnt = int(len(g))

        hint_any = " ".join([str(x) for x in g["Hint"].dropna().unique().tolist()]).strip().lower()
        ttm_hint = ("last 4 quarters" in hint_any) or ("need latest quarter" in hint_any)

        fields = _locate_form_fields_for_missing_label(str(inp), bucket)
        if fields:
            where_chunks = [sec + (" (Quarterly/TTM needs quarters)" if ttm_hint else "") for sec, _, _ in fields]
            key_chunks   = [key for _, _, key in fields]
        else:
            where_chunks = ["Not in form (maybe alias / add field)"]
            key_chunks   = ["—"]

        metric_str = "; ".join(metrics[:8]) + (f" … +{len(metrics)-8}" if len(metrics) > 8 else "")
        period_str = ", ".join(periods[:10]) + (f" … +{len(periods)-10}" if len(periods) > 10 else "")

        grouped.append({
            "Input": inp,
            "Where to Fill": ", ".join(dict.fromkeys(where_chunks)),
            "Form Keys": ", ".join(dict.fromkeys(key_chunks)),
            "Blocked Metrics": metric_str,
            "Periods": period_str,
            "Count": cnt,
        })

    out = pd.DataFrame(grouped).sort_values(["Count","Input"], ascending=[False, True]).head(int(top_n))
    return out.reset_index(drop=True)

def decorate_soldier_worm_missing_table(
    warn_df: pd.DataFrame,
    *,
    bucket: str,
) -> pd.DataFrame:
    """
    Add 2 UX columns to the Missing-Inputs table:
      - Where to Fill
      - Form Keys

    Keeps original columns untouched, returns a new DataFrame.
    """
    if warn_df is None or warn_df.empty:
        return warn_df

    def _map_one(mis_str: str) -> tuple[str, str]:
        if not mis_str:
            return "", ""
        parts = [p.strip() for p in str(mis_str).split(",") if p.strip()]
        where_all = []
        keys_all = []
        for p in parts:
            base = p.split("—")[0].strip()
            fields = _locate_form_fields_for_missing_label(base, bucket)
            if fields:
                for sec, _, key in fields:
                    where_all.append(sec)
                    keys_all.append(key)
        where = ", ".join(dict.fromkeys(where_all)) if where_all else "—"
        keys  = ", ".join(dict.fromkeys(keys_all)) if keys_all else "—"
        return where, keys

    out = warn_df.copy()
    mapped = out["Missing Inputs"].apply(_map_one)
    out["Where to Fill"] = [x[0] for x in mapped]
    out["Form Keys"] = [x[1] for x in mapped]

    # insert after "Missing Inputs"
    cols = out.columns.tolist()
    for c in ["Where to Fill", "Form Keys"]:
        if c in cols:
            cols.remove(c)
    try:
        mi_idx = cols.index("Missing Inputs") + 1
    except ValueError:
        mi_idx = len(cols)
    cols = cols[:mi_idx] + ["Where to Fill","Form Keys"] + cols[mi_idx:]
    return out[cols]
