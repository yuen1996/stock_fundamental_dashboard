...
# --- Missing-value helper: None or NaN are missing; 0 is NOT missing ---
def _is_missing(x) -> bool:
    # pd.isna(None) and pd.isna(np.nan) are True; pd.isna(0) is False
    try:
...
def _requirements_catalog(bucket: str) -> dict[str, list[list[str]]]:
    ...
    common = {
        ...
        "Three Fees Ratio (%)":        [["revenue","selling_expenses","admin_expenses","interest_expense"]],
...
def build_soldier_worm_report(
    ...
    *,
    ...
    # --- Ensure TTM strip metrics are scanned even if not in config ---
    ttm_strip = [
        "Revenue", "Gross Profit", "Operating Profit", "EBITDA", "Net Profit",
        "EPS", "DPS", "Order Backlog", "Backlog Coverage (×)", "Capacity Utilization",
    ]
    if bucket == "Banking":
        ttm_strip.extend([
            ...
            "LCR (%)", "NSFR (%)", "Loan-to-Deposit Ratio (%)", "Loan-Loss Coverage (×)",
            "CASA Ratio (%)", "CASA (Core, %)"
        ])

    _seen = {m for _, m in ordered_metrics}
    for _m in ttm_strip:
        if _m not in _seen:
            ordered_metrics.append(("TTM Strip", _m))
            _seen.add(_m)        
    ...
    if not annual_df.empty and "Year" in annual_df.columns:
        ...
        for idx, (_, r) in enumerate(A.iterrows()):
            ...
            for cat, metric in ordered_metrics:
                ...
                # We have a calculator; run it and treat NaN like None (0 is OK)
                try:
                    val = fn(row, prev, ctx)
                except Exception:
                    val = None
