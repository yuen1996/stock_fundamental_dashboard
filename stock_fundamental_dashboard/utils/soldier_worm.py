...
def build_soldier_worm_cagr_kpis(
    ...
    *,
    ...
    for lab in (labels or []):
        ...
        # --- NEW: TTM end overrides to mirror View Stock -----------------
        ttm_end = None
        if end_basis == "TTM":
            # flows / EPS-like bases from TTM raw
            if base in ("Revenue", "EBITDA", "Net Profit", "CFO", "Orderbook"):
                ttm_end = _to_num(raw_ttm.get(base)) if raw_ttm else None
                # keep the explicit CFO override if we already computed it
                if base == "CFO" and ttm_cfo is not None:
                    ttm_end = ttm_cfo

            # banking balances — prefer latest quarter (stock), else last FY
