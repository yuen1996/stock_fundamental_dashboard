    ...
    *,
    ...
    for lbl in (labels or []):
        ...
        if fn is not None:
            ...
            # NEW: even if there is no calc fn (or val is still None), try a path-only passthrough
            if _num(val) is None and p:
                path, inputs = _select_satisfied_path(ttm, prev, {"price_fallback": price_fallback}, p)
                if path:
                    # if it’s a simple passthrough (single token), take the value from inputs
                    if len(path) == 1:
                        k = path[0]
                        val = _num((inputs.get(k) or (None, ""))[0])
                        formula = f"{k} (ttm raw)"
                    # still keep `path` / `inputs` so the trace shows something useful
