...
def _requirements_catalog(bucket: str) -> dict[str, list[list[str]]]:
    ...
    elif bucket == "Plantation":
        base = {**common, **{
            "Gearing (Debt/Assets, %)":     [["borrowings","total_assets"]],
            "Unit Cash Cost (RM/t)":       [["unit_cash_cost"]],
            "CPO ASP (RM/t)":              [["cpo_asp"]],
            "OER (CPO/FFB Input, %)":      [["oer_pct"], ["cpo_output","ffb_input"]],
            "Yield per Hectare (FFB t/ha)":[["yield_per_hectare"], ["ffb_harvested","mature_area"], ["ffb_harvested","planted_hectares"]],
            "Cash Margin per ton (RM/t)":  [["cash_margin_per_ton"], ["cpo_asp","unit_cash_cost"]],
            "EBITDA per ton (RM/t)":       [["ebitda_per_ton"], ["ebitda","tonnes_sold"]],
        }}
