...
@supports (-webkit-touch-callout: none) and (not (hover: hover)) {
  ...
  /* stop iOS auto text zoom; keep sizing consistent */
  html { -webkit-text-size-adjust: 100%; }

  /* prevent ugly overflows/wrapping */
  body { word-break: break-word; overflow-wrap: anywhere; }

  /* keep content inside viewport with a bit of side padding */
  .container, .page, .wrap, .content {
    max-width: 100vw;
    padding-left: 12px;
    padding-right: 12px;
  }

  /* resilient cards (less reflow jank) */
  .card, .panel { contain: layout paint; }

  /* responsive grids that don’t spill on iPad widths */
  .grid, .cards, .tile-grid {
    display: grid !important;
    grid-template-columns: repeat(auto-fit, minmax(280px, 1fr));
    gap: 12px;
  }

  /* tables: fixed layout + ellipsis to avoid exploding columns */
  ...
  /* slightly larger small-text sizes to avoid iOS zoom quirks */
