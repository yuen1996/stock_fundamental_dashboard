...
def render_page_title(page_name: str) -> None:
    """Render a premium hero header for every page."""
    from datetime import datetime
    now = datetime.now().strftime("%Y-%m-%d %H:%M")
    st.markdown(
        f"""<div class='hero'>
  <div class='left'>
    <div class='brand'>📊 Fundamentals Dashboard</div>
    <div class='title'>{page_name}</div>
    <div class='subtitle'>Clean, consistent layout • fast signals • audit-friendly outputs</div>
  </div>
  <div class='right'>
...
html, body, [data-testid="stAppViewContainer"] *{ font-family: Inter, system-ui, -apple-system, Segoe UI, Roboto, Helvetica, Arial, "Apple Color Emoji","Segoe UI Emoji"; letter-spacing:.1px; color:var(--text); }
/* --- Restore Streamlit icon fonts (prevents icon names like 'keyboard_arrow_down' showing as text) --- */
...
[data-testid="stIcon"] span{
  font-family: "Material Symbols Rounded","Material Symbols Outlined","Material Symbols Sharp",
               "Material Icons","Material Icons Outlined","Material Icons Round","Material Icons Sharp" !important;
...
/* Sidebar controls: premium dark inputs (avoid washed-out white controls on dark sidebar) */
...
[data-testid="stSidebar"] .stSlider [data-testid="stTickBar"]{ opacity:.55; }
[data-testid="stSidebar"] .stSlider [role="slider"]{ background: rgba(255,255,255,.18) !important; }
[data-testid="stSidebar"] hr{ border-color: rgba(255,255,255,.12) !important; }



/* Sidebar expanders: soft panels */
...
/* Hero header */
.hero{
  background: radial-gradient(120% 120% at 100% 0%, rgba(79,70,229,.18) 0%, rgba(79,70,229,0) 55%),
...
.hero .right{ display:flex; gap:.5rem; flex-wrap:wrap; align-items:center; justify-content:flex-end; }
.hero .chip{
...
/* Premium containers for charts + tables */
...
div[data-testid="stDataFrame"]{ padding: .6rem !important; }
...
/* Sidebar reopen control (when sidebar collapsed) */
