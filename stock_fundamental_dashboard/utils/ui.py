...
def render_soldier_worm_diagnostics(warn_df, trace_df, *, key_prefix: str = "sw"):
    ...
    - warn_df: output of calculations.build_soldier_worm_report(...)
    - trace_df: output of calculations.build_soldier_worm_calc_trace(...)

    UI-only (no calculation changes).
    """
    import pandas as pd
    ...
    def _h(n, row_h=28, base=140, max_h=620):
        try:
            ...
            n = 0
        return min(max_h, base + row_h * max(1, min(n, 18)))
    ...
    with tab1:
        if warn_df.empty:
            ...
            st.dataframe(
                ...
                height=_h(len(focus), row_h=26, base=120, max_h=520),
            )
    ...
    with tab2:
        if warn_df.empty:
            ...
            w = warn_df.copy()
            if "Missing Count" in w.columns:
                try:
                    w = w.sort_values(["Missing Count", "Category", "Metric", "Period"], ascending=[False, True, True, True])
                except Exception:
                    pass
            show_cols = [c for c in ["Category", "Metric", "Period", "Missing Count", "Missing Inputs"] if c in w.columns]
            st.dataframe(
                w[show_cols],
                use_container_width=True,
                height=_h(len(w), row_h=26, base=160, max_h=620),
            )

    with tab3:
        if trace_df.empty:
            st.info("No trace rows to display.")
        else:
            t = trace_df.copy()
            prefer = [c for c in ["Category", "Metric", "Period", "Value", "Formula", "Inputs"] if c in t.columns]
            rest = [c for c in t.columns if c not in prefer]
            t = t[prefer + rest]
            st.dataframe(
                t,
                use_container_width=True,
                height=_h(len(t), row_h=24, base=180, max_h=620),
            )
