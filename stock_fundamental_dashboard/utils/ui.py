...
def render_page_title(page_name: str) -> None:
    """Render a premium hero header for every page (shared look)."""
    from html import escape
    from datetime import datetime
    now = datetime.now().strftime("%Y-%m-%d %H:%M")
    title = escape(str(page_name))
    st.markdown(
        f"""
        <div class='hero'>
          <div class='row'>
            <div>
              <div class='kicker'>Fundamentals Dashboard</div>
              <div class='title'>{title}</div>
              <div class='sub'>Research workspace • Updated {now}</div>
            </div>
            <div class='meta'>
              <span class='pill'><span class='dot'></span>Live</span>
              <span class='pill'>Malaysia (MY)</span>
            </div>
          </div>
        </div>
        """,
...
/* ---------------- Premium App Shell ---------------- */
.block-container{ padding-top: 1.05rem !important; padding-bottom: 2.2rem !important; max-width: 1320px; }
@media (max-width: 1200px){ .block-container{ max-width: 1040px; } }
@media (max-width: 900px){ .block-container{ max-width: 100%; padding-left: 1.15rem !important; padding-right: 1.15rem !important; } }
...
/* Premium hero header */
.hero{
  background: linear-gradient(135deg, rgba(79,70,229,.10) 0%, rgba(14,165,233,.08) 45%, rgba(16,185,129,.08) 100%);
...
.hero .row{ display:flex; justify-content:space-between; gap: 14px; flex-wrap:wrap; align-items:flex-start; }
.hero .kicker{ font-weight: 800; font-size: .78rem; letter-spacing: .12em; text-transform: uppercase; color: rgba(71,85,105,.9); }
.hero .title{
  font-weight: 950; font-size: 1.65rem; line-height: 1.1; margin-top: .2rem;
  background: linear-gradient(90deg, #0f172a 0%, #312e81 55%, #0f766e 100%);
  -webkit-background-clip: text; background-clip: text; color: transparent;
}
.hero .sub{ color: var(--muted); font-size: .95rem; margin-top: .35rem; }
.hero .meta{ display:flex; gap: 8px; align-items:center; flex-wrap:wrap; justify-content:flex-end; }
.pill{
  display:inline-flex; align-items:center; gap:.4rem;
  padding: .22rem .55rem; border-radius: 999px;
  background: rgba(255,255,255,.70); border: 1px solid rgba(15,23,42,.12);
  color: rgba(15,23,42,.85); font-weight: 800; font-size: .78rem;
  box-shadow: 0 8px 18px rgba(15,23,42,.08);
}
.pill .dot{ width:7px; height:7px; border-radius:999px; background: var(--primary); display:inline-block; }
...
.stButton > button:hover{ transform: translateY(-1px); box-shadow: 0 14px 26px rgba(15,23,42,.10) !important; }

/* Sidebar collapsed reopen control (always visible) */
