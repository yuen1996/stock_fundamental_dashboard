...
def build_summary_table(
    ...
    *,
    ...
    if include_ttm and not quarterly_df.empty:
        ...
        # --- Prefer capital ratios from the latest annual (synthetic TTM) row when provided ---
        # This fixes cases where CET1/Tier1 don’t have quarterly paths and you’ve appended a TTM row
        # (e.g., Year=2025 in your annual raw). We explicitly prefer those values for TTM.
        if prev_row:
            ...
            # CET1
            v = _first_present(prev_row, "CET1 Ratio (%)", "CET1 Ratio")
            if v is not None:
                raw_ttm["CET1 Ratio (%)"] = v  # calculators pick this first

            # Tier 1
            v = _first_present(prev_row, "Tier 1 Capital Ratio (%)", "Tier 1 Ratio (%)", "Tier 1 Ratio")
            if v is not None:
                raw_ttm["Tier 1 Capital Ratio (%)"] = v  # calculators pick this first
        ...
        if prev_row:
            ...
            for k in [
                ...
                # capital ratios – include both plain and (%) variants
                "CET1 Ratio", "CET1 Ratio (%)",
                "Tier 1 Ratio", "Tier 1 Ratio (%)", "Tier 1 Capital Ratio (%)",
