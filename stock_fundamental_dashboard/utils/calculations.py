...
def compute_ttm(
    ...
    *,
    price_override: Optional[float] = None,  # NEW: force the "current price" (momentum/manual)
) -> dict:
    """
    ...
    ----------
    df : pd.DataFrame
        Mixed annual + quarterly rows for all names.
    name : str
        Target name to compute.
    bucket : Optional[str]
        Industry bucket to drive derivations (fallback inferred).
    price_override : Optional[float]
        If provided, this price is used to build/compute TTM (matches View page choice).
    """
    if df is None or df.empty:
        return {}
    ...
    # ---- compute raw TTM with a price hint that respects override ----
    price_hint = (
        _to_num(price_override) if price_override is not None
        else (_last(a.get("Price")) if isinstance(a, pd.DataFrame) and "Price" in a.columns else None)
    )
    raw_ttm = ttm_raw_row_from_quarters(q, current_price=price_hint)
    if not raw_ttm:
        return {}

    # ---- fill derivatives (industry-aware) ----
    enrich_three_fees_inplace(raw_ttm)
    enrich_auto_raw_row_inplace(
        raw_ttm,
        bucket=bucket or _infer_bucket_for_rows(a) or "General"
    )
    ...
    # Price: prefer the TTM row (already seeded with override) then explicit override
    px  = _to_num(raw_ttm.get("Price")) or _to_num(price_override)
...
def _ctx_price(row: dict, ctx: dict) -> Optional[float]:
    """
    Single source of truth for the price used in all valuation calcs.
    Order of precedence:
      1) ctx["price_override"]  → the View page’s chosen current price (momentum/manual)
      2) row["Price"]           → if the row already carries a Price
      3) ctx["price_fallback"]  → last-known fallback if provided
    """
    if ctx:
        ovr = _to_num(ctx.get("price_override"))
        if ovr is not None:
            return ovr
    px = _to_num(row.get("Price"))
    if px is not None:
        return px
    return _to_num(ctx.get("price_fallback") if ctx else None)
