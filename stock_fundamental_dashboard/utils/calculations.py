...
def compute_ttm(stock_df: pd.DataFrame, current_price: float | None = None) -> dict:
    ...
    # Absolute fallback: scan all Shares aliases and use the first with data
    if (not valid(eps_ttm) or float(eps_ttm) == 0.0) and valid(ttm_np):
        for name in TTM_ALIASES["SharesOutstanding"]:
            if name in stock_df.columns:
                ss = _to_num(stock_df[name]).dropna()
                if not ss.empty:
                    shares_avg = float(ss.tail(4).mean()) if len(ss) >= 2 else float(ss.iloc[-1])
                    if shares_avg and shares_avg > 0:
                        eps_ttm = float(ttm_np) / shares_avg
                        break
