...
def compute_factor_scores(stock_name, stock_df, ttm,
    ...
    # ── GROWTH ───────────────────────────────────────────────
    def _pos_rate(col):
        if col not in stock_df.columns: return None
        s = pd.to_numeric(stock_df[col],errors="coerce")
        pos = sum((s-s.shift(4))>0); tot = s.count()-4
        return pos/tot if tot>0 else None

    r_pos = _pos_rate("Q_Revenue") or _pos_rate("Revenue")
    e_pos = _pos_rate("Q_EPS")     or _pos_rate("EPS")

    g1 = _score_linear(r_pos,0.4,0.9)
    g2 = _score_linear(e_pos,0.4,0.9)

    _push(details,"Growth","Rev YoY %",r_pos,g1,
          "Quarterly Q_Revenue vs q-4",{})
    _push(details,"Growth","EPS YoY %",e_pos,g2,
          "Quarterly Q_EPS vs q-4",{})

    parts=[p for p in (g1,g2) if p is not None]
    scores["Growth"]=int(sum(parts)/len(parts)) if parts else 0

    # ── CASH ────────────────────────────────────────────────
    nd_ebitda = ttm.get("Net Cash (Debt)")*-1 / (ttm.get("TTM EBITDA") or 1)
    icov      = ttm.get("Interest Coverage (EBITDA/Fin)")
    debt_fcf  = ttm.get("Debt / FCF (yrs)")

    c1=_score_linear(nd_ebitda,4,0,True)
    c2=_score_linear(min(icov,20) if icov else None,2,12)
    c3=_score_linear(debt_fcf,8,0,True)

    _push(details,"Cash","ND/EBITDA",nd_ebitda,c1,"(Debt-Cash) ÷ EBITDA_TTM",{})
    _push(details,"Cash","Coverage",icov,c2,"EBITDA_TTM ÷ FinanceCost_TTM",{})
    _push(details,"Cash","Debt/FCF",debt_fcf,c3,"(Debt-Cash) ÷ FCF_TTM",{})

    parts=[c for c in (c1,c2,c3) if c is not None]
    scores["Cash"]=int(sum(parts)/len(parts)) if len(parts)>=2 else int(parts[0]*0.7) if parts else 0
