    ...
    *,
    ...
    if include_ttm and not quarterly_df.empty:
        ...
        if prev_row:
            ...
            # --- ensure NIM (%) shows up in TTM if quarters don't provide it ---
            nim_prev = _pick(prev_row, "nim_pct") if prev_row else None
            if raw_ttm is not None and nim_prev is not None:
                # write to aliases your picker understands
                raw_ttm["NIM"] = nim_prev           # covers "NIM"
                raw_ttm["NIM (%)"] = nim_prev       # covers explicit percent header        
