...
# ---------- Summary precedence wrapper (all industries) ----------
...
def _div_safe_pct(num, den):
    v = _safe_div(num, den)
    return (v * 100.0) if v is not None else None

def _bank_overlay_vals(row: pd.Series) -> dict:
    """
    Compute Banking metrics from TP_Bank_* Num/Den + raw parts if available.
    Uses your existing _to_num/_safe_div helpers.
    """
    g = lambda k: _to_num(row.get(k))
    vals = {}

    # NIM (%): as-reported "NIM" -> TP -> core (NII / Avg EA)
    nim_as_reported = _to_num(row.get("NIM"))
    nim_from_tp     = _div_safe_pct(g("TP_Bank_NIM_Num"), g("TP_Bank_NIM_Den"))
    nim_from_core   = _div_safe_pct(g("NII (incl Islamic)"), g("Average Earning Assets"))
    vals["NIM (%)"] = _first_not_none(nim_as_reported, nim_from_tp, nim_from_core)

    # Cost-to-Income Ratio
    cir_from_tp   = _div_safe_pct(g("TP_Bank_CIR_Num"), g("TP_Bank_CIR_Den"))
    cir_from_core = _div_safe_pct(g("Operating Expenses"), g("Operating Income"))
    vals["Cost-to-Income Ratio (%)"] = _first_not_none(cir_from_tp, cir_from_core)

    # Financial Leverage
    lev_from_tp   = _safe_div(g("TP_Bank_Lev_Num"), g("TP_Bank_Lev_Den"))
    lev_from_core = _safe_div(g("Total Assets"), g("Equity"))
    vals["Financial Leverage (Assets/Equity)"] = _first_not_none(lev_from_tp, lev_from_core)

    # CASA & CASA (Core)
    casa_as_reported = _to_num(row.get("CASA Ratio"))
    casa_parts = _div_safe_pct((g("Demand Deposits") or 0) + (g("Savings Deposits") or 0), g("Deposits"))
    vals["CASA Ratio (%)"] = _first_not_none(casa_as_reported, casa_parts)

    casa_core_as_reported = _to_num(row.get("CASA (Core, %)"))
    core_den = (g("Demand Deposits") or 0) + (g("Savings Deposits") or 0) + (g("Fixed/Time Deposits") or 0)
    casa_core_parts = _div_safe_pct((g("Demand Deposits") or 0) + (g("Savings Deposits") or 0), core_den)
    vals["CASA (Core, %)"] = _first_not_none(casa_core_as_reported, casa_core_parts)

    # LDR, NPL, Coverage
    ldr_as_reported = _to_num(row.get("Loan-to-Deposit Ratio"))
    ldr_from_parts  = _div_safe_pct(g("Gross Loans"), g("Deposits"))
    vals["Loan-to-Deposit Ratio (%)"] = _first_not_none(ldr_as_reported, ldr_from_parts)

    vals["NPL Ratio (%)"]          = _div_safe_pct(g("Non-Performing Loans"), g("Gross Loans"))
    vals["Loan-Loss Coverage (×)"] = _safe_div(g("Loan Loss Reserve"), g("Non-Performing Loans"))

    # Valuation/returns if parts exist
    eps = _safe_div(g("Net Profit"), g("Shares"))
    pb  = _safe_div(g("Price"), _safe_div(g("Equity"), g("Shares")))
    pe  = _safe_div(g("Price"), eps)
    dy  = _div_safe_pct(g("DPS"), g("Price"))
    roe = _div_safe_pct(g("Net Profit"), g("Equity"))
    roa = _div_safe_pct(g("Net Profit"), g("Total Assets"))
    vals["EPS (RM)"] = eps
    vals["P/B (×)"]  = pb
    vals["P/E (×)"]  = pe
    vals["Dividend Yield (%)"] = dy
    vals["ROE (%)"] = roe
    vals["ROA (%)"] = roa

    # Pass-through regulatory ratios if present
    for k in ("CET1 Ratio", "Tier 1 Ratio", "Total Capital Ratio", "LCR", "NSFR"):
        v = _to_num(row.get(k))
        if v is not None:
            # your summary labels use "(%)" on these
            vals[f"{k} (%)"] = v
    return {k: v for k, v in vals.items() if v is not None and math.isfinite(v)}

def _overlay_as_reported(sum_df: pd.DataFrame, annual_df: pd.DataFrame, bucket: str) -> pd.DataFrame:
    """
    Precedence overlay:
      1) if an annual column with the SAME name as 'Metric' exists, prefer it (as-reported)
      2) for Banking only, apply TP_Bank_* precedence
      3) otherwise keep existing calculated values
    """
    if sum_df is None or sum_df.empty:
        return sum_df

    out = sum_df.copy()
    years = [c for c in out.columns if isinstance(c, (int, np.integer))]
    if not isinstance(annual_df, pd.DataFrame) or annual_df.empty or "Year" not in annual_df.columns:
        return out

    a = annual_df.copy()
    a["__Year"] = pd.to_numeric(a["Year"], errors="coerce")

    for y in years:
        row = a[a["__Year"] == int(y)].tail(1)
        if row.empty:
            continue
        r = row.iloc[0]

        # 1) exact-name as-reported overlay
        for i, label in out["Metric"].items():
            if label in row.columns:
                v = _to_num(r.get(label))
                if v is not None and math.isfinite(v):
                    out.at[i, y] = v

        # 2) banking TP overlay
        if str(bucket).lower() == "banking":
            bank_vals = _bank_overlay_vals(r)
            for i, label in out["Metric"].items():
                if label in bank_vals and bank_vals[label] is not None:
                    out.at[i, y] = bank_vals[label]

    return out

def build_summary_table_with_precedence(
    *, annual_df: pd.DataFrame, quarterly_df: pd.DataFrame, bucket: str,
    include_ttm: bool = True, price_fallback: Optional[float] = None
) -> pd.DataFrame:
    """
    Call your existing build_summary_table(...), then apply precedence overlay.
    Works for all industries; Banking gets TP_* precedence too.
    """
    base = build_summary_table(
        annual_df=annual_df,
    ...
    return _overlay_as_reported(base, annual_df, bucket)
...
RAW_ALIASES: Dict[str, List[str]] = {
    ...
    "earning_assets": ["Earning Assets", "Interest Earning Assets"],
...
RAW_ALIASES.update({
    ...
    # Plantation
    "oer_pct": ["OER (CPO/FFB Input, %)", "OER (%)"],
    "yield_per_hectare": ["Yield per Hectare (FFB t/ha)", "Yield per Hectare"],
    ...
    def q_sum(canon):
        ...
        for col in cols:
            if col in last4.columns:
                s = pd.to_numeric(last4[col], errors="coerce").sum(skipna=True)
                if pd.notna(s):
                    return float(s)
        return None
...
def _common_calcs() -> Dict[str, Calculator]:
    ...
    d: Dict[str, Calculator] = {
        ...
        "Operating CF Margin (%)": lambda r, p, c: (
            (_pick(r, "cfo") / _pick(r, "revenue") * 100.0)
            if (_pick(r, "cfo") is not None and _pick(r, "revenue") not in (None, 0)) else None
        ),
...
def _banking_calcs() -> Dict[str, Calculator]:
    ...
    d: Dict[str, Calculator] = {
        # Income & Efficiency
        "NIM (%)": lambda r, p, c: (lambda nii_num, ea_cur, ea_prev, manual_den: pct(
            nii_num,
            manual_den if manual_den not in (None, 0) else _avg2(ea_cur, ea_prev)
        ))(
        ...
        "CASA Ratio (%)": lambda r, p, c: (lambda dd, sd, dep, td: (
            ((dd or 0.0) + (sd or 0.0)) / (dep if dep not in (None, 0) else ((dd or 0.0) + (sd or 0.0) + (td or 0.0)))
            * 100.0 if (dep not in (None, 0) or any(v is not None for v in (dd, sd, td))) else None
        ))(
            _pick(r, "demand_dep"),
            _pick(r, "savings_dep"),
            _pick(r, "deposits"),
        ...
        "CASA (Core, %)": lambda r, p, c: (lambda dd, sd, td: (
            ((dd or 0.0) + (sd or 0.0)) / (((dd or 0.0) + (sd or 0.0) + (td or 0.0))) * 100.0
        ...
        "CET1 Ratio (%)":           lambda r, p, c: _to_num(r.get("CET1 Ratio")),
        "Tier 1 Capital Ratio (%)": lambda r, p, c: _to_num(r.get("Tier 1 Ratio")),
        "Total Capital Ratio (%)":  lambda r, p, c: _to_num(r.get("Total Capital Ratio")),
        "LCR (%)":                  lambda r, p, c: _to_num(r.get("LCR")),
        "NSFR (%)":                 lambda r, p, c: _to_num(r.get("NSFR")),
...
def _plantation_calcs() -> Dict[str, Calculator]:
    base = _common_calcs()
    base.update({
        "OER (CPO/FFB Input, %)":       lambda r, p, c: _pick(r, "oer_pct"),
        "Yield per Hectare (FFB t/ha)": lambda r, p, c: _pick(r, "yield_per_hectare"),
        "Cash Margin per ton (RM/t)":   lambda r, p, c: _pick(r, "cash_margin_per_ton"),
        "EBITDA per ton (RM/t)":        lambda r, p, c: _pick(r, "ebitda_per_ton"),
        "Gearing (Debt/Assets, %)":     lambda r, p, c: (
            _safe_div(_pick(r, "borrowings"), _pick(r, "total_assets")) * 100.0
            if (_pick(r, "borrowings") is not None and _pick(r, "total_assets") not in (None, 0)) else None
        ),
    })
    return base
...
# --- TTM-only enrichers for Summary (EPS YoY, Payout Ratio) ---
def _append_eps_yoy_and_payout_rows(pivot: pd.DataFrame) -> pd.DataFrame:
    """
    Adds/updates:
      • 'EPS YoY (%)'  = (EPS_TTM / EPS_lastFY - 1) * 100
      • 'Payout Ratio (%)' (TTM) = Dividend Yield (%)_TTM × P/E (×)_TTM
    Safe: if inputs are missing, nothing is added/changed.
    """
    df = pivot.copy()
    if df is None or df.empty or "Metric" not in df.columns:
        return df

    # Find TTM column (e.g., "TTM 2025") and last FY col (max int)
    ...
    def _ensure_row(metric: str, category: str):
        ...
        if mask.any():
            return mask
        # create a blank row with proper columns
    ...
    if pd.notna(eps_yoy):
        m = "EPS YoY (%)"
        _ensure_row(m, "Returns")
        df.loc[df["Metric"] == m, ttm_col] = float(eps_yoy)

    # ---------- Payout Ratio (%) — TTM-first ----------
    # Identity: payout% = (DPS/EPS * 100) = (DY% × P/E)
    payout = np.nan
    try:
    ...
    if pd.notna(payout):
        m = "Payout Ratio (%)"
        _ensure_row(m, "Valuation")
        df.loc[df["Metric"] == m, ttm_col] = float(payout)
...
def _append_eps_yoy_and_payout_rows(pivot: pd.DataFrame) -> pd.DataFrame:
    """
    Adds/updates two TTM-only rows inside the Summary table:
      • EPS YoY (%)        = (EPS_TTM / EPS_lastFY - 1) * 100
...
def build_summary_table(
    ...
    *,
    ...
    if include_ttm and not quarterly_df.empty:
        ...
        # --- BACK-FILL TTM FROM LATEST ANNUAL IF MISSING (Banking-friendly) ---
        if prev_row:
            # For any missing flows/ratios the UI expects, adopt the latest Annual value.
            for k in [
                ...
                # capital ratios usually disclosed at period-end
...
__all__ = [
    ...
    "build_summary_table_with_precedence",
]
