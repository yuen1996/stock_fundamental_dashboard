...
def _with_q_and_raw(names: Iterable[str]) -> List[str]:
    """Return aliases preferring ``Q_`` versions but including raw fallbacks."""
    ordered: List[str] = []
    seen = set()
    for name in names:
        q_name = name if str(name).startswith("Q_") else f"Q_{name}"
        for candidate in (q_name, name):
...
Q_ALIASES.update({
    "orderbook": _with_q_and_raw([
        "Orderbook", "Order Book", "Order Backlog", "Unbilled Sales"
    ]),
    "capacity_utilization": _with_q_and_raw([
        "Capacity Utilization", "Capacity Utilization (%)", "Utilization",
        "Capacity Utilization pct", "Capacity Utilization Pct", "Capacity Utilization %"
    ]),

    "cet1_ratio_pct": _with_q_and_raw([
        "CET1 Ratio (%)", "CET1", "CET1 (%)"
    ]),
    "tier1_ratio_pct": _with_q_and_raw([
        "Tier 1 Capital Ratio (%)", "Tier 1 Ratio (%)", "Tier 1 (%)"
    ]),
    "total_capital_ratio_pct": _with_q_and_raw([
