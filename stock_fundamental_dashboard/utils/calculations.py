    ...
    *,
    ...
    if include_ttm and not quarterly_df.empty:
        ...
        if prev_row:
            ...
            try:
                last_q_year = int(
                    pd.to_numeric(quarterly_df.get("Year"), errors="coerce")
                    .dropna()
                    .iloc[-1]
                )
            except Exception:
            ...
            try:
                last_a_year = int(
                    _to_num(prev_row.get("Year"))
                    or int(annual_df["Year"].dropna().astype(int).iloc[-1])
                )
            except Exception:
                last_a_year = None
            ...
            for k in [
                ...
                if (
                    raw_ttm.get(k) is None
                    and (k in prev_row)
                    and (_to_num(prev_row.get(k)) is not None)
                ):
                    raw_ttm[k] = _to_num(prev_row.get(k))

            # 1a) Dividends: if TTM DPS/DPU is missing, copy from last annual using aliases
            if raw_ttm.get("DPS") is None:
                ann_dps = _pick(prev_row, "dps")  # uses RAW_ALIASES (DPS, Dividend per Share (TTM, RM), etc.)
                if ann_dps is not None:
                    raw_ttm["DPS"] = ann_dps
            if raw_ttm.get("DPU") is None:
                ann_dpu = _pick(prev_row, "dpu")  # Distribution per Unit
                if ann_dpu is not None:
                    raw_ttm["DPU"] = ann_dpu
            ...
            for k_plain, k_label in _cap_keys:
                ...
                if ann_v is None:
                    continue
                has_q_val = (
                    _to_num(raw_ttm.get(k_label)) is not None
                    or _to_num(raw_ttm.get(k_plain)) is not None
                )
                ...
                if (
                    last_a_year is not None
                    and last_q_year is not None
                    and last_a_year > last_q_year
                ):
                    raw_ttm[k_plain] = ann_v
        ...
        ttm_series.index = pd.MultiIndex.from_tuples(
            ttm_series.index, names=["Category", "Metric"]
        )
