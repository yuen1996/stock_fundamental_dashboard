...
def _push(details: list, pillar: str, label: str,
          raw: float | None, score: float | None, source: str):
    details.append({
        "Pillar": pillar,
        "Input":  label,
...
def compute_factor_scores(stock_name, stock_df, ttm,
    ...
    """
    Return 0-100 scores for five pillars
        Value · Quality · Growth · Cash · Momentum
    and attach a `_detail` list for UI diagnostics.
    """
    scores = {k: 0 for k in ["Value", "Quality", "Growth", "Cash", "Momentum"]}
    details: list[dict] = []   # collect per-metric rows

    # helper to record one row
    def _push(pillar, label, raw, score, source):
        details.append({
            "Pillar": pillar,
            "Input":  label,
            "Raw":    None if raw is None else float(raw),
    ...
    # ────────── QUALITY ────────
    cc = (ttm.get("Cash Conversion (CFO/NP, %)") or 0) / 100.0
    gm = None
    try:
    ...
    _push("Quality", "Cash-Conversion", cc, q1, "CFO_TTM ÷ NetProfit_TTM")
    _push("Quality", "Gross-Margin",    gm, q2, "(Revenue – COGS) ÷ Revenue")
    ...
    growth = 0
    try:
        q = stock_df[stock_df.get("IsQuarter", False)]
        if not q.empty and {"Year", "Quarter"}.issubset(q.columns):
            ...
            _push("Growth", "Rev YoY %", r_pos, g1, "Quarterly Q_Revenue vs q-4")
            _push("Growth", "EPS YoY %", e_pos, g2, "Quarterly Q_EPS vs q-4")
    ...
    nd_ebitda = None
    try:
        net_cash = ttm.get("Net Cash (Debt)")
        net_debt = -float(net_cash) if net_cash is not None else None
    ...
    _push("Cash", "ND/EBITDA", nd_ebitda, c1, "(Debt – Cash) ÷ EBITDA_TTM")
    _push("Cash", "Coverage",  icov,      c2, "EBITDA_TTM ÷ FinanceCost_TTM")
    _push("Cash", "Debt/FCF",  debt_fcf,  c3, "(Debt – Cash) ÷ FCF_TTM")

    parts = [c for c in (c1, c2, c3) if c is not None]
    scores["Cash"] = int(sum(parts) / len(parts)) if len(parts) >= 2 \
    ...
    try:
        if isinstance(ohlc_latest, dict):
            ...
            _push("Momentum", "12-m Return", ret12, m1,
                  "(Price ÷ Price_-252d) – 1")
            _push("Momentum", "200-DMA flag", above, m2,
                  "Price ≥ 200-DMA")
