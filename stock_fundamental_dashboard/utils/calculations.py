...
# --- P/E policy toggle + helper -----------------------------------------
# Set ALLOW_NEGATIVE_PE = True in config.py to allow signed P/E when EPS < 0
ALLOW_NEGATIVE_PE = bool(getattr(config, "ALLOW_NEGATIVE_PE", False))

def _pe(px, eps):
    px = _to_num(px); eps = _to_num(eps)
    if px in (None, 0) or eps in (None, 0):
        return None
    # allow negative EPS only if toggled on
    return (px / eps) if (ALLOW_NEGATIVE_PE or eps > 0) else None
# ------------------------------------------------------------------------
