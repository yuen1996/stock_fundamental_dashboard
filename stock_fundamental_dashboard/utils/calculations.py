...
RAW_ALIASES.update({
    ...
    # Plantation
    # --- direct KPIs (may be entered manually / extracted from ops section) ---
    "unit_cash_cost": ["Unit Cash Cost (RM/t)", "Unit Cash Cost"],
    "cpo_asp": ["CPO ASP (RM/t)", "Average CPO Selling Price (RM/t)", "Average CPO Selling Price", "ASP (RM/t)"],

    # --- raw plantation operating stats (used to compute derived KPIs) ---
    "cpo_output": ["CPO Output (t)", "Total CPO Production Volume (tonnes)", "Total CPO Production Volume"],
    "ffb_input": ["FFB Input (t)", "FFB Input"],
    "ffb_harvested": ["FFB Harvested (t)", "FFB Production Volume (tonnes)", "FFB Production Volume"],
    "tonnes_sold": ["Tonnes Sold (t)", "FFB Sales Volume (t)", "FFB Sales Volume (tonnes)", "FFB Sales Volume"],
    "planted_hectares": ["Planted Hectares (ha)", "Total Planted Area (hectares)", "Total Planted Area"],
    "mature_area": ["Mature Area (ha)", "Mature (4th-20th year) (hectares)", "Mature (4th-20th year)"],

    # --- derived plantation KPIs ---
    "oer_pct": ["OER (CPO/FFB Input, %)", "OER (%)", "Average Oil Extraction Rate (CPO) (%)", "Average Oil Extraction Rate (CPO, %)", "Average Oil Extraction Rate (CPO)"],
    "yield_per_hectare": ["Yield per Hectare (FFB t/ha)", "Yield per Hectare", "FFB Yield (mt/ha)", "FFB Yield"],
...
def _plantation_calcs() -> Dict[str, Calculator]:
    ...
    def _oer_pct(r, p, c):
        # Prefer manually-entered KPI if present; else compute from ops stats.
        v = _pick(r, "oer_pct")
        if v is not None:
            return v
        cpo = _pick(r, "cpo_output")
        ffb = _pick(r, "ffb_input")
        d = _safe_div(cpo, ffb)
        return (d * 100.0) if d is not None else None

    def _yield_per_ha(r, p, c):
        # Prefer manually-entered KPI if present; else compute from FFB harvested and (mature area -> planted hectares).
        v = _pick(r, "yield_per_hectare")
        if v is not None:
            return v
        ffb_h = _pick(r, "ffb_harvested")
        denom = _pick(r, "mature_area")
        if denom in (None, 0):
            denom = _pick(r, "planted_hectares")
        return _safe_div(ffb_h, denom)

    def _cash_margin_per_ton(r, p, c):
        # Prefer manual; else proxy = CPO ASP - unit cash cost (common plantation definition).
        v = _pick(r, "cash_margin_per_ton")
        if v is not None:
            return v
        asp = _pick(r, "cpo_asp")
        ucc = _pick(r, "unit_cash_cost")
        if asp is None or ucc is None:
            return None
        return asp - ucc

    def _ebitda_per_ton(r, p, c):
        # Prefer manual; else EBITDA / FFB sales volume (tonnes sold).
        v = _pick(r, "ebitda_per_ton")
        if v is not None:
            return v
        e = _pick(r, "ebitda")
        t = _pick(r, "tonnes_sold")
        return _safe_div(e, t)
    ...
    base.update({
        ...
        "OER (CPO/FFB Input, %)":        _oer_pct,
        "Yield per Hectare (FFB t/ha)":  _yield_per_ha,
        "Cash Margin per ton (RM/t)":    _cash_margin_per_ton,
