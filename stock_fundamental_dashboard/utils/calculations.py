...
def ttm_raw_row_from_quarters(
    ...
    # LCR / NSFR: expose both plain and "(%)" labels so Summary/SoldierWorm can pick either
    _lcr  = q_last("lcr_pct")
    _nsfr = q_last("nsfr_pct")
    ttm["LCR"]      = _lcr
    ttm["LCR (%)"]  = _lcr
    ttm["NSFR"]     = _nsfr
    ttm["NSFR (%)"] = _nsfr
...
def _banking_calcs() -> Dict[str, Calculator]:
    ...
    d: Dict[str, Calculator] = {
        ...
        # Plain-label aliases (some configs list these without "(%)")
        "LCR":      lambda r, p, c: _pick(r, "lcr_pct"),
        "NSFR":     lambda r, p, c: _pick(r, "nsfr_pct"),
    ...
    *,
    ...
    if include_ttm and not quarterly_df.empty:
        ...
        if prev_row:
            ...
            # also handle liquidity ratios
            _liq_keys = [("LCR", "LCR (%)"), ("NSFR", "NSFR (%)")]
            ...
            # LCR/NSFR: adopt Annual if Quarterly missing or older
            for k_plain, k_label in _liq_keys:
                ann_v = _to_num(prev_row.get(k_plain)) or _to_num(prev_row.get(k_label))
                if ann_v is None:
                    continue
                has_q_val = (_to_num(raw_ttm.get(k_label)) is not None) or (_to_num(raw_ttm.get(k_plain)) is not None)
                if not has_q_val:
                    raw_ttm[k_label] = ann_v
                    raw_ttm[k_plain] = ann_v
                    continue
                if (last_a_year is not None) and (last_q_year is not None) and (last_a_year > last_q_year):
                    raw_ttm[k_label] = ann_v
