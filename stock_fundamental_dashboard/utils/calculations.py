...
# add quarterly-prefixed aliases — accept BOTH 'Q_' and unprefixed headers
Q_ALIASES: Dict[str, List[str]] = {}
for k, names in RAW_ALIASES.items():
    alts: List[str] = []
    for name in names:
        nm = str(name)
        # allow plain header (no Q_) because many quarter files are unprefixed
        alts.append(nm)
        # also allow Q_-prefixed version
        alts.append(nm if nm.startswith("Q_") else f"Q_{nm}")
    # de-dupe preserving order
    seen = set()
    Q_ALIASES[k] = [x for x in alts if not (x in seen or seen.add(x))]

# ensure quarterly synonyms explicitly include these too (both plain & Q_)
Q_ALIASES.update({
    "orderbook": [
        "Orderbook", "Order Book", "Order Backlog", "Unbilled Sales",
    ...
    "capacity_utilization": [
        "Capacity Utilization", "Capacity Utilization (%)", "Utilization",
        "Capacity Utilization pct", "Capacity Utilization %",
    ...
    "cet1_ratio_pct": [
        "CET1 Ratio (%)", "CET1", "CET1 (%)", "CET1 Ratio",
        "Q_CET1 Ratio (%)", "Q_CET1", "Q_CET1 (%)", "Q_CET1 Ratio",
    ],
    "tier1_ratio_pct": [
        "Tier 1 Capital Ratio (%)", "Tier 1 Ratio (%)", "Tier 1 (%)", "Tier 1 Ratio",
        "Q_Tier 1 Capital Ratio (%)", "Q_Tier 1 Ratio (%)", "Q_Tier 1 (%)", "Q_Tier 1 Ratio",
    ],
    "total_capital_ratio_pct": [
        "Total Capital Ratio (%)", "Total Capital Ratio", "Total Capital (%)",
        "Q_Total Capital Ratio (%)", "Q_Total Capital Ratio", "Q_Total Capital (%)",
    ],
})
...
def _pick(row: dict, *canon_keys: str, quarterly: bool = False):
    ...
    """
    # primary alias table
    primary = Q_ALIASES if quarterly else RAW_ALIASES
    # secondary fallback (lets quarterly accept unprefixed headers too)
    secondary = RAW_ALIASES if quarterly else {}

    def _try_alias_tables(tables: List[Dict[str, List[str]]]) -> Optional[float]:
        # Map lowercased actual column names -> real key
        key_map = {str(k).strip().lower(): k for k in row.keys()}
        for table in tables:
            for ck in canon_keys:
                for col in table.get(ck, []):
                    # exact header first
                    if col in row:
    ...
    # try primary, then secondary (only different when quarterly=True)
    return _try_alias_tables([primary, secondary] if secondary else [primary])
