...
def compute_factor_scores(stock_name, stock_df, ttm,
    ...
    conf["Cash"]   = _present_count(nd_ebitda, icov, debt_fcf) / 3.0
           ...
           # ────────── MOMENTUM ────────
    # Inputs come from ohlc_latest built on the View page (price CSV).
    mom = 0
    m_present = 0
    if isinstance(ohlc_latest, dict):
        ...
        # M1: 12-month total return; map −20%..+40% ⇒ 0..100
        m1 = _score_linear(ret12, -0.20, 0.40) if ret12 is not None else None

        # M2: 200-DMA flag (1 if price ≥ 200-DMA else 0) scaled to 0/100
        ...
        # Detail rows (show raw and score)
        _push(details, "Momentum", "12-month return", ret12, m1,
              "(Price ÷ Price_-252d) – 1", {"price": price})
        _push(details, "Momentum", "200-DMA flag", 1.0 if above else 0.0, m2,
              "Price ≥ 200-DMA", {"price": price, "ma200": ma200})

        # Pillar score = average of available sub-scores
        subs = [v for v in (m1, m2) if v is not None]
        mom = int(sum(subs) / len(subs)) if subs else 0
        m_present = len(subs) / 2.0

    scores["Momentum"] = mom
    conf["Momentum"]   = m_present
