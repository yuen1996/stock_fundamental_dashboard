...
RAW_ALIASES: Dict[str, List[str]] = {
    ...
    "cet1_ratio_pct":           ["CET1 Ratio (%)", "CET1", "CET1 (%)"],
    "tier1_ratio_pct":          ["Tier 1 Capital Ratio (%)", "Tier 1 Ratio (%)", "Tier 1 (%)"],
...
Q_ALIASES.update({
    ...
    "cet1_ratio_pct":           ["Q_CET1 Ratio (%)", "Q_CET1", "Q_CET1 (%)"],
    "tier1_ratio_pct":          ["Q_Tier 1 Capital Ratio (%)", "Q_Tier 1 Ratio (%)", "Q_Tier 1 (%)"],
...
def ttm_raw_row_from_quarters(
    ...
    ttm["NIM"]                  = q_last("nim_pct")   # carry latest quarterly NIM (%)
...
def build_summary_table(
    ...
    *,
    ...
    if include_ttm and not quarterly_df.empty:
        ...
        if prev_row:
            # Try several label variants when copying from the latest annual row.
            def _copy_if_missing(ttm_key: str, *annual_aliases: str):
                if raw_ttm.get(ttm_key) is None:
                    for akey in annual_aliases:
                        val = _to_num(prev_row.get(akey))
                        if val is not None:
                            raw_ttm[ttm_key] = val
                            break

            # banking flows we show on the TTM cards
            _copy_if_missing("NII (incl Islamic)", "NII (incl Islamic)")
            _copy_if_missing("Operating Income", "Operating Income", "Operating income")
            _copy_if_missing("Operating Expenses", "Operating Expenses", "Opex")
            _copy_if_missing("Provisions", "Provisions", "Provisions / Impairment (RM)")

            # inputs useful for NIM fallback
            _copy_if_missing("Interest Income", "Interest Income")
            _copy_if_missing("Interest Expense", "Interest Expense", "Finance Cost", "Finance Costs")
            _copy_if_missing("Net Islamic Income", "Net Islamic Income")
            _copy_if_missing("Earning Assets", "Earning Assets", "Interest Earning Assets")

            # capital ratios — include (%) labels and common variants
            _copy_if_missing("CET1 Ratio (%)", "CET1 Ratio (%)", "CET1 Ratio", "CET1 (%)", "CET1")
            _copy_if_missing("Tier 1 Capital Ratio (%)", "Tier 1 Capital Ratio (%)", "Tier 1 Ratio (%)", "Tier 1 Ratio", "Tier 1 (%)")
            _copy_if_missing("Total Capital Ratio (%)", "Total Capital Ratio (%)", "Total Capital Ratio")
