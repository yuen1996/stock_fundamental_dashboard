...
RAW_ALIASES: Dict[str, List[str]] = {
    ...
    "oer_pct": ["Average Oil Extraction Rate (CPO) (%)", "Average Oil Extraction Rate (%)", "Oil Extraction Rate (OER) (%)"],
    "cpo_asp": ["Average CPO Selling Price (RM/t)", "Average CPO Selling Price (RM/tonne)", "CPO ASP (RM/t)", "CPO Selling Price (RM/t)"],
    "tonnes_sold": ["Tonnes Sold (t)", "FFB Sales Volume (t)", "FFB Sales Volume"],
    "unit_cash_cost": ["Unit Cash Cost (RM/t)", "Unit Cash Cost"],
    "cash_margin_per_ton": ["Cash Margin per ton (RM/t)", "Cash Margin per tonne (RM/t)"],
    "ebitda_per_ton": ["EBITDA per ton (RM/t)", "EBITDA per tonne (RM/t)"],
}
...
RAW_ALIASES.update({
    ...
    # Plantation
    "oer_pct": ["OER (CPO/FFB Input, %)", "OER (%)", "Average Oil Extraction Rate (CPO) (%)", "Average Oil Extraction Rate (%)", "Oil Extraction Rate (OER) (%)"],
...
def _common_calcs() -> Dict[str, Calculator]:
    ...
    d: Dict[str, Calculator] = {
        ...
        "CFO/EBITDA (%)":        lambda r, p, c: (lambda d: (d * 100.0) if d is not None else None)(_safe_div(_pick(r, "cfo"), _derive_ebitda(r))) if _derive_ebitda(r) not in (None, 0) else None,
...
def _plantation_calcs() -> Dict[str, Calculator]:
    ...
    def _ffb_sales_vol(r, p, c):
        # “per ton” here = per FFB tonnes sold (from annual report / keyed in)
        return _pick(r, "tonnes_sold")

    def _unit_cash_cost(r, p, c):
    ...
    def _oer(r, p, c):
        return _pick(r, "oer_pct")

    def _yield_ha(r, p, c):
        return _pick(r, "yield_per_hectare")

    def _cash_margin(r, p, c):
        # Simple industry convention (if both are per ton of CPO): ASP - unit cash cost
        asp = _cpo_asp(r, p, c)
        cost = _unit_cash_cost(r, p, c)
        if asp is None or cost is None:
            return None
        return asp - cost

    def _ebitda_per_ton(r, p, c):
        # EBITDA per ton of FFB sold = EBITDA / FFB Sales Volume (t)
        # If you prefer per ton of CPO, change denominator to CPO sales/production.
        ebitda = _pick(r, "ebitda")
        vol = _ffb_sales_vol(r, p, c)
        if ebitda is None or vol in (None, 0):
            return None
        return ebitda / vol

    base.update({
        "FFB Sales Volume (t)":          _ffb_sales_vol,
        "Unit Cash Cost (RM/t)":         _unit_cash_cost,
        "CPO ASP (RM/t)":                _cpo_asp,
        "OER (CPO/FFB Input, %)":        _oer,
        "Yield per Hectare (FFB t/ha)":  _yield_ha,
        "Cash Margin per ton (RM/t)":    _cash_margin,
        "EBITDA per ton (RM/t)":         _ebitda_per_ton,

        # keep your existing gearing definition (uses Borrowings / Total Assets)
