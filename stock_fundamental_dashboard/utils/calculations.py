...
RAW_ALIASES.update({
    ...
    # Plantation
    "oer_pct": ["OER", "OER (CPO/FFB Input, %)", "OER (%)"],
...
# --- FINAL bucket calc overrides (keep LAST; prevents later re-definitions from wiping them) ---
try:
    # Ensure industry-specific KPI calculators are actually used
    BUCKET_CALCS["Transportation/Logistics"] = _transport_calcs()
    BUCKET_CALCS["Energy/Materials"] = _energy_materials_calcs()
    BUCKET_CALCS["Plantation"] = _plantation_calcs()
except Exception:
    # If anything goes wrong at import time, fall back to existing mappings.
    pass
