...
def compute_factor_scores(stock_name, stock_df, ttm, ohlc_latest=None, industry=None):
    """
    Return 0-100 scores for 5 pillars:
        Value     – Earnings-Yield & FCF-Yield
        Quality   – Cash-Conversion & Gross-Margin
        Growth    – % of quarterly YoY positives on Revenue & EPS
        Cash      – Interest-Coverage, Debt/FCF, Net-Debt/EBITDA
...
details: list[dict] = []
    ...
    v1 = _score_linear(ey, 0.03, 0.10)  if ey is not None else None   # 3-10 % EY scale
    v2 = _score_linear(fy, 0.02, 0.08)  if fy is not None else None   # 2-8 % FCF-Yld

_push(details, "Value", "Earnings-Yield",
      ey, v1, "NetProfit_TTM ÷ (Price × Shares)")
_push(details, "Value", "FCF-Yield",
      fy, v2, "FCF_TTM ÷ MarketCap")
