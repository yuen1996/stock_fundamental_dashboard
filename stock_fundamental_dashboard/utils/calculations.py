...
def compute_ttm(stock_df: pd.DataFrame, current_price: float | None = None) -> dict:
    ...
    # ---------- Data Health (simple, robust) ----------
    missing = []
    # FCF requires CFO & CapEx
    if out.get("TTM CFO") is None or out.get("TTM CapEx") is None:
        missing.append("CFO or CapEx (for FCF)")
    # Interest coverage needs EBITDA and Finance cost
    if out.get("Interest Coverage (EBITDA/Fin)") is None:
        missing.append("Finance cost or EBITDA (for Coverage)")
    # Net cash (debt) needs debt & cash
    if out.get("Net Cash (Debt)") is None:
        missing.append("Debt/Cash (for Net Cash)")

    out["DataHealth"] = {
        "missing":   missing,
        "estimated": []  # (kept for extension if you later tag EBITDA fallbacks)
    }
...
def compute_factor_scores(stock_name, stock_df, ttm, ohlc_latest=None, industry=None):
    ...
    # ----- Growth (YoY positive frequency over last 12 quarters if available) -----
    growth = 0
    try:
        ...
        if not q.empty and {"Year","Quarter"}.issubset(q.columns):
            ...
            def yoy_pos_frac(series_name):
                if series_name not in q.columns: return None
                v = q[series_name].astype(float).values
                pos, tot = 0, 0
                for i in range(len(v)):
                    if i-4 >= 0 and v[i-4] not in (None, 0):
                        try:
            ...
            r_pos = yoy_pos_frac("Revenue")
            e_pos = yoy_pos_frac("Q_EPS")
            ...
            g_list = [g for g in (g1, g2) if g is not None]
            growth = int(sum(g_list)/len(g_list)) if g_list else 0
    except Exception:
        growth = 0
    ...
    c1 = _score_linear(icov, 2.0, 12.0) if icov is not None else None
    c2 = _score_linear(debt_fcf, 8.0, 0.0, reverse=True) if debt_fcf is not None else None
    c_list = [c for c in (c1, c2) if c is not None]
    scores["Cash"] = int(sum(c_list)/len(c_list)) if c_list else 0

    # ----- Momentum (placeholder, shows 0 unless you pass ohlc_latest later) -----
    scores["Momentum"] = 0
