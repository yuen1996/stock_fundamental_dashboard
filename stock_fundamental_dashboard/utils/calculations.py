...
def compute_ttm(stock_df: pd.DataFrame, current_price: float | None = None) -> dict:
    ...
    # EPS TTM: prefer quarterly EPS sum; else NetProfit / average shares
eps_ttm = tsum(col_eps)

# fallback also when EPS exists but equals 0.0
need_eps_fallback = (
    eps_ttm is None
    or (isinstance(eps_ttm, (int, float, np.floating)) and float(eps_ttm) == 0.0)
)

if need_eps_fallback and valid(ttm_np) and col_sh:
    # shares may be in quarterly rows or overall table
    shares_series = q4[col_sh] if col_sh in q4.columns else stock_df[col_sh]
    # prefer the last 4 quarters; if missing, use latest available
    s4 = _to_num(shares_series).dropna().tail(4)
    if not s4.empty:
        shares_avg = float(s4.mean())
    else:
        sall = _to_num(shares_series).dropna()
        shares_avg = float(sall.iloc[-1]) if not sall.empty else None
