    ...
    *,
    ...
    if include_ttm and not quarterly_df.empty:
        ...
        if raw_ttm:
            ...
            # NEW: fallback DPS/DPU for TTM if quarterly dividends are missing
            if prev_row and (raw_ttm.get("DPS") in (None, 0) and raw_ttm.get("DPU") in (None, 0)):
                annual_dps = _pick(prev_row, "dps")
                annual_dpu = _pick(prev_row, "dpu")
                if annual_dps is not None:
                    # normal shares case
                    raw_ttm["DPS"] = annual_dps
                elif annual_dpu is not None:
                    # REITs/units case
                    raw_ttm["DPU"] = annual_dpu
