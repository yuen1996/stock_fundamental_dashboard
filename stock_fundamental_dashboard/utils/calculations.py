...
def _last_or_none(values):
    """Return the last non-None value; else None."""
...
def ttm_raw_row_from_quarters(
    ...
    def q_sum(canon):
        """
        Sum the last 4 quarter values for a field.
        If all 4 are blank/None/NaN -> return None (NOT 0).
        """
        cols = Q_ALIASES.get(canon, [])
        for col in cols:
            if col in last4.columns:
                s = pd.to_numeric(last4[col], errors="coerce")
                # None if every quarter is NaN
                if not s.notna().any():
                    return None
    ...
    def q_last(canon):
        """
        Carry the last non-None/finite value from the last 4 quarters.
        """
        cols = Q_ALIASES.get(canon, [])
        for col in cols:
            if col in last4.columns:
                vals = pd.to_numeric(last4[col], errors="coerce").tolist()
                return _last_or_none(vals)
        return None
