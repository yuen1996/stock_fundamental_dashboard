...
TTM_ALIASES = {
    # Quarterly flow items (sum last 4) — added your exact names
    ...
    # Shares & EV pieces (quarterly OR annual — pick latest)
...
def compute_ttm(stock_df: pd.DataFrame, current_price: float | None = None) -> dict:
    ...
    # --- resolve columns present in this stock’s data (MUST be indented here) ---
    ...
    def tsum(col):
        return float(_to_num(q4[col]).tail(4).sum()) if col and col in q4.columns else None
    ...
    # EPS TTM: prefer quarterly EPS sum; else NetProfit / avg shares
eps_ttm = tsum(col_eps)

# 👉 also fallback when EPS is present but equals 0.0
need_eps_fallback = (
    not valid(eps_ttm) or
    (isinstance(eps_ttm, (int, float, np.floating)) and float(eps_ttm) == 0.0)
)
if need_eps_fallback and valid(ttm_np) and col_sh:
    shares_series = q4[col_sh] if col_sh in q4.columns else stock_df[col_sh]
    shares_avg = _to_num(shares_series).dropna().tail(4).mean()
    ...
    # --- margins ---
    def pct(a, b):
        if not valid(a) or not valid(b) or float(b) == 0:
            return None
    ...
    # --- valuation multiples ---
    # Market cap: prefer column; else price × shares
    mc = None
    ...
    if mc is None and (current_price is not None) and col_sh:
        s = _to_num(stock_df[col_sh]).dropna()
        if not s.empty:
            mc = float(current_price) * float(s.iloc[-1])

    if valid(current_price) and valid(eps_ttm) and float(eps_ttm) != 0.0:
        out["P/E (TTM)"] = float(current_price) / float(eps_ttm)
    if valid(mc) and valid(ttm_rev) and float(ttm_rev) != 0.0:
    ...
    if valid(mc) and valid(ttm_ebit) and float(ttm_ebit) != 0.0:
