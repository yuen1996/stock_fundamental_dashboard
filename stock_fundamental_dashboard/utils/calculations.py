...
def compute_factor_scores(stock_name, stock_df, ttm,
    ...
    if isinstance(ohlc_latest, dict):
        ...
        # M1: 12-month return mapped from −20%..+40% → 0..100
        m1 = _score_linear(ret12, -0.20, 0.40) if ret12 is not None else None

        # M2: 200-DMA flag (1 if price ≥ 200-DMA else 0), then scaled to 0/100
        ...
        _push(details, "Momentum", "200-DMA flag", 1.0 if above else 0.0 if (price is not None and ma200 is not None) else None,
              m2, "Price ≥ 200-DMA", {"price": price, "ma200": ma200})
