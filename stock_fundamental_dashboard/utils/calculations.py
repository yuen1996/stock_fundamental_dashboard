...
RAW_ALIASES.update({
    ...
    # Plantation
    "tonnes_sold": [
        "Tonnes Sold (t)", "Tonnes Sold",
        "FFB Sales Volume (t)", "FFB Sales Volume",
        "FFB Sold (t)", "FFB Sold"
    ],
    "unit_cash_cost": [
        "Unit Cash Cost (RM/t)", "Unit Cash Cost",
        "Unit Cash Cost (RM/tonne)", "Unit Cash Cost (RM/mt)"
    ],
    "cpo_asp": [
        "Average CPO Selling Price (RM/t)",
        "CPO ASP", "CPO ASP (RM/t)",
        "Average CPO Selling Price",
        "ASP (RM/t)", "ASP"
    ],
    "oer_pct": [
        "Average Oil Extraction Rate (CPO) (%)",
        "OER", "OER (%)",
        "OER (CPO/FFB Input, %)"
    ],
    "yield_per_hectare": [
        "FFB Yield (mt/ha)",
        "FFB Yield",
        "Yield per Hectare (FFB t/ha)",
        "Yield per Hectare"
    ],
    "cash_margin_per_ton": [
        "Cash Margin per ton (RM/t)", "Cash Margin per ton"
    ],
    "ebitda_per_ton": [
        "EBITDA per ton (RM/t)", "EBITDA per ton"
    ],
...
def _plantation_calcs() -> Dict[str, Calculator]:
    ...
    def _ffb_sales(r, p, c):
        return _pick(r, "tonnes_sold")

    def _unit_cost(r, p, c):
        return _pick(r, "unit_cash_cost")

    def _cpo_asp(r, p, c):
        return _pick(r, "cpo_asp")

    def _cash_margin(r, p, c):
        v = _pick(r, "cash_margin_per_ton")
        if v is not None:
            return v
        asp = _cpo_asp(r, p, c)
        cost = _unit_cost(r, p, c)
        if asp is None or cost is None:
            return None
        return asp - cost

    def _ebitda_per_ton(r, p, c):
        v = _pick(r, "ebitda_per_ton")
        if v is not None:
            return v
        e = _pick(r, "ebitda")
        vol = _ffb_sales(r, p, c)
        if e is None or vol in (None, 0):
            return None
        return e / vol

    base.update({
        "FFB Sales Volume (t)":          _ffb_sales,
        "Unit Cash Cost (RM/t)":         _unit_cost,
        "CPO ASP (RM/t)":                _cpo_asp,
        "OER (CPO/FFB Input, %)":        lambda r, p, c: _pick(r, "oer_pct"),
        "Yield per Hectare (FFB t/ha)":  lambda r, p, c: _pick(r, "yield_per_hectare"),
        "Cash Margin per ton (RM/t)":    _cash_margin,
        "EBITDA per ton (RM/t)":         _ebitda_per_ton,
...
# === PATCH (2026-02-07): Fix KPI label mismatches + add missing Construction/REITs calcs ===
# This block is intentionally appended so it applies after BUCKET_CALCS is fully constructed.

def _dio_days(r, p, c):
    inv = _pick(r, "inventory")
    cogs = _mag(_pick(r, "cogs"))
    return (inv / cogs * 365.0) if (inv is not None and cogs not in (None, 0)) else None

def _dso_days(r, p, c):
    ar = _pick(r, "receivables")
    rev = _pick(r, "revenue")
    return (ar / rev * 365.0) if (ar is not None and rev not in (None, 0)) else None

def _dpo_days(r, p, c):
    ap = _pick(r, "payables")
    cogs = _mag(_pick(r, "cogs"))
    return (ap / cogs * 365.0) if (ap is not None and cogs not in (None, 0)) else None

def _ebitda_margin_pct(r, p, c):
    rev = _pick(r, "revenue")
    e = _derive_ebitda(r)
    v = _safe_div(e, rev)
    return (v * 100.0) if v is not None else None

def _net_margin_pct(r, p, c):
    rev = _pick(r, "revenue")
    np = _pick(r, "net_profit")
    v = _safe_div(np, rev)
    return (v * 100.0) if v is not None else None

def _eps_rm(r, p, c):
    return _safe_div(_pick(r, "net_profit"), _pick(r, "shares"))

# 1) Construction: missing working-capital days + label variants
if "Construction" in BUCKET_CALCS:
    d = BUCKET_CALCS["Construction"]
    d.setdefault("DIO (days)", _dio_days)
    d.setdefault("DSO (days)", _dso_days)
    d.setdefault("DPO (days)", _dpo_days)

    # config uses (x), calculations uses (×)
    if "Backlog Coverage (×)" in d and "Backlog Coverage (x)" not in d:
        d["Backlog Coverage (x)"] = d["Backlog Coverage (×)"]

    # config uses "Gross Profit Margin (%)" in Construction
    if "Gross Margin (%)" in d and "Gross Profit Margin (%)" not in d:
        d["Gross Profit Margin (%)"] = d["Gross Margin (%)"]

# 2) Telco: legacy label
if "Telco" in BUCKET_CALCS:
    d = BUCKET_CALCS["Telco"]
    if "Net Debt / EBITDA (×)" in d and "Net Debt/EBITDA (x)" not in d:
        d["Net Debt/EBITDA (x)"] = d["Net Debt / EBITDA (×)"]

# 3) Property: label variant for P/NAV
if "Property" in BUCKET_CALCS:
    d = BUCKET_CALCS["Property"]
    if "P/NAV (×)" in d and "P/NAV (x)" not in d:
        d["P/NAV (x)"] = d["P/NAV (×)"]

# 4) REITs: config expects generic margins + EPS (in addition to NPI Margin)
if "REITs" in BUCKET_CALCS:
    d = BUCKET_CALCS["REITs"]
    d.setdefault("EBITDA Margin (%)", _ebitda_margin_pct)
    d.setdefault("Net Margin (%)", _net_margin_pct)
    d.setdefault("EPS (RM)", _eps_rm)
