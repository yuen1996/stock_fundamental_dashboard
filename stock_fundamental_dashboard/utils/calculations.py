...
def compute_factor_scores(stock_name, stock_df, ttm,
    ...
    if isinstance(ohlc_latest, dict):
        ...
        _push(details, "Momentum", "200-DMA flag", 1.0 if above else 0.0 if m2 is not None else None, m2,
              "Price ≥ 200-DMA", {"price": price, "ma200": ma200})

        usable = [m for m in (m1, m2) if m is not None]
        if usable:
            mom = int(sum(usable)/len(usable))
        m_present = _present_count(ret12, (price if ma200 is not None else None))
    scores["Momentum"] = mom
    conf["Momentum"]   = (m_present / 2.0) if m_present else 0.0
