...
def _common_calcs() -> Dict[str, Calculator]:
    ...
    # working-capital label variants used by some industry summaries
    d["DIO (days)"] = d["Inventory Days (days)"]
    d["DSO (days)"] = d["Receivable Days (days)"]
    d["DPO (days)"] = d["Payable Days (days)"]

    # gross profit margin naming variants (some industries use this label)
    d["Gross Profit Margin (%)"] = d["Gross Margin (%)"]
    d["Gross Profit Margin"] = d["Gross Margin (%)"]

    # Net Debt/EBITDA variants without spaces around slash
    d["Net Debt/EBITDA (×)"] = d["Net Debt / EBITDA (×)"]
    d["Net Debt/EBITDA"] = d["Net Debt / EBITDA (×)"]
    ...
    d["Net Debt/EBITDA (x)"] = d.get("Net Debt / EBITDA (x)", d["Net Debt / EBITDA (×)"])
...
def _reits_calcs() -> Dict[str, Calculator]:
    """REITs = common ratios + REIT-specific portfolio & distribution metrics.

    Important: we inherit _common_calcs() so EBITDA Margin, Net Margin, EPS, etc. work out of the box.
    """
    base = _common_calcs()
    ...
    base.update({
        ...
        "Distribution Yield (%)":       lambda r, p, c: (lambda dpu, px: (
                                                ((abs(dpu) / px) * 100.0) if (dpu is not None and px not in (None, 0)) else None
        ...
        "P/NAV (×)":                    lambda r, p, c: (lambda navps, px: (px / navps) if (px and navps and navps > 0) else None)(
                                                _safe_div(_pick(r, "equity"), _pick(r, "shares")), _ctx_price(r, c)),
        "NPI Margin (%)":               lambda r, p, c: (
                                                (_safe_div(_pick(r, "npi"), _pick(r, "revenue")) * 100.0)
    ...
    base["Interest Coverage (x)"]       = base["Interest Coverage (×)"]
    base["P/NAV (x)"]                   = base["P/NAV (×)"]
    base["Average Cost of Debt (%)"]    = base["Avg Cost of Debt (%)"]
    base["Gearing (%)"]                 = base["Gearing (Debt/Assets, %)"]
    return base
...
def _plantation_calcs() -> Dict[str, Calculator]:
    ...
    Notes:
      - For OER and Yield/ha: prefer direct inputs if user keyed them in; otherwise compute from
        CPO Output ÷ FFB Input, and FFB Harvested ÷ Planted Hectares.
      - EBITDA per ton is defined as EBITDA ÷ FFB Sales Volume (Tonnes Sold).
      - Cash Margin per ton defaults to (CPO ASP − Unit Cash Cost) if not provided.
    """
    base = _common_calcs()
    ...
    base.update({
        "FFB Sales Volume (t)":          lambda r, p, c: _pick(r, "ffb_sales_volume_t"),
        ...
        "OER (CPO/FFB Input, %)":        lambda r, p, c: (
            _pick(r, "oer_pct") if _pick(r, "oer_pct") is not None
            else pct(_pick(r, "cpo_output_t"), _pick(r, "ffb_input_t"))
        ),

        "Yield per Hectare (FFB t/ha)":  lambda r, p, c: (
            _pick(r, "yield_per_hectare") if _pick(r, "yield_per_hectare") is not None
            else _safe_div(_pick(r, "ffb_harvested_t"), _pick(r, "planted_hectares"))
        ),

        "Cash Margin per ton (RM/t)":    lambda r, p, c: (
            _pick(r, "cash_margin_per_ton") if _pick(r, "cash_margin_per_ton") is not None
            else (lambda asp, cost: (asp - cost) if (asp is not None and cost is not None) else None)(
                _pick(r, "cpo_asp"), _pick(r, "unit_cash_cost")
            )
        ),

        "EBITDA per ton (RM/t)":         lambda r, p, c: (
            _safe_div(_derive_ebitda(r), _pick(r, "ffb_sales_volume_t"))
        ),

        "Gearing (Debt/Assets, %)":      lambda r, p, c: pct(_pick(r, "borrowings"), _pick(r, "total_assets")),
    })
    return base
