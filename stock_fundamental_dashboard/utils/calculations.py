...
def compute_factor_scores(stock_name, stock_df, ttm,
    ...
    # ────────── MOMENTUM ────────
    # Softer scale + weighting: 70% 12-mo return, 30% 200-DMA flag
    mom = 0
    mom_conf = 0.0
    try:
        price = ohlc_latest.get("price") if isinstance(ohlc_latest, dict) else None
        ret12 = ohlc_latest.get("ret_12m") if isinstance(ohlc_latest, dict) else None
        ma200 = ohlc_latest.get("ma200") if isinstance(ohlc_latest, dict) else None
        above = (price is not None and ma200 is not None and price >= ma200)

        # Less punitive mapping: -10% → 0, +30% → 100
        m1 = _score_linear(ret12, -0.10, 0.30) if ret12 is not None else None
        # 200-DMA flag contributes if we can evaluate it
        ...
        _push(details, "Momentum", "200-DMA flag",
            (1.0 if above else 0.0) if m2 is not None else None, m2,
            "Price ≥ 200-DMA",
            {"price": price, "ma200": ma200})

        parts = [v for v in (m1, m2) if v is not None]
        if parts:
            # When both are present, 70/30 weighting; otherwise just use whatever is available
        ...
        # Confidence reflects how many momentum inputs we could evaluate
