...
def _to_num(x, *, none_ok: bool = True) -> Optional[float]:
    """Convert to float; tolerant to commas, trailing %, and (negative) parentheses."""
    try:
        if x is None:
        ...
        if isinstance(x, str):
            ...
            if s == "":
                return None if none_ok else 0.0
            # negatives in parentheses: (123.4) -> -123.4
            neg = False
            if s.startswith("(") and s.endswith(")"):
                neg = True
                s = s[1:-1].strip()
            # drop commas and a trailing percent sign
            s = s.replace(",", "")
            if s.endswith("%"):
                s = s[:-1].strip()
            v = float(s)
            ...
            v = float(x)
        return v if math.isfinite(v) else (None if none_ok else 0.0)
    except Exception:
        return None if none_ok else 0.0
