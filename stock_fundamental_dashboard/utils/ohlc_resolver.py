...
# ---------- Indexing ----------
@_cache
def _build_index(ohlc_dir: str, _sig: int = 0) -> Dict[str, Path]:
    """
...
def _dir_sig(path: str) -> int:
    """Directory signature for cache-busting (mtime in ns)."""
    try:
...
def resolve_ohlc_path(
    ...
    sig = _dir_sig(base_dir)
    index = _build_index(base_dir, sig)
    if not index:
        return None, "not_found"
...
def list_known(ohlc_dir: Optional[str] = None) -> List[str]:
    ...
    base_dir = ohlc_dir or str(default_ohlc_dir())
    sig = _dir_sig(base_dir)
    return sorted(set(_build_index(base_dir, sig).keys()))