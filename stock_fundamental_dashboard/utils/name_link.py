...
def resolve_ohlc_dir() -> str:
    ...
    Order:
      ...
      3) <repo>/data/ohlc
      4) <repo>/stock_fundamental_dashboard/data/ohlc
      5) <cwd>/data/ohlc
      6) <cwd>/stock_fundamental_dashboard/data/ohlc

    If none exist, create the best default (SFD_DATA_DIR/ohlc if available,
    otherwise <repo>/data/ohlc).
    """
    here = os.path.dirname(__file__)

    env = (os.environ.get("SFD_OHLC_DIR") or "").strip()
    data_env = (os.environ.get("SFD_DATA_DIR") or "").strip()
    data_ohlc = os.path.abspath(os.path.join(os.path.expanduser(data_env), "ohlc")) if data_env else None

    candidates = [
        env or None,
        data_ohlc,
        # standard spot next to utils/
    ...
    # default: create in SFD_DATA_DIR if provided, else create the standard spot next to utils/
    d = data_ohlc or os.path.abspath(os.path.join(here, "..", "data", "ohlc"))
