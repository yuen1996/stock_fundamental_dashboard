...
def push_trade_candidate(
    ...
    *,
    ...
    Extra planning fields are optional; remains backward compatible with older callers.
    Also logs an UPSERT event to the queue audit.
    """
...
def mark_live_from_queue(name: str, strategy: str, *, open_price: float | None = None, open_date: str | None = None) -> bool:
    """
    Move a (Name, Strategy) row from Trade Queue to Open Trades.
    - Uses queue Entry if present, otherwise provided `open_price`, otherwise CurrentPrice.
    - Copies Stop/Take/Shares/RR/TPs/Reasons.
    ...
    # prefer explicit queue Entry; else open_price; else CurrentPrice
    entry = row.get("Entry")
    ...
    # Upsert into open_trades (unique by Name, Strategy)
    open_df = load_open_trades()
    omask = open_df["Name"].astype(str).str.lower().eq(str(name).lower()) & \
            open_df["Strategy"].astype(str).str.lower().eq(str(strategy).lower())
    open_df = open_df.loc[~omask]
