...
def mark_friend_live_row(row_id: int, friend_name: str) -> bool:
    ...
    and write an audit record ('MARK_LIVE_FRIEND').
    """
    try:
        q = load_trade_queue()
        if q is None or q.empty or row_id < 0 or row_id >= len(q):
        ...
        try:
            payload = dict(row)
            payload["FriendName"] = friend_name
            append_queue_audit("MARK_LIVE_FRIEND", payload, audit_reason=f"friend:{friend_name}")
        except Exception:
            pass

        # --- Move to friend live/open list
        row_df = q.iloc[[row_id]].copy()
        ...
        # add friend name
        row_df["FriendName"] = friend_name

        # read/prepare friend live file and make sure it has needed columns
        live = pd.DataFrame(columns=list(row_df.columns))
        try:
        ...
        live = pd.concat([live, row_df], ignore_index=True)
        _atomic_write_csv(live, FRIEND_LIVE_FILE)
...
# Friend-trades live/closed files (separate from your own)
FRIEND_OPEN_TRADES_COLUMNS = OPEN_TRADES_COLUMNS + ["FriendName"]
FRIEND_CLOSED_TRADES_COLUMNS = CLOSED_TRADES_COLUMNS + ["FriendName"]
...
def close_friend_open_trade_row(rowid: int, close_price: float, close_reason: str) -> bool:
    """
    Close ONE friend open-trades row by its current CSV index (RowId), and move it to FRIEND_CLOSED_FILE.
    """
    try:
