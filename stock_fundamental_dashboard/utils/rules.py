...
# ---- Momentum helpers (share the same files as Momentum page) ----
import os
...
def _mom_change_pct_df(df, months: int = 12) -> float | None:
    ...
    start_dt = last_dt - pd.Timedelta(days=365 if months >= 12 else 30*months)
        ...
        if STRICT_NO_COMPUTE:
            ...
            if key in ("mom_12m_pct","price_change_12m_pct"):
                ...
                if "Name" in stock_df.columns:
                    ...
                    if not s.empty: stock_name = str(s.iloc[0])
...
def funnel_rule(
    *,
    ...
    s_cash, c_cash = _score_block_from_anchors(stock_df, anchors_all.get("cashflow_first", {}), fd_rate_decimal, bucket=bucket)
    s_ttm,  c_ttm  = _score_block_from_anchors(stock_df, anchors_all.get("ttm_vs_lfy", {}),     fd_rate_decimal, bucket=bucket)
    s_gq,   c_gq   = _score_block_from_anchors(stock_df, anchors_all.get("growth_quality", {}), fd_rate_decimal, bucket=bucket)
    s_div,  c_div  = _score_block_from_anchors(stock_df, anchors_all.get("dividend", {}),       fd_rate_decimal, bucket=bucket)
    s_mom,  c_mom  = _score_block_from_anchors(stock_df, anchors_all.get("momentum", {}),       fd_rate_decimal, bucket=bucket)
