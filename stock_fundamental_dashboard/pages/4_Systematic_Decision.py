...
st.markdown(
    section("🔧 Manage Queue", "Tick Friend to send to Friend Trades. Mark Live / Delete — acts on exact RowId", "warning"),
    unsafe_allow_html=True,
)
...
if tq.empty:
    ...
    table.insert(1, "FriendTrade", False)
    ...
    with c1:
        if st.button("✅ Mark Live selected"):
            sel = edited_q[edited_q["Select"] == True].copy()
            if sel.empty:
                st.info("Nothing selected.")
            else:
                # Block rows with incomplete plan
                blocked = sel[sel.get("PlanOK", False) != True]
                ready   = sel[sel.get("PlanOK", False) == True]

                blocked_ids = [int(x) for x in blocked.get("RowId", pd.Series([], dtype=int)).tolist()]

                # If any selected rows are marked as FriendTrade, ask for the friend name first
                friend_ready = ready[ready.get("FriendTrade", False) == True]
                if not friend_ready.empty:
                    st.session_state["mq_friend_mark_payload"] = {
                        "row_ids": [int(x) for x in ready["RowId"].tolist()],
                        "friend_row_ids": [int(x) for x in friend_ready["RowId"].tolist()],
                        "blocked_ids": blocked_ids,
                    }
                    st.session_state["mq_friend_mark_open"] = True
                else:
                    moved = 0
                    # process in descending RowId (because each mark-live removes a row from the queue)
                    for rid in sorted([int(x) for x in ready["RowId"].tolist()], reverse=True):
                        if ioh.mark_live_row(rid):
                            moved += 1
        ...
        # Friend-name popup (uses st.dialog if available; otherwise inline expander)
        if st.session_state.get("mq_friend_mark_open", False):
            payload = st.session_state.get("mq_friend_mark_payload") or {}
            _row_ids = payload.get("row_ids", [])
            _friend_ids = set(payload.get("friend_row_ids", []))
            _blocked_ids = payload.get("blocked_ids", [])

            dialog_fn = getattr(st, "dialog", None) or getattr(st, "experimental_dialog", None)

            def _do_friend_mark(friend_name: str):
                friend_name = (friend_name or "").strip()
                if not friend_name:
                    st.warning("Please enter a friend name.")
                    return

                moved_self, moved_friend = 0, 0
                # process all ready rows (self + friend) in descending RowId
                for rid in sorted([int(x) for x in _row_ids], reverse=True):
                    if rid in _friend_ids:
                        if ioh.mark_friend_live_row(rid, friend_name):
                            moved_friend += 1
                    else:
                        if ioh.mark_live_row(rid):
                            moved_self += 1

                st.session_state["mq_friend_mark_open"] = False
                st.session_state["mq_friend_mark_payload"] = None

                if moved_self or moved_friend:
                    msg = []
                    if moved_self:
                        msg.append(f"{moved_self} self")
                    if moved_friend:
                        msg.append(f"{moved_friend} friend ({friend_name})")
                    st.success("Marked live: " + ", ".join(msg) + " row(s).")
                    (getattr(st, "rerun", None) or getattr(st, "experimental_rerun", lambda: None))()
                elif _blocked_ids:
                    st.warning(
                        f"{len(_blocked_ids)} row(s) blocked — plan incomplete (need Entry, Stop, Take, Shares, RR): "
                        + ", ".join(map(str, _blocked_ids))
                        + ". Open **Risk / Reward Planner** to finish the plan."
                    )
                else:
                    st.info("Nothing moved.")
            ...
            if dialog_fn:
                @dialog_fn("👥 Friend trade", width="small")
                def _friend_dialog():
                    st.markdown("Enter the friend name for the selected **FriendTrade** row(s).")
                    friend_name = st.text_input("Friend name", key="mq_friend_name_input")
                    c_ok, c_cancel = st.columns([1, 1])
                    with c_ok:
                        if st.button("✅ Confirm"):
                            _do_friend_mark(friend_name)
                    with c_cancel:
                        if st.button("Cancel"):
                            st.session_state["mq_friend_mark_open"] = False
                            st.session_state["mq_friend_mark_payload"] = None
                ...
                with st.expander("👥 Friend trade — enter friend name", expanded=True):
                    friend_name = st.text_input("Friend name", key="mq_friend_name_input_inline")
                    c_ok, c_cancel = st.columns([1, 1])
                    with c_ok:
                        if st.button("✅ Confirm friend mark", key="mq_friend_mark_confirm_inline"):
                            _do_friend_mark(friend_name)
                    with c_cancel:
                        if st.button("Cancel", key="mq_friend_mark_cancel_inline"):
                            st.session_state["mq_friend_mark_open"] = False
                            st.session_state["mq_friend_mark_payload"] = None
                            (getattr(st, "rerun", None) or getattr(st, "experimental_rerun", lambda: None))()
