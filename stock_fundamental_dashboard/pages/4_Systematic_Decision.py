...
import pandas as pd
import numpy as np
from utils import bus, name_link  # sync price sources with View Stock
...
# --- Manual price refresh (forces cache bust across cached loaders) ---
PRICE_REFRESH_EPOCH = st.session_state.get("PRICE_REFRESH_EPOCH", 0)
if st.button("🔄 Refresh prices"):
    st.session_state["PRICE_REFRESH_EPOCH"] = int(PRICE_REFRESH_EPOCH) + 1
    st.rerun()
...
# --- View-Stock–synced price helpers (replaces the old _resolve_price_for_name) ---

def _current_price_from_rows(rows: pd.DataFrame) -> float | None:
...
@st.cache_data(show_spinner=False, ttl=60)  # refresh at most every 60s
def _load_ohlc_for_name(stock_name: str, _etag: int, _epoch: int, *, ticker: str | None = None) -> pd.DataFrame | None:
    # Reuse your existing _resolve_ohlc_dir() in this file
...
def _momentum_close_before_today(stock_name: str, stock_rows: pd.DataFrame) -> tuple[float | None, pd.Timestamp | None]:
    ...
    etag = _ohlc_dir_etag()
    dfp  = _load_ohlc_for_name(stock_name, etag, PRICE_REFRESH_EPOCH, ticker=ticker_hint)
...
def _resolve_price_for_name(name: str) -> float | None:
    ...
    # use the exact per-stock choice set on View Stock
    pref = st.session_state.get(f"price_src_{_safe_key(name)}", "Momentum")
    pref = str(pref or "Auto").strip().lower()

    manual = _current_price_from_rows(rows)
...
def _ohlc_dir_etag() -> int:
    # Prefer cross-page bus etag; if not bumped, fall back to dir mtime
    try:
        v = int(bus.etag("ohlc"))
        if v:
            return v
...
def _load_ohlc_for(stock_name: str, _etag: int, _epoch: int) -> pd.DataFrame | None:
...
def _ret_12m_pct_from_ohlc(stock_name: str) -> float | None:
    df_px = _load_ohlc_for(stock_name, _ohlc_dir_etag(), PRICE_REFRESH_EPOCH)
    if df_px is None:
        return None
