...
def _ttm_summary(stock_df: pd.DataFrame, bucket: str):
    """Build TTM Summary only when NOT in strict mode. In strict mode we rely on the View snapshot."""
...
def _value_for_metric(
    ...
    *,
    ...
    # prefer View snapshot; fall back to recomputed TTM (when allowed)
    def _via(label: str):
        # 1) exact value saved by View page
        if stock_name:
            v_snap = _from_view_snapshot(stock_name, label)
            if v_snap is not None:
                return v_snap
        # 2) fallback to on-the-fly TTM column (only if available)
    ...
    def _src_for(label: str, v):
        if v is None:
            return "—"
        if stock_name and _from_view_snapshot(stock_name, label) is not None:
            return f"View Snapshot: {canon(stock_name, label)}"
        return f"TTM Summary: {label}"
    ...
    def _yoy_from_annual(names: list[str]):
    ...
    if STRICT_NO_COMPUTE:
        ...
        if key == "revenue_yoy_pct":
            ...
            return v, _src_for("Revenue YoY (%)", v), "%"
        ...
        if key in direct:
            v = _via(direct[key])
            return v, _src_for(direct[key], v), "%" if key.endswith("_pct") else ""
        ...
        if key == "dy_vs_fd_x":
            ...
            return v, _src_for("Dividend Yield (%)", v), "%"

        return None, "—", ""

    # --- Non-strict: keep previous fallbacks (same UX as before) ---
    if key == "cfo_margin_pct":
        v = _via("Operating CF Margin (%)") or _via("CFO Margin (%)")
        if v is not None: return v, _src_for("CFO Margin (%)", v), "%"
    ...
    if key == "fcf_margin_pct":
        v = _via("FCF Margin (%)")
        if v is not None: return v, _src_for("FCF Margin (%)", v), "%"
    ...
    if key in ("cash_conv_cfo_ebitda","cash_conv_cfo_ebitda_pct"):
        ...
        if v is None:
            ...
            if v_pct is not None:
                return float(v_pct)/100.0, _src_for("CFO/EBITDA (%)", v_pct), "x"
        if v is not None: return v, _src_for("CFO/EBITDA (×)", v), "x"
        cfo = _last_float(a, ["CFO","Operating Cash Flow","Cash Flow from Ops"])
    ...
    if key == "ebitda_margin_pct":
        v = _via("EBITDA Margin (%)")
        if v is not None: return v, _src_for("EBITDA Margin (%)", v), "%"
    ...
    if key in ("icr_x","icr"):
        v = _via("Interest Coverage (x)") or _via("Interest Coverage (×)")
        if v is not None: return v, _src_for("Interest Coverage (x)", v), "x"
    ...
    if key == "avg_cost_debt_pct":
        v = _via("Average Cost of Debt (%)")
        if v is not None: return v, _src_for("Average Cost of Debt (%)", v), "%"
    ...
    if key == "capex_to_rev_pct":
        v = _via("Capex/Revenue (%)") or _via("Capex to Revenue (%)")
        if v is not None: return v, _src_for("Capex/Revenue (%)", v), "%"
    ...
    if key == "dy_vs_fd_x":
        ...
        return v, _src_for("Dividend/Distribution Yield (%)", v), "%"

    if key == "payout_pct":
        v = _via("Payout Ratio (%)")
        if v is not None: return v, _src_for("Payout Ratio (%)", v), "%"
    ...
    if key in ("mom_12m_pct", "price_change_12m_pct"):
        v = _via("12M Price Change (%)") or _via("Price Change 12M (%)") or _via("Total Return 1Y (%)")
        src = _src_for("12M Price Change (%)", v) if v is not None else ""
    ...
    if key in direct_map:
        ...
        return v, _src_for(direct_map[key][0], v), "%" if key.endswith("_pct") else ""
