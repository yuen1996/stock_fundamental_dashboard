...
def _value_for_metric(
    ...
    *,
    ...
    if STRICT_NO_COMPUTE:
        ...
        if key == "eps_yoy_pct":
            # 1) direct YoY saved on Summary
            v = _via("EPS YoY (%)")
            if v is not None:
                return v, "TTM Summary: EPS YoY (%)", "%"

            # 2) compute from TTM EPS vs last FY EPS (calculator TTM column)
            base_lbl = "EPS (RM)"
            ttm_eps = _via(base_lbl)
            if ttm_eps is None and stock_name:
                ttm_eps = _from_ttm_card(stock_name, base_lbl) or _from_view_snapshot(stock_name, base_lbl)
            prev_eps, fy_col = _via_lfy(base_lbl)

            if ttm_eps is not None and prev_eps not in (None, 0):
                yoy = (float(ttm_eps) / float(prev_eps) - 1.0) * 100.0
                return yoy, f"TTM Summary/Card: {base_lbl} / {base_lbl} [{fy_col}]", "%"
            if prev_eps == 0:
                return None, "Not meaningful (prev FY = 0)", "%"

            # 3) last resort: annual YoY
        ...
        if key == "revenue_yoy_pct":
            ...
            # 2) compute from TTM Revenue vs last FY Revenue (calculator TTM column)
            base_lbl = "Revenue"
            ttm_rev = _via(base_lbl)
            if ttm_rev is None and stock_name:
                ttm_rev = _from_ttm_card(stock_name, base_lbl) or _from_view_snapshot(stock_name, base_lbl)
            prev_rev, fy_col = _via_lfy(base_lbl)

            if ttm_rev is not None and prev_rev not in (None, 0):
                yoy = (float(ttm_rev) / float(prev_rev) - 1.0) * 100.0
                return yoy, f"TTM Summary/Card: {base_lbl} / {base_lbl} [{fy_col}]", "%"
            if prev_rev == 0:
                return None, "Not meaningful (prev FY = 0)", "%"

            # 3) last resort: annual YoY
            v = _yoy_from_annual([base_lbl])
            if v is not None:
