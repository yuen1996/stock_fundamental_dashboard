...
try:
    from utils.dividend_helpers import get_latest_dividend_yield
except Exception:  # pragma: no cover - legacy fallback when utils package not available
    from dividend_helpers import get_latest_dividend_yield  # type: ignore
...
def _value_for_metric(
    ...
    *,
    ...
    if key == "dy_vs_fd_x":
        v = get_latest_dividend_yield(stock_name or "") if stock_name else None
        return v, ("Latest Dividend Yield (%)" if v is not None else "—"), "%"
