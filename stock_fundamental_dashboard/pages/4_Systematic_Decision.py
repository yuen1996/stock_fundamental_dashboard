...
if tq.empty:
    ...
    edited_q = st.data_editor(
        ...
        column_config={
            ...
            "FriendName":   st.column_config.TextColumn("Friend Name"),
    ...
    def _queue_clear_pending():
        ...
        st.session_state["queue_prompt_friend_name"] = ""
    ...
    if st.session_state.get("queue_friend_prompt_open") and (st.session_state.get("queue_pending_missing_friend_rowids") or []):
        ...
        if hasattr(st, "dialog"):
            ...
            def _friend_name_dialog():
                ...
                name = st.text_input("Friend name", key="queue_prompt_friend_name", placeholder="e.g. John / Ah Seng")
                ...
                with b1:
                    if st.button("Apply & Mark Live"):
                        if not str(name).strip():
                            ...
                            return
                        moved = _queue_execute_pending(friend_name_fill=str(name).strip())
            ...
            st.text_input("Friend name", key="queue_prompt_friend_name")

            c_ok, c_cancel = st.columns([1, 1])
            with c_ok:
                if st.button("Apply & Mark Live", key="queue_apply_mark_live_no_dialog"):
                    name = str(st.session_state.get("queue_prompt_friend_name") or "").strip()
                    if not name:
                        st.error("Please enter a friend name.")
                    else:
    ...
    with c1:
        if st.button("✅ Mark Live selected"):
            ...
            if not to_move and blocked == 0:
                ...
                st.session_state.setdefault("queue_prompt_friend_name", "")
                _queue_rerun()
            else:
                # Execute immediately
                st.session_state["queue_pending_moves"] = to_move
