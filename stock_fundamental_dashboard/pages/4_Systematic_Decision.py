...
# NEW — read the exact TTM number the View page saved (with canonical label)
def _from_view_snapshot(stock: str, label: str):
    try:
        d = get_summary_ttm(stock)
        if not d:
            return None
        key = canon(stock, label)
        v = d.get(key)
        return _coerce_float(v)
    except Exception:
        return None
    
# NEW — try reading the ⏱️ TTM KPI card snapshot (uses canonical labels)
def _from_ttm_card(stock: str, label: str):
    try:
        _, vals = get_ttm_kpis(stock)
        key = canon(stock, label)
        return _coerce_float((vals or {}).get(key) or (vals or {}).get(label))
    except Exception:
        return None
    ...
    *,
    ...
    if STRICT_NO_COMPUTE:
        ...
        if key == "revenue_yoy_pct":
            # 0) EXACT match with View ⏱️ TTM KPIs snapshot (what the card shows)
            v = None
            if stock_name:
                v = _from_ttm_card(stock_name, "Revenue YoY (%)") or _from_ttm_card(stock_name, "Revenue YoY")
            if v is not None:
                return v, "View ⏱️ TTM KPIs snapshot", "%"

            # 1) direct YoY saved on Summary (TTM column)
            v = _via("Revenue YoY (%)") or _via("Revenue YoY")
            if v is not None:
                return v, "TTM Summary: Revenue YoY (%)", "%"

            # 2) last resort: FY→FY YoY
    ...
    if key == "revenue_yoy_pct":
        # 0) Try the View ⏱️ TTM KPI card snapshot first (keeps Decision = View)
        v = None
        if stock_name:
            v = _from_ttm_card(stock_name, "Revenue YoY (%)") or _from_ttm_card(stock_name, "Revenue YoY")
        if v is not None:
        ...
        # 1) TTM Summary value (if present)
        v = _via("Revenue YoY (%)") or _via("Revenue YoY")
        if v is not None:
            return v, "TTM Summary: Revenue YoY (%)", "%"

        # 2) Fallback: compute FY→FY YoY
...
def _valuation_breakdown(stock_df: pd.DataFrame, bucket: str, current_price: Optional[float]) -> Tuple[pd.DataFrame, str]:
    ...
    # 1) rules.py aggregator (base values)
    vals = {}
    try:
    ...
    # 2) Hard override from the exact TTM numbers your View page saved (STRICT-safe)
    stock_name = None
    if "Name" in stock_df.columns:
        s_name = stock_df["Name"].dropna()
        if not s_name.empty:
    ...
    def _snap_any(labels: list[str]):
        if not stock_name:
            return None
        for lbl in labels:
            v = _from_view_snapshot(stock_name, lbl)
            if v is not None:
                return float(v)
        return None

    pe_snap   = _snap_any(["P/E (x)", "P/E (×)", "P/E", "PE (x)", "PE"])
    pb_snap   = _snap_any(["P/B (x)", "P/B (×)", "P/B", "PB (x)", "PB", "P/BV", "PBV"])
    fcfy_snap = _snap_any(["FCF Yield (%)", "FCF yield (%)"])
    dy_snap   = _snap_any(["Dividend Yield (%)", "Distribution/Dividend Yield (%)", "Distribution Yield (%)", "DY (%)"])
    pnav_snap = _snap_any(["P/NAV (x)", "P/NAV (×)", "P/NAV"])
    prnav_snap= _snap_any(["P/RNAV (x)", "P/RNAV (×)", "P/RNAV"])

    if pe_snap   is not None: vals["pe"]        = pe_snap
    if pb_snap   is not None: vals["pb"]        = pb_snap
    if fcfy_snap is not None: vals["fcf_yield"] = fcfy_snap
    if dy_snap   is not None: vals["dy"]        = dy_snap
    if pnav_snap is not None: vals["p_nav"]     = pnav_snap
    if prnav_snap is not None: vals["p_rnav"]   = prnav_snap

    # 3) (Optional) If a recomputed TTM Summary exists, let it override too
    sum_df, ttm_col = _ttm_summary(stock_df, bucket)
    if sum_df is not None and ttm_col:
        def via(label: str) -> Optional[float]:
            row = sum_df.loc[sum_df["Metric"] == label, ttm_col]
            s = pd.to_numeric(row, errors="coerce").dropna()
            return float(s.iloc[0]) if not s.empty else None

        def any_of(labels: list[str]) -> Optional[float]:
            for L in labels:
                v = via(L)
                if v is not None:
                    return v
        ...
        pe_ttm   = any_of(["P/E (x)", "P/E (×)", "P/E"])
        pb_ttm   = any_of(["P/B (x)", "P/B (×)", "P/B", "P/BV"])
        fcfy_ttm = any_of(["FCF Yield (%)", "FCF yield (%)"])
        dy_ttm   = any_of(["Dividend Yield (%)", "Distribution/Dividend Yield (%)", "Distribution Yield (%)", "DY (%)"])
        pnav_ttm = any_of(["P/NAV (x)", "P/NAV (×)", "P/NAV"])
        prnav_ttm= any_of(["P/RNAV (x)", "P/RNAV (×)", "P/RNAV"])

        if pe_ttm   is not None: vals["pe"]        = pe_ttm
        if pb_ttm   is not None: vals["pb"]        = pb_ttm
        if fcfy_ttm is not None: vals["fcf_yield"] = fcfy_ttm
        if dy_ttm   is not None: vals["dy"]        = dy_ttm
        if pnav_ttm is not None: vals["p_nav"]     = pnav_ttm
        if prnav_ttm is not None: vals["p_rnav"]   = prnav_ttm
...
if tq.empty:
    ...
    # Friend trade routing
    if "Friend" not in table.columns:
        table.insert(1, "Friend", False)
    if "FriendName" not in table.columns:
        table.insert(2, "FriendName", "")

    # Build dropdown options for FriendName (from existing friend trades + current queue)
    def _collect_existing_friend_names() -> list[str]:
        names = set()
        # from current queue table
        try:
            if isinstance(table, pd.DataFrame) and "FriendName" in table.columns:
                for v in table["FriendName"].tolist():
                    s = str(v or "").strip()
                    if s:
                        names.add(s)
        except Exception:
            pass
        # from friend open/closed trades
        for loader in ("load_friend_open_trades", "load_friend_closed_trades"):
            try:
                if hasattr(ioh, loader):
                    df_ = getattr(ioh, loader)()
                    if isinstance(df_, pd.DataFrame) and "FriendName" in df_.columns:
                        for v in df_["FriendName"].tolist():
                            s = str(v or "").strip()
                            if s:
                                names.add(s)
            except Exception:
                pass
        # sorted, with blank option first
        return [""] + sorted(names, key=lambda x: str(x).lower())

    friend_name_options = _collect_existing_friend_names()
    ...
    edited_q = st.data_editor(
        ...
        column_config={
            ...
            "Friend":       st.column_config.CheckboxColumn("Friend"),
            "FriendName":   (st.column_config.SelectboxColumn("Friend Name", options=friend_name_options) if hasattr(st.column_config, "SelectboxColumn") else st.column_config.TextColumn("Friend Name")),
    ...
    # Friend Name popup: when marking Friend rows live, ask for a name in a dialog (instead of a bottom field)
    def _queue_rerun():
        (getattr(st, "rerun", None) or getattr(st, "experimental_rerun", lambda: None))()

    def _queue_clear_pending():
        st.session_state["queue_friend_prompt_open"] = False
        st.session_state["queue_pending_moves"] = []
        st.session_state["queue_pending_blocked_ids"] = []
        st.session_state["queue_pending_missing_friend_rowids"] = []
    def _queue_execute_pending(friend_name_fill: str = "") -> int:
        moved = 0
        for item in (st.session_state.get("queue_pending_moves") or []):
            rowid = int(item.get("RowId"))
            is_friend = bool(item.get("Friend"))
            fn = str(item.get("FriendName") or "").strip()
            if is_friend and not fn:
                fn = str(friend_name_fill or "").strip()

            if is_friend:
                if not fn:
                    continue
                if hasattr(ioh, "mark_friend_live_row") and ioh.mark_friend_live_row(rowid, fn):
                    moved += 1
            else:
                if ioh.mark_live_row(rowid):
                    moved += 1
        return moved

    # Render popup if needed
    if st.session_state.get("queue_friend_prompt_open") and (st.session_state.get("queue_pending_missing_friend_rowids") or []):
        missing_ids = st.session_state.get("queue_pending_missing_friend_rowids") or []
        if hasattr(st, "dialog"):
            @st.dialog("Friend name required")
            def _friend_name_dialog():
                st.write("Some selected rows are marked as **Friend** but have empty **Friend Name**:")
                st.code(", ".join(map(str, missing_ids)))
                pick = st.selectbox("Choose existing friend (optional)", options=friend_name_options, key="queue_prompt_friend_pick")
                name = st.text_input("Or type a new friend name", key="queue_prompt_friend_name", placeholder="e.g. John / Ah Seng")
                final_name = str(name or "").strip() or str(pick or "").strip()

                b1, b2 = st.columns(2)
                with b1:
                    if st.button("Apply & Mark Live"):
                        if not final_name:
                            st.error("Please enter a friend name.")
                            return
                        moved = _queue_execute_pending(friend_name_fill=final_name)
                        blocked_ids = st.session_state.get("queue_pending_blocked_ids") or []
                        _queue_clear_pending()

                        if moved > 0:
                            st.success(f"Marked live: {moved} row(s).")
                        if blocked_ids:
                            st.warning(
                                f"{len(blocked_ids)} row(s) blocked — plan incomplete (need Entry, Stop, Take, Shares, RR): "
            ...
            # Fallback if running on older Streamlit (no dialog)
            st.warning("Friend name is required for selected Friend row(s): " + ", ".join(map(str, missing_ids)))
            st.selectbox("Choose existing friend (optional)", options=friend_name_options, key="queue_prompt_friend_pick")
            st.text_input("Or type a new friend name", key="queue_prompt_friend_name")

            c_ok, c_cancel = st.columns([1, 1])
            with c_ok:
                if st.button("Apply & Mark Live", key="queue_apply_mark_live_no_dialog"):
                    typed = str(st.session_state.get("queue_prompt_friend_name") or "").strip()
                    picked = str(st.session_state.get("queue_prompt_friend_pick") or "").strip()
                    name = typed or picked
                    if not name:
                        st.error("Please enter a friend name.")
                    else:
                        moved = _queue_execute_pending(friend_name_fill=name)
                        blocked_ids = st.session_state.get("queue_pending_blocked_ids") or []
                        _queue_clear_pending()
            ...
            with c_cancel:
                if st.button("Cancel", key="queue_cancel_no_dialog"):
                    _queue_clear_pending()
                    _queue_rerun()
    ...
    with c1:
        if st.button("✅ Mark Live selected"):
            # Build selection; prompt for Friend name only when missing
            moved, blocked = 0, 0
            blocked_ids = []
            to_move = []
            missing_friend_rowids = []
            ...
            for _, r in edited_q.iterrows():
                ...
                item = {
                    "RowId": int(r.RowId),
                    "Friend": bool(r.get("Friend", False)),
                    "FriendName": str(r.get("FriendName") or "").strip(),
                }
                to_move.append(item)
                if item["Friend"] and not item["FriendName"]:
                    missing_friend_rowids.append(item["RowId"])

            if not to_move and blocked == 0:
                st.info("Nothing selected.")
            elif missing_friend_rowids:
                # Open popup to ask friend name (fills blanks only)
                st.session_state["queue_pending_moves"] = to_move
                st.session_state["queue_pending_blocked_ids"] = blocked_ids
                st.session_state["queue_pending_missing_friend_rowids"] = missing_friend_rowids
                st.session_state["queue_friend_prompt_open"] = True
                st.session_state["queue_prompt_friend_name"] = ""  # reset input when opening prompt
                st.session_state["queue_prompt_friend_pick"] = ""  # reset dropdown when opening prompt
                _queue_rerun()
            else:
                # Execute immediately
                st.session_state["queue_pending_moves"] = to_move
                st.session_state["queue_pending_blocked_ids"] = blocked_ids
                moved = _queue_execute_pending(friend_name_fill="")
                _queue_clear_pending()
