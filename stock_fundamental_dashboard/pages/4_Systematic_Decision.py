...
if tq.empty:
    ...
    # Existing friend names for quick-pick (so trades can be grouped under the same friend)
    def _existing_friend_names() -> list[str]:
        names = set()

        def _add_from_df(df: pd.DataFrame):
            try:
                if df is None or df.empty or "FriendName" not in df.columns:
                    return
                for x in df["FriendName"].tolist():
        ...
        try:
            _add_from_df(ioh.load_friend_open_trades())
        except Exception:
            pass
        try:
            _add_from_df(ioh.load_friend_closed_trades())
        except Exception:
            pass
        try:
            if "FriendName" in tq.columns:
                _add_from_df(tq[["FriendName"]].copy())
        except Exception:
            pass

        return sorted(names, key=lambda s: s.lower())

    friend_name_options = _existing_friend_names()
    ...
    edited_q = st.data_editor(
        ...
        column_config={
            ...
            "FriendName":   st.column_config.SelectboxColumn("Friend Name", options=([""] + friend_name_options), help="Pick an existing friend (or leave blank; you can also set a default friend name below)."),
    ...
    with c3:
        # Default friend name (used only when a Friend row's FriendName is empty)
        pick_friend = st.selectbox(
            "Friend name (pick existing)",
            options=[""] + friend_name_options,
            key="queue_default_friend_pick",
            help="Optional: pick an existing friend quickly.",
        )
        typed_friend = st.text_input(
            "Or type a new friend name",
            key="queue_default_friend_name_typed",
            placeholder="e.g. John / Ah Seng",
        )
        default_friend_name = str(typed_friend or pick_friend or "").strip()
        ...
        if st.button("✅ Mark Live selected"):
            ...
            for _, r in edited_q.iterrows():
                ...
                if not bool(r.get("PlanOK", False)):
                    ...
                    continue
                # Route to Friend Trades if Friend is checked
                if bool(r.get("Friend", False)):
                    fn = str(r.get("FriendName") or default_friend_name or "").strip()
                    if not fn:
                        blocked += 1
                        blocked_ids.append(int(r.RowId))
                        continue
                    if hasattr(ioh, "mark_friend_live_row") and ioh.mark_friend_live_row(int(r.RowId), fn):
                        moved += 1
                else:
                    # normal live
            ...
            if moved > 0:
                ...
                st.warning(
                    f"{blocked} row(s) blocked — plan incomplete OR missing Friend Name (need Entry, Stop, Take, Shares, RR; and Friend Name if Friend ticked): "
