...
# --- Unified OHLC resolver/cleaner (same as Momentum Data) ---
from utils.ohlc_resolver import resolve_ohlc_path, load_ohlc, default_ohlc_dir
...
# ---- OHLC resolver ----
def _resolve_ohlc(name: str) -> tuple[str | None, str]:
    """Return (path, how) using the shared resolver."""
    try:
        p, how = resolve_ohlc_path(name)
        return (str(p) if p else None), how
    except Exception:
