...
# --- RSI (line-only, with 30/70 band)
row_idx = 2
if rsi_series is not None:
    fig.add_trace(go.Scatter(x=df["Date"], y=rsi_series, name="RSI(14)", mode="lines"),
                  row=row_idx, col=1)
    fig.add_hrect(y0=30, y1=70, line_width=0, fillcolor="LightGray", opacity=0.25,
                  row=row_idx, col=1)
