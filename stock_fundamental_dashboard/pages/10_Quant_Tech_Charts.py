...
# --- Sidebar controls ---
# --- Sidebar controls (premium grouped) ---
with st.sidebar:
    st.markdown("### ⚙️ Controls")

    with st.expander("📌 Stocks & Timeframe", expanded=True):
    ...
    with st.expander("📊 Panels", expanded=True):
    ...
    with st.expander("🔔 Signals", expanded=False):
        ...
        st.caption("Signals are cues only — not guarantees.")

    st.markdown("---")
    # Quick actions (visual only; doesn't touch your trading logic)
    c1, c2 = st.columns(2)
    with c1:
...
# Premium: readable timeframe label
...
# ----------------------- Premium: Quick overview strip -----------------------
# (UI only — does not change your indicator logic)
try:
    _last_date = df["Date"].iloc[-1]
    _prev_close = float(price_close.iloc[-2]) if len(df) > 1 else None
    _last_close = float(price_close.iloc[-1]) if len(df) > 0 else None
    _chg = (_last_close - _prev_close) if (_last_close is not None and _prev_close) else None
    _chg_pct = (_chg / _prev_close * 100.0) if (_chg is not None and _prev_close) else None

    _trend = "Bull" if float(ema10.iloc[-1]) >= float(ema30.iloc[-1]) else "Bear"
    _trend_tone = "good" if _trend == "Bull" else "bad"

    _rsi_last = float(rsi_series.iloc[-1]) if (rsi_series is not None and len(rsi_series) > 0) else None
    _atr_last = float(atr_series.iloc[-1]) if (atr_series is not None and len(atr_series) > 0) else None
    _vol_last = float(df["Volume"].iloc[-1]) if ("Volume" in df.columns and len(df) > 0 and pd.notna(df["Volume"].iloc[-1])) else None

    quick_items = [
        {"label": "Symbol", "value": str(main_name).upper(), "badge": title_tf},
        {"label": "Last Candle", "value": _last_date.strftime("%Y-%m-%d"), "badge": freq},
        {"label": "Close", "value": _fmt_px(_last_close), "badge": (f"{_chg_pct:+.2f}%" if _chg_pct is not None else "") , "tone": ("good" if (_chg_pct is not None and _chg_pct >= 0) else ("bad" if _chg_pct is not None else ""))},
        {"label": "Trend", "value": _trend, "badge": "EMA10/30", "tone": _trend_tone},
        {"label": "RSI(14)", "value": (f"{_rsi_last:.1f}" if _rsi_last is not None else "—"), "badge": "14"},
        {"label": "ATR(14)", "value": (_fmt_px(_atr_last) if _atr_last is not None else "—"), "badge": "14"},
    ]
except Exception:
    quick_items = []
...
fig_main.update_layout(
    ...
    xaxis=dict(showspikes=True, rangeslider=dict(visible=False)),
...
# ----------------------- Tabs -----------------------
# Premium quick stats (top strip)
if quick_items:
    render_stat_cards(quick_items, columns=6)
...
with tab_chart:
    _render_kpis(df)
    st.plotly_chart(fig_main, use_container_width=True, key="main_chart", config={"displayModeBar": "hover", "displaylogo": False})
    if fig_rsi is not None:
        st.plotly_chart(fig_rsi, use_container_width=True, key="rsi_chart", config={"displayModeBar": "hover", "displaylogo": False})
    if fig_atr is not None:
        st.plotly_chart(fig_atr, use_container_width=True, key="atr_chart", config={"displayModeBar": "hover", "displaylogo": False})
    if fig_obv is not None:
        st.plotly_chart(fig_obv, use_container_width=True, key="obv_chart", config={"displayModeBar": "hover", "displaylogo": False})
...
with st.expander("🔎 Data source details", expanded=False):
    st.write("Resolved OHLC file:", f"`{p0}`" if p0 else "—")
    if p0 and os.path.exists(p0):
        st.write("File last modified:", pd.to_datetime(mtime0, unit="ns") if mtime0 else "—")
    st.write("Cache etag:", etag)
    if not df0.empty:
        st.write("Daily OHLC last date:", df0["Date"].iloc[-1].strftime("%Y-%m-%d"))
