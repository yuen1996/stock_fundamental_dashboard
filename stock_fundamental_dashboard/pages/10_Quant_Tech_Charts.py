...
# ----------------------- Tabs -----------------------
tab_overview, tab_signals = st.tabs(["📈 Overview", "🔔 Signals"])
...
with tab_overview:
    ...
    # --- Indicators (below main chart) ---
    if (fig_rsi is not None) or (fig_atr is not None) or (fig_obv is not None):
        st.markdown(section("Indicators", "RSI / ATR / OBV (based on the selected timeframe)", ""), unsafe_allow_html=True)

        # RSI full width (most readable)
        if fig_rsi is not None:
            st.plotly_chart(fig_rsi, use_container_width=True, key="rsi_chart")

        # ATR + OBV side-by-side when both enabled
        if (fig_atr is not None) and (fig_obv is not None):
            c1, c2 = st.columns(2)
            with c1:
