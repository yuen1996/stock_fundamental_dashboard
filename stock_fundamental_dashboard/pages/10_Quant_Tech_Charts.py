...
def _load_ohlc(name: str, _etag: int, _file_mtime_ns: int) -> pd.DataFrame | None:
    """Load OHLC CSV with tolerant filename matching; return tidy DataFrame.
    Cache busts on bus etag AND on the CSV file's mtime.
    """
    try:
...
# ----------------------- Chart helpers -----------------------
def _clean_ohlc_for_plot(open_s: pd.Series, high_s: pd.Series, low_s: pd.Series, close_s: pd.Series):
    """Remove impossible OHLC (<=0) which would force the y-axis to include 0 and push candles to the top."""
    ...
    bad = (o <= 0) | (h <= 0) | (l <= 0) | (c <= 0)
    if bad.any():
        o = o.mask(bad); h = h.mask(bad); l = l.mask(bad); c = c.mask(bad)
    return o, h, l, c

def _compute_padded_range(low_s: pd.Series, high_s: pd.Series):
    """Return (ymin, ymax) with padding; None if not computable."""
    try:
        pmin = float(np.nanmin(pd.to_numeric(low_s, errors="coerce").values))
        pmax = float(np.nanmax(pd.to_numeric(high_s, errors="coerce").values))
    except Exception:
        return None
    if not np.isfinite(pmin) or not np.isfinite(pmax) or pmax <= 0:
        return None
    if pmax <= pmin:
        pad = max(pmax * 0.03, 0.01)
    else:
        pad = max((pmax - pmin) * 0.10, pmax * 0.02)
    return (max(pmin - pad, 0), pmax + pad)
...
p0 = _ohlc_path_for(main_name)
mtime0 = int(os.stat(p0).st_mtime_ns) if (p0 and os.path.exists(p0)) else 0
df0 = _load_ohlc(main_name, etag, mtime0)
...
# Clean impossible OHLC (<=0) so the price axis doesn't snap to 0
price_open, price_high, price_low, price_close = _clean_ohlc_for_plot(price_open, price_high, price_low, price_close)
...
# Keep candles visually centered (avoid huge empty space when bad 0 values exist)
_price_range = _compute_padded_range(price_low, price_high)
if _price_range:
    fig_main.update_yaxes(range=list(_price_range), row=1, col=1, secondary_y=False)
...
with tab_chart:
    ...
    if fig_atr is not None:
        st.plotly_chart(fig_atr, use_container_width=True, key="atr_chart")
    if fig_obv is not None:
        st.plotly_chart(fig_obv, use_container_width=True, key="obv_chart")
