...
if not stocks:
    ...
    st.stop()
# --- Premium: show which file is used (helps debug stale dates) ---
with st.expander("🔎 Data source details", expanded=False):
    st.write({
        "selected": main_name,
        "resolved_path": p0,
        "file_mtime_ns": mtime0,
        "bus_etag": _bus_etag,
        "data_version_etag": _data_etag(),
        "effective_etag": etag,
        "last_date_in_df": str(df0["Date"].iloc[-1].date()) if ("Date" in df0.columns and not df0.empty) else None,
