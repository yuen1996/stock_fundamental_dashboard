...
# === MAIN FIGURE (Price/EMA/Volume + optional ATR/OBV) ===
main_rows = 1 + int(atr_series is not None) + int(obv_series is not None)
specs = [[{"secondary_y": True}]] + [[{"secondary_y": False}] for _ in range(main_rows - 1)]
row_heights = [0.60] + [0.40 / (main_rows - 1) for _ in range(main_rows - 1)] if main_rows > 1 else [0.70]

fig_main = make_subplots(
    rows=main_rows, cols=1, shared_xaxes=True, vertical_spacing=0.035,
...
if df["Volume"].notna().sum() > 0:
    ...
    fig_main.update_yaxes(title_text="Price", row=r_price, col=1, secondary_y=False)
    fig_main.update_yaxes(title_text="Vol",   row=r_price, col=1, secondary_y=True)

# ATR row (if any)
next_row = 2
if atr_series is not None:
    fig_main.add_trace(go.Scatter(x=df["Date"], y=atr_series, name="ATR(14)", mode="lines"),
                       row=next_row, col=1)
    fig_main.update_yaxes(title_text="ATR", row=next_row, col=1)
    next_row += 1

# OBV row (if any)
if obv_series is not None:
    fig_main.add_trace(go.Scatter(x=df["Date"], y=obv_series, name="OBV", mode="lines"),
                       row=next_row, col=1)
    fig_main.update_yaxes(title_text="OBV", row=next_row, col=1)
...
fig_main.update_layout(
    height=700 if main_rows > 1 else 560,
    margin=dict(l=6, r=6, t=10, b=70),
    xaxis=dict(showspikes=True),
    legend=dict(orientation="h", yanchor="top", y=-0.18, xanchor="left", x=0)
)

# === RSI FIGURE (pure line-only; separate figure so nothing can leak in) ===
fig_rsi = None
if rsi_series is not None:
    ...
    fig_rsi.update_layout(height=180, margin=dict(l=6, r=6, t=10, b=10))
...
if _rb:
    ...
    if fig_rsi is not None:
        fig_rsi.update_xaxes(rangebreaks=_rb)

# Backwards-compatibility alias so downstream code using `fig` still works.
fig = fig_main
