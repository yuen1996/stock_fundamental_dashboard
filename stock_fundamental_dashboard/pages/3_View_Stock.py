...
# === Price override + source helpers (DROP-IN) ===============================

PRICE_OVR_KEY = "price_overrides"

import math

def _resolve_price_with_source(rows: pd.DataFrame) -> tuple[float, str]:
    """
    Return (price_value, source_label) by scanning common columns.
    """
    for k in ("CurrentPrice", "EndQuarterPrice", "Price", "SharePrice"):
        s = rows.get(k)
        if s is not None and s.dropna().size:
...
def _get_effective_price(stock_name: str, rows: pd.DataFrame) -> tuple[float, str]:
    """
    Use a session-state override if present; else the latest stored price with its source.
    """
    st.session_state.setdefault(PRICE_OVR_KEY, {})
    ovr = st.session_state[PRICE_OVR_KEY].get(stock_name)
    if isinstance(ovr, (int, float)) and np.isfinite(float(ovr)) and float(ovr) > 0:
        return float(ovr), "Override"
    v, src = _resolve_price_with_source(rows)
    return (v if np.isfinite(v) else float("nan")), (src or "—")
# =============================================================================
...
def _render_compare(a: str, b: str):
    ...
    price_a, src_a = _get_effective_price(a, sub_a)
    price_b, src_b = _get_effective_price(b, sub_b)


    # ---------- quick facts header ----------
    def _facts(sub, nm):
        p, src = _get_effective_price(nm, sub)
        return [
            ("Current Price", f"{p:,.4f}" if np.isfinite(p) else "—"),
            ("Price Source",  src or "—"),
    ...
    facts_a = _facts(sub_a, a)
    facts_b = _facts(sub_b, b)
...
with st.expander(name, expanded=True):
    ...
    price_now, price_src = _get_effective_price(name, stock)
    ...
    # --- Price: show source + allow two-way override (applies live to all KPIs) ---
    with st.container(border=True):
        colp1, colp2, colp3 = st.columns([1.2, 1.8, 1])
        with colp1:
            st.markdown("**Price source**")
            st.caption(price_src or "—")
        with colp2:
            _default_price = float(price_now) if (isinstance(price_now, (int,float)) and np.isfinite(price_now)) else 0.0
            new_price = st.number_input(
                "Set/override price (used for P/E, DY, MOS, etc.)",
                value=_default_price, min_value=0.0, step=0.0001, format="%.4f",
                key=f"price_edit_{safe_name}"
            )
        with colp3:
            apply = st.button("Apply", key=f"apply_price_{safe_name}", use_container_width=True)
            clear = st.button("Clear", key=f"clear_price_{safe_name}", type="secondary", use_container_width=True)

        if apply and new_price and np.isfinite(float(new_price)) and float(new_price) > 0:
            st.session_state.setdefault(PRICE_OVR_KEY, {})
            st.session_state[PRICE_OVR_KEY][name] = float(new_price)
            st.success(f"Using override price {float(new_price):,.4f} for all valuation KPIs.")
            st.rerun()

        if clear and name in st.session_state.get(PRICE_OVR_KEY, {}):
            st.session_state[PRICE_OVR_KEY].pop(name, None)
            st.info("Cleared price override — back to data column value.")
            st.rerun()

    st.caption(
        f"Valuation KPIs on this page use **{(price_now if np.isfinite(price_now) else float('nan')):,.4f}** "
        f"from **{price_src or '—'}**."
    )
