...
with st.expander(name, expanded=True):
    ...
    with tabs[0]:
        ...
        def _ohlc_dir_etag() -> int:
            ...
            try:
                return int(bus.etag("ohlc"))          # ← key change
            except Exception:
                try:
        ...
        def _ohlc_path_for(stock_name: str) -> str | None:
            """Exact match first; else tolerant alias + relaxed prefix (both directions)."""
            ...
            try:
                ...
                for cand in _alias_candidates(stock_name):
                    ...
                    target = f"{cand_lower}.csv"
                    if target in files:
                        return files[target]
                    # relaxed prefix — match longer or shorter names either way
                    for fname, full in files.items():
                        root = fname[:-4]  # strip ".csv"
                        if root.startswith(cand_lower) or cand_lower.startswith(root):
                            return full
            except Exception:
                pass
            return None
        ...
        def _load_ohlc_for(stock_name: str, _etag: int) -> pd.DataFrame | None:
            ...
            # Accept Adj Close if Close is absent
