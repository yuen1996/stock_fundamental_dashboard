...
for stock_name in stocks:
    with st.expander(stock_name, expanded=False):
        ...
        # ---- Current Price metric (prefer LIVE price; then quarterly; then annual)
cur_val = None

# 1) LIVE current price if present (best for TTM P/E & P/S)
if "CurrentPrice" in stock.columns:
    s = stock["CurrentPrice"].dropna()
    if not s.empty:
        cur_val = s.iloc[-1]

# 2) Fallback: latest QUARTERLY price (your dataset label)
if cur_val is None or float(cur_val) == 0.0:
    for qprice_col in ["Q_EndQuarterPrice", "Q_SharePrice", "Each end per every quarter price", "Price"]:
        if qprice_col in stock.columns:
            s = stock[qprice_col].dropna()
            if not s.empty:
                cur_val = s.iloc[-1]
                break

# 3) Fallback: last ANNUAL price (end-of-year snapshot)
if cur_val is None or float(cur_val) == 0.0:
