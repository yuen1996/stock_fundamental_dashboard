...
def _show_styled(styler, height=360):
    """
    Render a pandas Styler in a sandboxed iframe for consistent horizontal
    scrolling and to avoid any stray HTML (like a lone </div>).
    """
    from streamlit.components.v1 import html as html_component

    # Get HTML from the Styler (or fall back to the raw DataFrame)
    try:
...
<html>
  <head>
    ...
    <style>
      html, body {{
        margin: 0; padding: 6px; background: #ffffff;
...
"""
    # Render inside an iframe with scrolling
    html_component(page, height=max(200, int(height)), scrolling=True)
