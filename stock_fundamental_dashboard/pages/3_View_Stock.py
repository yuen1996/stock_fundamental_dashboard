...
for stock_name in stocks:
    with st.expander(stock_name, expanded=False):
        ...
        with tabs[0]:
            ...
            # Toggle the lookback window for CAGR/Est EPS
            win_label = st.radio(
            ...
            if len(years) >= 2:
                # Always anchor on latest year; pick the earliest year available within the last N years
                last_year = years[-1]
                ...
                period = max(1, last_year - first_year)  # protect division if data gaps

                # Pull latest + window-start values from the existing tables
                ...
                if eps_first is not None and not pd.isna(eps_first) and eps_first > 0 and eps_last is not None:
                    cagr_eps = ((eps_last / eps_first) ** (1/period) - 1) * 100
                else:
                    cagr_eps = None

                # Simple linear projection of EPS for the chosen horizon
                if (eps_first is not None and not pd.isna(eps_first)
                    ...
                    est_eps_ny     = eps_last + avg_eps_growth * win
                else:
                    avg_eps_growth = None
                    est_eps_ny     = None
                ...
                # Graham value & MOS using the latest EPS and EPS CAGR
                graham_val = eps_last * (8.5 + 2 * (cagr_eps or 0)) if (eps_last is not None and not pd.isna(eps_last)) else None
                ...
                c2.metric(
                    f"Est EPS ({win}-yr)",
                    f"{float(est_eps_ny):,.4f}" if (est_eps_ny is not None and np.isfinite(float(est_eps_ny))) else "N/A"
                )
