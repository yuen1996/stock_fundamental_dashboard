    ...
    with st.expander(stock_name, expanded=False):
        ...
        with tabs[0]:
            ...
            # === Snowflake & Momentum (inside the same Annual tab) ===
                        ...
                        # Build ohlc_latest → feeds Momentum (robust: 365-day date fallback)
            ohlc_latest = None
            try:
                try:
                ...
                if oh is not None and not oh.empty:
                    ...
                    if not df.empty:
                        ...
                        # MA200 needs ≥200 trading rows
                        ma200 = float(close.rolling(200, min_periods=200).mean().iloc[-1]) if len(close) >= 200 else None

                        # 12-month return:
                        #   1) try exact index look-back (~252 trading days)
                        #   2) else use date-based 365-day fallback (first row on/after cutoff)
                        ret_12m = None
            ...
            # Momentum messages (so you see why the spoke might be 0)
