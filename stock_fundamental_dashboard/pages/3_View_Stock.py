...
def _drag_labels(labels, key_suffix):
    ...
    IMPORTANT: pass a unique key to avoid DuplicateWidgetID.
    """
...
def multi_panel_charts(count, options, x_labels, series_getter, key_prefix):
    """
    Render up to 4 charts (each single-select). Arranged 1–4 in a 2x2 grid.
    """
    count = max(1, min(4, int(count)))
    # Prepare selectors defaults
    option_labels = [o[0] for o in options]
    for i in range(count):
        # Build selection UI in grid
        if count <= 2:
        ...
        with container:
            ...
            if options:
                ...
                if y is None or (pd.isna(pd.Series(y)).all()):
                    ...
                    plot_single_series(x_labels, y, sel, sel)
        ...
        with tabs[0]:
            ...
            if ann_numeric.empty:
                ...
                if ann_raw_layout.startswith("Years"):
                    ...
                    # remember current X order for charts later
                    ...
                    disp_num = disp_num[new_cols]
                    # remember X as Years order (index)
            ...
            if ratio_df.empty:
                ...
                if ratio_layout.startswith("Years"):
                    ...
                    # remember X labels and available metrics
                ...
                # ---- Undervalue/Overvalue Bar (unchanged)
                ...
                # ===== NEW: Place ALL annual comparison charts *under* the bar =====
                ...
                years_order = st.session_state.get(f"ann_raw_x_{stock_name}",
                ...
                ann_count = st.slider("Number of raw-data charts", min_value=1, max_value=4, value=2,
                                      key=f"ann_raw_chartcount_{stock_name}")
                multi_panel_charts(
                    ann_count,
                    ann_opts,
                    years_order,
                ...
                ratio_x = st.session_state.get(f"ann_ratio_x_{stock_name}",
                                               [str(int(y)) for y in ratio_df.index.tolist()])
                ratio_metrics = st.session_state.get(f"ann_ratio_metrics_{stock_name}", list(ratio_df.columns))

                def ratio_series_getter(metric_name):
                    if metric_name not in ratio_df.columns:
                        return None
                    y = ratio_df[metric_name]
                    return pd.Series(y).reindex([str(x) for x in ratio_x]).values

                ratio_count = st.slider("Number of ratio charts", min_value=1, max_value=4, value=2,
                                        key=f"ann_ratio_chartcount_{stock_name}")
                multi_panel_charts(
                    ratio_count,
                    [(m, m) for m in ratio_metrics],
                    ratio_x,
                    ratio_series_getter,
...
st.caption("Annual comparison charts are now under the Undervalue/Overvalue Bar. Drag the chips above tables to reorder columns. Raw Data still moves empty rows to the bottom.")
