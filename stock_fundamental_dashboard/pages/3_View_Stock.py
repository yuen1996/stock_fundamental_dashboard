...
/* Drag area container */
.sort-help { color:#475569; font-size:.90rem; margin:.25rem 0 .35rem 0; }
.sort-box {
  border:1px dashed #cbd5e1; background:#f8fafc; border-radius:12px;
  padding:.25rem .4rem;               /* tighter padding */
  max-height:120px; overflow:auto;    /* limit height + scroll */
}

/* ---- Compact pill styling ---- */
/* For streamlit-sortables (generic DOM draggable) */
.sort-box [draggable="true"]{
  display:inline-flex !important;     /* inline pills */
  width:auto !important;
  border-radius:9999px !important;
  background:#eef2ff !important;      /* soft color */
  border:1px solid #c7d2fe !important;
  color:#0f172a !important;
  font-size:11px !important;          /* smaller */
  padding:2px 6px !important;         /* smaller */
...
/* Ensure the list wraps instead of stacking full width */
...
def drag_reorder(columns, key_suffix, help_text="Drag to set column order. Left-most = first."):
    ...
    # Compact toggle instead of expander – avoids Streamlit 1.26 expander issues
    ...
    if show:
        ...
        # Keep current order if the user doesn't open the drag area
        new_label_order = labels

    # Map labels back to original column objects
