...
with st.expander(name, expanded=True):
    ...
    with tabs[0]:
        ...
        def _val_for_tile(label: str):
            ...
            v = kpis_master.get(canon)
            if v is not None:
                return v
            # fall back to your previous logic (Summary row → raw TTM formula bits)
            ...
            if row is not None and (ttm_col in row):
                try:
                    return float(row[ttm_col])
                except Exception:
                    return None
            # raw fallbacks you had before (keep as last resort)
            L = label.strip().lower()
            if L in ("revenue","gross profit","operating profit","ebitda","net profit","eps","dps"):
