...
# ─── Patch: compute quarterly EPS so Growth · EPS YoY works ───
stock = stock.copy()  # avoid pandas SettingWithCopyWarning
stock["Q_EPS"] = (
    pd.to_numeric(stock["Q_NetProfit"], errors="coerce")
    / pd.to_numeric(stock["Q_NumShares"], errors="coerce")
)
# ─────────────────────────────────────────────────────────────
