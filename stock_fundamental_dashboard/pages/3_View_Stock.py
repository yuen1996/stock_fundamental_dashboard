...
_GRANDP = os.path.abspath(os.path.join(_THIS, ".", "."))   # repo root

for p in (_PARENT, _GRANDP):
    if p not in sys.path:
        sys.path.insert(0, p)

# try package-level imports, fallback to utils/ if moved
try:
...
import streamlit as st  # <<< MUST come before any st.* calls

# (compat shim for old st_aggrid that expects components.components.MarshallComponentException)
try:
    import streamlit.components.v1 as _stc
    # old st_aggrid does: components.components.MarshallComponentException
    if not hasattr(_stc, "components"):
        _stc.components = _stc
    # make sure MarshallComponentException exists on BOTH the module and the alias
    if not hasattr(_stc, "MarshallComponentException"):
        class MarshallComponentException(Exception):
            pass
        _stc.MarshallComponentException = MarshallComponentException
    if not hasattr(_stc.components, "MarshallComponentException"):
        _stc.components.MarshallComponentException = _stc.MarshallComponentException
except Exception:
    pass
...
# ---- Page setup (keep this RIGHT after importing streamlit) ----
...
def _ag_height(fullscreen: bool, df: pd.DataFrame, base: int = 420) -> int:
    return (_auto_height(df) if fullscreen else base)

def show_aggrid_ratio(df_in: pd.DataFrame, *, metrics_in_columns: bool, fullscreen: bool, grid_key: str):
    df = df_in.copy()
    # if metrics are rows, expose index as 'Metric' column so JS can read which rule to use
    metric_col = None
    if not metrics_in_columns:
        df = df.reset_index()
        metric_col = df.columns[0]  # name of the index column (e.g., "Metric")

    gb = GridOptionsBuilder.from_dataframe(df)
    gb.configure_default_column(resizable=True, sortable=True, filter=True)
    gb.configure_grid_options(ensureDomOrder=True)

    # JS color function when metric is determined by the ROW (metrics in index)
    if not metrics_in_columns:
        rules_map = {k: [d, float(g), float(o)] for k, (d, g, o, _) in THRESHOLDS.items()}
        rules_js = json.dumps(rules_map)
        js = JsCode(f"""
        function(params){{
          var v = parseFloat(params.value);
          if (isNaN(v)) return {{}};
          var rules = {rules_js};
          var mname = params.data['{metric_col}'];
          var arr = rules[mname];
          if (!arr) return {{}};
          var dir = arr[0], great = arr[1], ok = arr[2];
          var state = null;
          if (dir === "ge") state = (v >= great) ? "good" : ((v >= ok) ? "ok" : "bad");
          else              state = (v <= great) ? "good" : ((v <= ok) ? "ok" : "bad");
          var colors = {{ "good":"#16a34a22", "ok":"#3b82f622", "bad":"#ef444422" }};
        ...
        for c in df.columns:
            if c == metric_col: 
                ...
                continue
            gb.configure_column(c, cellStyle=js)

    # JS color when metric is determined by the COLUMN (metrics in columns)
    else:
        tpl = """
        function(params){
          var v = parseFloat(params.value);
          if (isNaN(v)) return {};
          var dir = "%s", great = %s, ok = %s;
          var state = null;
          if (dir === "ge") state = (v >= great) ? "good" : ((v >= ok) ? "ok" : "bad");
          else              state = (v <= great) ? "good" : ((v <= ok) ? "ok" : "bad");
          var colors = { "good":"#16a34a22", "ok":"#3b82f622", "bad":"#ef444422" };
        ...
        for c in df.columns:
            rule = THRESHOLDS.get(c)
            if rule:
                d, g, o, _ = rule
                gb.configure_column(c, cellStyle=JsCode(tpl % (d, float(g), float(o))))

    go = gb.build()
    AgGrid(
        df, gridOptions=go,
        height=_ag_height(fullscreen, df, base=360),
...
def show_aggrid_raw(df_in: pd.DataFrame, *, periods_in_columns: bool, fullscreen: bool, grid_key: str):
    # highlight BIG jump between latest and previous period (≥100% and material)
    df = df_in.apply(pd.to_numeric, errors="coerce").copy()

    gb = GridOptionsBuilder.from_dataframe(df.reset_index() if not periods_in_columns else df)
    gb.configure_default_column(resizable=True, sortable=True, filter=True)
    gb.configure_grid_options(ensureDomOrder=True)

    if periods_in_columns:
        if len(df.columns) >= 2:
            prev_col, last_col = str(df.columns[-2]), str(df.columns[-1])
            # spike per-row flag
            med = np.nanmedian(np.abs(df.values), axis=1)
            floor = np.maximum(1e-12, 0.05 * med)   # 5% of median
            prev = df.iloc[:, -2].astype(float).values
            last = df.iloc[:, -1].astype(float).values
            base = np.maximum(np.abs(prev), floor)
            pct  = (last - prev) / base
            spike = (np.isfinite(pct)) & (pct >= 1.0) & (np.abs(last - prev) >= floor)
            df_view = df.copy()
            df_view["__SPIKE__"] = spike
            gb = GridOptionsBuilder.from_dataframe(df_view)
            gb.configure_default_column(resizable=True, sortable=True, filter=True)
            gb.configure_column("__SPIKE__", hide=True)
            js = JsCode(f"""
            function(params){{
              var field = params.colDef.field;
              var sp = params.data["__SPIKE__"] === true;
              if (!sp) return {{}};
              if (field === "{last_col}" || field === "{prev_col}") {{
                return {{ "backgroundColor":"#f59e0b33", "border":"1px solid #f59e0b" }};
              }}
              return {{}};
            }}""")
            for c in df.columns:
            ...
            go = gb.build()
            AgGrid(
                df_view, gridOptions=go,
                height=_ag_height(fullscreen, df_view),
                fit_columns_on_grid_load=False,
                allow_unsafe_jscode=True,
                theme="balham",
                key=grid_key,
            ...
            AgGrid(df, gridOptions=gb.build(),
                   height=_ag_height(fullscreen, df), fit_columns_on_grid_load=False,
                   allow_unsafe_jscode=True, theme="balham", key=grid_key)
    else:
        # periods are ROWS → detect spike per column (compare last vs prev row)
        if len(df.index) >= 2:
            prev_row, last_row = df.index[-2], df.index[-1]
            flagged = {}
            for c in df.columns:
                a = float(df.loc[prev_row, c]) if pd.notna(df.loc[prev_row, c]) else np.nan
                b = float(df.loc[last_row, c]) if pd.notna(df.loc[last_row, c]) else np.nan
                if not np.isfinite(a) or not np.isfinite(b): 
                    flagged[str(c)] = False
                    continue
                med = np.nanmedian(np.abs(df[c].values))
                floor = max(1e-12, 0.05 * (med if np.isfinite(med) else 0.0))
                base = max(abs(a), floor)
                pct = (b - a) / base
                flagged[str(c)] = (np.isfinite(pct) and pct >= 1.0 and abs(b - a) >= floor)

            df_view = df.reset_index()
            mask_last = (np.asarray(df.index) == last_row)
            mask_prev = (np.asarray(df.index) == prev_row)
            df_view["__IS_LAST__"] = mask_last.astype(bool)
            df_view["__IS_PREV__"] = mask_prev.astype(bool)


            gb = GridOptionsBuilder.from_dataframe(df_view)
            gb.configure_default_column(resizable=True, sortable=True, filter=True)
            gb.configure_column("__IS_LAST__" , hide=True)
            gb.configure_column("__IS_PREV__" , hide=True)

            flagged_js = json.dumps(flagged)
            js = JsCode(f"""
            function(params){{
              var flagged = {flagged_js};
              var field = params.colDef.field;
              var isLast = (params.data["__IS_LAST__"] === true);
              var isPrev = (params.data["__IS_PREV__"] === true);
              if ((isLast || isPrev) && flagged[field] === true){{
                return {{ "backgroundColor":"#f59e0b33", "border":"1px solid #f59e0b" }};
              }}
              return {{}};
            }}""")
            # pin the index columns on the left
            idx0 = df_view.columns[0]
            gb.configure_column(idx0, pinned="left")
            for c in df.columns:
                gb.configure_column(str(c), cellStyle=js)
            ...
            AgGrid(df.reset_index(), gridOptions=gb.build(),
                   height=_ag_height(fullscreen, df), fit_columns_on_grid_load=False,
                   allow_unsafe_jscode=True, theme="balham", key=grid_key)
    ...
    with st.expander(stock_name, expanded=False):
        ...
        with tabs[0]:
            ...
            if ann_numeric.empty:
                ...
                full_ann_raw = st.checkbox("Full screen height", value=False, key=f"ann_raw_full_{stock_name}")
                show_aggrid_raw(disp_num, periods_in_columns=ann_raw_layout.startswith("Years"), fullscreen=full_ann_raw, grid_key=f"ann_raw_{stock_name}")
                st.caption("⚠️ Alerts: latest vs previous period ≥100%% jump and material (≥5%% of median level).")
            ...
            if ratio_df.empty:
                ...
                if ratio_layout.startswith("Years"):
                    ...
                    full_ann_ratio = st.checkbox("Full screen height", value=False, key=f"ann_ratio_full_{stock_name}")
                    show_aggrid_ratio(disp, metrics_in_columns=ratio_layout.startswith("Metrics"), fullscreen=full_ann_ratio, grid_key=f"ann_ratio_{stock_name}")
...
# ---- Quarterly Raw table: Full-height / Interactive (resizable) option
full_h_q_raw = st.checkbox("Full height (no vertical scroll)", value=False, key=f"q_raw_full_{stock_name}")
inter_q_raw  = st.checkbox("Interactive table (resizable columns, no colors/tooltips)", value=False, key=f"q_raw_inter_{stock_name}")

if inter_q_raw:
    st.dataframe(disp_qnum, use_container_width=True, height=_auto_height(disp_qnum) if full_h_q_raw else 420)
else:
    if q_raw_layout.startswith("Periods"):
    ...
    _show_styled(styled_q, height=9999 if full_h_q_raw else 420)
    ...
    if qratio_df.empty:
        ...
        if qratio_layout.startswith("Periods"):
            ...
            full_h_q_ratio = st.checkbox("Full height (no vertical scroll)", value=False, key=f"q_ratio_full_{stock_name}")
            inter_q_ratio  = st.checkbox("Interactive table (resizable columns, no colors/tooltips)", value=False, key=f"q_ratio_inter_{stock_name}")

            if inter_q_ratio:
                st.dataframe(disp_qratio, use_container_width=True, height=_auto_height(disp_qratio) if full_h_q_ratio else 360)
            else:
                _show_styled(styled, height=9999 if full_h_q_ratio else 360)
