...
_VALID_PRICE_SRCS = {"Momentum", "Add/Edit", "Auto"}
...
def _norm_price_src(v, *, default="Momentum") -> str:
    ...
    if s in {"add/edit", "addedit", "add edit", "manual"}:
        return "Add/Edit"
    if s in {"auto", "automatic", "default"}:
        return "Auto"
    # anything else (None, "None", "", unexpected) → default
    return default

def _get_price_src_prefs() -> dict:
    """{ safe_stock_key -> 'Momentum' | 'Add/Edit' | 'Auto' } (normalized)"""
    d = _load_settings()
...
def _resolve_price_for_view(stock_name: str, stock_rows: pd.DataFrame, source_preference: str):
    ...
    Returns (price, asof_date, source_used)
      source_preference: "Momentum" | "Add/Edit" | "Auto"
    """
    pref = (source_preference or "Auto").strip().lower()
    ...
    # Momentum / Auto (prefer Momentum; fall back to Add/Edit if missing)
    mom, asof = _momentum_close_before_today(stock_name, stock_rows)
    if mom is not None:
...
def compute_ttm_kpis_for_tiles(
    ...
    *,
    ...
    that comes from `_resolve_price_for_view(...)` (Momentum/Add-Edit/Auto),
...
with st.expander(name, expanded=True):
    ...
    if hasattr(st, "segmented_control"):
        src_pref = st.segmented_control(
            "Price source",
            options=["Momentum", "Add/Edit", "Auto"],
            selection_mode="single",
            key=_price_src_key,   # value already valid in session_state
        )
    else:
        src_pref = st.radio(
            "Price source",
