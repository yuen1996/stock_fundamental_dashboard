...
def _get_price_src_prefs() -> dict:
    """{ safe_stock_key -> 'Momentum' | 'Add/Edit' | 'Auto' }"""
    d = _load_settings()
    v = d.get("price_src_by_stock") or {}
...
def _set_price_src_pref(stock_name: str, value: str) -> None:
    ...
    prefs[_safe_key(stock_name)] = str(value)
...
def _render_compare(a: str, b: str):
    ...
    def _facts(sub):
        ...
        src_pref = st.session_state.get(sp_key, saved or "Momentum")
...
with st.expander(name, expanded=True):
    ...
    _price_src_key = f"price_src_{safe_name}"
    _saved_pref = _get_price_src_prefs().get(_safe_key(name))

    # Seed the widget once from disk
    if _price_src_key not in st.session_state and _saved_pref:
        st.session_state[_price_src_key] = _saved_pref
    ...
    if hasattr(st, "segmented_control"):
        src_pref = st.segmented_control(
            ...
            default=st.session_state.get(_price_src_key, _saved_pref or "Momentum"),
            key=_price_src_key
        )
    else:
        current = st.session_state.get(_price_src_key, _saved_pref or "Momentum")
        src_pref = st.radio(
            "Price source",
            options=["Momentum", "Add/Edit", "Auto"],
            index=["Momentum","Add/Edit","Auto"].index(current),
            horizontal=True,
            key=_price_src_key
        )
