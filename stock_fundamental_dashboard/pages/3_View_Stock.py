...
def _drag_labels(labels, key_suffix):
    ...
    if DRAG_LIB == "sortables":
        # streamlit-sortables returns the list in the new order
        from streamlit_sortables import sort_items  # local import to satisfy some hot-reloaders
        with st.container():
    ...
    if DRAG_LIB == "draggable-list":
        # DraggableList expects list[dict{id, order, name}] and returns list in new order
        ...
        if isinstance(result, list):
            # If list of dicts with 'name', keep that order; else convert to str
            if result and isinstance(result[0], dict) and "name" in result[0]:
                return [d["name"] for d in result]
            return [str(x) for x in result]
        return labels

    # No drag library — show a hint, keep order
...
st.caption("Drag the chips above any table to reorder columns. Raw Data still moves empty rows to the bottom.")
