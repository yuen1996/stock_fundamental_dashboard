...
COLORS = {
    "good": "rgba(22,163,74,0.13)",
    "ok":   "rgba(59,130,246,0.13)",
    "bad":  "rgba(239,68,68,0.13)",
}
...
def _show_styled_raw(styler, height=420):
    """
    Render a pandas Styler with its inline <td style="…"> intact
    by bypassing Streamlit’s Markdown sanitizer.
    """
    # 1) Grab the raw HTML (includes inline-cell styles)
    raw_html = styler.to_html()

    # 2) (Optional) Inject wrapper CSS to match your existing table look
    wrapper_css = """
    <style>
      :root {--border:#e5e7eb; --hover:#f8fafc;}
      table {border-collapse:collapse; width:max-content; min-width:100%;}
      th, td {border:1px solid var(--border) !important; padding:8px 10px; white-space:nowrap;}
      thead th {position:sticky; top:0; background:#f9fafb; z-index:5;}
      tbody tr:nth-child(2n) td {background:#fcfcfd;}
      tbody tr:hover td {background:var(--hover);}
    </style>
    """

    # 3) Render un-sanitized
    st_html(wrapper_css + raw_html, height=height + 30)
        ...
        with tabs[1]:
            ...
            if qratio_df.empty:
                ...
                if qratio_layout.startswith("Periods"):
                    ...
                    _show_styled_raw(styled, height=360)
                    st.caption("Legend: 🟩 Great  •  🟦 OK  •  🟥 Fails threshold")
