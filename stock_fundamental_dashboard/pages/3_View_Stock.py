...
for stock_name in stocks:
    with st.expander(stock_name, expanded=False):
        ...
        # ---- Current Price metric (prefer live; then quarterly; then annual)
        cur_val = None

        # 1) live/current price if present
        if "CurrentPrice" in stock.columns:
            s = stock["CurrentPrice"].dropna()
            if not s.empty:
                cur_val = s.iloc[-1]

        # 2) latest quarterly price (your dataset uses this label)
        ...
        # 3) fall back to last annual SharePrice
        if cur_val is None or float(cur_val) == 0.0:
