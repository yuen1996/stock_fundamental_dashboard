...
def _resolve_ohlc_dir() -> str:
    ...
    candidates = [
        os.path.abspath(os.path.join(_PARENT, "data", "ohlc")),   # Momentum writes here
...
def _load_ohlc_for_name(stock_name: str, _etag: int, *, ticker: str | None = None) -> pd.DataFrame | None:
    """
    Load OHLC CSV for a stock using tolerant name/ticker matching.
    """
...
with st.expander(name, expanded=True):
    ...
    with tabs[0]:
        ...
        # ---- Momentum helpers: robust OHLC resolver (uses utils/name_link) ----
        def _resolve_ohlc_dir() -> str:
            # First existing path wins
            candidates = [
                os.path.abspath(os.path.join(_PARENT, "data", "ohlc")),   # project root (Momentum writes here)
                os.path.abspath(os.path.join(os.getcwd(), "data", "ohlc")),
                os.path.abspath(os.path.join(_THIS, "..", "data", "ohlc")),
                os.path.abspath(os.path.join(_GRANDP, "data", "ohlc")),
            ]
        ...
        def _ohlc_dir_etag() -> int:
            """Bust cache when Momentum page bumps the OHLC bus tag; fall back to dir mtime."""
            try:
        ...
        def _load_ohlc_for(stock_name: str, _etag: int, ticker: str | None = None) -> pd.DataFrame | None:
            """
            Load OHLC CSV for a stock using tolerant name matching + optional ticker hint.
            """
            path = name_link.find_ohlc_path(stock_name, ohlc_dir=OHLC_DIR, ticker=ticker)
            if not path or not os.path.exists(path):
                return None
        ...
        def _get_momentum(label: str):
            ...
            # 3) OHLC CSV fallback
            # try to extract a ticker/code from the selected stock rows, if present
            ticker_hint = None
            for col in ("Ticker","Code","Symbol"):
            ...
            df_px = _load_ohlc_for(name, _ohlc_dir_etag(), ticker=ticker_hint)
