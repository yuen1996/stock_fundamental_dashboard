...
def _load_ohlc_for_name(stock_name: str, _etag: int, *, ticker: str | None = None) -> pd.DataFrame | None:
    ...
    Cache is keyed by bus.etag('ohlc') via _etag.
    """
    try:
        # unified resolver (same behavior as Add/Edit page)
        # if your resolver supports ticker hints, you can pass ticker=ticker
    ...
    date_col = next((lower.get(c) for c in ("date", "timestamp", "time", "datetime") if c in lower), None)
...
with st.expander(name, expanded=True):
    ...
    with tabs[0]:
        ...
        # --- FIX: force TTM Dividend Yield (%) to use DPS_TTM / current price ---
        try:
            if sum_df is not None and not sum_df.empty and "Metric" in sum_df.columns:
                # find the TTM column (e.g. "TTM 2026")
                ttm_col = next(
                    (c for c in sum_df.columns if isinstance(c, str) and c.upper().startswith("TTM")),
                    None,
                )
                if ttm_col:
                    metrics = sum_df["Metric"].astype(str)

                    # locate "Dividend Yield (%)" row (case-insensitive; allow unitless "Dividend Yield")
                    dy_mask = metrics.str.strip().str.lower().eq("dividend yield (%)")
                    if not dy_mask.any():
                        dy_mask = metrics.str.strip().str.lower().eq("dividend yield")

                    if dy_mask.any():
                        row_idx = sum_df.index[dy_mask][0]

                        # 1) TTM DPS from quarters (central helper)
                        dps_ttm = None
                        try:
                            ttm_raw = ttm_raw_row_from_quarters(qtr, current_price=price_now) or {}
                            dps_ttm = ttm_raw.get("DPS")
                        except Exception:
                            dps_ttm = None

                        # 2) Fallback: latest annual DPS
                        if (
                            dps_ttm is None
                            or not isinstance(dps_ttm, (int, float))
                            or not math.isfinite(dps_ttm)
                        ):
                            if isinstance(annual, pd.DataFrame) and not annual.empty and "DPS" in annual.columns:
                                s_dps = pd.to_numeric(annual["DPS"], errors="coerce").dropna()
                                if not s_dps.empty:
                                    dps_ttm = float(s_dps.iloc[-1])

                        # 3) If we have DPS_TTM and a valid price, overwrite only the TTM cell
                        if (
                            isinstance(dps_ttm, (int, float))
                            and math.isfinite(dps_ttm)
                            and isinstance(price_now, (int, float))
                            and math.isfinite(price_now)
                            and price_now != 0
                        ):
                            dy_ttm = (float(dps_ttm) / float(price_now)) * 100.0
                            # write into the Industry Summary TTM column (e.g. "TTM 2026")
                            sum_df.at[row_idx, ttm_col] = dy_ttm
        except Exception:
            # never break the page because of this cosmetic fix
            pass
        # --- END FIX ---
