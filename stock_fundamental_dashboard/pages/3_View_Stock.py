...
def _render_compare(a: str, b: str):
    ...
    def _facts(sub):
        ...
        src_pref = st.session_state.get(sp_key, "Momentum")
...
with st.expander(name, expanded=True):
    ...
    # --- Price source switch (per-stock; remembered in session) ---
    _price_src_key = f"price_src_{safe_name}"
    if hasattr(st, "segmented_control"):
        src_pref = st.segmented_control("Price source",
                                        options=["Momentum", "Add/Edit", "Auto"],
                                        selection_mode="single",
                                        default=st.session_state.get(_price_src_key, "Momentum"),
                                        key=_price_src_key)
    else:
        # Fallback widget
        src_pref = st.radio("Price source", options=["Momentum", "Add/Edit", "Auto"],
                            index=["Momentum","Add/Edit","Auto"].index(st.session_state.get(_price_src_key, "Momentum")),
                            horizontal=True, key=_price_src_key)
