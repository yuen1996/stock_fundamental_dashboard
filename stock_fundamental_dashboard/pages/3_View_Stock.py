...
for stock_name in stocks:
    with st.expander(stock_name, expanded=False):
        ...
        df["Raw"] = df["Raw"].apply(
            lambda x: "–" if x is None or pd.isna(x) else f"{float(x):,.4f}"
        )
        df["Score"] = df["Score"].apply(
            lambda x: "–" if x is None or pd.isna(x) else f"{int(round(float(x))):d}"
        )
        st.dataframe(
            ...
            use_container_width=True, height=320
                        ...
                        st.info("Diagnostic data not available (old cache or compute error).")
