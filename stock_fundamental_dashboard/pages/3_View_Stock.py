...
def compute_ttm_kpis_for_tiles(annual_df, quarterly_df, bucket: str,
    ...
    # ---------- Use ONE price for all TTM math (prefer latest quarter/current price) ----------
    try:
    ...
    try:
        sum_df = calculations.build_summary_table(
            ...
            bucket=bucket,
            include_ttm=True,
            # make Summary’s TTM use the SAME price the raw TTM used
            price_fallback=(raw_ttm.get("Price") or price_now)
        )
    except Exception:
        sum_df = None
    ...
    # RAW-first tiles with improved fallback order and opt-out of FY
    for tile, key in raw_first.items():
        ...
        if tile == "EPS (RM)" and v is None:
            ...
            if v is not None and src is None:
                src = "raw_ttm"  # derived from raw_ttm inputs

        values[tile] = v
        sources[tile] = src

    # SUMMARY-first ratios
...
with st.expander(name, expanded=True):
    ...
    with tabs[0]:
        ...
        kpis_master, kpi_sources, kpi_fy_year = compute_ttm_kpis_for_tiles(
            annual, qtr, bucket, strict_no_fy_fallback=STRICT_TTM_ONLY
        )
