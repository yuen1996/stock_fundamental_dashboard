...
.sort-box { border:1px dashed #94a3b8; padding:.35rem .5rem; border-radius:10px; background:#f8fafc; }
</style>
"""
...
def nice_label(col):
    """Readable label for a column (handles MultiIndex column as 'Section • Field')."""
...
def drag_reorder(columns, key_suffix, help_text="Drag to set column order. Left-most = first."):
    """Show a draggable list of column labels, return same column objects in new order."""
...
def field_options(sections):
    """Return list of (label, (section,label)) for selects."""
    opts = []
...
def plot_single_series(x_values, y_values, title, yname):
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=x_values, y=y_values, mode="lines+markers", name=yname))
    fig.update_layout(margin=dict(l=10, r=10, t=40, b=10), title=title, xaxis_title="", yaxis_title="")
    st.plotly_chart(fig, use_container_width=True)

def pick_and_plot_two_panels(
    left_label, right_label, options, x_labels, series_getter, key_prefix
):
    """
    Show two side-by-side charts. Each chart has a single-select from `options` and
    uses `series_getter(opt)` -> 1D numeric array/Series aligned to x_labels.
    """
    c1, c2 = st.columns(2)

    with c1:
        sel_left = st.selectbox(
            left_label,
        ...
        if options:
            # find the tuple payload for sel_left
            payload = dict(options)[sel_left]
            y = series_getter(payload)
            if y is None or (pd.isna(pd.Series(y)).all()):
                st.info("No data for this selection.")
            else:
                plot_single_series(x_labels, y, sel_left, sel_left)

    with c2:
        sel_right = st.selectbox(
            right_label,
            options=[o[0] for o in options],
            index=min(1, len(options)-1) if len(options) > 1 else 0 if options else None,
            key=f"{key_prefix}_right",
        )
        if options:
            payload = dict(options)[sel_right]
            y = series_getter(payload)
            if y is None or (pd.isna(pd.Series(y)).all()):
                st.info("No data for this selection.")
            else:
                plot_single_series(x_labels, y, sel_right, sel_right)
...
for stock_name in stocks:
    with st.expander(stock_name, expanded=False):
        ...
        with tabs[0]:
            ...
            # ---- Raw Data with layout toggle + empties-last + DRAG column ordering
            ...
            if ann_numeric.empty:
                ...
                if ann_raw_layout.startswith("Years"):
                    # Columns = Years (already); Rows = Fields
                    disp_num = _reorder_empty_last(ann_numeric)
                    # Drag to reorder Year columns
                    ...
                    # Columns = Fields (MultiIndex); Rows = Years
                    disp_num = _reorder_empty_last(ann_numeric.T)   # rows=Years
                    field_cols = disp_num.columns.tolist()          # list of tuples (Section, Field)
                    new_cols = drag_reorder(
                        field_cols,
                ...
                # ==== NEW: Raw Data comparison charts (two panels) ====
                st.markdown("##### 📈 Compare Raw Data (Year‑over‑Year)")
                # options from ANNUAL_SECTIONS
                ann_opts = field_options(ANNUAL_SECTIONS)
                years = new_cols if ann_raw_layout.startswith("Years") else disp_num.index.astype(str).tolist()
                # series_getter: given (sec, lbl) -> numeric series aligned to 'years'
                def ann_series_getter(sec_lbl):
                    ...
                    # align to 'years' order
                    return pd.Series(y).reindex([str(x) for x in years]).values
                pick_and_plot_two_panels(
                    "Chart A – pick a field", "Chart B – pick a field",
                    ann_opts, years, ann_series_getter, f"annual_raw_chart_{stock_name}"
                )

            # ---- Calculated Ratios with layout toggle + DRAG column ordering
            ...
            if ratio_df.empty:
                ...
                if ratio_layout.startswith("Years"):
                    ...
                    # ==== NEW: Ratios comparison charts (two panels) ====
                    st.markdown("##### 📈 Compare Ratios (Year‑over‑Year)")
                    years_r = new_cols
                    metrics = disp.index.tolist()
                    def ratio_series_getter(metric):
                        if metric not in disp.index:
                            return None
                        return pd.Series(disp.loc[metric, :]).reindex(years_r).values
                    pick_and_plot_two_panels(
                        "Chart A – pick a ratio", "Chart B – pick a ratio",
                        [(m, m) for m in metrics], years_r, ratio_series_getter, f"annual_ratio_chart_{stock_name}_years"
                    )
                else:
                    # Columns = Metrics; Rows = Years
                    disp = ratio_df
                    new_cols = drag_reorder(
                        disp.columns.tolist(),
                    ...
                    # ==== NEW: Ratios comparison charts (two panels) ====
                    st.markdown("##### 📈 Compare Ratios (Year‑over‑Year)")
                    years_r = disp.index.astype(str).tolist()
                    metrics = new_cols
                    def ratio_series_getter(metric):
                        if metric not in disp.columns:
                            return None
                        return pd.Series(disp[metric]).reindex(disp.index).values
                    pick_and_plot_two_panels(
                        "Chart A – pick a ratio", "Chart B – pick a ratio",
                        [(m, m) for m in metrics], years_r, ratio_series_getter, f"annual_ratio_chart_{stock_name}_metrics"
                    )
        ...
        with tabs[1]:
            ...
            # ---- Raw Quarterly Data with layout toggle + empties-last + DRAG column ordering
            ...
            if q_numeric.empty:
                ...
                if q_raw_layout.startswith("Periods"):
                    ...
                    # Columns = Fields (MultiIndex); Rows = Periods
                ...
                # ==== NEW: Quarterly Raw Data comparison charts (two panels) ====
                st.markdown("##### 📈 Compare Raw Data (Quarterly Periods)")
                q_opts = field_options(QUARTERLY_SECTIONS)
                periods = new_cols if q_raw_layout.startswith("Periods") else disp_qnum.index.astype(str).tolist()
                def q_series_getter(sec_lbl):
                    ...
                    return pd.Series(y).reindex(periods).values
                pick_and_plot_two_panels(
                    "Chart A – pick a quarterly field", "Chart B – pick a quarterly field",
                    q_opts, periods, q_series_getter, f"quarter_raw_chart_{stock_name}"
                )

            # ---- Quarterly ratios with layout toggle + DRAG column ordering
            ...
            if qratio_df.empty:
                ...
                if qratio_layout.startswith("Periods"):
                    ...
                    # ==== NEW: Quarterly Ratios comparison charts (two panels) ====
                    st.markdown("##### 📈 Compare Ratios (Quarterly Periods)")
                    periods_r = new_cols
                    ratios_list = disp_qratio.index.tolist()
                    def qratio_series_getter(metric):
                        if metric not in disp_qratio.index:
                            return None
                        return pd.Series(disp_qratio.loc[metric, :]).reindex(periods_r).values
                    pick_and_plot_two_panels(
                        "Chart A – pick a ratio", "Chart B – pick a ratio",
                        [(m, m) for m in ratios_list], periods_r, qratio_series_getter, f"quarter_ratio_chart_{stock_name}_periods"
                    )
                else:
                    # Columns = Metrics; Rows = Periods
                    ...
                    # ==== NEW: Quarterly Ratios comparison charts (two panels) ====
                    st.markdown("##### 📈 Compare Ratios (Quarterly Periods)")
                    periods_r = disp_qratio.index.astype(str).tolist()
                    ratios_list = new_cols
                    def qratio_series_getter(metric):
                        if metric not in disp_qratio.columns:
                            return None
                        return pd.Series(disp_qratio[metric]).reindex(disp_qratio.index).values
                    pick_and_plot_two_panels(
                        "Chart A – pick a ratio", "Chart B – pick a ratio",
                        [(m, m) for m in ratios_list], periods_r, qratio_series_getter, f"quarter_ratio_chart_{stock_name}_metrics"
                    )

st.caption("Tables keep your drag order. Below each table you now have two single‑select charts to compare any two fields/ratios over time (YoY or by quarter).")
