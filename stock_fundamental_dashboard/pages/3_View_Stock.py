...
def style_ratio_table(df_in: pd.DataFrame) -> "pd.io.formats.style.Styler":
    """
    Apply green/blue/red background based on THRESHOLDS whether metrics are
    in the columns *or* in the index (works for both orientations).
    Also builds tooltips explaining the rating.
    """
    df = df_in.copy()
    ...
    for r in df.index:
        row_rule = THRESHOLDS.get(r, None)  # if metrics are in index
        for c in df.columns:
            col_rule = THRESHOLDS.get(c, None)  # if metrics are in columns
            # prefer column rule; otherwise use row rule
            rule = col_rule if col_rule is not None else row_rule
            ...
            if state:
                ...
                if rule:
                    ...
                    label = c if col_rule is not None else r
                    try:
                        vtxt = f"{float(val):,.4f}"
                    except Exception:
                        vtxt = str(val)
                    tips.loc[r, c] = f"{label}: {vtxt} → {state.upper()} (good {bound}, OK {mid}). {note}"
...
def _show_styled(styler, height=420):
    """
    Render a pandas Styler in an iframe that matches Streamlit's modern
    DataFrame look-and-feel: sticky header, borders, hover, zebra rows,
    and horizontal scroll. No stray HTML will leak into the page.
    """
    from streamlit.components.v1 import html as html_component

    # Get HTML from the Styler (fall back to raw DataFrame if needed)
    try:
        html_table = styler.to_html()
    except Exception:
        try:
            html_table = styler.data.to_html()
        except Exception:
            # Last resort: show as a native dataframe (no coloring)
            st.dataframe(getattr(styler, "data", styler), use_container_width=True, height=height)
            return
...
<html>
  <head>
    ...
    <style>
      ...
      /* Card wrapper like Streamlit's grid container */
      .wrap {{
        border:1px solid var(--border);
        border-radius:12px;
        box-shadow:var(--shadow);
      ...
      /* Put the table inside a scroll pane, enable horizontal scroll */
      .scroll {{
        overflow-x:auto;
        overflow-y:auto;
      ...
      /* Make the table look like the modern grid */
      table {{
        border-collapse:separate;
        border-spacing:0;
        width:max-content;               /* allow horizontal scroll when wide */
        min-width:100%;
      }}
      thead th {{
        position:sticky; top:0; z-index:5;
        background:#f9fafb;              /* subtle header bg like Streamlit */
      ...
      th, td {{
        border-bottom:1px solid var(--border);
        padding:8px 10px;
        white-space:nowrap;              /* keep year/metric labels on one line */
      }}
      ...
      /* Keep the first (index) column visible when scrolling horizontally */
      tbody th {{
        position:sticky; left:0; z-index:4; background:#fff;
        border-right:1px solid var(--border);
      }}
      thead th:first-child {{
        position:sticky; left:0; z-index:6; background:#f9fafb;
        border-right:1px solid var(--border);
      }}

      /* Pandas Styler outputs a surrounding <style> and classes — keep them */
      .pd-styler {{
        width:max-content;
      }}
    </style>
  ...
  <body>
    <div class="wrap">
      <div class="scroll">{html_table}</div>
    </div>
  </body>
</html>
"""
...
for stock_name in stocks:
    with st.expander(stock_name, expanded=False):
        ...
        with tabs[0]:
            ...
            if show_cf:
                ...
                with r2c4:
                    st.metric("Market Cap (est.)", _fmt2((ttm.get("P/S (TTM)") or 0) * (ttm.get("TTM Revenue") or 0), 0))
                       ...
                       # === FORCE place Snowflake INSIDE the Annual tab ===
            with tabs[0]:

                           # === Snowflake & Momentum (inside the same Annual tab) ===
            # Build ohlc_latest → feeds Momentum
            ohlc_latest = None
            try:
                try:
                    import io_helpers as _ioh  # project-root import
                except Exception:
                    from utils import io_helpers as _ioh  # utils/ fallback

                oh = _ioh.load_ohlc(stock_name)
                if oh is not None and not oh.empty and "Close" in oh.columns:
                    close = pd.to_numeric(oh["Close"], errors="coerce").dropna()
                    if not close.empty:
                        ...
                        ret_12m = (
                            float(price / float(close.iloc[-252]) - 1.0)
                            if len(close) >= 252 and close.iloc[-252] not in (0, np.nan)
                            else None
                        )
            ...
            # Momentum messages (so you see why the spoke is 0)
            if ohlc_latest is None:
                ...
                if any(d.get("Input","").startswith("EPS YoY %") and d.get("Score") is None for d in det):
                ...
                if dh.get("missing"):
                    if st.checkbox("Show missing inputs affecting metrics",
                                   key=f"dh_missing_{stock_name}"):
                        for m in dh["missing"]:
                            st.write("-", m)


                                # ---- Show per-metric breakdown (raw, score, source) ---------
                if st.checkbox("ℹ️  Show Snowflake calculation detail",
                               key=f"sf_detail_{stock_name}"):
                    ...
                    if detail:
                        ...
                        df["Raw"] = df["Raw"].apply(
                            lambda x: "–"
                            if x is None or pd.isna(x) else f"{float(x):,.4f}"
                        )
                        df["Score"] = df["Score"].apply(
                            lambda x: "–"
                            if x is None or pd.isna(x) else f"{int(round(float(x))):d}"
                        )
                        ...
                        for _, row in df.iterrows():
                            ...
                            if comp:
                                st.markdown(
                                    f"<details>"
                                    f"<summary><b>{row['Pillar']} · {row['Input']} components</b></summary>",
                                    unsafe_allow_html=True,
                                )
