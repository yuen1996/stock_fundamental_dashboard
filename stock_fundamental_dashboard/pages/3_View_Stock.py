...
for stock_name in stocks:
    with st.expander(stock_name, expanded=False):
        ...
        # --- 🚦 Systematic Decision (per-stock; placed below charts) ---
        ...
        if not annual_rows.empty:
            ...
            with st.expander(f"🚦 Systematic Decision — {stock_name}", expanded=False):
                strategy = st.selectbox(
                    "Strategy",
                    list(rules_engine.RULESETS.keys()),
                    key=f"{stock_name}_strategy"
                )
                min_score = st.slider(
                    "Minimum score",
                    0, 100, 60, 5,
                    key=f"{stock_name}_minscore"
                )
            ...
            st.info("No annual data available for systematic decision.")
