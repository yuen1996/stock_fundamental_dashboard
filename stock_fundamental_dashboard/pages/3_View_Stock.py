...
# ─────────────────────────────────────────
# Snowflake — Overall Performance (5 Pillars)
# ─────────────────────────────────────────
try:
    ...
    # Simple overall composite (equal-weighted)
    ...
    st.info("Upgrade needed: calculations.compute_factor_scores(.) not found. I can add it for you.")

# Data Health badge (from TTM)
dh = (ttm or {}).get("DataHealth", {})
ok = not dh.get("missing")
st.markdown(f"**Data Health:** {'✅ OK' if ok else '⚠️ Incomplete'}")
if dh.get("missing"):
    with st.expander("Missing inputs affecting metrics"):
        for m in dh["missing"]:
            st.write("-", m)
