...
def _load_ohlc_for_name(stock_name: str, _etag: int, *, ticker: str | None = None) -> pd.DataFrame | None:
    """
    Load OHLC CSV for a stock using the shared resolver in utils.name_link.
    Falls back to filename candidates if the smart matcher can't find a path.
    Cache is keyed by bus.etag('ohlc') via the _etag argument.
    """
    # 1) Resolve the canonical OHLC directory
    try:
        ohlc_dir = name_link.resolve_ohlc_dir()
    except Exception:
        # ultra-safe fallback: old local candidates logic
        candidates = [
        ...
        ohlc_dir = next((d for d in candidates if os.path.isdir(d)), candidates[0])
        os.makedirs(ohlc_dir, exist_ok=True)

    # 2) Try the tolerant matcher (handles aliases like “Maybank” ⇄ “Malayan Banking”)
    path = None
    try:
    ...
    # 3) Filename guesses as last resort
    if not path:
        path = next((p for p in _candidate_paths(ohlc_dir, stock_name, ticker) if os.path.exists(p)), None)
        if not path:
            return None

    # 4) Parse CSV → tidy DataFrame
    try:
...
def _momentum_close_before_today(stock_name: str, stock_rows: pd.DataFrame) -> tuple[float | None, pd.Timestamp | None]:
    ...
    available trading day STRICTLY BEFORE today. Weekends/holidays handled
    automatically by taking the latest row < today.
    """
    ticker_hint = None
...
with st.expander(name, expanded=True):
    ...
    # === 🧪 Derivatives Health — paste this block right above the "📚 Financial Reports" section ===
    try:
        from utils.derivatives import DerivInputs, compute_deriv_health
    except Exception:
        DerivInputs = None
        compute_deriv_health = None

    st.markdown(
        section("🧪 Derivatives Health", "Quick risk read from annual report derivatives notes"),
        unsafe_allow_html=True
    )

    if (DerivInputs is None) or (compute_deriv_health is None):
        st.info("Add utils/derivatives.py to enable this section.")
    else:
        import os as _os
        import pandas as _pd
        # Try prefill from CSV: <repo-root>/data/derivatives/derivatives_notes.csv
        # Expected columns:
        # ticker,total_notional,equity,deriv_pnl_ttm,ebit_ttm,collateral_posted,top1_counterparty_notional,level3_fv,total_deriv_fv
        _csv = _os.path.join(_GRANDP, "data", "derivatives", "derivatives_notes.csv")
        _prefill = {}
        if _os.path.exists(_csv):
            try:
                _dfd = _pd.read_csv(_csv)
                if "ticker" in _dfd.columns:
                    # match by Name or ticker/code/symbol from your stock rows
                    _hints = [str(name)]
                    for _col in ("Ticker", "Code", "Symbol"):
                        if _col in stock.columns and stock[_col].dropna().size:
                            _hints.append(str(stock[_col].dropna().iloc[-1]))
                    _m = _pd.Series(False, index=_dfd.index)
                    for _h in _hints:
                        _m = _m | (_dfd["ticker"].astype(str).str.upper() == str(_h).upper())
                    _hit = _dfd[_m]
                    if not _hit.empty:
                        _prefill = _hit.iloc[0].to_dict()
            except Exception as _e:
                st.caption(f"Could not read derivatives CSV: {_e}")
        ...
        # Pull defaults from CSV (None if missing)
        _defaults = {
            "total_notional": _num_or_none(_prefill.get("total_notional")),
            "equity": _num_or_none(_prefill.get("equity")),
            "deriv_pnl_ttm": _num_or_none(_prefill.get("deriv_pnl_ttm")),
            "ebit_ttm": _num_or_none(_prefill.get("ebit_ttm")),
            "collateral_posted": _num_or_none(_prefill.get("collateral_posted")),
            "top1_counterparty_notional": _num_or_none(_prefill.get("top1_counterparty_notional")),
            "level3_fv": _num_or_none(_prefill.get("level3_fv")),
            "total_deriv_fv": _num_or_none(_prefill.get("total_deriv_fv")),
        }

        # Helper: treat 0.0 from the UI as "unknown" (None) so it doesn't look deceptively safe
        def _none_if_zero(x):
            try:
                f = float(x)
        ...
        with st.expander("Edit inputs (from derivatives note / CSV)"):
            st.caption("Tip: Leave as 0.0 if unknown; this panel treats 0.0 as ‘unknown’ (⚪), not as a real zero.")
            c1, c2, c3 = st.columns(3)
            v_total_notional = c1.number_input("Total notional", value=float(_defaults["total_notional"] or 0.0), step=1.0, key=f"dN_{safe_name}")
            v_equity         = c2.number_input("Equity",          value=float(_defaults["equity"] or 0.0), step=1.0, key=f"dE_{safe_name}")
            v_deriv_pnl_ttm  = c3.number_input("Derivatives P&L (TTM)", value=float(_defaults["deriv_pnl_ttm"] or 0.0), step=1.0, key=f"dP_{safe_name}")
            c4, c5, c6 = st.columns(3)
            v_ebit_ttm      = c4.number_input("EBIT (TTM)", value=float(_defaults["ebit_ttm"] or 0.0), step=1.0, key=f"dEB_{safe_name}")
            v_collateral    = c5.number_input("Collateral posted", value=float(_defaults["collateral_posted"] or 0.0), step=1.0, key=f"dC_{safe_name}")
            v_top1_cp       = c6.number_input("Top-1 counterparty notional", value=float(_defaults["top1_counterparty_notional"] or 0.0), step=1.0, key=f"dCP_{safe_name}")
            c7, c8 = st.columns(2)
            v_level3_fv     = c7.number_input("Level-3 FV", value=float(_defaults["level3_fv"] or 0.0), step=1.0, key=f"dL3_{safe_name}")
            v_total_fv      = c8.number_input("Total derivatives FV", value=float(_defaults["total_deriv_fv"] or 0.0), step=1.0, key=f"dFV_{safe_name}")

        _inp = DerivInputs(
            total_notional=_none_if_zero(v_total_notional),
            equity=_none_if_zero(v_equity),
            deriv_pnl_ttm=_none_if_zero(v_deriv_pnl_ttm),
            ebit_ttm=_none_if_zero(v_ebit_ttm),
            collateral_posted=_none_if_zero(v_collateral),
            top1_counterparty_notional=_none_if_zero(v_top1_cp),
            level3_fv=_none_if_zero(v_level3_fv),
            total_deriv_fv=_none_if_zero(v_total_fv),
        )

        _res = compute_deriv_health(_inp)

        # Render results
        st.markdown(f"**Overall:** {_res['overall']}")
        cols = st.columns(len(_res["metrics"]))
        for i, (label, value, light, unit) in enumerate(_res["metrics"]):
            with cols[i]:
                if value is None:
                    st.metric(label=f"{light} {label}", value="N/A")
                else:
                    if unit == "×":
                        txt = f"{float(value):,.2f}×"
                    else:
                        txt = f"{float(value) * 100:,.2f}%"
                    st.metric(label=f"{light} {label}", value=txt)

        st.caption(
            "Rules of thumb: Notional/Equity ≤ 3× (green), 3–5× (yellow), >5× (red); "
            "Derivatives P&L/EBIT ≤10% (green), 10–20% (yellow), >20% (red); "
            "Collateral/Equity ≤10% (green), 10–20% (yellow), >20% (red); "
            "Top-1 counterparty share ≤40% (green), 40–50% (yellow), >50% (red); "
            "Level-3 FV share ≤30% (green), 30–60% (yellow), >60% (red)."
        )
    # === end Derivatives Health block ===
    ...
    with tabs[0]:
        ...
        def _val_for_tile(label: str):
            ...
            # raw fallbacks (last resort)
            if L in ("revenue","gross profit","operating profit","ebitda","net profit","eps","dps"):
                return ttm_raw.get(label)

            if L == "dividend yield" and np.isfinite(ttm_raw.get("DPS", np.nan)) and np.isfinite(ttm_raw.get("Price", np.nan)):
        ...
        for label in ttm_items:
            ...
            shown = (_fmt_pct(val) if is_pct else _fmt_num(val))
                ...
                if L in ("revenue","gross profit","operating profit","ebitda","net profit","eps","dps"):
                    ...
                    if (last_eps is None or not np.isfinite(float(last_eps))) and isinstance(annual, pd.DataFrame):
                        if "EPS" in annual.columns:
                            ...
                            sh_s = pd.to_numeric(annual["Shares"], errors="coerce").replace(0, np.nan).dropna()  # <- ignore zeros
                            if not np_s.empty and not sh_s.empty:
                                last_eps = float(np_s.iloc[-1]) / float(sh_s.iloc[-1])  # safe: shares != 0
        ...
        def _mom_change_pct_df(df: pd.DataFrame, months: int = 12) -> float | None:
            ...
            if not prior.empty:
                ...
                # ~1 year of trading days as a len-based fallback
                if len(df) < 200:
                    return None
