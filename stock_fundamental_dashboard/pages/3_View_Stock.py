...
def _show_styled(styler, height=360):
    """
    Render a pandas Styler safely across Streamlit versions.

    1) Try native Styler rendering (gives built-in horizontal scroll).
    2) If that fails, render as HTML inside a horizontally scrollable div
       (so you still get left-right scrolling + tooltips/colors).
    3) As a last resort, show the raw DataFrame without styles.
    """
    # Try Streamlit native rendering of Styler
    try:
    ...
    # Fallback: HTML with horizontal scroll
    try:
        html = styler.to_html()
        wrapper = f"""
        <div style="
    ...
    # Last resort: unstyled data
    try:
        st.dataframe(styler.data, use_container_width=True, height=height)
    except Exception:
        st.write(styler.data)
