...
def _ttm_agg_for(category: str, key: str, label: str) -> str:
    """
    Decide how to aggregate last-4 quarters for a given field.
    Returns one of: "sum" (flows), "last" (stocks/levels), "mean" (averages).
    """
    cat = (category or "").strip()
    txt = (label or key or "").strip().lower()

    # Clear-cut by category first
    if cat in ("Income Statement", "Cash Flow"):
        return "sum"
    if cat == "Balance Sheet":
        # balance-sheet stocks carry the period-end level
        # but allow "(Avg)" / "Average ..." items to be averaged
        if "avg" in txt or txt.startswith("average "):
            return "mean"
        return "last"

    # "Other" / "Industry KPIs" — decide by name
    # flows explicitly marked as TTM-like
    if ("ttm" in txt) or ("new orders" in txt) or ("dividend per share" in txt) or ("dpu" in txt):
        return "sum"

    # averages
    if ("(avg" in txt) or (" avg" in txt) or txt.startswith("average "):
        return "mean"

    # ratios / percentages / rates — carry forward latest reading
    if any(w in txt for w in ["ratio", "%", "margin", "yield", "rate", "nim", "lcr", "nsfr", "cir", "casa",
                              "occupancy", "wale", "reversion", "hedged debt"]):
        return "last"

    # levels / counts — carry latest
    if any(w in txt for w in ["shares", "units outstanding", "price", "store count", "subscribers",
                              "fleet size", "room count", "teu throughput"]):
        return "last"

    # default to last (safer than summing an unknown)
    return "last"
...
def _ttm_dict_from_quarters(q_df: pd.DataFrame, bucket: str) -> dict:
    ...
    for category, items in schema.items():
        for it in items:
            ...
            how = _ttm_agg_for(category, key, label)
            if how == "sum":
...
def _ttm_kpis_to_yearfirst_json(*, labels: list[str], sum_df: pd.DataFrame | None,
    ...
    for label in (labels or []):
        ...
        # 2) raw TTM fallbacks for common items
...
with st.expander(name, expanded=True):
    ...
    with tabs[0]:
            ...
            if label in cand_map:
                for k in cand_map[label]:
                    if k in annual.columns:
                        ...
                        if s.notna().sum() >= 2:
                            years_raw = pd.to_numeric(annual["Year"], errors="coerce")
                            return pd.Series({int(y): float(v) for y, v in zip(years_raw, s)})
            return None
        ...
        with st.expander(f"📈 CAGR / MOS / PEG — Diagnostics  —  missing: {cg_miss_cnt} · trace rows: {cg_trace_cnt}", expanded=False):
            ...
            try:
                ...
                payload_cg_yf = _cagr_results_to_yearfirst_json(
                    items=cagr_list, N=int(win), end_basis=end_basis,
        ...
        # Use the already-chosen toggles from the CAGR panel if present
        use_ttm_state = st.session_state.get(f"cagr_use_ttm_{safe_name}", True)
        N = int(st.session_state.get(f"cagr_win_{safe_name}", 5))
        ...
        def _resolve_ohlc_dir() -> str:
            ...
            for d in candidates:
                if os.path.isdir(d):
                    return d
            # default to project-root path even if it doesn't exist yet
            return candidates[0]

        OHLC_DIR = _resolve_ohlc_dir()

        def _ohlc_dir_etag() -> int:
            """Used only to bust cache when files change."""
            try:
        ...
        def _safe_ohlc_name(x: str) -> str:
            # match the sanitizer used on the Momentum page
            return re.sub(r"[^0-9A-Za-z]+", "_", str(x)).strip("_")

        def _ohlc_path_for(stock_name: str) -> str:
            return os.path.join(OHLC_DIR, f"{_safe_ohlc_name(stock_name)}.csv")
        ...
        def _series_from_annual(label: str) -> pd.Series | None:
            """
            Build a clean Series {FY -> value} from ANNUAL rows, skipping any bad/blank/TTM years.
            Returns None if fewer than 2 valid points.
            """
            if annual is None or annual.empty or "Year" not in annual.columns:
                return None

            # Always coerce Year to numeric first (NaNs allowed, we filter later)
            years = pd.to_numeric(annual["Year"], errors="coerce")

            def _build_series(yrs: pd.Series, vals: pd.Series) -> pd.Series | None:
                # keep rows where both year and value are numeric
                mask = yrs.notna() & pd.to_numeric(vals, errors="coerce").notna()
                if mask.sum() < 2:
                    return None
                yrs_ok  = yrs[mask].astype("Int64").dropna().astype(int)
                vals_ok = pd.to_numeric(vals[mask], errors="coerce").astype(float)
                # pair into a stable series (index = int years)
            ...
            mapping = {
                ...
                "ARR":          ["ARR"],
                "AUM":          ["AUM"],
                # helpful for banking/REIT variants
                "NAV per Unit": ["NAV per Unit","NAV/Unit","NAVPU","NAV"],
                "Gross Loans":  ["Gross Loans","Gross Loan"],
                "Deposits":     ["Deposits","Customer Deposits","Total Deposits"],
            }

            # Special case: FCF = CFO - |Capex|
            if label == "FCF":
                cfo  = None
                capx = None
                if "CFO" in annual.columns:
                    cfo = pd.to_numeric(annual["CFO"], errors="coerce")
                for cap_col in ("Capex", "Capital Expenditure"):
                    if cap_col in annual.columns:
                        capx = pd.to_numeric(annual[cap_col], errors="coerce").abs()
                        break
                if cfo is not None and capx is not None:
                    fcf = cfo - capx
                    return _build_series(years, fcf)
                return None

            # Normal path via mapped columns
            for cand in mapping.get(label, []):
                if cand in annual.columns:
                    s = pd.to_numeric(annual[cand], errors="coerce")
                    ser = _build_series(years, s)
                    if ser is not None:
                        return ser
