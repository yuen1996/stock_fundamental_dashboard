...
# Keep prior selection within the SAME session; otherwise start EMPTY
seed = [s for s in st.session_state.get("sel_names", []) if s in names][:2]  # no auto-pick

try:
    sel_names = st.multiselect(
        "Pick up to 2 stocks to analyze / compare",
        options=names,
        default=seed,                 # [] on first visit/new login
        key="sel_names",
...
# If nothing selected, show a tip and stop rendering the rest of the page
