...
/* ===== Soldier Worm (Data Quality Radar) ===== */
.sw-panel{
...
.sw-chip.success{ border-color:#bbf7d0; background:rgba(240,253,244,.8); }
.sw-chip.warning{ border-color:#fde68a; background:rgba(255,251,235,.85); }
.sw-chip.danger{  border-color:#fecaca; background:rgba(254,242,242,.85); }
.sw-subtle{ color:#6b7280; font-size:.84rem; margin-top:.25rem; }
.sw-table-hint{ color:#6b7280; font-size:.82rem; margin:.2rem 0 .6rem; }
...
from utils.ohlc_resolver import load_ohlc as _load_ohlc_raw
...
def _safe_ohlc_name(x: str) -> str:
    # keep identical behavior to the inner helper
    return re.sub(r"[^0-9A-Za-z]+", "_", str(x)).strip("_")

def _candidate_paths(ohlc_dir: str, stock_name: str, ticker_hint: str | None):
    """
    Simple filename guesses if name_link.find_ohlc_path() returns nothing.
    e.g., data/ohlc/Malayan_Banking.csv, data/ohlc/MAYBANK.csv
    """
    def _variants(s: str):
        s = _safe_ohlc_name(s)
    ...
    names = set()
    if stock_name:
        names.update(_variants(stock_name))
    if ticker_hint:
        names.update(_variants(ticker_hint))

    return [os.path.join(ohlc_dir, f"{n}.csv") for n in names if n]
...
# === Persisted price-source prefs (per stock) ===
# Only two modes now. Any legacy "Auto" is treated as Momentum.
_VALID_PRICE_SRCS = {"Momentum", "Add/Edit"}

def _norm_price_src(v, *, default="Momentum") -> str:
    s = str(v or "").strip().lower()
    if s in {"momentum", "auto", "automatic", "default"}:
        # map legacy "Auto" (or similar) to Momentum
        return "Momentum"
    if s in {"add/edit", "addedit", "add edit", "manual"}:
        return "Add/Edit"
    # anything else (None, "None", "", unexpected) → default (Momentum)
    return default


def _get_price_src_prefs() -> dict:
    """{ safe_stock_key -> 'Momentum' | 'Add/Edit' } (normalized; legacy 'Auto' is mapped to 'Momentum')"""
    d = _load_settings()
    raw = d.get("price_src_by_stock") or {}
    out, changed = {}, False
    for k, v in raw.items():
        norm = _norm_price_src(v)
        out[k] = norm
        if norm != v:
            changed = True
    if changed:
        d["price_src_by_stock"] = out
        _save_settings(d)  # write back once so bad values don't reappear
    return out

def _set_price_src_pref(stock_name: str, value: str) -> None:
    d = _load_settings()
    prefs = d.get("price_src_by_stock") or {}
    prefs[_safe_key(stock_name)] = _norm_price_src(value)
    d["price_src_by_stock"] = prefs
    _save_settings(d)  # bumps .data.version so other pages notice
...
# ========= Price source helpers (Momentum vs Add/Edit) =========

def _ohlc_file_mtime(stock_name: str, ticker_hint: str | None) -> int:
    """
    Per-stock OHLC CSV mtime (ns).
    Used ONLY to bust the Streamlit cache when that specific file changes.
    This makes View Stock pick up fresh Momentum prices immediately after
    your auto-sync finishes, even within the same session.
    """
    try:
        ohlc_dir = os.path.join(_GRANDP, "data", "ohlc")
        if not os.path.isdir(ohlc_dir):
            return 0

        for p in _candidate_paths(ohlc_dir, stock_name, ticker_hint):
            if os.path.exists(p):
                try:
...
def _load_ohlc_for_name(
    ...
    *,
    ...
    """
    Load OHLC using the unified utils.ohlc_resolver (tolerant to spaces/underscores).

    Cache key = (stock_name, _etag, _file_mtime, ticker)
      • _etag       – global bus.etag("ohlc") (if used by your Momentum page)
      • _file_mtime – per-stock CSV mtime (so each stock busts independently)

    We don’t need to *use* _etag / _file_mtime inside the function – they just
    participate in the Streamlit cache hash.
    """
    try:
        # unified resolver (same behavior as Add/Edit page)
        # if your resolver supports ticker hints, you can extend this call,
        # but we keep it as-is to match your current utils.ohlc_resolver API.
        df, _path, _how = _load_ohlc_raw(stock_name)
    except FileNotFoundError:
        return None
    except Exception:
        return None

    # Ensure there is a 'Date' column for the rest of this page
    lower = {c.lower(): c for c in df.columns}
    date_col = next(
        (lower.get(c) for c in ("date", "timestamp", "time", "datetime") if c in lower),
        None,
    )
...
def _momentum_close_before_today(
    stock_name: str,
    stock_rows: pd.DataFrame,
) -> tuple[float | None, pd.Timestamp | None]:
    """
    Return (price, asof_date) where price is the Yahoo 'Close' for the last
    available trading day STRICTLY BEFORE today.

    • Uses utils.ohlc_resolver via _load_ohlc_for_name()
    • Cache busts when:
        – bus.etag("ohlc") changes, OR
        – the specific CSV file’s mtime changes
    """
    ticker_hint = None
    for col in ("Ticker", "Code", "Symbol"):
        if col in stock_rows.columns and stock_rows[col].dropna().size:
            ticker_hint = str(stock_rows[col].dropna().iloc[-1])
            break

    etag = int(bus.etag("ohlc")) if hasattr(bus, "etag") else 0
    file_mtime = _ohlc_file_mtime(stock_name, ticker_hint)

    # NOTE: _file_mtime is only for the cache key – the helper itself does not
    #       need to read it. This ensures per-stock freshness.
    dfp = _load_ohlc_for_name(stock_name, etag, file_mtime, ticker=ticker_hint)
...
def _resolve_price_for_view(stock_name: str, stock_rows: pd.DataFrame, source_preference: str):
    """
    Returns (price, asof_date, source_used)
      source_preference: "Momentum" | "Add/Edit"
    """
    pref = (source_preference or "Momentum").strip().lower()

    def _manual_price():
        v = _current_price(stock_rows)
        return (float(v) if v is not None and np.isfinite(v) else None)

    if pref == "add/edit":
        manual = _manual_price()
        if manual is not None:
            return manual, None, "Add/Edit"
        mom, asof = _momentum_close_before_today(stock_name, stock_rows)
        return mom, asof, "Momentum (fallback)"

    # Momentum (prefer); fall back to Add/Edit if missing
    mom, asof = _momentum_close_before_today(stock_name, stock_rows)
    if mom is not None:
        return mom, asof, "Momentum"
    manual = _manual_price()
    return manual, None, ("Add/Edit (fallback)" if manual is not None else "None")
...
def _ttm_dict_from_quarters(q_df: pd.DataFrame, bucket: str) -> dict:
    ...
    for category, items in schema.items():
        for it in items:
            ...
            # baseline rule from config
            how = config._ttm_agg_for(category, key, label)

            # ---- OVERRIDE: NIM (%) should be SUM of last 4 quarters for TTM ----
            lab_l = str(label or key).strip().lower()
            if ("(%)" in lab_l) and (
                "nim" in lab_l
                or ("net interest" in lab_l and "margin" in lab_l)
                or ("net interest/financing" in lab_l and "margin" in lab_l)
            ):
                how = "sum"
            ...
            if how == "sum":
                vals = series.dropna()
                v = vals.sum() if not vals.empty else np.nan   # keep blank if all missing
            elif how == "mean":
...
def _latest_annual_value(df, col):
    if df is None or df.empty or col not in df.columns:
        return None
    ser = df.dropna(subset=["Year"]).sort_values("Year")[col]
    v = ser.iloc[-1] if len(ser) else None
    try:
        v = float(v)
...
def _summary_ttm(sum_df, label):
    if sum_df is None or sum_df.empty:
        return None
    # accept “TTM …” column names (e.g., TTM 2025)
    ttm_col = next((c for c in sum_df.columns if isinstance(c, str) and c.upper().startswith("TTM")), None)
    if not ttm_col:
        return None
    row = sum_df[sum_df["Metric"].astype(str).str.lower() == str(label).lower()]
    if row.empty:
        return None
    v = row.iloc[0].get(ttm_col)
    try:
        v = float(v)
        return v if np.isfinite(v) else None
    except Exception:
...
def compute_ttm_kpis_for_tiles(
    ...
    *,
    ...
    IMPORTANT: All **valuation** math (P/E, P/B, DY, MOS, PEG) uses the injected `price_now`
    that comes from `_resolve_price_for_view(...)` (Momentum or Add/Edit),
    never the Add/Edit Price columns.

    """
    strict_no_fy_fallback = set(map(str, (strict_no_fy_fallback or set())))

    # ---------- Use ONE price for all TTM math (respect the user’s selection) ----------
    # If caller didn’t provide a usable price, fall back gracefully.
    if price_now is None or not (isinstance(price_now, (int, float)) and np.isfinite(price_now)):
        try:
            _rows_for_price = pd.concat([annual_df, quarterly_df], ignore_index=True)
        except Exception:
            _rows_for_price = annual_df
        price_now = _current_price(_rows_for_price) or last_price_from_annual(annual_df)

    # ---------- RAW TTM ----------
    raw_ttm = ttm_raw_row_from_quarters(quarterly_df, current_price=price_now) or {}
    try:
        enrich_auto_raw_row_inplace(raw_ttm, bucket=bucket)
    except Exception:
        pass

    # ---------- SUMMARY TTM (for ratios) ----------
    # Force Summary to use the same price as the cards
    try:
    ...
    # ---------- Annual + synthetic TTM row (next FY) ----------
    try:
        annual_plus_ttm = _annual_with_appended_ttm(annual_df, quarterly_df, bucket)
        ttm_from_annual = _ttm_raw_from_annual_row(annual_plus_ttm) or {}
    except Exception:
        annual_plus_ttm = annual_df
        ttm_from_annual = {}

    # latest FY year (for tagging when we fall back to latest FY values)
    fy_year = _last_fy(annual_df)

    # tiles to fill
    raw_first = {
        "Net Profit": "Net Profit",
        "NII (incl Islamic)": "NII (incl Islamic)",
    ...
    # RAW-first tiles (with improved fallback and strict no-FY option)
    for tile, key in raw_first.items():
        v = None
        src = None

        if key in raw_ttm and raw_ttm.get(key) is not None:
            v = raw_ttm.get(key); src = "raw_ttm"
        elif key in ttm_from_annual and ttm_from_annual.get(key) is not None:
            v = ttm_from_annual.get(key); src = "ttm_annual"
        else:
            if tile not in strict_no_fy_fallback:
                v = _latest_annual_value(annual_df, key)
                if v is not None:
                    src = "latest_fy"

        # EPS fallback (Net Profit / Shares) if needed — uses the same TTM raw inputs
        if tile == "EPS (RM)" and v is None:
            npf = raw_ttm.get("Net Profit")
            shr = raw_ttm.get("Shares") or _latest_annual_value(annual_df, "Shares")
            v = (float(npf) / float(shr)) if (npf is not None and shr not in (None, 0)) else None
            if v is not None and src is None:
                src = "raw_ttm"

        values[tile] = v
        sources[tile] = src

    # SUMMARY-first ratios (the Summary was built with price_fallback=price_now)
    def _summary_ttm_val(sum_df, label):
        if sum_df is None or sum_df.empty:
            return None, None
        ttm_col = next((c for c in sum_df.columns if isinstance(c, str) and c.upper().startswith("TTM")), None)
        if not ttm_col:
        ...
        if row.empty:
            return None, None
        try:
            v = float(row.iloc[0].get(ttm_col))
            return (v if np.isfinite(v) else None), ("summary_ttm" if np.isfinite(v) else None)
        except Exception:
            return None, None

    for tile in summary_first:
        v, src = _summary_ttm_val(sum_df, tile)
        values[tile] = v
        sources[tile] = src

    # Nice fallback for NIM (%) from annual if Summary couldn’t compute it
    if values.get("NIM (%)") is None:
        v = _latest_annual_value(annual_df, "NIM")
        values["NIM (%)"] = v
        sources["NIM (%)"] = "latest_fy" if v is not None else None

    return values, sources, fy_year
    ...
    *, labels: list[str], sum_df: pd.DataFrame | None,
    ...
    for label in (labels or []):
        ...
        if val is None and isinstance(ttm_raw, dict):
            ...
            elif L in ("dividend yield", "dividend yield (%)", "dividend yield%", "dividend yield (ttm)", "dividend yield (ttm, %)") \
                 and ttm_raw.get("DPS") not in (None, 0) and ttm_raw.get("Price") not in (None, 0):
                try:
...
def _cagr_results_to_yearfirst_json(
    *, items: list[str], N:int, end_basis:str,
    ...
    if EB == "TTM" and ttm_end is None and base in ("Gross Loans","Deposits"):
        # prefer latest quarter balance; else last FY (reuses helpers above if available in scope)
        ttm_end = _last_quarter_value(quarterly, base) if 'quarterly' in locals() else None
        if ttm_end is None:
            # final fallback to last FY value from the annual series
...
# --- NEW: full-schema JSON for Annual & Quarterly (include nulls for missing) ---
def _full_schema_json(df_in: pd.DataFrame, bucket: str, *, quarterly: bool, name: str, industry: str) -> dict:
    schema = _schema_by_category(bucket)  # {Category: [{"key","label"}, ...]}
    rows = []
    ...
    if quarterly:
        if "Qnum" not in df.columns and "Quarter" in df.columns:
            df["Qnum"] = df["Quarter"].map(_qnum)
        df = df.dropna(subset=["Year","Qnum"], how="any").sort_values(["Year","Qnum"])
    else:
        if "Year" in df.columns:
            df = df.dropna(subset=["Year"], how="any").sort_values(["Year"])

    for _, r in df.iterrows():
        out = {}
        # Period
        try:
            out["Year"] = int(float(r.get("Year"))) if pd.notna(r.get("Year")) else None
        except Exception:
            out["Year"] = None
        if quarterly:
            q = r.get("Quarter")
            out["Quarter"] = (str(q).upper().strip() if pd.notna(q) else None)

        # All schema fields (follow industry; null if missing)
        for cat, items in (schema or {}).items():
            for it in (items or []):
                key   = it.get("key")
                label = it.get("label", key)
                col   = f"Q_{key}" if quarterly else key
                ...
                if pd.isna(v):
                    ...
                    n = _num_or_none(v)
                    out[label] = n if n is not None else str(v)
        rows.append(out)
    ...
    return {
        ...
        "schema":  [{"category": cat, "key": it["key"], "label": it.get("label", it["key"])}
                    for cat, items in (schema or {}).items() for it in (items or [])],
        "rows":    rows
    }
...
@st.cache_data(show_spinner=False)
def _load_df(_etag_app: int = 0, _etag_ohlc: int = 0) -> pd.DataFrame:
    df0 = io_helpers.load_data()
    return df0 if df0 is not None else pd.DataFrame()

df = _load_df(_data_etag(), int(bus.etag("ohlc")) if hasattr(bus, "etag") else 0)

if st.button("🔄 Refresh data"):
    # Clear ALL cached data sources used on this page
    _load_df.clear()
    try:
        _load_ohlc_for_name.clear()
    except Exception:
        # safe-guard: in case Streamlit version changes API
        pass
    try:
        st.rerun()
    except Exception:
...
def _clear_filters():
    ...
    # Clear only this page's UI selection (optional). Do NOT clear the global saved picks.
    st.session_state.pop("sel_names_view", None)
...
# ---- Persistent picker (remembers across pages; only clears on logout/session end) ----
GLOBAL_KEY = "sel_names_global"    # shared across ALL pages
UI_KEY     = "sel_names_view"      # page-local key (do not reuse on other pages)

# Keep prior picks across pages in this session
st.session_state.setdefault(GLOBAL_KEY, [])

# Intersect saved picks with what's available on THIS page to seed the UI
seed = [s for s in st.session_state[GLOBAL_KEY] if s in names][:2]

# Defer-clear must happen BEFORE the widget is created
if st.session_state.pop("__clear_sel_flag", False):
    st.session_state.pop(UI_KEY, None)

# --- Initialize the widget value via Session State ONLY (avoid Streamlit warning)
if UI_KEY not in st.session_state:
    st.session_state[UI_KEY] = seed
# Clamp current value to valid options & max=2, in case filters changed
st.session_state[UI_KEY] = [s for s in st.session_state[UI_KEY] if s in names][:2]

def _clear_selection_now():
    # clear this page’s picker + the cross-page remembered picks
    st.session_state[UI_KEY] = []
    st.session_state[GLOBAL_KEY] = []

st.button("Clear selection", key="clear_sel", on_click=_clear_selection_now)

# UI widget writes into the page-local key so it never wipes the global one by accident
try:
    sel_names_ui = st.multiselect(
        "Pick up to 2 stocks to analyze / compare",
        options=names,
        key=UI_KEY,  # <-- no `default=`; session_state is the single source of truth
    ...
    sel_names_ui = st.multiselect(
        "Pick up to 2 stocks to analyze / compare",
        options=names,
        key=UI_KEY,  # <-- no `default=`
        help="Pick one to analyze, two to compare headers side-by-side (UI only).",
    )

# If the user picked something on this page, sync it back to the global store
if sel_names_ui:
    st.session_state[GLOBAL_KEY] = sel_names_ui[:2]

# Use the UI selection for this page's rendering
calc_sel_names = sel_names_ui[:2] if sel_names_ui else []

# If nothing selected, either because the saved pick isn't in this filtered list,
# or the user hasn't picked yet, show a tip but DO NOT wipe the global selection.
if not calc_sel_names:
    saved = st.session_state.get(GLOBAL_KEY, [])
    if saved and not any(s in names for s in saved):
        st.info("Your saved selection isn’t in the current list (filters differ). Change filters or pick a stock above. Your saved selection is kept for other pages.")
    else:
        st.info("Pick 1 stock above to view details, or 2 to compare.")
    st.stop()

# From here on we know at least one stock is selected
name = calc_sel_names[0]
...
def _render_compare(a: str, b: str):
    ...
    # ---------- quick facts header ----------
    def _facts(sub):
        nm = str(sub["Name"].dropna().iloc[-1]) if sub.get("Name") is not None and sub["Name"].dropna().size else ""
        # Use each stock's saved source selection if available; otherwise default to Momentum
        sp_key = f"price_src_{_safe_key(nm)}"
        saved = _get_price_src_prefs().get(_safe_key(nm))
        src_pref = _norm_price_src(st.session_state.get(sp_key, saved or "Momentum"))
        px, asof, _src_used = _resolve_price_for_view(nm, sub, src_pref)

        return [
            ("Current Price", f"{px:,.4f}" if (px is not None and np.isfinite(px)) else "—"),
    ...
    def _ttm_ratio_series(annual_df: pd.DataFrame, quarterly_df: pd.DataFrame, *, bucket: str, price_now: float | None):
        ...
        try:
            sum_df = build_summary_table(
                ...
                bucket=bucket,
                include_ttm=True,
                price_fallback=price_now   # ← was price_hint (undefined in this scope)
            )
        except Exception:
            sum_df = pd.DataFrame()
...
with st.expander(name, expanded=True):
    ...
    # --- Price source switch (per-stock; persisted across pages/sessions) ---
    _price_src_key = f"price_src_{_safe_key(name)}"
    _saved_pref = _norm_price_src(_get_price_src_prefs().get(_safe_key(name)), default="Momentum")

    # Seed/fix the session value BEFORE rendering the widget
    _cur = st.session_state.get(_price_src_key, _saved_pref)
    st.session_state[_price_src_key] = _norm_price_src(_cur, default=_saved_pref)

    # Render
    if hasattr(st, "segmented_control"):
        src_pref = st.segmented_control(
            "Price source",
            options=["Momentum", "Add/Edit"],
    ...
    if src_pref != _saved_pref:
        _set_price_src_pref(name, src_pref)


    # Resolve price per selected source (with graceful fallback)
    price_now, mom_asof, source_used = _resolve_price_for_view(name, stock, src_pref)
    ...
    render_kpi_text_grid([
        ("Current Price", f"{price_now:,.4f}" if (price_now is not None and np.isfinite(price_now)) else "—"),
        ("Industry",      industry or "—"),
        ("Bucket",        bucket or "—"),
    ...
    # Source remark
    if str(source_used).startswith("Momentum") and mom_asof is not None:
        st.caption(f"Current Price from Momentum (Yahoo close as of {mom_asof.strftime('%d/%m/%Y')}).")
    elif "Add/Edit" in str(source_used):
        st.caption("Current Price from Add/Edit (latest saved).")
    ...
    with tabs[0]:
        ...
        if hasattr(calculations, "build_summary_table"):
            try:
                sum_df = calculations.build_summary_table(
                    ...
                    bucket=bucket,
                    include_ttm=True,
                    price_fallback=price_now,   # <- ensures valuation ratios use current price
                )
        ...
        with st.expander(f"🪱 Soldier Worm  —  missing: {miss_cnt} · trace rows: {trace_cnt}", expanded=False):
            st.markdown("<div class='sw-panel'>", unsafe_allow_html=True)

            # ----- Fix List (what to fill first) -----
            try:
                warn_df_ui = calculations.decorate_soldier_worm_missing_table(warn_df, bucket=bucket)
            except Exception:
                warn_df_ui = warn_df

            try:
                fix_df = calculations.build_soldier_worm_fixlist(warn_df_ui, bucket=bucket, top_n=50)
            except Exception:
                fix_df = pd.DataFrame()
            ...
            section(
                "🪱 Soldier Worm — Data Quality Radar",
                "Detect why KPIs are empty (missing raw inputs / broken paths). Fill the Fix List first to unlock Industry Summary.",
                tone="info",
            )
            ...
            try:
                _cat_map = (
                    config.INDUSTRY_SUMMARY_RATIOS_CATEGORIES.get(bucket)
                    or config.INDUSTRY_SUMMARY_RATIOS_CATEGORIES.get("General", {})
                )
                total_metrics = int(sum(len(v) for v in _cat_map.values()))
            except Exception:
                total_metrics = None

            base_df = warn_df_ui if (warn_df_ui is not None and not warn_df_ui.empty) else warn_df
            missing_rows = int(len(base_df)) if base_df is not None else 0
            missing_metrics = (
                int(base_df["Metric"].nunique())
                if (base_df is not None and not base_df.empty and "Metric" in base_df.columns)
                else 0
            )

            score = None
            if total_metrics and total_metrics > 0:
                score = int(round(100.0 * (1.0 - (missing_metrics / float(total_metrics)))))
                score = max(0, min(100, score))

            uniq_inputs = 0
            top_blocker = ""
            try:
                if fix_df is not None and not fix_df.empty and "Input" in fix_df.columns:
                    uniq_inputs = int(fix_df["Input"].nunique())
                    top_blocker = str(fix_df.iloc[0]["Input"])
            except Exception:
                pass
            ...
            render_stat_cards(
                [
                    {"label": "Completeness", "value": f"{score}%" if score is not None else "—", "tone": tone, "badge": "SCORE"},
                    {"label": "Missing metrics", "value": f"{missing_metrics} / {total_metrics}" if total_metrics else f"{missing_metrics}", "tone": "danger" if missing_metrics else "success"},
                    {"label": "Unique missing inputs", "value": str(uniq_inputs), "tone": "warning" if uniq_inputs else "success"},
                    {"label": "Top blocker", "value": top_blocker or "—", "tone": "warning" if top_blocker else ""},
                ],
                columns=4,
            )
            ...
            if score is not None:
                ...
                st.markdown(
                    f"<div class='sw-subtle'>{100 - score}% gap • {missing_rows} missing rows across periods</div>",
                    unsafe_allow_html=True,
                )
                st.markdown(
                    f"<span class='sw-chip {tone}'>Completeness {score}%</span> "
                    f"<span class='sw-chip'>{missing_metrics} missing metrics</span> "
                    f"<span class='sw-chip warning'>{uniq_inputs} raw inputs to fill</span>",
                    unsafe_allow_html=True,
                )
            else:
                st.markdown(
                    f"<div class='sw-subtle'>{missing_rows} missing rows across periods</div>",
                    unsafe_allow_html=True,
                )

            if missing_rows > 0:
                if score is not None and score < 70:
                    st.error("High risk: many KPIs in Industry Summary will stay empty. Start with **Fix List** below.")
                else:
                    st.warning("Some KPIs are missing raw inputs. Start with **Fix List** below.")
            else:
                st.success("All KPIs in Industry Summary are computable for the available periods.")

            st.markdown("<div class='sw-divider'></div>", unsafe_allow_html=True)


            st.subheader("Fix List (what to fill first)", divider=True)
            if fix_df is None or fix_df.empty:
                st.caption("No missing inputs detected (your Summary is fully computable for the available periods).")
            else:
                c1, c2, c3 = st.columns(3)
                with c1:
                    st.metric("Unique missing inputs", int(fix_df["Input"].nunique()))
                with c2:
                    st.metric("Total missing rows", int(miss_cnt))
                with c3:
                    st.metric("Top blocker", str(fix_df.iloc[0]["Input"]))

                top_only = st.toggle("Show only top 15", value=True, key=f"sw_fix_top_{safe_name}")
                show_df = fix_df.head(15) if top_only else fix_df
                try:
                    sty_fix = show_df.style.hide(axis="index")
                except Exception:
                    sty_fix = show_df.style
                st.dataframe(sty_fix, use_container_width=True, height=_auto_h(show_df, row_h=26, base=120))
        ...
        # Tiles that must NOT fall back to FY if TTM is missing
        STRICT_TTM_ONLY = {"Provisions"}  # add more labels here if you like

        kpis_master, kpi_sources, kpi_fy_year = compute_ttm_kpis_for_tiles(
            annual, qtr, bucket,
            price_now=price_now,                       
            strict_no_fy_fallback=STRICT_TTM_ONLY
        )

        # NEW: label-aware numeric formatter for TTM tiles
        def _fmt_ttm_val(label: str, value):
            """
            Number formatting for TTM KPI tiles:
              • DPS / EPS / Price-like stuff → 4 decimal places
            ...
            try:
                ...
                if not np.isfinite(f):
                    return "—"
                lab = str(label).lower()
                if "dps" in lab or "eps" in lab or "price" in lab:
                    return f"{f:,.4f}"
                return f"{f:,.2f}"
            except Exception:
                return "—"

        def _val_for_tile(label: str):
            # prefer canonical label to improve hit-rate
            canon = (_canonical(label, syn_idx) or label).strip()
            v = (kpis_master or {}).get(canon)
            if v is not None:
                return v

            # try the calculated Summary TTM column
            row = _find_row(sum_df, label, syn_idx) if (isinstance(sum_df, pd.DataFrame) and not sum_df.empty) else None
            if isinstance(row, pd.Series) and ttm_col and (ttm_col in row.index):
                try:
                    return float(row.get(ttm_col))
                except Exception:
                    pass  # fall through to custom branches

            L = str(label).strip().lower()

            # --- Revenue YoY (%) (from Summary or fallbacks) ---
            if L in ("revenue yoy", "revenue yoy (%)", "sales yoy", "sales yoy (%)"):
                ttm_rev, last_rev = None, None

                # Prefer Summary's Revenue row: TTM vs latest FY
                rev_row = _find_row(sum_df, "Revenue", syn_idx) if (isinstance(sum_df, pd.DataFrame) and not sum_df.empty) else None
                if isinstance(rev_row, pd.Series):
                    if ttm_col and (ttm_col in rev_row.index):
                        ttm_rev = pd.to_numeric(pd.Series([rev_row.get(ttm_col)]), errors="coerce").iloc[0]
                    fy_cols = [c for c in rev_row.index if isinstance(c, (int, np.integer))]
                    if fy_cols:
                        last_rev = pd.to_numeric(pd.Series([rev_row.get(max(fy_cols))]), errors="coerce").iloc[0]

                # Fallbacks: raw TTM Revenue, then latest FY Revenue from annual
                if (ttm_rev is None or not np.isfinite(float(ttm_rev))) and isinstance(ttm_raw, dict):
                    ttm_rev = _clean_num(ttm_raw.get("Revenue"))

                if (last_rev is None or not np.isfinite(float(last_rev))) and isinstance(annual, pd.DataFrame):
                    for col in ("Revenue", "Total Revenue", "TotalRevenue", "Sales"):
                        if col in annual.columns:
                            s = pd.to_numeric(annual[col], errors="coerce").dropna()
                            if not s.empty:
                                last_rev = float(s.iloc[-1])
                                break

                try:
                    t, p = float(ttm_rev), float(last_rev)
                    return ((t / p) - 1.0) * 100.0 if (np.isfinite(t) and np.isfinite(p) and p != 0.0) else None
                except Exception:
                    return None

            # --- Raw fallbacks for absolute TTM numbers ---
            # revenue / profit / EPS-like metrics
            if L in ("revenue", "gross profit", "operating profit", "ebitda", "net profit", "eps"):
                return ttm_raw.get(label)

            # DPS-style labels: map to the 'DPS' key in ttm_raw
            if L.startswith("dps") or "dividend per share" in L:
                if isinstance(ttm_raw, dict):
                    if "DPS" in ttm_raw:
                        return ttm_raw.get("DPS")
                    # try to find a DPS-like key
                    for k, v in ttm_raw.items():
                        kl = str(k).lower()
                        if "dps" in kl or ("dividend" in kl and "per share" in kl):
                            return v
                return None

            # Dividend Yield from DPS / Price if needed
            if L in (
                "dividend yield",
                "dividend yield (%)",
                "dividend yield%",
                ...
                if isinstance(ttm_raw, dict) and ttm_raw.get("DPS") not in (None, 0) and ttm_raw.get("Price") not in (None, 0):
                    try:
                        return (float(ttm_raw["DPS"]) / float(ttm_raw["Price"])) * 100.0
                    except Exception:
                        return None
                return None

            # P/E fallback from Price / EPS
            if L == "p/e" and np.isfinite(ttm_raw.get("EPS", np.nan)) and np.isfinite(ttm_raw.get("Price", np.nan)) and float(ttm_raw["EPS"]) != 0:
                return float(ttm_raw["Price"]) / float(ttm_raw["EPS"])
        ...
        for label in ttm_items:
            ...
            # 1) try the new consolidated dict
            val = (kpis_master or {}).get(canon)
            src = (kpi_sources or {}).get(canon)

            # 2) if missing, fall back to the previous logic
            if val is None:
                val = _val_for_tile(label)   # ← this restores Revenue/EBITDA/margins/etc.
                src = None                   # no FY-tag for this older fallback

            # format & show
            is_pct = any(x in canon.lower() for x in ["(%)", "margin", "yield", " ratio", "yoy"])
            shown = (_fmt_pct(val) if is_pct else _fmt_ttm_val(canon, val))


            if src == "latest_fy" and kpi_fy_year is not None:
                shown = f"{shown}  • FY {int(kpi_fy_year)}"

            ttm_pairs.append((label, shown))
        ...
        for label in (ttm_items or []):
            ...
            if row is not None and (ttm_col in row) and pd.notna(row[ttm_col]):
                ...
                # Absolute TTM figures we can pull straight from ttm_raw
                if L in ("revenue", "gross profit", "operating profit", "ebitda", "net profit", "eps"):
                    val = ttm_raw.get(label)

                # DPS-style labels → map to the 'DPS' key in ttm_raw
                elif L.startswith("dps") or "dividend per share" in L:
                    if isinstance(ttm_raw, dict):
                        if "DPS" in ttm_raw:
                            val = ttm_raw.get("DPS")
                        else:
                ...
                # Dividend Yield from DPS / Price
                    ...
                    "dividend yield (ttm)",
                    "dividend yield (ttm, %)",
                ) and isinstance(ttm_raw, dict) and ttm_raw.get("DPS") not in (None, 0) and ttm_raw.get("Price") not in (None, 0):
                    try:
                ...
                # EPS YoY (%)
                    ...
                    # Prefer Summary EPS (TTM + latest FY)
                    ttm_eps, last_eps = None, None

                    eps_row = _find_row(sum_df, "EPS", syn_idx) if (isinstance(sum_df, pd.DataFrame) and not sum_df.empty) else None
                    if isinstance(eps_row, pd.Series) and (ttm_col in eps_row.index):
                        ttm_eps = pd.to_numeric(pd.Series([eps_row.get(ttm_col)]), errors="coerce").iloc[0]
                        fy_cols = [c for c in eps_row.index if isinstance(c, (int, np.integer))]
                        if fy_cols:
                            last_eps = pd.to_numeric(pd.Series([eps_row.get(max(fy_cols))]), errors="coerce").iloc[0]

                    # Raw TTM fallback
                    if (ttm_eps is None or not np.isfinite(float(ttm_eps))) and isinstance(ttm_raw, dict):
                        ttm_eps = _clean_num(ttm_raw.get("EPS"))

                    # Latest FY EPS fallback from annual (zero-safe for Shares)
                    if (last_eps is None or not np.isfinite(float(last_eps))) and isinstance(annual, pd.DataFrame):
                        ...
                        elif {"Net Profit", "Shares"}.issubset(annual.columns):
                            np_s = pd.to_numeric(annual["Net Profit"], errors="coerce").dropna()
                            sh_s = pd.to_numeric(annual["Shares"], errors="coerce").replace(0, np.nan).dropna()
                            if not np_s.empty and not sh_s.empty:
                                last_eps = float(np_s.iloc[-1]) / float(sh_s.iloc[-1])

                    try:
                        t, p = float(ttm_eps), float(last_eps)
                        val = ((t / p) - 1.0) * 100.0 if (np.isfinite(t) and np.isfinite(p) and p != 0.0) else np.nan
                    except Exception:
                        val = np.nan

                # Revenue YoY (%)
                elif L in ("revenue yoy", "revenue yoy (%)", "sales yoy", "sales yoy (%)"):
                    # Prefer Summary: compute from Summary's Revenue row (TTM vs latest FY)
                    ttm_rev, last_rev = None, None
                    rev_row = _find_row(sum_df, "Revenue", syn_idx) if (isinstance(sum_df, pd.DataFrame) and not sum_df.empty) else None
                    if isinstance(rev_row, pd.Series):
                        # TTM revenue if Summary provided it
                        if ttm_col and (ttm_col in rev_row.index):
                            ttm_rev = pd.to_numeric(pd.Series([rev_row.get(ttm_col)]), errors="coerce").iloc[0]
                        # Latest FY revenue for prior period
                        fy_cols = [c for c in rev_row.index if isinstance(c, (int, np.integer))]
                        if fy_cols:
                            last_rev = pd.to_numeric(pd.Series([rev_row.get(max(fy_cols))]), errors="coerce").iloc[0]

                    # Fallback to raw TTM & Annual if Summary is missing/incomplete
                    if (ttm_rev is None or not np.isfinite(float(ttm_rev))) and isinstance(ttm_raw, dict):
                        ttm_rev = _clean_num(ttm_raw.get("Revenue"))
        ...
        ttm_overrides = {
            ...
            "Gross Loans": (ttm_raw.get("Gross Loans") if isinstance(ttm_raw, dict) else None),
            "Deposits":    (ttm_raw.get("Deposits")    if isinstance(ttm_raw, dict) else None),
        ...
        for item in cagr_items:
            ...
            # helper: latest quarterly stock-level value (good for balances)
            def _last_quarter_value(qdf: pd.DataFrame, *keys) -> float | None:
                if qdf is None or qdf.empty:
                    return None
                q = qdf.copy()
                if "Qnum" not in q.columns and "Quarter" in q.columns:
                    q["Qnum"] = q["Quarter"].str.extract(r"(\d+)").astype(float)
                q = q.dropna(subset=["Year","Qnum"]).sort_values(["Year","Qnum"])
                for k in keys:
                    if k in q.columns:
                        s = pd.to_numeric(q[k], errors="coerce").dropna()
                        if not s.empty:
            ...
            if g is None:
                ...
                if end_basis == "TTM":
                    ...
                    # last-resort overrides so TTM CAGR has a valid end-point
                    if override is None and base_label in (
                        "Orderbook","Store Count","ARR","AUM","NAV per Unit",
                        "Gross Loans","Deposits"  # ← add these
                    ):
                        # prefer the latest quarterly balance, fall back to last FY
                        if base_label in ("Gross Loans","Deposits"):
                            override = _last_quarter_value(qtr, base_label) or _last_fy_value(base_label)
                        else:
        ...
        # ---- Momentum helpers: reuse global OHLC loader (uses utils/name_link) ----
        def _ohlc_bus_etag() -> int:
            """Bust cache when Momentum updates OHLC files; fall back to dir mtime of the OHLC dir."""
            try:
        ...
        @st.cache_data(show_spinner=False)
        def _load_ohlc_for(stock_name: str, _etag: int, ticker: str | None = None) -> pd.DataFrame | None:
            """
            Delegate to the global loader so all pages share the same behavior.
            Uses the top-level _load_ohlc_for_name(...) defined earlier in this file.
            """
            return _load_ohlc_for_name(stock_name, _etag, ticker=ticker)

        def _mom_change_pct_df(df: pd.DataFrame, months: int = 12) -> float | None:
            """
            Trailing price change (%) using a calendar-month window.

            • Takes the last available Close as "now".
            • Looks back `months` (≈ 30 days each, or 365 days for 12M+) to find
              the last price on/before that start date.
            • If there’s no such row, falls back to ~252 trading days (~1 year).
            • Returns % change (e.g. +12.34 for +12.34%), or None if not computable.
            """
            ...
            # Anchor on the last trading day
        ...
        def _get_momentum(label: str):
            ...
            3) OHLC CSVs in data/ohlc (fallback; shared global loader)
            """
            L = (label or "").upper()

            def _bucket(L):
                if "1M" in L or "30D" in L:
                    return "1M"
                if "3M" in L or "90D" in L:
                    return "3M"
                if "6M" in L or "180D" in L:
                    return "6M"
                return "12M"
            ...
            # 2) Summary table candidates (if available in this scope)
            for cand in [label, "12M Price Change (%)", "Price Change 12M (%)", "Total Return 1Y (%)"]:
                try:
                    v = _get_from_summary(cand)  # defined earlier in this Snowflake block
                    if v is not None:
                        return v
                except Exception:
                    pass
            ...
            ticker_hint = None
            for col in ("Ticker", "Code", "Symbol"):
                if col in stock.columns and stock[col].dropna().size:
                    ticker_hint = str(stock[col].dropna().iloc[-1])
                    break

            df_px = _load_ohlc_for(name, _ohlc_bus_etag(), ticker=ticker_hint)
        ...
        __ttm_dict = _ttm_dict_from_quarters(qtr if isinstance(qtr, pd.DataFrame) else pd.DataFrame(), bucket) or {}
        ...
        ttm_overrides = {
            ...
            # balances from quarter-derived TTM dict
            "Gross Loans": __sf_num_or_none(__ttm_dict.get("Gross Loans")),
            "Deposits":    __sf_num_or_none(__ttm_dict.get("Deposits")),
        }
    ...
    with tabs[1]:
        ...
        if annual is None or annual.empty:
            ...
            a_payload = _full_schema_json(annual_plus, bucket, quarterly=False, name=name, industry=industry)
            st.download_button(
                label="⬇️ Download Annual (JSON, raw inputs)",
                data=(__import__("json").dumps(a_payload, indent=2, ensure_ascii=False)).encode("utf-8"),
    ...
    with tabs[2]:
        ...
        if qtr is None or qtr.empty:
            ...
            q_payload = _full_schema_json(qtr, bucket, quarterly=True, name=name, industry=industry)
            st.download_button(
                label="⬇️ Download Quarterly (JSON, raw inputs)",
                data=(__import__("json").dumps(q_payload, indent=2, ensure_ascii=False)).encode("utf-8"),
