...
def _raw_df_from_schema(df_in: pd.DataFrame, bucket: str, *, quarterly: bool) -> pd.DataFrame:
    ...
    # ---- alias a few common raw column names so "Revenue" is never all-NaN ----
    def _alias_common_cols(df: pd.DataFrame) -> pd.DataFrame:
        aliases = [
...
def _add_yoy_to_multiheader(df_multi: pd.DataFrame, *, is_quarterly: bool):
    ...
    # tolerant label finder (case-insensitive; matches display label in the MultiIndex)
    def _find_lab(labs):
        want = {str(x).strip().lower() for x in labs}
        for c in df.columns:
            if isinstance(c, tuple):
                lab = str(c[1]).strip().lower()
                if lab in want:
                    return c
        return None

    # Accept common synonyms so we actually find the right column
    rev_col = _find_lab(["Revenue", "Total Revenue", "TotalRevenue", "Sales"])
    np_col  = _find_lab(["Net Profit", "NetProfit"])
    eps_col = _find_lab(["EPS", "EPS (RM)"])
    ...
    def _yoy(series: pd.Series) -> pd.Series:
        ...
        # guard: need at least two numeric points to compute pct_change
        if s.notna().sum() < 2:
