...
def _raw_df_from_schema(df_in: pd.DataFrame, bucket: str, *, quarterly: bool) -> pd.DataFrame:
    ...
    # NOTE: pandas cannot reliably select/reindex a DataFrame with a *non-unique* MultiIndex columns.
    # Some buckets may accidentally define duplicate (Category, Label) pairs in the schema.
    # We defensively de-duplicate column tuples here to avoid:
    ...
    for cat, items in schema.items():
        for it in items:
            ...
            # Ensure unique MultiIndex (cat, label)
            cand = (cat, str(label))
            if cand in seen_cols:
                cand = (cat, f"{label} [{key}]")
            if cand in seen_cols:
                base = cand[1]
                i = 2
                while (cat, f"{base} ({i})") in seen_cols:
                    i += 1
                cand = (cat, f"{base} ({i})")
