        ...
        with tabs[0]:
            ...
            with tabs[0]:
                ...
                try:
                    ...
                    if not oh.empty and "Close" in oh.columns:
                        close = oh["Close"].astype(float)
                        price = float(close.iloc[-1])

                        ma200 = float(
                            close.rolling(200, min_periods=200).mean().iloc[-1]
                        ) if len(close) >= 200 else None

                        ret_12m = None
                        if len(close) >= 252 and pd.notna(close.iloc[-252]) and close.iloc[-252] != 0:
...
# --- Momentum visibility / sanity notes on the View page ---
