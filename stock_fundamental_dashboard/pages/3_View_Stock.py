...
for stock_name in stocks:
    with st.expander(stock_name, expanded=False):
        ...
        with tabs[0]:
            ...
            # ---- Annual Raw table: Full-height / Interactive (resizable) option
            full_h_ann_raw = st.checkbox("Full height (no vertical scroll)", value=False, key=f"ann_raw_full_{stock_name}")
            inter_ann_raw  = st.checkbox("Interactive table (resizable columns, no colors/tooltips)", value=False, key=f"ann_raw_inter_{stock_name}")
            ...
            if ratio_df.empty:
                ...
                if ratio_layout.startswith("Years"):
                    ...
                    full_h_ann_ratio = st.checkbox("Full height (no vertical scroll)", value=False, key=f"ann_ratio_full_{stock_name}")
                    inter_ann_ratio  = st.checkbox("Interactive table (resizable columns, no colors/tooltips)", value=False, key=f"ann_ratio_inter_{stock_name}")
