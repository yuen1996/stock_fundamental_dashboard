...
with st.expander(name, expanded=True):
    ...
    _price_src_key = f"price_src_{_safe_key(name)}"   # use _safe_key so it matches saved prefs & compare
    _saved_pref = _get_price_src_prefs().get(_safe_key(name)) or "Momentum"

    # Seed once, then let the widget own the value
    st.session_state.setdefault(_price_src_key, _saved_pref)
