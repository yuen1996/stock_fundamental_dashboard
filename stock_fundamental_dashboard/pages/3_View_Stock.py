...
# --- Latest candle (OHLC) helper: reads the same Momentum CSVs used elsewhere ---
def _latest_candle_ohlc(stock_name: str) -> dict | None:
    """
    Returns dict with last candle info from data/ohlc:
      { date, open, high, low, close, volume, change_pct }
    change_pct is vs prior close if available.
    """
    try:
        etag = int(bus.etag("ohlc")) if hasattr(bus, "etag") else 0
    except Exception:
    ...
    dfp = _load_ohlc_for_name(stock_name, etag, ticker=None)
    if dfp is None or dfp.empty or "Date" not in dfp.columns:
        return None

    dfp = dfp.dropna(subset=["Date"]).sort_values("Date").reset_index(drop=True)
    if dfp.empty:
        return None

    # tolerant column lookup (handles Adj Close, etc.)
    ...
    c_high  = _col("High")
    c_low   = _col("Low")
    c_close = _col("Close", "Adj Close", "Adj_Close")
    c_vol   = _col("Volume", "Vol")

    last = dfp.iloc[-1]
    prev = dfp.iloc[-2] if len(dfp) >= 2 else None
    ...
    open_  = _num(last[c_open])  if c_open  else None
    high   = _num(last[c_high])  if c_high  else None
    low    = _num(last[c_low])   if c_low   else None
    close  = _num(last[c_close]) if c_close else None
    vol    = _num(last[c_vol])   if c_vol   else None

    change_pct = None
    if prev is not None and c_close:
        pclose = _num(prev[c_close])
        if pclose and close and pclose != 0:
            change_pct = (close / pclose - 1.0) * 100.0
...
with st.expander(name, expanded=True):
    ...
    # --- Latest Candle KPIs (no extra charts; clean, just the numbers) ---
    info = _latest_candle_ohlc(name)
    ...
    if info:
        ...
        date_str = info["date"].strftime("%d/%m/%Y") if pd.notna(info["date"]) else "—"
        kpis = [
            ("Last Candle Date", date_str),
            ("Open",  _f4(info["open"])),
            ("High",  _f4(info["high"])),
            ("Low",   _f4(info["low"])),
            ("Close", _f4(info["close"])),
        ]
        render_kpi_text_grid(kpis)

        # Small note for extra context (optional, stays tidy)
        note_bits = []
        if isinstance(info.get("change_pct"), (int, float)) and np.isfinite(info["change_pct"]):
            note_bits.append(f"Δ vs prior close: {info['change_pct']:+.2f}%")
        if isinstance(info.get("volume"), (int, float)) and np.isfinite(info["volume"]):
            note_bits.append(f"Vol: {int(info['volume']):,}")
        if note_bits:
            st.caption(" · ".join(note_bits))
    else:
        st.caption("No OHLC data found for this stock (data/ohlc missing?).")
