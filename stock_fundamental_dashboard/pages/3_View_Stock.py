...
# NEW: drag-and-drop list for ordering
...
def drag_reorder(columns, key_suffix, help_text="Drag to set column order. Left-most = first."):
    """
    Show a draggable list of column labels, return the same columns (objects) in the new order.
    This uses streamlit-sortables' sort_items().
    """
    labels = [nice_label(c) for c in columns]
    st.markdown(f'<div class="sort-help">{help_text}</div>', unsafe_allow_html=True)
    # Keep the drag list visually compact
    with st.container():
        st.markdown('<div class="sort-box">', unsafe_allow_html=True)
        # sort_items returns new list of strings in dropped order
        new_order = sort_items(labels)  # component keeps its own UI state
        st.markdown('</div>', unsafe_allow_html=True)

    # map back to column objects
    label_to_col = {nice_label(c): c for c in columns}
    ordered_cols = [label_to_col[l] for l in new_order if l in label_to_col]
