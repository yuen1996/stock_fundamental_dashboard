...
def _raw_df_from_schema(df_in: pd.DataFrame, bucket: str, *, quarterly: bool) -> pd.DataFrame:
    ...
    # Guard: pandas cannot select from a non-unique MultiIndex (raises ValueError).
    # This happens if schema labels collide (same (category,label) appears more than once).
    # We keep the first occurrence and suffix the rest to make columns unique.
    if isinstance(out.columns, pd.MultiIndex) and (not out.columns.is_unique):
        seen = {}
        new_cols = []
        for a, b in out.columns.to_list():
            key = (a, b)
            if key not in seen:
                seen[key] = 1
                new_cols.append((a, b))
                ...
                seen[key] += 1
                new_cols.append((a, f"{b} ({seen[key]})"))
...
with st.expander(name, expanded=True):
    ...
    with tabs[0]:
        ...
        with st.expander(f"🪱 Diagnostics  —  missing: {miss_cnt} · trace rows: {trace_cnt}", expanded=False):
            render_soldier_worm_diagnostics(warn_df, trace_df, key_prefix=f"sw_{safe_name}")
