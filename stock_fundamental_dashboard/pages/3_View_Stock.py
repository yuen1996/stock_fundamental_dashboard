...
COLORS = {
    "good": "#16a34a22",   # green (light)
    "ok":   "#3b82f622",   # blue  (light)
    "bad":  "#ef444422",   # red   (light)
}

def _rate(value, rule):
    """Return ('good'|'ok'|'bad', expl) for value using rule=(dir, great, ok, note)."""
...
def style_ratio_table(df_in: pd.DataFrame) -> "pd.io.formats.style.Styler":
    """
    Color each cell based on THRESHOLDS and attach a tooltip explaining the rule.
    Works with either orientation (Years as rows or columns).
    """
    df = df_in.copy()
...
def _show_styled(styler, height=360):
    """Try to render Styler in st.dataframe; fall back to st.write if needed."""
    try:
...
for stock_name in stocks:
    with st.expander(stock_name, expanded=False):
        ...
        with tabs[0]:
            ...
            # ---- Raw Data table
            st.markdown(
                '<div class="sec info"><div class="t">🧾 Annual — Raw Data</div></div>',
                unsafe_allow_html=True
            )
...
st.markdown(
    '<div class="sec success"><div class="t">📐 Annual — Calculated Ratios</div></div>',
    unsafe_allow_html=True
)
...
if ratio_df.empty:
    ...
    if ratio_layout.startswith("Years"):
        ...
        # save state for multi-panel charts below
        ...
        st.session_state[f"ann_ratio_metrics_{stock_name}"] = new_cols_r
            ...
            st.markdown(
                '<div class="sec warning"><div class="t">📊 Annual — Comparison Charts</div></div>',
                unsafe_allow_html=True
            )
        ...
        with tabs[1]:
            ...
            st.markdown(
                '<div class="sec info"><div class="t">📄 Quarterly — Raw Data</div></div>',
                unsafe_allow_html=True
            )
            ...
            st.markdown(
    '<div class="sec success"><div class="t">📐 Quarterly — Calculated Ratios</div></div>',
    unsafe_allow_html=True
)
...
if qratio_df.empty:
    ...
    if qratio_layout.startswith("Periods"):
        ...
        st.caption("Legend: 🟩 Great  •  🟦 OK  •  🟥 Fails threshold")
            ...
            st.markdown(
                '<div class="sec warning"><div class="t">📊 Quarterly — Comparison Charts</div></div>',
                unsafe_allow_html=True
            )
