...
# --- Systematic decision for this stock (latest annual row) ---
annual_rows = annual.sort_values("Year")
if not annual_rows.empty:
    r_latest = annual_rows.tail(1).iloc[0]
    metrics = calculations.calc_ratios(r_latest)

    with st.expander("🚦 Systematic Decision", expanded=False):
        strategy = st.selectbox("Strategy", list(rules_engine.RULESETS.keys()), key=f"{stock_name}_strategy")
        min_score = st.slider("Minimum score", 0, 100, 60, 5, key=f"{stock_name}_minscore")
        ev = rules_engine.evaluate_ratios(metrics, strategy, min_score)
        st.metric("Decision Score", f"{ev['score']}%")

        st.write("**Mandatory checks**")
        for label, ok, reason in ev["mandatory"]:
            st.write(("✅ " if ok else "❌ ") + label + ("" if ok else f" — {reason}"))

        st.write("**Scored checks**")
        for label, ok, weight in ev["scored"]:
            st.write(("✅ " if ok else "❌ ") + f"{label} ({weight} pts)")

        if ev["pass"]:
            if st.button(f"Push {stock_name} → Trade Queue", key=f"push_queue_{stock_name}"):
                io_helpers.push_trade_candidate(
                    stock_name, strategy, ev["score"], cur_val, "; ".join(ev["reasons"])
                )
                st.success("Pushed to Trade Queue.")
        else:
            st.info("Does not PASS. Fix unmet criteria or adjust strategy/threshold to proceed.")
