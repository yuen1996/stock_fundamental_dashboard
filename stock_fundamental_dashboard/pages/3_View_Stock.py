...
def compute_ttm_kpis_for_tiles(annual_df, quarterly_df, bucket: str):
    """
    Return a dict {tile label -> numeric value or None}
    Raw-first for banking absolutes; Summary-first for ratios.
    """
    # ---------- RAW TTM ----------
    price_hint = last_price_from_annual(annual_df)
    raw_ttm = ttm_raw_row_from_quarters(quarterly_df, current_price=price_hint) or {}
    # fill some banking-derived raw fields if your quarters miss them
    try:
    ...
    try:
        sum_df = calculations.build_summary_table(
            ...
            bucket=bucket,
            include_ttm=True,
            price_fallback=price_hint  # ← use the local price_hint (was undefined price_now)
        )
    except Exception:
        sum_df = None

    # ---------- Build an annual with an appended TTM row (next FY) ----------
    # We’ll use this as a *preferred* fallback before latest FY for RAW-first tiles
    try:
    ...
    # 1) tiles that should come straight from RAW (then TTM-annual, then latest FY)
    raw_first = {
        ...
        "DPS": "DPS",
        # Capital & liquidity levels -> raw latest FY if TTM absent
        "CET1 Ratio (%)": "CET1 Ratio",
        "Tier 1 Capital Ratio (%)": "Tier 1 Ratio",
        "Total Capital Ratio (%)": "Total Capital Ratio",
        # Optional if you show it as a tile:
        "EPS (RM)": "EPS (RM)",
    }

    # 2) tiles that should come from SUMMARY (calculated)
    summary_first = [
        "NIM (%)",
    ...
    # RAW-first with improved fallback order:
    # raw TTM (quarters) → appended TTM annual row (e.g., “TTM 2025”) → latest actual FY
    for tile, key in raw_first.items():
        v = raw_ttm.get(key)
        if v is None:
            v = ttm_from_annual.get(key)  # ← prefer synthetic TTM annual
        if v is None:
            v = _latest_annual_value(annual_df, key)
        out[tile] = v

    # EPS (RM) fallback = Net Profit / Shares if you don’t have EPS raw
    if out.get("EPS (RM)") is None:
        npf = raw_ttm.get("Net Profit")
        shr = raw_ttm.get("Shares") or _latest_annual_value(annual_df, "Shares")
        out["EPS (RM)"] = (float(npf) / float(shr)) if (npf is not None and shr not in (None, 0)) else None
    ...
    for tile in summary_first:
        out[tile] = _summary_ttm(sum_df, tile)

    # Nice fallback for NIM if Summary couldn’t compute it
    if out.get("NIM (%)") is None:
        out["NIM (%)"] = _latest_annual_value(annual_df, "NIM")
...
with st.expander(name, expanded=True):
    ...
    with tabs[0]:
        ...
        # NEW: pre-compute a master dict for tiles (handles raw-vs-summary sourcing)
        kpis_master = compute_ttm_kpis_for_tiles(annual, qtr, bucket)
        ...
        for label in ttm_items:
            val = _val_for_tile(label)

            # choose % vs number formatting based on label
            is_pct = any(x in label.lower() for x in ["(%)", "margin", "yield", " ratio", "yoy"])
