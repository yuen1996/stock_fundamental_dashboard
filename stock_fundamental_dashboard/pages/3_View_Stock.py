...
# ---------- Systematic Decision (read-only, per stock) ----------
# Use the latest ANNUAL row to evaluate strict rules
annual_rows_for_eval = annual.sort_values("Year")
if not annual_rows_for_eval.empty:
    latest_row = annual_rows_for_eval.iloc[-1]
    metrics = calculations.calc_ratios(latest_row)

    # Choose the default strategy to display here
    ev = rules.evaluate(metrics, "Quality-Value")  # or "Dividend"

    badge = "✅ PASS" if ev["pass"] else "❌ REJECT"
    st.markdown(f"**Systematic Decision:** {badge} · Score {ev['score']}%")

    # Show unmet mandatory reasons if any
    if not ev["pass"] and ev["reasons"]:
        st.caption("Unmet (mandatory): " + "; ".join(ev["reasons"]))
