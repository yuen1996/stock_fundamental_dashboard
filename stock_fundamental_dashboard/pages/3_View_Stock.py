...
# ---- ❄️ Snowflake — Overall Performance (inside Annual tab)
try:
    # Compute 0..100 scores from your TTM metrics (uses your calculations.compute_factor_scores)
    scores = calculations.compute_factor_scores(
        stock_name, stock, ttm, ohlc_latest=None, industry=None
    )
    base = ["Value", "Quality", "Growth", "Cash", "Momentum"]
    labels = ["Future Value", "Earnings Quality", "Growth Consistency", "Cash Strength", "Momentum"]
    vals = [int(scores.get(k, 0) or 0) for k in base]
except AttributeError:
    # compute_factor_scores not available yet
    labels = ["Future Value", "Earnings Quality", "Growth Consistency", "Cash Strength", "Momentum"]
    vals = [0, 0, 0, 0, 0]

# Radar
try:
    import plotly.graph_objects as go  # safe if already imported
    ...
    st.info("Snowflake chart could not be rendered (missing Plotly?).")

# Data Health badge (from TTM) — use a checkbox, not an expander (expanders cannot nest)
