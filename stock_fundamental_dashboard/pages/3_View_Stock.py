...
def plot_single_series(x_values, y_values, title, yname, height=320):
    ...
    fig.update_layout(
        height=height,  # slightly smaller to fit neatly
...
def multi_panel_charts(count, options, x_labels, series_getter, key_prefix, chart_height=320):
    """
    Render up to 4 charts (each single-select) in a consistently aligned 2-column grid:
      - If count == 1: left column filled, right left blank (keeps alignment)
      - If count == 2: left & right on the same row
      - If count == 3 or 4: 2 rows x 2 columns; charts 3 & 4 appear directly below 1 & 2
    """
    count = max(1, min(4, int(count)))
    option_labels = [o[0] for o in options]

    # First row (always 2 columns to lock alignment)
    row1 = st.columns(2)
    # Second row if needed (also 2 columns to stay aligned with row1)
    row2 = st.columns(2) if count > 2 else (None, None)

    # Helper to render a single chart cell
    ...
    # Place chart 1 and 2 on the first row (left, right)
    render_cell(row1[0], 0)
    if count >= 2:
        render_cell(row1[1], 1)

    # Place chart 3 and 4 on the second row (left, right)
...
st.caption("Annual comparison charts align in a 2×2 grid (left/right), with charts 3 & 4 rendering directly below 1 & 2. Column drag-order still works; raw-data rows with all blanks move to the bottom.")
