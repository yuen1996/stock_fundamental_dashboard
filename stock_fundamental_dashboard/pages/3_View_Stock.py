...
def compute_ttm_kpis_for_tiles(annual_df, quarterly_df, bucket: str,
    ...
    # ---------- RAW TTM ----------
    price_hint = last_price_from_annual(annual_df)
    raw_ttm = ttm_raw_row_from_quarters(quarterly_df, current_price=price_hint) or {}
    try:
    ...
    raw_first = {
        ...
        "DPS": "DPS",
        # use percent-labeled keys (match Annual/TTM 2025)
    ...
    # aliases to accept both percent and non-percent column names
    _aliases = {
        "CET1 Ratio (%)": ["CET1 Ratio (%)", "CET1 Ratio", "CET1 (%)", "CET1"],
        "Tier 1 Capital Ratio (%)": ["Tier 1 Capital Ratio (%)", "Tier 1 Ratio (%)", "Tier 1 Ratio", "Tier 1 (%)"],
    ...
    for tile, key in raw_first.items():
        v = None
        src = None
        keys_try = _aliases.get(tile, [key])

        # 1) raw TTM (from quarters)
        for k in keys_try:
            if raw_ttm.get(k) is not None:
                v = raw_ttm.get(k); src = "raw_ttm"; break

        # 2) appended TTM annual row (Year = last_fy+1)
        if v is None:
            for k in keys_try:
                if ttm_from_annual.get(k) is not None:
                    v = ttm_from_annual.get(k); src = "ttm_annual"; break

        # 3) latest FY (only if allowed)
        if v is None and tile not in strict_no_fy_fallback:
            for k in keys_try:
                v_try = _latest_annual_value(annual_df, k)
                if v_try is not None:
                    v = v_try; src = "latest_fy"; break

        # EPS fallback (unchanged)
