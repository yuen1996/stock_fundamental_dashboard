...
def _show_styled(styler, height=360):
    ...
    1) Try the native Styler renderer.
    2) If that fails, render as HTML (preserves tooltips & colors).
    3) As a last resort, show the raw DataFrame without styles.
    """
    try:
        # Modern Streamlit can render Styler directly with height
        st.dataframe(styler, use_container_width=True, height=height)
        return
    except Exception:
        pass

    # Fallback: HTML render of the styled table (keeps tooltips/colors)
    try:
        html = styler.to_html()
    ...
    # Last resort: show unstyled data to avoid hard failure
    try:
