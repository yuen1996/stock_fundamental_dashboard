...
def _data_etag() -> int:
    """Cache-buster for anything derived from fundamentals.csv (prices, TTM DPS, etc.).

    We include BOTH:
    - .data.version mtime (manual bump / other pages use it)
    - fundamentals.csv mtime (bulk price edits update the file but may not bump .data.version)
    """
    v1 = 0
    v2 = 0
    try:
        v1 = int(os.stat(_VERSION_FILE).st_mtime_ns)
    except Exception:
        v1 = 0
    try:
        v2 = int(os.stat(io_helpers.DATA_FILE).st_mtime_ns)
    except Exception:
        v2 = 0
    # combine to a single integer for st.cache_data keys
    return v1 ^ v2
...
# ─────────────────────────────────────────
# Alerts: stop hit / near stop / target hit (visual scanning)
# ─────────────────────────────────────────
stop = pd.to_numeric(filtered.get("Stop"), errors="coerce")
take = pd.to_numeric(filtered.get("Take"), errors="coerce")
tp1 = pd.to_numeric(filtered.get("TP1"), errors="coerce") if "TP1" in filtered.columns else pd.Series(np.nan, index=filtered.index)
target = take.copy()
target = target.where(target.notna(), tp1)

alert = pd.Series([""] * len(filtered), index=filtered.index, dtype="object")
cond_stop = stop.notna() & cur.notna() & (cur <= stop)
alert = np.where(cond_stop, "🔴 Stop hit", alert)

cond_target = (pd.Series(alert, index=filtered.index) == "") & target.notna() & cur.notna() & (cur >= target)
alert = np.where(cond_target, "🟡 Target hit", alert)

cond_near = (pd.Series(alert, index=filtered.index) == "") & stop.notna() & cur.notna() & (cur > stop) & (cur <= stop * 1.02)
alert = np.where(cond_near, "🟠 Near stop", alert)

filtered["Alert"] = pd.Series(alert, index=filtered.index).astype(str)
...
for fn in friends:
    ...
    with st.expander(label, expanded=(fn != "Unassigned")):
        ...
        # Place "Alert" right after Current (if present)
        if "Alert" in table.columns and "Current" in table.columns:
            cols = list(table.columns)
            if "Alert" in cols:
                cols.remove("Alert")
            try:
                cur_idx = cols.index("Current")
                cols.insert(cur_idx + 1, "Alert")
                table = table[cols]
            except Exception:
                pass
        ...
        edited = st.data_editor(
            ...
            column_config={
                ...
                "Alert": st.column_config.TextColumn("Alert", disabled=True, help="🔴 Stop hit | 🟠 Near stop | 🟡 Target hit"),
