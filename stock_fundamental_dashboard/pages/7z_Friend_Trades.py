...
# ─────────────────────────────────────────
# Friend Accounts (summary)
# ─────────────────────────────────────────

st.markdown(
    section("👥 Friend Accounts", "Performance summary grouped by friend"),
    unsafe_allow_html=True,
)

# --- TTM DPS helper (use session sync if available; else derive from last 4 quarters) ---
...
def _ttm_dps_from_quarters_cached(_etag: int, name: str) -> float | None:
    """Best-effort TTM DPS from quarterly rows in fundamentals.csv."""
    try:
        df = io_helpers.load_data()
        ...
        for c in q.columns:
            sc = str(c).lower()
            if ("dps" in sc) or ("dividend per share" in sc) or ("dpu" in sc):
                dps_col = c
                if sc.startswith("q_"):
                    break
        if not dps_col:
        ...
        vals = pd.to_numeric(q[dps_col], errors="coerce").dropna()
        if vals.empty:
            return None
        v = float(vals.tail(min(4, len(vals))).sum())
        return v if np.isfinite(v) and v >= 0 else None
    except Exception:
        return None
...
def _friend_metrics(df: pd.DataFrame) -> dict:
    _shares = pd.to_numeric(df.get("Shares"), errors="coerce").fillna(0)
    _entry  = pd.to_numeric(df.get("Entry"),  errors="coerce").fillna(0)
    _cur    = pd.to_numeric(df.get("Current"), errors="coerce")

    total_cost = float((_shares * _entry).sum())
    cur_value  = float((_shares * _cur).sum(skipna=True))
    pnl        = float((_shares * (_cur - _entry)).sum(skipna=True))

    ttm_dps = df["Name"].map(lambda n: get_ttm_dps(str(n))).astype("float").fillna(0.0)
    proj_div_rm = float((_shares * ttm_dps).sum())
    div_yield = (proj_div_rm / cur_value * 100.0) if cur_value > 0 else None

    rr_init = pd.to_numeric(df.get("RR"), errors="coerce")
    avg_rr = float(rr_init.dropna().mean()) if rr_init.notna().any() else None

    open_dates = pd.to_datetime(df.get("OpenDate"), errors="coerce")
    avg_hold_days = (
    ...
    return {
        ...
        "div_yield": div_yield,
        "avg_rr": avg_rr,
        "avg_hold": avg_hold_days,
    }


def _tone_pnl(v: float) -> str:
    if v > 0:
        return "good"
    if v < 0:
        return "bad"
...
# Group by FriendName (fallback to "Unknown" if missing)
if "FriendName" in filtered.columns:
    _fn = filtered["FriendName"].fillna("").astype(str).str.strip()
    filtered["FriendName"] = _fn.replace({"": "Unknown"})
    friend_names = sorted(filtered["FriendName"].unique().tolist())
else:
    filtered["FriendName"] = "Unknown"
    friend_names = ["Unknown"]

# If a single friend is selected, show only that friend (no tabs)
if len(friend_names) == 1:
    fn = friend_names[0]
    fdf = filtered[filtered["FriendName"] == fn]
    m = _friend_metrics(fdf)
    st.markdown(f"### {fn}")
    render_stat_cards(
        [
            {"label": "Open Positions", "value": f"{m['n']:,}", "badge": "Shown"},
            {"label": "Total Cost (RM)", "value": f"{m['cost']:,.2f}", "badge": "Exposure"},
            {"label": "Cur Value (RM)", "value": f"{m['value']:,.2f}", "badge": "Marked"},
            {
                "label": "Unrealized P&L",
                "value": f"{m['pnl']:,.2f}",
                "badge": "Live",
                "tone": _tone_pnl(m['pnl']),
            },
            {
            ...
            {
                "label": "Div Yield (wgt)",
                "value": (f"{m['div_yield']:.2f}%" if m['div_yield'] is not None else "—"),
                "badge": "TTM",
            },
            {
                "label": "Avg RR Init",
                "value": (f"{m['avg_rr']:.2f}×" if m['avg_rr'] is not None else "—"),
                "badge": "Quality",
                "tone": _tone_rr(m['avg_rr']),
            },
            {
                "label": "Avg Holding (d)",
                "value": (f"{m['avg_hold']:,}" if m['avg_hold'] is not None else "—"),
                "badge": "Duration",
            },
        ],
        columns=4,
    ...
    tabs = st.tabs(friend_names)
    for t, fn in zip(tabs, friend_names):
        with t:
            fdf = filtered[filtered["FriendName"] == fn]
            m = _friend_metrics(fdf)
            render_stat_cards(
...
edited = st.data_editor(
    ...
    column_config={
        ...
        "FriendName": st.column_config.TextColumn("Friend", help="Editable: assign/rename friend account"),
        ...
        "Shares": st.column_config.NumberColumn("Shares", format="%d", help="Editable: adjust quantity/units"),
...
# ─────────────────────────────────────────
# Save edits (Friend / Shares)
# ─────────────────────────────────────────
st.markdown(
    '<div class="sec"><div class="t">💾 Save edits</div>'
    '<div class="d">You can edit <b>Friend</b> and <b>Shares</b> directly in the table, then save.</div></div>',
    unsafe_allow_html=True,
)

# Detect edits (FriendName / Shares) compared to the filtered snapshot
_orig = table[["RowId"] + (["FriendName"] if "FriendName" in table.columns else []) + (["Shares"] if "Shares" in table.columns else [])].copy()
_new  = edited[["RowId"] + (["FriendName"] if "FriendName" in edited.columns else []) + (["Shares"] if "Shares" in edited.columns else [])].copy()

# Normalize
if "FriendName" in _new.columns:
    _new["FriendName"] = _new["FriendName"].fillna("").astype(str).str.strip()
if "Shares" in _new.columns:
    _new["Shares"] = pd.to_numeric(_new.get("Shares"), errors="coerce")

chg_rows = []
for _, r in _new.iterrows():
    rid = int(r.get("RowId"))
    o = _orig[_orig["RowId"] == rid]
    if o.empty:
        continue
    o = o.iloc[0]
    changed = False
    ...
    if "FriendName" in _new.columns:
        old_fn = ("" if pd.isna(o.get("FriendName")) else str(o.get("FriendName"))).strip()
        new_fn = ("" if pd.isna(r.get("FriendName")) else str(r.get("FriendName"))).strip()
        if new_fn != old_fn and new_fn != "":
            payload["FriendName"] = new_fn
            changed = True

    if "Shares" in _new.columns:
        old_sh = pd.to_numeric(o.get("Shares"), errors="coerce")
        new_sh = pd.to_numeric(r.get("Shares"), errors="coerce")
        if pd.notna(new_sh) and (pd.isna(old_sh) or float(new_sh) != float(old_sh)):
            payload["Shares"] = float(new_sh)
            changed = True

    if changed:
        chg_rows.append(payload)

if not chg_rows:
    st.caption("No Friend/Shares edits detected.")
else:
    st.caption(f"Detected {len(chg_rows)} change(s). Click **Save** to write to CSV.")

c_save, c_hint = st.columns([1, 2])
with c_save:
    if st.button("Save Friend/Shares changes", disabled=(len(chg_rows) == 0)):
        # Best-effort: update the underlying friend open-trades CSV
        try:
            if hasattr(io_helpers, "load_friend_open_trades") and hasattr(io_helpers, "save_friend_open_trades"):
                base_df = io_helpers.load_friend_open_trades()
                base_df = base_df.reset_index(drop=True)
            else:
                raise RuntimeError("Friend trades IO helpers not found")

            for upd in chg_rows:
                rid = int(upd["RowId"])
                if rid < 0 or rid >= len(base_df):
                    continue
                if "FriendName" in upd:
                    base_df.at[rid, "FriendName"] = upd["FriendName"]
                if "Shares" in upd:
                    base_df.at[rid, "Shares"] = upd["Shares"]

            io_helpers.save_friend_open_trades(base_df)

            # bump the data etag to invalidate cached price / DPS reads
            try:
                import os, time
                os.makedirs(os.path.dirname(_VERSION_FILE), exist_ok=True)
                with open(_VERSION_FILE, "a", encoding="utf-8"):
                    pass
...
with c_hint:
    st.caption("Tip: after saving, the page will refresh and P&L / dividend numbers will update using the new Shares.")
