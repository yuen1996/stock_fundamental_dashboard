...
ttm_div_yield_pct_sess = name_series.map(_ttm_div_yield_pct_from_session)
ttm_dps_sess = name_series.map(_ttm_dps_from_session)
ttm_dps_q    = name_series.map(lambda n: _ttm_dps_from_quarters_cached(_data_etag(), n))

# If we have DY%, prefer implied DPS (keeps DPS consistent with DY%)
ttm_dps = pd.Series(np.nan, index=filtered.index, dtype="float64")
with np.errstate(divide="ignore", invalid="ignore"):
    ttm_dps = ttm_dps.where(~ttm_div_yield_pct_sess.notna(), (cur * (ttm_div_yield_pct_sess / 100.0)))
ttm_dps = ttm_dps.where(ttm_dps.notna(), ttm_dps_sess)
ttm_dps = ttm_dps.where(ttm_dps.notna(), ttm_dps_q)

# Dividend yield (%): use session DY% if available else compute from DPS and current price
ttm_div_yield_pct = pd.Series(ttm_div_yield_pct_sess, index=filtered.index, dtype="float64")
with np.errstate(divide="ignore", invalid="ignore"):
...
# Projected dividends
filtered["Proj Div (RM)"] = (shares * ttm_dps).round(2)
