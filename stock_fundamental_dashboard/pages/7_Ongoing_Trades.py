...
# ─────────────────────────────────────────────────────────────────────────────
# Version etag (invalidate caches when data updates) + paths
# ─────────────────────────────────────────────────────────────────────────────
...
# View Stock helpers to locate OHLC + etag bus
try:
    from utils import bus, name_link
except Exception:
...
# ─────────────────────────────────────────────────────────────────────────────
# View Stock price-source sync (Momentum vs Add/Edit)
# ─────────────────────────────────────────────────────────────────────────────
...
def _get_live_pref_from_session(stock_name: str) -> Optional[str]:
    """Mirror View Stock's live toggle (check the common keys it writes)."""
    sk = _safe_key(stock_name)

    # 1) Per-stock direct keys commonly used by View Stock pages
    for key in (f"price_src_{sk}", f"view_price_src_{sk}"):
        v = st.session_state.get(key)
        if isinstance(v, str) and v.strip():
            return _norm_price_src(v)

    # 2) Dict mapping stock -> src
...
def latest_current_price(master_df: pd.DataFrame, stock_name: str) -> float | None:
    """Return live price following View Stock source selection for this stock."""
...
.inspect .pos { color:#10b981; }
.inspect .neg { color:#ef4444; }
...
# ─────────────────────────────────────────────────────────────────────────────
# Live price + P&L columns (follows View Stock selection)
# ─────────────────────────────────────────────────────────────────────────────
...
# Sync "Current" with View Stock (Momentum last close OR Add/Edit manual)
_master = io_helpers.load_data()
filtered["Current"] = filtered["Name"].map(lambda n: latest_current_price(_master, str(n))).astype("float")
...
# Overview
# ─────────────────────────────────────────────────────────────────────────────
st.markdown(section("📊 Overview", "Open positions & exposure (filtered)"), unsafe_allow_html=True)
st.caption("**Current** follows your **View Stock** source (Momentum last close vs Add/Edit manual) per stock.")
try:
...
edited = st.data_editor(
    ...
    column_order=[
        ...
        "Entry","Stop","TP1","TP2","TP3","Take",
        "Shares","RR","OpenDate",
        "Current","R live","Return (%)","Unrealized PnL (RM)",
        "Δ to TP1 (RM)","Δ to TP2 (RM)","Δ to TP3 (RM)",
        "Δ to TP1 (%)","Δ to TP2 (%)","Δ to TP3 (%)",
        "Δ to SL (RM)","SL Breach?","Reasons",
...
def _open_inspect_dialog(row: pd.Series):
    ...
    entry  = row.get("Entry"); cur = row.get("Current"); stop = row.get("Stop")
    tp1 = row.get("TP1"); tp2 = row.get("TP2"); tp3 = row.get("TP3")
    shares = int(row.get("Shares") or 0)
    pnl = row.get("Unrealized PnL (RM)"); ret = row.get("Return (%)"); rlive = row.get("R live")
    ...
    p1 = _pct_to_target(tp1); p2 = _pct_to_target(tp2); p3 = _pct_to_target(tp3); ps = _sl_buffer()
    ...
    if hasattr(st, "dialog"):
        ...
        def _dlg():
            ...
            with c1: st.markdown("<div class='inspect kpi'><h4>Entry</h4>"  f"<div class='v'>{entry:.4f}</div></div>",  unsafe_allow_html=True)
            with c2: st.markdown("<div class='inspect kpi'><h4>Current</h4>"f"<div class='v'>{(f'{cur:.4f}' if pd.notna(cur) else '—')}</div></div>", unsafe_allow_html=True)
            with c3: st.markdown("<div class='inspect kpi'><h4>R live</h4>" f"<div class='v'><span class='pill {pill_tone}'>{'—' if pd.isna(rlive) else f'{rlive:.2f}'}</span></div></div>", unsafe_allow_html=True)
            c4, c5, c6 = st.columns(3)
            with c4: st.markdown("<div class='inspect kpi'><h4>P&L (RM)</h4>" f"<div class='v {pnl_cls}'>{'—' if pd.isna(pnl) else f'{pnl:,.2f}'}</div></div>", unsafe_allow_html=True)
            with c5: st.markdown("<div class='inspect kpi'><h4>Return (%)</h4>" f"<div class='v'>{'—' if pd.isna(ret) else f'{ret:.2f}'}</div></div>", unsafe_allow_html=True)
            with c6: st.markdown("<div class='inspect kpi'><h4>Shares</h4>" f"<div class='v'>{shares:,d}</div></div>", unsafe_allow_html=True)
...
""", unsafe_allow_html=True)
        ...
        with st.form(key=f"wiz_form_{idx}", clear_on_submit=False, border=True):
            ...
            if submit:
                if price <= 0:
                    st.error("Close Price must be > 0."); st.stop()
                if need_detail and not (detail or "").strip():
                    st.error("Please specify detail for 'Other'."); st.stop()
...
# Open the wizard once per run if active
