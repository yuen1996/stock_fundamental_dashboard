...
def _latest_price_for_cached(name: str, _etag: int) -> float | None:
...
def _latest_price_for(name: str) -> float | None:
    # Keep the public name the same; just key the cache by the global data etag
    return _latest_price_for_cached(name, _data_etag())
