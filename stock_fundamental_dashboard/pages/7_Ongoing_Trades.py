...
# Dividend snapshot (TTM Dividend Yield % preferred; DPS fallback)
def _norm_name(x: str) -> str:
    return re.sub(r"\s+", " ", str(x or "").strip()).upper()

def _name_key(x: str) -> str:
    """Compact matching key for name: remove spaces/punct so 'LayHong' == 'Lay Hong'."""
    s = _norm_name(x)
...
def _ttm_div_yield_pct_from_data(_etag: int, name: str) -> float | None:
    ...
    try:
        ...
        keys = df["Name"].astype(str).map(_name_key)
        sub = df[keys == target].copy()
        if sub.empty and target:
            # fallback: relaxed containment match (rare)
            try:
                mask = keys.str.contains(target, na=False)
                sub = df[mask].copy()
            except Exception:
                sub = sub
        if sub.empty:
        ...
        if ttm.empty:
            # fallback to latest annual Dividend Yield (%) if TTM row not present
            ann = sub[sub.get("IsQuarter") != True].copy()
            if ann is not None and not ann.empty and "Year" in ann.columns:
                ynum = pd.to_numeric(ann["Year"], errors="coerce")
                ann = ann[~ynum.isna()].copy()
                if not ann.empty:
                    latest = float(ynum.max())
                    ann_latest = ann[ynum == latest].copy()
                    ttm = ann_latest
...
def _ttm_dps_from_quarters_cached(_etag: int, name: str) -> float | None:
    ...
    try:
        ...
        for c in q.columns:
            s = str(c).lower()
            if ("dividend" in s) and (("per share" in s) or ("dps" in s) or ("dpu" in s)):
                cand.append((3, c))
            elif ("dividend per share" in s) or ("dividendps" in s):
                cand.append((2, c))
            elif ("dps" in s) or ("dpu" in s):
                cand.append((1, c))
        cand.sort(key=lambda t: t[0], reverse=True)
        dps_col = cand[0][1] if cand else None
        if not dps_col:
            return None
...
filtered["Stock Div Yield %"] = np.where(
