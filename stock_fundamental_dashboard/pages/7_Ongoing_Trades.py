...
# NEW: Momentum filename resolver + optional etag bus
...
# ---- Momentum (OHLC) helpers: match View Stock’s “Auto” behavior ----
...
def _ohlc_dir_etag() -> int:
    # cache-buster: prefer Momentum’s bus tag; else directory mtime
    try:
...
def _current_price_from_addedit(rows: pd.DataFrame) -> float | None:
...
def _momentum_close_before_today(stock_name: str, stock_rows: pd.DataFrame) -> tuple[float | None, pd.Timestamp | None]:
    # optional hint: Ticker / Code / Symbol
    ticker_hint = None
    ...
    dfp = _load_ohlc_for_name(stock_name, _ohlc_dir_etag(), ticker=ticker_hint)
...
@st.cache_data(show_spinner=False)
def _live_price_for_cached(name: str, _etag_app: int, _etag_ohlc: int) -> float | None:
    # “Auto”: Momentum (close before today) → fallback to Add/Edit current price
    df = io_helpers.load_data()
    if df is None or df.empty:
        return None
    rows = df[df["Name"].astype(str) == str(name)].sort_values(["Year","Quarter"])
    if rows.empty:
        return None
    ...
    return _current_price_from_addedit(rows)
...
# Latest price (Auto): Momentum → fallback to Add/Edit (same as View Stock)
