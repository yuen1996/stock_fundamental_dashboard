...
# Dividend snapshot (TTM Dividend Yield % preferred; DPS from last 4 quarters)
try:
    import calculations  # local module
except Exception:
    calculations = None
...
def _norm_key_alnum(x: str) -> str:
    return re.sub(r"[^A-Z0-9]+", "", _norm_name(x))


def _lookup_ttm_sync_record(name: str) -> dict | None:
    store = st.session_state.get("TTM_KPI_SYNC", {}) or {}
    if not store:
        return None
    if name in store:
        return store.get(name)
    nkey = _norm_key_alnum(name)
    for k, v in store.items():
        if _norm_key_alnum(k) == nkey:
            return v
    return None
...
def _ttm_div_yield_pct_from_session(name: str) -> float | None:
    rec = _lookup_ttm_sync_record(name)
    if not rec:
        return None
    vals = (rec.get("values") or {}) if isinstance(rec, dict) else {}
    if not isinstance(vals, dict):
        return None

    # prefer canonical label from View Stock
    for k in [
        "Dividend Yield (%)",
        "Div Yield (%)",
        "Dividend Yield %",
        ...
        if k in vals:
            v = _num_or_none(vals.get(k))
            if v is not None:
                return v
    # last-resort fuzzy match
    for kk, vv in vals.items():
        key = str(kk or "").strip().lower()
        if ("dividend" in key and "yield" in key) or key in ("dy", "div yield"):
            v = _num_or_none(vv)
            if v is not None:
                return v
    return None
...
@st.cache_data(show_spinner=False)
def _ttm_div_yield_pct_from_data_cached(_etag: int, name: str) -> float | None:
    """Read TTM Dividend Yield (%) from fundamentals.csv (works even after full app refresh)."""
    try:
        df = io_helpers.load_data()
        if df is None or df.empty or "Name" not in df.columns:
            return None

        name_u = _norm_name(name)
        sub = df[df["Name"].astype(str).str.upper().str.strip() == name_u]
        if sub.empty:
            key = _norm_key_alnum(name)
            nm2 = df["Name"].astype(str).str.upper().str.replace(r"[^A-Z0-9]+", "", regex=True)
            sub = df[nm2 == key]
        if sub.empty:
            return None

        # Locate the TTM row (some datasets store it in Year, some in Quarter)
        ttm = pd.DataFrame()
        if "Year" in sub.columns:
            y = sub["Year"].astype(str).str.upper().str.strip()
            ttm = sub[y == "TTM"]
        if ttm.empty and "Quarter" in sub.columns:
            q = sub["Quarter"].astype(str).str.upper().str.strip()
            ttm = sub[q == "TTM"]
        if ttm.empty:
            return None

        # Find a dividend-yield column
        dy_col = None
        for c in ttm.columns:
            cl = str(c).strip().lower()
            if ("dividend" in cl) and ("yield" in cl):
                dy_col = c
                # Prefer the canonical label if present
                if cl in ("dividend yield (%)", "dividend yield %", "dividend yield"):
                    break
        if dy_col is None:
            return None

        v = pd.to_numeric(ttm[dy_col], errors="coerce").dropna()
        if v.empty:
            return None
        out = float(v.iloc[-1])
...
def _get_ttm_div_yield_pct(name: str) -> float | None:
    """Prefer View Stock sync (session), else fall back to fundamentals.csv."""
    v = _ttm_div_yield_pct_from_session(name)
    if v is None:
        v = _ttm_div_yield_pct_from_data_cached(_data_etag(), name)
    return v
...
def _ttm_dps_from_session(name: str) -> float | None:
    ...
    if not isinstance(vals, dict):
        return None
    # Prefer explicit DPS/DPU keys if present (we'll sanity-check later by using DY first)
    for k in [
        "DPS",
    ...
    for kk, vv in vals.items():
        key = str(kk or "").strip().lower()
        if ("dividend" in key and "per" in key and "share" in key) or key in ("dps", "dpu"):
            v = _num_or_none(vv)
            if v is not None:
                return abs(v)
    return None
...
def _ttm_dps_from_quarters_cached(_etag: int, name: str) -> float | None:
    """Compute TTM DPS/DPU by summing last 4 quarters (no dependency on View Stock page)."""
    ...
    isq = sub.get("IsQuarter")
    if isq is None:
        return None
    try:
        mask_q = (isq == True) | (isq.astype(str).str.lower().isin(["true", "1", "yes", "y"]))
    except Exception:
        mask_q = (isq == True)
    qdf = sub[mask_q]
    if qdf.empty:
    ...
    try:
        raw = calculations.ttm_raw_row_from_quarters(qdf, current_price=1.0) or {}
    except Exception:
        return None
...
# Build per-row DY% + projected dividends
name_series = filtered["Name"].astype(str)

ttm_div_yield_pct_pref = pd.to_numeric(name_series.map(_get_ttm_div_yield_pct), errors="coerce")
ttm_dps_q = pd.to_numeric(
    name_series.map(lambda n: _ttm_dps_from_quarters_cached(_data_etag(), n)),
    errors="coerce",
)

# If we have DY%, prefer implied DPS (keeps DPS consistent with DY% and avoids DPS/EPS mixups)
ttm_dps = pd.Series(np.nan, index=filtered.index, dtype="float64")
with np.errstate(divide="ignore", invalid="ignore"):
    ttm_dps = ttm_dps.where(ttm_div_yield_pct_pref.isna(), (cur * (ttm_div_yield_pct_pref / 100.0)))
ttm_dps = ttm_dps.where(ttm_dps.notna(), ttm_dps_q)

# Dividend yield (%): prefer DY% (session/data). Otherwise compute from DPS and current price.
ttm_div_yield_pct = pd.Series(ttm_div_yield_pct_pref, index=filtered.index, dtype="float64")
with np.errstate(divide="ignore", invalid="ignore"):
    ttm_div_yield_pct = ttm_div_yield_pct.where(
        ttm_div_yield_pct.notna(),
        np.where(cur > 0, (ttm_dps / cur) * 100.0, np.nan),
    )

# Projected dividends (force numeric dtype to avoid pandas object-round issues)
shares_num = pd.to_numeric(shares, errors="coerce").fillna(0.0)
ttm_dps_num = pd.to_numeric(ttm_dps, errors="coerce")
filtered["Proj Div (RM)"] = (shares_num * ttm_dps_num).round(2)
proj_div_gross = float(pd.to_numeric(filtered["Proj Div (RM)"], errors="coerce").fillna(0.0).sum())
proj_div_yield = (proj_div_gross / cur_value * 100.0) if cur_value > 0 else None

# Per-stock dividend yield (%)
filtered["Stock Div Yield %"] = pd.to_numeric(ttm_div_yield_pct, errors="coerce").round(2)
...
render_stat_cards(
    [
        ...
        {
            "label": "Div Yield (wgt)",
            "value": (f"{proj_div_yield:.2f}%" if proj_div_yield is not None else "—"),
            "badge": "TTM",
        },
        {
...
edited = st.data_editor(
    ...
    column_config={
        ...
        "Proj Div (RM)": st.column_config.NumberColumn("Proj Div (RM)", format="%.2f", disabled=True),
