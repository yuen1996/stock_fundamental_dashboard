...
def _get_price_src_prefs_cached(_etag: int) -> dict:
    # cache preferences by file mtime so changes on View Stock page are picked up
    return _get_price_src_prefs()

@st.cache_data(show_spinner=False)
def _load_master_data_cached(_etag: int):
    # cache master data by your global data etag
    return io_helpers.load_data()
...
def _get_live_pref_from_session(stock_name: str) -> Optional[str]:
    ...
    # 3) Global single toggle (fallback)
...
def latest_current_price(
    master_df: pd.DataFrame,
    stock_name: str,
    *,
    prefs: Optional[dict] = None
) -> float | None:
    """Follow View Stock price source: 'Momentum' (last close) or 'Add/Edit' (manual)."""
    rows = _rows_for_stock(master_df, stock_name)
    if rows is None or rows.empty:
        return None

    # live (session) overrides saved preference
    live_pref = _get_live_pref_from_session(stock_name)

    # use cached prefs (by settings file mtime) so we don't re-open the file per row
    if prefs is None:
...
prefs = _get_price_src_prefs_cached(_settings_etag())
_master = _load_master_data_cached(_data_etag())

filtered["Current"] = (
    filtered["Name"]
    .map(lambda n: latest_current_price(_master, str(n), prefs=prefs))
    .astype("float")
)
