...
# --- Dividend Yield helpers (pull EXACT TTM yield from View Stock cache) ----
import math
...
def _ttm_cache_record(name: str, code: str | None = None) -> dict | None:
    """Find a TTM_KPI_SYNC record with tolerant matching (Name or Code)."""
    try:
        ...
        # normalized map
        norm_map = { _norm_key(k): k for k in cache.keys() }
        nk = _norm_key(name)
        if nk and nk in norm_map:
            return cache.get(norm_map[nk])

        if code:
            ck = _norm_key(code)
            if ck and ck in norm_map:
                return cache.get(norm_map[ck])
        return None
    except Exception:
        return None

def get_ttm_dividend_yield_pct(name: str, code: str | None = None) -> float | None:
    """Return Dividend Yield (%) from View Stock's TTM KPI cache (as shown on View Stock)."""
    rec = _ttm_cache_record(name, code)
    if not rec:
        return None
    ...
    # direct common keys (View Stock canonicalizes to the (%) version most of the time)
    direct = (
        "Dividend Yield (%)",
        "Dividend Yield",
        "Dividend Yield (TTM, %)",
    ...
    # fuzzy fallback (still only dividend/distribution yield)
    for k, v in vals.items():
        kl = str(k).lower()
        if ("dividend yield" in kl) or ("distribution yield" in kl):
            try:
                fv = float(v)
            except Exception:
                continue
...
def simple_avg_dividend_yield_pct(df: pd.DataFrame) -> float | None:
    """Simple (unweighted) average of per-stock TTM dividend yields across unique holdings."""
    ...
    for _, r in df.iterrows():
        ...
        for c in ("Code", "Ticker", "StockCode", "Symbol"):
            if c in df.columns:
                code = str(r.get(c) or "").strip() or None
                if code:
                    break
        key = _norm_key(code or nm)
        if not key or key in seen:
            continue
        y = get_ttm_dividend_yield_pct(nm, code)
        if y is not None:
            ys.append(float(y))
        seen.add(key)
...
avg_div_yield = simple_avg_dividend_yield_pct(filtered)
...
render_stat_cards(
    [
        ...
        {"label": "Avg Div Yield (TTM)", "value": (f"{avg_div_yield:.2f}%" if avg_div_yield is not None else "—"), "badge": "Simple Avg"},
