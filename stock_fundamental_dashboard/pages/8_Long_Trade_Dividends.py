...
# === Price lookup helper (sync with Bulk Price Edit) =========================
_THIS = os.path.dirname(__file__)
_GRANDP = os.path.abspath(os.path.join(_THIS, "..", ".."))
_VERSION_FILE = os.path.join(_GRANDP, ".data.version")

def _data_etag() -> int:
    """Token changes when either .data.version OR fundamentals data file changes."""
    mts = []
    try:
...
def _build_price_index(etag: int) -> dict:
    # etag is only used to invalidate Streamlit cache
    _ = etag
    df = io_helpers.load_data()
    if df is None or df.empty:
        return {}

    # columns vary by dataset; support common ones
    name_col = next((c for c in ["Name", "name", "Company", "company"] if c in df.columns), None)
    code_col = next((c for c in ["Code", "code", "Ticker", "ticker", "Symbol", "symbol"] if c in df.columns), None)
    price_col = next((c for c in ["CurrentPrice", "current_price", "Price", "price", "Close", "close"] if c in df.columns), None)
    ...
    out = {}
    # map by name (lower)
    for n, p in zip(d[name_col].astype(str), d[price_col].astype(float)):
        out[("name", n.lower())] = float(p)

    if code_col is not None:
        for c, p in zip(d[code_col].astype(str), d[price_col].astype(float)):
            if c and c.lower() != "nan":
                out[("code", c.lower())] = float(p)

    return out

def _lookup_latest_price(name: str | None, code: str | None = None) -> float | None:
    idx = _build_price_index(_data_etag())
    if not idx:
        return None
    if code:
        p = idx.get(("code", str(code).strip().lower()))
        if p is not None:
            return p
    if name:
        return idx.get(("name", str(name).strip().lower()))
    return None
...
filtered["Current"] = filtered.apply(lambda r: _lookup_latest_price(r.get("Name"), r.get("Code") or r.get("Ticker") or r.get("Symbol")), axis=1)
cur = pd.to_numeric(filtered.get("Current"), errors="coerce")


# --- Alerts (Stop-loss / Take-profit) ---
stop = pd.to_numeric(filtered.get("Stop"), errors="coerce")
take = pd.to_numeric(filtered.get("Take"), errors="coerce") if "Take" in filtered.columns else pd.Series(np.nan, index=filtered.index)
tp1 = pd.to_numeric(filtered.get("TP1"), errors="coerce") if "TP1" in filtered.columns else pd.Series(np.nan, index=filtered.index)
...
edited = st.data_editor(
    ...
    column_order=[
        "Select","RowId","Name","Alert","Strategy","Entry","Current","Stop","TP1","TP2","TP3","Take",
        "Shares","RR","OpenDate","R live","Return (%)","Unrealized PnL (RM)",
