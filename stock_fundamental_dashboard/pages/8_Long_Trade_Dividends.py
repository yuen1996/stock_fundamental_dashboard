...
# --- Dividend Yield % + implied DPS (same as Ongoing Trades) ---
...
render_stat_cards(
    [
        ...
        {"label": "Proj Dividend (RM)",  "value": f"{proj_div_gross:,.2f}", "badge": "TTM"},
        {"label": "Div Yield (WGT)",     "value": (f"{(proj_div_gross/cur_value*100.0):.2f}%" if cur_value > 0 else "—"), "badge": "TTM"},
...
st.caption(
    ...
    "Net = Gross × (1 − Tax%). Dividend metrics prefer **View Stock → TTM KPI (Dividend Yield %)** if available; "
