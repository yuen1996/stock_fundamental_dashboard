...
port_div_yield = (float(div_net_total) / cur_value * 100.0) if cur_value > 0 else None
...
render_stat_cards(
    [
        ...
        {"label": "Portfolio Div Yield (NET)", "value": (f"{port_div_yield:.2f}%" if port_div_yield is not None else "—"), "badge": "Weighted", "tone": ("good" if (port_div_yield or 0) >= 4 else ("warn" if (port_div_yield or 0) >= 2 else "neutral"))},
            ],
    columns=3,
)
