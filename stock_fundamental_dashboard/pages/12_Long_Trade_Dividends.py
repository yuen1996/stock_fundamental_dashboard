...
def _find_q_dps_col(df: pd.DataFrame) -> str | None:
    """
    Try common quarterly DPS column names. Accepts 'Q_*' prefixed or not, and
    anything that contains 'dps' or 'dividend per share' or 'dpu'.
    """
...
def _ttm_dps_from_quarters(name: str) -> float | None:
    """Fallback: sum the latest up to 4 quarters of DPS."""
    try:
        df = io_helpers.load_data()
        if df is None or df.empty:
            return None
