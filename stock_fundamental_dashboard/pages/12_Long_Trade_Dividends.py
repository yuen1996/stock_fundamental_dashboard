...
if not dps_by_year.empty:
    now_y = int(pd.Timestamp.now().year)
    row = dps_by_year[dps_by_year["Year"] == now_y]
    if row.empty: row = dps_by_year.tail(1)
    if not row.empty:
        latest_div = f'{float(row["Total Dividend (Net RM)"].iloc[0]):,.2f}'
...
st.caption("Projection = **DPS × Shares × held-fraction** by calendar year from your **OpenDate** (prorated Daily/Monthly). **Tax** uses per-stock `TaxPct` if present; else the default above.")
...
# ─────────────────────────────────────────
# Manage Queue — unchanged (handy here too)
# ─────────────────────────────────────────
st.markdown(
...
if tq.empty:
    ...
    def _plan_ok_row(r):
        ...
        return (np.isfinite(e) and e>0 and np.isfinite(s) and s>0 and e>s and np.isfinite(t) and t>0 and np.isfinite(rr) and (pd.notna(sh) and (int(sh) if not pd.isna(sh) else 0)>0))
    tq["PlanOK"] = tq.apply(_plan_ok_row, axis=1)

    view_cols = ["RowId","Name","Strategy","CurrentPrice","Score","Entry","Stop","Take","Shares","RR","TP1","TP2","TP3","Timestamp","Reasons","PlanOK"]
    ...
    with c1:
        if st.button("✅ Mark Live selected"):
            ...
            for _, r in edited_q.iterrows():
                if not r.Select: continue
                if not bool(r.get("PlanOK", False)):
                    blocked += 1; blocked_ids.append(int(r.RowId)); continue
    ...
    with c2:
        if st.button("🗑️ Delete selected"):
            ...
            if not sel_ids:
                ...
                if hasattr(io_helpers, "delete_trade_rows"):
                    deleted = io_helpers.delete_trade_rows(sel_ids)
                else:
                    deleted = 0
                    for i in sorted(set(sel_ids), reverse=True):
                        if io_helpers.delete_trade_row(int(i)): deleted += 1
                if deleted > 0:
