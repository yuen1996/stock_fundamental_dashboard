...
with st.sidebar:
    ...
    if (model or "").strip().startswith("gpt-5"):
        st.caption("ℹ️ For gpt-5* models, the API always uses default temperature = 1. Slider is ignored.")
...
def _chat_openai(api_key: str, model: str, messages: List[Dict[str,Any]], temperature: float = 0.2):
    ...
    # Build kwargs dynamically so we can omit unsupported params for certain models
    kwargs: Dict[str, Any] = {
        "model": m,
        "messages": messages,
        "stream": True,
    }
    # gpt-5* family currently only supports default temperature=1.
    # API throws 400 if you send any explicit temperature.
