...
# ========== 0) Pick an existing queued row (REQUIRED explicit choice) ==========
qdf_raw = io_helpers.load_trade_queue()
queue_df = (
    qdf_raw.reset_index().rename(columns={"index": "RowId"})
    if (qdf_raw is not None and not qdf_raw.empty) else
    pd.DataFrame(columns=["RowId","Name","Strategy","Entry","Stop","Take","RR"])
)

queue_options = ["— Choose queued idea —", "— New Plan —"] + [
    f"{int(r.RowId)} — {r.Name} ({r.Strategy})"
    for _, r in queue_df.iterrows()
]

sel = st.selectbox(
    "Queued idea to edit (or choose “New Plan”)",
    queue_options,
    index=0
)

# Gate: user must choose either a queued row or 'New Plan'
if sel == "— Choose queued idea —":
    st.info("Please pick a **queued idea** to edit, or select **— New Plan —** to start a fresh plan.")
    st.stop()

editing_rowid = None
prefill = {}
if sel != "— New Plan —" and not queue_df.empty:
    try:
...
# --- tester: clear EV safety lock now ---
clear_key = f"clear_ev_block_btn_{strategy or 'ALL'}"
if st.button("Clear EV safety lock (testing)", key=clear_key, help="Removes the 7-day EV lock immediately"):
    st.session_state[block_key] = None
    st.info("EV safety lock cleared for this strategy.")
