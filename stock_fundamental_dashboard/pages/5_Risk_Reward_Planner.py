...
def _get_live_pref_from_session(stock_name: str, stock_rows: pd.DataFrame) -> str | None:
    """Mirror View Stock's live toggle robustly."""
    ...
    for key in (f"price_src_{sk}", f"view_price_src_{sk}"):
        ...
        if isinstance(v, str) and v.strip():
            s = v.strip().lower()
            if s in ("momentum", "add/edit", "manual", "auto", "default"):
                return _norm_price_src(v)

    # Dicts mapping stock -> src
    ...
    # Global single toggle
    for key in ("view_price_src", "price_src", "price_source", "PRICE_SRC", "PRICE_SOURCE"):
        v = st.session_state.get(key)
        if isinstance(v, str) and v.strip():
            return _norm_price_src(v)

    # Wildcard scan: any matching key containing this stock name
    for key, v in list(st.session_state.items()):
        if isinstance(v, str):
            nv = _norm_price_src(v, default="")
            if nv in ("Momentum", "Add/Edit") and sk in _safe_key(str(key)):
                return nv
...
sel_pref   = _norm_price_src(live_pref or saved_pref or "Momentum")
...
chosen_label = "Add/Edit (manual)" if sel_pref == "Add/Edit" else "Momentum (last close)"
used_label   = label_map.get(used_src, "—")
st.caption(f"Price source used: {used_label}  •  (Selected: {chosen_label}). Change it on View Stock.")
