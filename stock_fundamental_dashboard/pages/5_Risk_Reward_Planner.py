...
# ---- Shared "Price source" (same key View Stock uses) ----
_price_pref_key = f"price_src_{_safe_key(stock_name)}"
st.radio(
    "Price source (shared with View Stock)",
    ["Momentum", "Add/Edit"],
    index=0 if (str(st.session_state.get(_price_pref_key, "Momentum")).lower() == "momentum") else 1,
    key=_price_pref_key,
    horizontal=True,
    help="Momentum = last close from OHLC/Yahoo (from Momentum page). Add/Edit = your manual CurrentPrice."
)
...
with c1:
    # Where should Entry come from?
    entry_src_key = f"entry_src_{editing_rowid}"
    # Default to Follow View Stock if we have a valid price_now; else fall back to Momentum or Manual
    _pref_now = "Follow View Stock" if (price_now is not None) else ("Momentum (auto)" if mom_px is not None else "Manual")
    entry_src = st.radio(
        "Entry source",
        ["Follow View Stock", "Momentum (auto)", "Manual"],
        index=["Follow View Stock", "Momentum (auto)", "Manual"].index(
            st.session_state.get(entry_src_key, _pref_now)
        ),
        horizontal=True,
        key=entry_src_key,
    )

    # Figure out what View Stock is set to (for the caption)
    _ps = str(st.session_state.get(_price_pref_key, "Momentum")).lower()  # "momentum" or "add/edit"

    if entry_src == "Follow View Stock":
        entry = float(price_now) if price_now is not None else float(mom_px or 0.0)
    ...
    elif entry_src == "Momentum (auto)":
        if mom_px is None:
            st.warning("No Momentum OHLC found for this stock — switching to Manual.")
            ...
            st.number_input("Entry Price", value=entry, min_value=0.0, step=0.001, format="%.4f", disabled=True, key="rr_entry")
            if mom_asof is not None:
                st.caption(f"As of {mom_asof.date()} • source: Momentum last close")

    else:  # Manual
        entry_default = float(
            qget("entry", float, None)
            or (row.Entry if _is_num(row.Entry) else (price_now or mom_px or 0.0))
        )
        entry = st.number_input("Entry Price", min_value=0.0, value=entry_default, step=0.001, format="%.4f", key="rr_entry")
