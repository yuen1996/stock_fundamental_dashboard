...
def _latest_current_price(stock_rows: pd.DataFrame) -> float | None:
    """
    Mirror View Stock selection. If the View Stock radio hasn't been touched
    this session, use the SAVED preference (Momentum / Add/Edit / Auto).
    """
    ...
    skey = _safe_key(stock_name)
    saved_pref = _get_price_src_prefs().get(skey, "Auto")
    session_pref = st.session_state.get(f"price_src_{skey}", None)
    pref = _norm_price_src(session_pref if session_pref is not None else saved_pref)

    # keep session in-sync so downstream reads see a valid value
    st.session_state[f"price_src_{skey}"] = pref
...
saved_pref = _get_price_src_prefs().get(_safe_key(stock_name), "Auto")
sel_pref = _norm_price_src(st.session_state.get(_price_pref_key, saved_pref), default=saved_pref)
