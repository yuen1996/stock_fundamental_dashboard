...
if entry and entry > 0 and stop is not None:
    ...
    # Allowed lots (baseline) = min(risk, cost) in lots
    allowed_lots_base = min(lots_risk, lots_cost) if lots_cost > 0 else lots_risk

    # Remaining after the user's planned lots. If planned_lots==0 → show 0.
    lots_planned = int(st.session_state.get("rr_planned_lots", 0) or 0)
    if lots_planned > 0:
        final_allowed_lots = max(allowed_lots_base - lots_planned, 0)
    else:
        final_allowed_lots = 0
...
min_rr = st.slider(
    ...
    help="Block Save/Live if plan R:R is below this"
)
rr_ok = bool(rr is not None and np.isfinite(rr) and rr >= min_rr and shares > 0)
ev_ok = bool(ev_r is not None and ev_r > 0)

if not rr_ok:
    st.warning("R:R below minimum. Increase take-profit or tighten stop.")
if not ev_ok:
    st.warning("Strategy EV ≤ 0 based on your closed-trade history.")

# We'll use rr_ok & ev_ok later to disable Save/Update buttons
...
disabled = not (shares > 0 and np.isfinite(risk_per_share) and risk_per_share > 0 and rr_ok and ev_ok)
