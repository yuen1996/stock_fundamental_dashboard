...
with c1:
    # Choose where Entry comes from
    entry_src_key = f"entry_src_{editing_rowid}"
    default_src = st.session_state.get(entry_src_key) or ("Momentum (auto)" if mom_px is not None else "Manual")
    entry_src = st.radio(
        "Entry source",
        ["Momentum (auto)", "Manual"],
        index=0 if default_src == "Momentum (auto)" else 1,
        horizontal=True,
        key=entry_src_key,
    )

    if entry_src == "Momentum (auto)":
        if mom_px is None:
            st.warning("No Momentum data found for this stock — using Manual.")
            ...
            if mom_asof is not None:
                st.caption(f"As of {mom_asof.date()} (Momentum last close)")
    else:
        # Manual mode
        entry_default = float(qget("entry", float, None) or (row.Entry if _is_num(row.Entry) else (price_now or mom_px or 0.0)))
        entry = st.number_input("Entry Price", min_value=0.0, value=entry_default, step=0.001, format="%.4f", key="rr_entry")
