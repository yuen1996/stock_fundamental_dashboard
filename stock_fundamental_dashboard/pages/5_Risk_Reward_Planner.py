...
def _current_pref(stock_name: str, stock_rows: pd.DataFrame) -> str:
    """
    Resolve current price-source pref with this priority:
      1) session[ticker_key], 2) session[name_key], 3) saved[ticker_key], 4) saved[name_key], else 'Auto'
    """
    prefs = _get_price_src_prefs()
    ...
    # session first
    for k in (tkey, nkey):
        if k and f"price_src_{k}" in st.session_state:
            return _norm_price_src(st.session_state[f"price_src_{k}"])
...
def _latest_current_price(stock_rows: pd.DataFrame) -> float | None:
    """
    Mirror View Stock selection using session-first + ticker-first resolution.
    """
    ...
    # If pref is Auto, prefer Add/Edit when a manual price exists.
    if pref == "Add/Edit" or (pref == "Auto" and manual is not None):
        return manual if manual is not None else mom
    # Momentum (or Auto w/o manual) falls back to manual if OHLC missing
