...
with cD:
    ...
    # Live helper: show the max you can afford by cost at this entry, rounded to lot size
    if entry and entry > 0:
        max_sh_cost = max((math.floor(acct / entry) // lot_size) * lot_size, 0)
        lots = (max_sh_cost // lot_size) if lot_size else 0
        st.caption(f"Max by cost: **{max_sh_cost:,} shares**  (≈ **{lots}** lots)")
    else:
        st.caption("Enter a valid Entry price to see the max shares/lots you can afford.")
...
if shares < shares_by_risk and max_sh_by_cost > 0:
