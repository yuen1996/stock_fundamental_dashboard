...
# ---- Safety locks (planned size + EV threshold & timed block) ----
min_rr = st.slider(
...
# EV threshold + timed block
ev_thresh = st.number_input(
    "EV block threshold (R)", min_value=-1.0, max_value=1.0,
    value=-0.25, step=0.01, format="%.2f",
    help="Block only if Strategy EV (per 1R) is BELOW this. Default −0.25R."
)
ev_block_days = st.number_input(
    "Block duration (days)", min_value=0, max_value=30,
    value=7, step=1, help="How long to block after EV drops below threshold"
)
...
# Start a new block window if EV is below threshold and not already blocked
if ev_bad and (not block_until or now >= block_until):
    st.session_state[block_key] = now + timedelta(days=int(ev_block_days))
    block_until = st.session_state[block_key]

# Manual reset
col_ev1, col_ev2 = st.columns([3,1])
with col_ev2:
    if st.button("Reset EV block", help="Clears the current EV block window"):
        st.session_state[block_key] = None
        block_until = None
        st.info("EV block has been reset for this strategy.")

# Active block?
blocked_active = bool(block_until and now < block_until)

# Note: translate EV and threshold to MYR per trade (using your planned 1R)
...
if blocked_active:
    st.error(f"EV block active for '{strategy}'. Unblocks on {block_until.strftime('%Y-%m-%d %H:%M')}.")
