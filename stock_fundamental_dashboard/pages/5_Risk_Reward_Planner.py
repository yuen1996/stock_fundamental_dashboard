...
def _data_etag() -> int:
    """
    Invalidate like Momentum & Systematic, plus a page-local nonce so
    this page can refresh without clearing global caches.
    """
    try:
        v = int(bus.etag("ohlc"))
    except Exception:
        v = 0
    try:
        dmt = int(os.stat(OHLC_DIR).st_mtime_ns)
    except Exception:
        dmt = 0
    local = int(st.session_state.get("_rrp_refresh_nonce", 0))
    return max(v, dmt) + local
...
def _get_price_src_prefs() -> dict:
    ...
    # IMPORTANT: normalize KEYS so lookups match _safe_key(...) everywhere
    return {_safe_key(str(k)): _norm_price_src(v) for k, v in raw.items()}
...
def _price_pref_for(stock_name: str, stock_rows: pd.DataFrame) -> str:
    ...
    # No saved pref: follow View Stock rule — manual if exists, else momentum
...
def _current_pref(stock_name: str, stock_rows: pd.DataFrame) -> str:
    """
    Priority:
      1) SAVED[ticker] or SAVED[name] (what View Stock set)
      2) If nothing saved: 'Add/Edit' when a manual price exists, else 'Momentum'
    """
    prefs = _get_price_src_prefs()
...
def _sync_pref_to_session(*_args, **_kwargs) -> None:
    # Read-only page: do NOT push price-source to session
    return None
...
with ref_col1:
    if st.button("🔄 Refresh prices & ATR", use_container_width=True):
        st.session_state["_rrp_refresh_nonce"] = st.session_state.get("_rrp_refresh_nonce", 0) + 1
        _safe_rerun()
...
def _latest_current_price(stock_rows: pd.DataFrame, *, prefer: str | None = None) -> float | None:
    ...
    # Use the caller’s preference if provided; otherwise fall back to saved/default rule
    pref = _norm_price_src(prefer) if prefer else _current_pref(stock_name, stock_rows)
...
# Saved pref first…
saved_pref = _current_pref(stock_name, stock_rows)

# …but if View Stock has a live toggle in this session, mirror it:
sp_key = f"price_src_{_safe_key(stock_name)}"
live_pref = st.session_state.get(sp_key, saved_pref)
sel_pref = _norm_price_src(live_pref, default=saved_pref)

using_addedit = (sel_pref == "Add/Edit")
...
price_now = _latest_current_price(stock_rows, prefer=sel_pref)
