...
def _latest_current_price(stock_rows: pd.DataFrame) -> float | None:
    """
    Mirror View Stock selection using the SAVED preference (ticker-first fallback to name).
    """
    ...
    # Prefer the saved pref (ticker-first), and sync session to it
    pref = _price_pref_for(stock_name, stock_rows)
...
# Use saved preference (ticker-first), and mirror it here
sel_pref = _price_pref_for(stock_name, stock_rows)
_price_pref_key = f"price_src_{_safe_key(stock_name)}"
st.session_state[_price_pref_key] = sel_pref  # keep session in sync
...
with c1:
    ...
    st.number_input("Entry Price", value=entry, min_value=0.0, step=0.001, format="%.4f", disabled=True, key="rr_entry")
