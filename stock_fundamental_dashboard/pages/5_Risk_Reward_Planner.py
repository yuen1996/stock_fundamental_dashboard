...
saved_pref = (prefs or {}).get(_safe_key(stock_name))
...
for k in ("view_price_src", "price_src", "price_source", "PRICE_SRC", "PRICE_SOURCE"):
    ...
    if isinstance(v, str) and v.strip():
        global_pref = _norm_price_src(v); break

# NEW: if nothing is set anywhere, DEFAULT to Add/Edit when a manual price exists
manual_hint = _manual_price_from_rows(stock_rows)
default_pref = "Add/Edit" if (manual_hint is not None) else "Momentum"
sel_pref = _norm_price_src(live_pref or saved_pref or global_pref or default_pref)

# NEW: visible per-stock switch that persists in session (keyed by stock)
src_choice = st.radio(
    "Price source (this stock)",
    ["Momentum", "Add/Edit"],
    horizontal=True,
    index=(1 if sel_pref == "Add/Edit" else 0),
    key=f"rr_src_{_safe_key(stock_name)}"
)

# Write the choice to the per-stock live session keys so other pages can see it too
st.session_state[f"price_src_{_safe_key(stock_name)}"] = src_choice
st.session_state["price_src_by_stock"] = {
    **(st.session_state.get("price_src_by_stock", {})),
    stock_name: src_choice,
    _safe_key(stock_name): src_choice,
}

# Finally resolve the price using the chosen source
price_now, used_src = _latest_current_price(stock_rows, prefer=src_choice, return_src=True)
