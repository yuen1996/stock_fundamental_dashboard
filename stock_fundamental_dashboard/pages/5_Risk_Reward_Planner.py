...
def _get_price_src_prefs() -> dict:
    ...
    out = {k: _norm_price_src(v) for k, v in raw.items()}
    if out != raw:  # heal bad values once
...
def _current_pref(stock_name: str, stock_rows: pd.DataFrame) -> str:
    """
    Resolve price-source pref with this priority:
      1) SAVED[ticker], 2) SAVED[name], 3) SESSION[ticker], 4) SESSION[name], else 'Auto'
    This ensures that changes made on View Stock (which persist to settings) win,
    even if RRP has stale session values.
    """
    prefs = _get_price_src_prefs()
...
def _latest_current_price(stock_rows: pd.DataFrame) -> float | None:
    """
    Use the exact preference (Momentum/Add/Edit/Auto).
    For Auto, prefer Momentum (last close); if missing, fall back to manual.
    """
    ...
    _sync_pref_to_session(stock_name, stock_rows, pref)  # just mirror to session
    ...
    # Auto: prefer Momentum, then manual
    return mom if mom is not None else manual
