...
# Live helper: show MAX BY RISK (primary), cost only if it binds
if entry and entry > 0:
    # risk per share uses your stop
    rps = max(entry - stop, 0.0)
    # risk-based cap (cash risk = acct * risk%)
    max_sh_risk_raw = math.floor((acct * (risk_pct / 100.0)) / rps) if rps > 0 else 0
    max_sh_risk = max((max_sh_risk_raw // lot_size) * lot_size, 0)
    lots_risk = (max_sh_risk // lot_size) if lot_size else 0
    st.caption(f"Max by **risk**: **{max_sh_risk:,} shares**  (≈ **{lots_risk}** lots)")

    # if buying-power is tighter, show it as a note
    max_sh_cost = max((math.floor(acct / entry) // lot_size) * lot_size, 0)
    if max_sh_cost and max_sh_cost < max_sh_risk:
        lots_cost = max_sh_cost // lot_size if lot_size else 0
        st.caption(f"_Note: buying-power cap reduces to **{max_sh_cost:,}** shares (≈ {lots_cost} lots)._")
else:
    st.caption("Enter a valid Entry & Stop to see the max shares/lots by risk.")
