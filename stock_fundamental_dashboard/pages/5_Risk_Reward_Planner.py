...
def _get_live_pref_from_session(stock_name: str, stock_rows: pd.DataFrame) -> str | None:
    """
    Mirror View Stock's live toggle.
    View Stock writes st.session_state['price_src_{safe_name}'] to 'Momentum'/'Add/Edit'
    and also persists it to app_settings.json.
    """
    safe_name = _safe_key(stock_name)
    # try exact key used by View Stock
    v = st.session_state.get(f"price_src_{safe_name}")
...
def _latest_current_price(stock_rows: pd.DataFrame, *, prefer: str | None = None) -> float | None:
    ...
    # use caller-provided pref, else live session, else saved setting, else momentum
    live_pref  = _get_live_pref_from_session(stock_name, stock_rows)
    saved_pref = _get_price_src_prefs().get(_safe_key(stock_name))
    pref = _norm_price_src(prefer or live_pref or saved_pref or "Momentum")
...
live_pref  = _get_live_pref_from_session(stock_name, stock_rows)
saved_pref = _get_price_src_prefs().get(_safe_key(stock_name))
sel_pref   = _norm_price_src(live_pref or saved_pref or "Momentum")  # what we're following

price_now  = _latest_current_price(stock_rows, prefer=sel_pref)

# small label so it’s obvious what we’re using (and a link to change it)
src_label = "Momentum (last close)" if sel_pref == "Momentum" else "Add/Edit (manual)"
st.caption(f"Price source: {src_label}. Change it on View Stock.")
try:
...
with c2:
    ...
    if stop_mode == "Manual price":
        # follow entry: default 5% below entry, locked
        stop = max(0.0, entry * 0.95)
        st.session_state["rr_stop"] = stop
        st.number_input(
            "Stop-loss Price",
            key="rr_stop",
            min_value=0.0, value=stop, step=0.001, format="%.4f",
            disabled=True,
        )
...
with c3:
    take = max(0.0, entry * 1.15)
    st.session_state["rr_take"] = take
    st.number_input(
        "Take-profit Price",
        key="rr_take",
        min_value=0.0, value=take, step=0.001, format="%.4f",
        disabled=True,
    )
with c4:
