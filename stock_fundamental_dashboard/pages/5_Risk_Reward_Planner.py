...
# --- Make the summary use the user's *planned* lots ---
lots_planned = int(st.session_state.get("rr_planned_lots", 0) or 0)
planned_shares = int(lots_planned * lot_size)

planned_cost = planned_shares * entry
planned_loss = planned_shares * max(entry - stop, 0.0)          # loss at your chosen stop
...
# Full-size (no scroll): use st.table instead of st.dataframe
st.table(detail.round(4))
