...
def _latest_current_price(stock_rows: pd.DataFrame) -> float | None:
    ...
    this session, behave like 'Auto' (prefer Add/Edit if available).
    """
    ...
    # What View Stock last chose for this name (Momentum / Add/Edit / Auto)
    pref = st.session_state.get(f"price_src_{_safe_key(stock_name)}", "Auto")
    ...
    # If View Stock is Auto, prefer Add/Edit when a manual price exists.
...
sel_pref = str(st.session_state.get(_price_pref_key, "Auto")).strip().lower()
...
# Compute what we are actually using (matches _latest_current_price logic)
...
# Lookups for current price (now guaranteed to follow View Stock)
