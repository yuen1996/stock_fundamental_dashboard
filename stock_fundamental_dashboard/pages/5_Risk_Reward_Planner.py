...
def _safe_rerun():
    """Call st.rerun() on new Streamlit; fall back to st.experimental_rerun() on older versions."""
    ...
    if callable(fn):
        fn()
    # If neither exists, do nothing (user can manually refresh)
...
def _data_etag() -> int:
    """
    Use the same invalidation signal as Momentum & Systematic Decision:
      1) bus.etag("ohlc")  (preferred)
      2) directory mtime    (fallback)
    """
    try:
        v = int(bus.etag("ohlc"))
        if v:
...
# ---- Shared settings access for price-source prefs ----
...
def _norm_price_src(v, *, default="Add/Edit") -> str:
    """
    Normalize price-source strings from settings/session.
    Accepts 'Add & Edit', 'Add/Edit', 'manual', etc.
    Default to 'Add/Edit' (since View Stock has no Auto anymore).
    """
    s = re.sub(r"[^a-z]+", "", str(v or "")).lower()
    if s in {"momentum", "mom"}:
        return "Momentum"
    if s in {"addedit", "addandedit", "manual", "edit"}:
        return "Add/Edit"
    if s in {"auto", "automatic", "default"}:
        # If something still writes 'Auto', treat it like Add/Edit when manual exists (handled below),
        # but normalize to 'Add/Edit' to avoid toggling.
        return "Add/Edit"
    return default
...
def _get_price_src_prefs() -> dict:
    ...
    # Normalize known values; do NOT downgrade unknowns to Momentum.
    out, changed = {}, False
    for k, v in raw.items():
        norm = _norm_price_src(v, default=v)  # keep original if unknown
        out[k] = norm
        if norm != v:
            changed = True
...
def _price_pref_for(stock_name: str, stock_rows: pd.DataFrame) -> str:
    """Resolve price-source pref using ticker (preferred) then name."""
    prefs = _get_price_src_prefs()
    keys = []

    # Try ticker-like columns first
    for col in ("Ticker", "Code", "Symbol"):
        if col in stock_rows.columns and stock_rows[col].dropna().size:
            keys.append(_safe_key(str(stock_rows[col].dropna().iloc[-1])))
            break

    # Fallback to name key
    keys.append(_safe_key(stock_name))

    # First matching saved pref wins; default Auto
    for k in keys:
        v = prefs.get(k)
        if v:
            # keep session in sync so other pages see the same value
            st.session_state[f"price_src_{k}"] = _norm_price_src(v)
            return _norm_price_src(v)
    return "Auto"

def _resolve_pref_keys(stock_name: str, stock_rows: pd.DataFrame) -> tuple[Optional[str], str]:
    """Return (ticker_key, name_key) used to store/read price-source prefs."""
    ticker_key = None
    for col in ("Ticker", "Code", "Symbol"):
        if col in stock_rows.columns and stock_rows[col].dropna().size:
            ticker_key = _safe_key(str(stock_rows[col].dropna().iloc[-1]))
            break
    name_key = _safe_key(stock_name)
    return ticker_key, name_key

def _current_pref(stock_name: str, stock_rows: pd.DataFrame) -> str:
    """
    Resolve price-source preference with this priority:
      1) SAVED[ticker], 2) SAVED[name], 3) SESSION[ticker], 4) SESSION[name],
      else (since View Stock has no Auto): 'Add/Edit' if a manual price exists, otherwise 'Momentum'.
    """
    prefs = _get_price_src_prefs()
    tkey, nkey = _resolve_pref_keys(stock_name, stock_rows)

    # SAVED first (what View Stock writes)
    for k in (tkey, nkey):
        if k and k in prefs:
            return _norm_price_src(prefs[k])

    # SESSION next (fallback only)
    for k in (tkey, nkey):
        v = st.session_state.get(f"price_src_{k}")
        if k and v:
            return _norm_price_src(v)

    # FINAL DEFAULT now that there's no Auto on View Stock:
    manual_exists = _manual_price_from_rows(stock_rows) is not None
    return "Add/Edit" if manual_exists else "Momentum"

def _sync_pref_to_session(stock_name: str, stock_rows: pd.DataFrame, pref: str) -> None:
    """Write the resolved pref to BOTH ticker and name keys so View Stock & RRP agree."""
    tkey, nkey = _resolve_pref_keys(stock_name, stock_rows)
    for k in (tkey, nkey):
        if k:
            st.session_state[f"price_src_{k}"] = _norm_price_src(pref)
...
ref_col1, ref_col2 = st.columns([1,6])
with ref_col1:
    if st.button("🔄 Refresh prices & ATR", use_container_width=True):
        try:
            st.cache_data.clear()
        except Exception:
            pass
...
def _latest_current_price(stock_rows: pd.DataFrame) -> float | None:
    """
    Use the exact preference (Momentum/Add/Edit).
    If anything still says 'Auto' internally, we treat it as:
      prefer manual (Add/Edit) when present; else Momentum.
    """
    ...
    pref = _current_pref(stock_name, stock_rows)
    _sync_pref_to_session(stock_name, stock_rows, pref)
    ...
    # Defensive fallback (in case something passed 'Auto' or unknown):
    return manual if manual is not None else mom

@st.cache_data(show_spinner=False)
def _load_ohlc_local(name: str, _etag: int) -> pd.DataFrame | None:
    """
    Load OHLC for ATR the same way View/Momentum do:
    - find the right CSV via name_link.find_ohlc_path(...)
    - invalidate on bus.etag("ohlc") via the _etag param
    """
    try:
        ohlc_dir = os.path.join(_PARENT, "data", "ohlc")
        # Prefer the shared resolver (handles ticker-aliased files too)
        path = name_link.find_ohlc_path(name, ohlc_dir=ohlc_dir)
        if not path or not os.path.exists(path):
            # last resort: "<raw_name>.csv" in the same folder
        ...
        # Ensure Close exists even if only Adj Close was present
...
def _manual_price_from_rows(rows: pd.DataFrame) -> float | None:
    # Common variations written by Add/Edit pages
    candidates = (
        "CurrentPrice", "Current Price", "ManualPrice", "Manual Price",
        "EndQuarterPrice", "Price", "SharePrice", "CMP",
...
# ---- Price source — read-only (follows View Stock page) ----
stock_rows = df[df["Name"] == stock_name].sort_values(["Year"])  # define first
manual_px  = _manual_price_from_rows(stock_rows)
mom_px, mom_asof = _momentum_close_before_today(stock_name, stock_rows)

# Resolve pref (session-first, ticker-first) and mirror to BOTH keys
sel_pref = _current_pref(stock_name, stock_rows)
_sync_pref_to_session(stock_name, stock_rows, sel_pref)

using_addedit = (sel_pref == "Add/Edit") or (sel_pref == "Auto" and manual_px is not None)
using_label   = "Add/Edit (manual)" if using_addedit else "Momentum (last close)"

st.markdown(f"**Price source:** {using_label}  ·  Change this on **View Stock**.")
try:
    st.page_link("pages/3_View_Stock.py", label="Open View Stock", icon="📈")
except Exception:
    pass

# Lookups for current price based on the same pref
...
with c1:
    entry = float(price_now or 0.0)
    st.number_input("Entry Price", value=entry, min_value=0.0, step=0.001, format="%.4f", disabled=True)

    if using_addedit:
        st.caption("Source: View Stock → Add/Edit current price")
    else:
        if mom_asof is not None:
            st.caption(f"As of {mom_asof.date()} • source: View Stock → Momentum last close")
        else:
            st.caption("Source: View Stock → Momentum last close")
