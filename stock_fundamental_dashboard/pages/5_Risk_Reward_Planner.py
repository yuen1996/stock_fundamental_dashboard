...
sel = st.selectbox("Queued idea to edit", options, index=0, key="rr_queue_sel")

# Ensure switching queued idea resets per-row widgets immediately (Entry/Stop/Take etc.)
_prev_sel = st.session_state.get("rr_queue_sel_prev")
if sel != _prev_sel:
    st.session_state["rr_queue_sel_prev"] = sel
    # Clear per-row widget state so defaults follow the newly selected queued idea
    for _k in (
        "rr_stock_locked",
        "rr_strategy_locked",
        "rr_entry",
...
with cA:
    ...
    st.text_input("Stock (from queue)", value=stock_name, disabled=True, key="rr_stock_locked")

with cB:
    strategy = str(row.Strategy)
    st.text_input("Strategy (from queue)", value=strategy, disabled=True, key="rr_strategy_locked")
