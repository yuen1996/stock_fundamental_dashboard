...
# ---- Settings (so we can read the persisted price source written by View Stock) ----
SETTINGS_PATH = os.path.join(_PARENT, "data", "app_settings.json")

def _settings_etag() -> int:
    try:
        return int(os.stat(SETTINGS_PATH).st_mtime_ns)
    except Exception:
        return 0

@st.cache_data(show_spinner=False)
def _get_price_src_prefs_cached(_etag: int) -> dict:
    # cache by settings file mtime so View Stock changes are picked up immediately
    return _get_price_src_prefs()
...
def _get_price_src_prefs() -> dict:
    """Read persisted {safe_name -> 'Momentum'|'Add/Edit'} written by View Stock."""
    d = _load_settings()
...
def _rows_for_stock(master_df: pd.DataFrame, name: str) -> pd.DataFrame:
    """Robust row lookup by exact and normalized name (handles small diffs)."""
    ...
    if not rows.empty:
        return rows.sort_values(["Year"]) if "Year" in rows.columns else rows
    safe = _safe_key(str(name))
    safe_series = master_df["Name"].astype(str).map(_safe_key)
    rows = master_df[safe_series == safe]
    return rows.sort_values(["Year"]) if "Year" in rows.columns else rows

def _get_live_pref_from_session(stock_name: str, stock_rows: pd.DataFrame) -> str | None:
    """Return ONLY a per-stock live preference written by View Stock."""
    sk = _safe_key(stock_name)

    # Exact per-stock keys
    for key in (f"price_src_{sk}", f"view_price_src_{sk}"):
        v = st.session_state.get(key)
        if isinstance(v, str) and v.strip():
            return _norm_price_src(v)

    # Dicts mapping stock -> src (per-stock)
    ...
    # Do NOT consider global keys here (we'll handle them as last fallback outside).
    return None
...
def _latest_current_price(
    ...
    *,
    ...
    """
    Get price following the selected source, but also tell which source was actually used.
    return_src: if True, returns (price, 'add'|'mom')
    """
    ...
    pref = _norm_price_src(prefer or "Momentum")
    manual = _manual_price_from_rows(stock_rows)
    mom, _asof = _momentum_close_before_today(stock_name, stock_rows)

    if pref == "Add/Edit":
        if manual is not None:
            return (manual, "add") if return_src else manual
        # manual missing → explicit fallback to Momentum (and we report it)
        return (mom, "mom") if return_src else mom
    else:
        if mom is not None:
            return (mom, "mom") if return_src else mom
        # momentum missing → fallback to manual
        return (manual, "add") if return_src else manual

def _resolve_saved_pref(stock_name: str, stock_rows: pd.DataFrame, prefs: dict) -> str | None:
    """
    Find the saved preference for this stock using multiple keys:
    - safe(Name)
    - safe(Ticker/Code/Symbol) if available
    - any JSON key that normalizes to the same safe key
    """
    if not isinstance(prefs, dict) or not prefs:
        return None

    cand_keys = {_safe_key(stock_name)}
    if stock_rows is not None and not stock_rows.empty:
        for col in ("Ticker", "Code", "Symbol"):
            if col in stock_rows.columns and stock_rows[col].dropna().size:
                cand_keys.add(_safe_key(str(stock_rows[col].dropna().iloc[-1])))
    ...
    # last resort: scan the dict and compare normalized forms
    for raw_k, v in prefs.items():
        if _safe_key(str(raw_k)) in cand_keys:
            return v
...
# Lookups for current price (mirror View Stock)
prefs      = _get_price_src_prefs_cached(_settings_etag())
stock_rows = _rows_for_stock(df, stock_name)

# 1) Per-stock live (session)
live_pref  = _get_live_pref_from_session(stock_name, stock_rows)

# 2) Saved per-stock (persisted by View Stock)
saved_pref = _resolve_saved_pref(stock_name, stock_rows, prefs or {})

# 3) Global only if nothing per-stock is present
global_pref = None
for k in ("view_price_src", "price_src", "price_source", "PRICE_SRC", "PRICE_SOURCE"):
    v = st.session_state.get(k)
    if isinstance(v, str) and v.strip():
        global_pref = _norm_price_src(v)
        break

sel_pref = _norm_price_src(live_pref or saved_pref or global_pref or "Momentum")

price_now, used_src = _latest_current_price(stock_rows, prefer=sel_pref, return_src=True)

# quick peek at both prices to explain any fallback
_m = _manual_price_from_rows(stock_rows)
_mom, _mom_asof = _momentum_close_before_today(stock_name, stock_rows)

label_map   = {"add": "Add/Edit (manual)", "mom": "Momentum (last close)"}
chosen_lbl  = "Add/Edit (manual)" if sel_pref == "Add/Edit" else "Momentum (last close)"
used_lbl    = label_map.get(used_src, "—")

source_note = f"Price source used: {used_lbl}  •  Selected: {chosen_lbl}"
if sel_pref == "Add/Edit" and used_src == "mom":
    source_note += "  •  Manual price missing → fell back to Momentum."

# SHOW IT (this line was missing in your last paste)
st.caption(source_note)

with st.expander("debug price lookup", expanded=False):
    st.write({
        "live_pref(session)": live_pref,
        "saved_pref(json)": saved_pref,
        "global_pref": global_pref,
        "sel_pref": sel_pref,
        "manual_found": _m,
        "momentum_found": _mom,
        "momentum_asof": (_mom_asof.date() if _mom_asof is not None else None),
    })
...
# Helpful hint if we had to fall back because manual is missing
manual_missing = (sel_pref == "Add/Edit" and used_src == "mom")
if manual_missing:
    source_note += "  •  Manual price missing → fell back to Momentum."
...
with c1:
    ...
    # force-sync the widget's value to the computed entry every run
    st.session_state["rr_entry"] = entry
    st.number_input(
        "Entry Price",
        key="rr_entry",
        value=entry, min_value=0.0, step=0.001, format="%.4f",
        disabled=True,
    )

    if sel_pref == "Momentum":
        mom_px, mom_asof = _momentum_close_before_today(stock_name, stock_rows)
        st.caption(f"As of {mom_asof.date() if mom_asof is not None else 'latest close'} • source: Momentum")
    else:
        st.caption("Source: Add/Edit current price")
