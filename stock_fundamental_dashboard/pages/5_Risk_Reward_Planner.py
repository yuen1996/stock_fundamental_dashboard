...
# Ensure switching queued idea resets per-row widgets immediately (Entry/Stop/Take etc.)
_RR_PER_ROW_KEYS = [
    "rr_stock_locked",
    "rr_strategy_locked",
    "rr_entry",
...
def _rr_on_queue_change():
    for _k in _RR_PER_ROW_KEYS:
        st.session_state.pop(_k, None)

sel = st.selectbox("Queued idea to edit", options, index=0, key="rr_queue_sel", on_change=_rr_on_queue_change)
...
# Row-scoped widget key helper (prevents stale widget state when switching queued ideas)
def _rk(base: str) -> str:
    return f"{base}__{editing_rowid}"
...
# Wrap inputs in an area card (LOCKED to the queued row)
st.markdown('<div class="area">', unsafe_allow_html=True)
cA, cB = st.columns([2, 1])

# Always follow the selected queued idea (read-only)
with cA:
    stock_name = str(row.Name)
    st.text_input("Stock (from queue)", value=stock_name, disabled=True, key=_rk("rr_stock_locked"))

with cB:
    strategy = str(row.Strategy)
    st.text_input("Strategy (from queue)", value=strategy, disabled=True, key=_rk("rr_strategy_locked"))
...
with c1:
    ...
    entry = st.number_input("Entry Price", min_value=0.0, value=entry_default, step=0.001, format="%.4f", key=_rk("rr_entry"))
with c2:
    # --- ATR Stop Mode support here ---
    stop_mode = st.radio("Stop-loss Mode",
                         ["Manual price", "% below entry", "ATR-based"],
                         index=0, horizontal=True, key=_rk("rr_stop_mode"))
    ...
    if stop_mode == "Manual price":
        ...
        stop  = st.number_input("Stop-loss Price", min_value=0.0, value=stop_default, step=0.001, format="%.4f", key=_rk("rr_stop"))
    elif stop_mode == "% below entry":
        pct = st.number_input("Stop % below entry", min_value=0.1, max_value=90.0,
                              value=5.0, step=0.1, format="%.1f", key=_rk("rr_stop_pct"))
        ...
        atr_mult = st.number_input("ATR multiplier (×)", min_value=0.25, max_value=10.0,
                                   value=2.0, step=0.25, format="%.2f", key=_rk("rr_atr_mult"))
...
with c3:
    ...
    take  = st.number_input("Take-profit Price", min_value=0.0, value=take_default, step=0.001, format="%.4f", key=_rk("rr_take"))
with c4:
    acct = st.number_input("Account Size (MYR)", min_value=0.0, value=10000.0, step=100.0, format="%.2f", key=_rk("rr_acct"))
    risk_pct = st.number_input("Risk % per Trade", min_value=0.1, max_value=30.0, value=1.0, step=0.1, format="%.1f", key=_rk("rr_riskpct"))
    lot_size = st.number_input("Lot Size (shares)", min_value=1, value=100, step=1, help="Shares per 1 lot (e.g. 100).", key=_rk("rr_lot"))
    planned_lots = st.number_input("Planned lots to buy", min_value=0, value=int(row.Shares // lot_size if _is_num(row.Shares) else 0), step=1, help="Your intended lots.", key=_rk("rr_planned_lots"))
...
lots_planned_now   = int(st.session_state.get(_rk("rr_planned_lots"), planned_lots) or 0)
...
# --- Testing override: allow clearing EV block (session only) ---
# This does NOT change your EV logic; it just sets a session flag to bypass the lock while you test.
with st.expander("🧪 Testing tools (optional)", expanded=False):
    if st.button("🔓 Clear EV block (this session)", use_container_width=True):
        st.session_state[global_clear_key] = True
        st.session_state[clear_key] = True
        st.session_state.pop(block_key, None)
        st.success("EV block cleared for this session. (It will remain bypassed until you close/restart the browser session.)")
        try:
            st.rerun()
        except Exception:
...
lots_planned   = int(st.session_state.get(_rk("rr_planned_lots"), lots_planned_now) or 0)
...
if atr_available and (atr_val is not None):
    atr_mult_used = float(st.session_state.get(_rk("rr_atr_mult"), 2.0) or 2.0)
...
with colL:
    st.text_area("Plan summary (stored as `Reasons`)", value=reason_text, height=110, key=_rk("rr_reason"))
...
with colR:
    if st.button("💾 Update queued plan", type="primary", use_container_width=True, disabled=disabled):
        ...
        fields = dict(
            ...
            Reasons=st.session_state.get(_rk("rr_reason"), reason_text),
        )
