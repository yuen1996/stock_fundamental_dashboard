...
_price_pref = str(st.session_state.get(_price_pref_key, "Momentum")).lower()
_src_human = "Momentum (last close)" if _price_pref == "momentum" else "Add/Edit (manual)"

st.markdown(f"**Price source:** {_src_human}  ·  Change this on **View Stock**.")
try:
    # Nice to have (works on newer Streamlit): jump to the page that controls this
...
with c1:
    # Entry always follows View Stock's current price selection
    ...
    # Helpful caption about where it came from
    if _ps == "momentum":
