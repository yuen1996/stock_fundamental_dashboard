...
# ---- Unified OHLC resolver (shared with Momentum / Add/Edit / View Stock) ----
try:
    from utils.ohlc_resolver import (
        load_ohlc as _load_ohlc_unified,
        default_ohlc_dir as _default_ohlc_dir,
    )
except Exception:
    _load_ohlc_unified = None
    _default_ohlc_dir = None
...
def _resolve_ohlc_dir() -> str:
    """
    Single source of truth for OHLC folder.

    Must match where 9_Momentum_Data.py saves:
      <project_root>/data/ohlc
    """

    # 1) If unified resolver exists (utils.ohlc_resolver), trust it.
    if _default_ohlc_dir is not None:
        try:
            d = os.path.abspath(str(_default_ohlc_dir()))
            if os.path.isdir(d):
                return d
        except Exception:
            pass

    # 2) Prefer <repo>/data/ohlc discovered relative to this file / cwd
    this_dir = os.path.abspath(os.path.dirname(__file__))
    candidates = [
        os.path.abspath(os.path.join(this_dir, "..", "data", "ohlc")),       # pages/.. -> repo_root/data/ohlc
        os.path.abspath(os.path.join(os.getcwd(), "data", "ohlc")),          # cwd/data/ohlc
        os.path.abspath(os.path.join(this_dir, "..", "..", "data", "ohlc")), # extra defensive
    ]
    for d in candidates:
        if os.path.isdir(d):
            return d

    # 3) Only as a fallback, respect name_link.resolve_ohlc_dir() if it exists & is real
    try:
        resolver = getattr(name_link, "resolve_ohlc_dir", None)
        if callable(resolver):
            d = os.path.abspath(str(resolver()))
            if os.path.isdir(d):
                return d
    except Exception:
        pass

    # 4) Last resort: create <repo>/data/ohlc next to pages/
    d = candidates[0]
...
def _load_ohlc_for_name(
    ...
    *,
    ...
    """
    Load OHLC for `stock_name` using:
      1) utils.ohlc_resolver.load_ohlc (if available)
      2) name_link.find_ohlc_path inside the resolved OHLC dir

    Accepts either company Name or Code/Ticker; tries several variants.
    """

    # candidate keys in order of preference
    keys: list[str] = []
    if ticker:
        keys.append(str(ticker).strip())
    if stock_name:
        s = str(stock_name).strip()
        if s:
            keys.append(s)
            keys.append(_safe_name(s))
    ...
    # --- 1) Preferred: unified resolver (utils.ohlc_resolver.load_ohlc) ---
    if _load_ohlc_unified is not None:
        for key in keys:
            try:
                df, path, how = _load_ohlc_unified(key)
            except Exception:
                df = None
    ...
    # --- 2) Fallback: name_link.find_ohlc_path in our resolved dir ---
    ohlc_dir = _resolve_ohlc_dir()
    finder = getattr(name_link, "find_ohlc_path", None)
    ...
    if callable(finder):
        for key in keys:
            ...
            try:
                # new-style signature
                path = finder(key, ohlc_dir=ohlc_dir)
            except TypeError:
                # old-style: (stock_name, ohlc_dir=..., ticker=...)
                try:
                    path = finder(stock_name, ohlc_dir=ohlc_dir, ticker=key)
                except Exception:
                    path = None
            except Exception:
                path = None
