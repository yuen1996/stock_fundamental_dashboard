...
def _segmented(label: str, options: list[str], default: str) -> str:
    """Prefer segmented_control; fallback to radio if not available."""
    try:
        return st.segmented_control(label, options=options, default=default)  # type: ignore[attr-defined]
    except Exception:
...
def _pct_change_from_ohlc(name: str, latest_row: Mapping[str, Any]) -> float | None:
    ...
    # use last two trading days strictly before today; fall back to last two rows
    if len(prior) >= 2:
        a, b = float(prior["Close"].iloc[-2]), float(prior["Close"].iloc[-1])
    elif len(dfp) >= 2:
        a, b = float(dfp["Close"].iloc[-2]), float(dfp["Close"].iloc[-1])
    else:
        return None
...
# Helper: first non-empty string from keys
...
# Helper: best-effort to find a % change column that your momentum page wrote
def _find_pct_change(row: Mapping[str, Any]) -> float | None:
    # Common names your momentum page might have saved
    CAND = [
        "ChangePct", "Change %", "Price Change %", "DailyChange%", "MomentumChangePct",
    ...
    # Fallback: derive from price vs previous close if present
    prev = _first_numeric(row, ("PrevClose", "PreviousClose", "ClosePrev", "Close_Yesterday"))
    curr = _first_numeric(row, _PRICE_KEYS)
...
# Build heatmap tiles from the latest rows we already computed above
tiles: list[dict[str, Any]] = []
for name, latest in latest_rows.items():
    pct = _find_pct_change(latest)
    if pct is None:
        # NEW: fallback to OHLC files written by your Momentum page
        pct = _pct_change_from_ohlc(name, latest)

    if pct is None:
        continue

    code = _first_str(latest, ("Code", "Ticker", "Symbol"))
    label = code or name  # requested: just stock code; falls back to Name if code missing
    tiles.append({
        "Name": name,
        "Label": label,
        "ChangePct": float(pct),
        "Size": abs(float(pct)) + 1e-4,  # avoid zeros so tiles still render
    })


if not tiles:
    st.info("No momentum % change found yet. Make sure your momentum page writes a '% Change' (or similar) column, or a previous close to derive it.")
else:
    hdf = pd.DataFrame(tiles)

    # Single-level treemap; size = |% change|, color = signed % change
    fig = px.treemap(
        hdf,
    ...
    fig.update_traces(
        # Show code/name and % change; we read % from custom_data to ensure correct rounding & sign
