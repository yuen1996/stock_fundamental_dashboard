...
if wl.empty:
    ...
    # ---- Promote to Trade Queue (NEW) ----
    st.markdown("#### ➕ Promote from Watchlist to Trade Queue")
    active_wl = keep[keep["Active"] == True] if "keep" in locals() else wl[wl["Active"] == True]
    if active_wl.empty:
        st.info("No active watchlist items to promote.")
    else:
        # Select a ruleset once (applies to each promote by default; can override per-item)
        default_ruleset = st.selectbox("Default ruleset for promotion", list(rules.RULESETS.keys()), index=0, key="promote_default_ruleset")
        st.caption("Choose a row below and click **Promote** (or **Plan Trade**) to move it to the queue.")
        for i, r in active_wl.reset_index(drop=True).iterrows():
            with st.container():
                c1, c2, c3, c4, c5 = st.columns([2, 1.2, 1.6, 1.2, 1.4])
                name = r["Name"]
                c1.write(f"**{name}**")
                # Allow override of strategy per row
                strat = c2.selectbox(
                    "Strategy",
                    list(rules.RULESETS.keys()),
                    index=list(rules.RULESETS.keys()).index(default_ruleset),
                    key=f"wl_strat_{i}_{name}"
                )
                # Use mapped latest price if available
                curp = latest_prices.loc[name, "CurrentPrice"] if name in latest_prices.index else np.nan
                c3.write(f"Current: {curp:,.4f}" if pd.notna(curp) else "Current: N/A")

                # Promote → minimal queue row; Reasons carry over from Notes
                if c4.button("Promote", key=f"wl_prom_{i}_{name}"):
                    try:
                        io_helpers.push_trade_candidate(
                            name=name,
                            strategy=strat,
                            score=0,
                            current_price=float(curp) if pd.notna(curp) else None,
                            reasons=str(r.get("Notes") or "")
                        )
                        st.success(f"Promoted **{name}** → Trade Queue")
                        try:
                            st.rerun()
                        except Exception:
                            st.experimental_rerun()
                ...
                # Optional: jump into Planner pre-filled
                if c5.button("Plan Trade", key=f"wl_plan_{i}_{name}"):
                    params = {
                        "stock":    name,
                        "strategy": strat,
                        "entry":    float(curp) if pd.notna(curp) else "",
                        "r":        2.0,
                        "riskpct":  1.0,
                        "lot":      100,
                    }
...
# Optional quick peek of raw queue (handy to confirm immediate updates)
