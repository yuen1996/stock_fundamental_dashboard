...
def _resolve_ohlc_dir() -> str:
    ...
    1) Prefer utils.ohlc_resolver.default_ohlc_dir()
    2) Fallback to legacy name_link.resolve_ohlc_dir()
    3) Fallback to ./data/ohlc
    """
    # Preferred: unified resolver (same as Momentum page)
    if _default_ohlc_dir is not None:
        try:
            return str(_default_ohlc_dir())
        except Exception:
            pass
...
def _load_ohlc_for_name(stock_name: str, _etag: int, *, ticker: str | None = None) -> pd.DataFrame | None:
    """
    Load OHLC using the SAME resolver as Momentum/Add/Edit/View.

    Primary: utils.ohlc_resolver.load_ohlc(...)
    Fallback: legacy name_link-based CSV lookup.
    """
    key = (ticker or stock_name or "").strip()
    if not key:
        return None

    # ---- 1) Unified resolver (preferred) ----
    if _load_ohlc_unified is not None:
        try:
            df, path, how = _load_ohlc_unified(key)
        except Exception:
            df, path, how = None, None, ""
    ...
    # ---- 2) Legacy fallback: name_link finder ----
    try:
