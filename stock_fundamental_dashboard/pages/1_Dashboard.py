...
if submitted:
    ...
    if not name_input:
        ...
        st.success(f"Watchlist updated for '{name_input}'. Refresh to see it below.")
...
if wl.empty:
    ...
    # enrich with computed cols
    disp = wl.copy()
    disp.insert(0, "Delete", False)
    disp["CurrentPrice"] = disp["Name"].map(lambda n: latest_prices.loc[n, "CurrentPrice"] if n in latest_prices.index else pd.NA)
    # numeric conversions
    ...
    # upside
    disp["Upside %"] = ((disp["TargetPrice"] - disp["CurrentPrice"]) / disp["CurrentPrice"] * 100).round(2)
    disp.loc[disp["CurrentPrice"].isna(), "Upside %"] = pd.NA
