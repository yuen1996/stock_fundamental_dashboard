...
# ---- Persistent picker (shared across pages) ----
GLOBAL_KEY = "sel_names_global"           # <-- keep this EXACT string on all pages
UI_KEY     = "sel_names_dashboard"        # <-- page-local key; must be UNIQUE per page

st.session_state.setdefault(GLOBAL_KEY, [])

# Use any previously saved picks that still exist on this page as the default
_seed = [s for s in st.session_state[GLOBAL_KEY] if s in selected_names][:2]
...
sel_names_ui = st.multiselect(
    ...
    key=UI_KEY,
    max_selections=2,
    help="Your pick is remembered across pages in this session."
)

# If the user picks something here, save it globally
if sel_names_ui:
    st.session_state[GLOBAL_KEY] = sel_names_ui[:2]

# For this page’s logic, prefer the UI picks; otherwise keep any saved picks that are valid here;
# otherwise leave 'selected_names' as-is (which currently means 'all companies').
if sel_names_ui:
    selected_names = sel_names_ui[:2]
elif st.session_state[GLOBAL_KEY]:
    selected_names = [s for s in st.session_state[GLOBAL_KEY] if s in selected_names][:2]

# Optional clear button (does NOT log you out; just clears the remembered picks)
if st.button("Clear selection", key="clear_sel_dashboard"):
