...
if not tiles:
    ...
    fig.update_traces(
        ...
        textfont_size=18,                       # bigger labels
        marker=dict(line=dict(width=1, color="rgba(0,0,0,.25)")),
        tiling=dict(pad=3)                      # a bit of spacing between tiles
    )
    fig.update_layout(
        margin=dict(l=0, r=0, t=0, b=0),
        coloraxis_colorbar=dict(title="%Δ", tickformat="+.1f"),
        uniformtext=dict(minsize=14, mode="show"),  # keep text visible on smaller tiles
