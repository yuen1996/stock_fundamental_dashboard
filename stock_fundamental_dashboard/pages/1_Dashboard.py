...
if not tiles:
    ...
    try:
        ...
        # Fallback for older Plotly that lacks px.Constant
        hdf["_Root"] = "All"
        fig = px.treemap(
            hdf,
            path=["_Root", "Label"],
    ...
    fig.update_traces(
        # Show WRAPPED label + a shorter % (1 decimal) to save space
        texttemplate="<b>%{customdata[1]}</b><br>%{customdata[0]:+.1f}%",
        root_color="rgba(0,0,0,0)",
        textfont_size=12,  # base; Plotly will shrink uniformly as needed
        marker=dict(line=dict(width=1, color="rgba(0,0,0,.25)")),
        tiling=dict(pad=2),  # a tad tighter than 3 → more room for text
    ...
    fig.update_layout(
        ...
        # IMPORTANT: force showing text even when small; shrink uniformly
