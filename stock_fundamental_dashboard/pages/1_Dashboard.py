...
# ----- PASTE THIS BLOCK ABOVE THE MOMENTUM HEATMAP SECTION -----
...
@st.cache_data(show_spinner=False, ttl=14_400)
def _load_klci_ohlc() -> pd.DataFrame | None:
    """Find KLCI OHLC CSV written by your Momentum page (tries several names)."""
    etag = int(getattr(bus, "etag", lambda *_: 0)("ohlc"))
    # try common names first
    for nm in ("FBMKLCI", "KLCI", "KLSE", "^KLSE", "FTSE Bursa Malaysia KLCI"):
        try:
            df = _load_ohlc_for_name(nm, etag, ticker=None)
            if df is not None and not df.empty:
                return df
        except Exception:
            pass
    # last resort: scan the OHLC folder for files that contain "klci"/"klse"
    try:
        ohlc_dir = _resolve_ohlc_dir()
        for fn in os.listdir(ohlc_dir):
            low = fn.lower()
            if low.endswith(".csv") and ("klci" in low or "klse" in low):
                d = pd.read_csv(os.path.join(ohlc_dir, fn))
...
klci = _load_klci_ohlc()
if klci is None or klci.empty:
    st.info("No KLCI OHLC found yet. On the Momentum page, save Yahoo OHLC for **FBMKLCI / ^KLSE**, then come back.")
else:
    ...
    if not ytd.empty:
        ytd_first = float(ytd.iloc[0]["Close"])
        ytd_pct = (last_px / ytd_first - 1.0) * 100.0 if ytd_first else 0.0
    else:
        ytd_pct = None
    ...
    _auto_refresh_ms(14_400_000, key="klci_autorefresh")  # 4 hours = 14,400,000 ms
