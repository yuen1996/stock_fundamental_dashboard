...
# --- View mode toggle for the editor table ---
view_mode = st.radio(
    "View mode for editor",
...
for name in sorted(df_view["Name"].dropna().unique()):
    st.markdown(f"### {name}")
    stock = df_view[df_view["Name"] == name].sort_values("Year")
    if stock.empty:
        st.info("No annual data for this stock.")
        continue
    ...
    if layout_choice.startswith("Years"):
        # Years as columns, metrics on the left
        disp = ratio_df.T
