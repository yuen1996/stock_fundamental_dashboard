...
def _lookup_yf_ticker(stock_name: str, latest_row: Mapping[str, Any]) -> str | None:
    """Return a Yahoo Finance ticker (e.g. 5012.KL) for this stock, if we can."""
    m = _load_yf_map()

    # Try key variants (Momentum saves multiple variants)
    safe = _safe_name(stock_name).lower()
    compact = re.sub(r"[^0-9A-Za-z]+", "", str(stock_name)).lower()
    for k in (stock_name, safe, compact):
        v = m.get(k)
        if isinstance(v, str) and v.strip():
            return v.strip()

    # Fallback: a column that already looks like a Yahoo-style ticker (has a dot)
    for col in ("YahooTicker", "Ticker", "Symbol", "Code"):
        v = latest_row.get(col)
        if isinstance(v, str) and v.strip() and "." in v:
            return v.strip()

    # Bursa numeric code -> Yahoo ticker, e.g. 5012 -> 5012.KL
    for col in ("Code", "code", "StockCode", "BursaCode", "Symbol"):
        v = latest_row.get(col)
        if v is None:
            continue
        s = str(v).strip()
        # Handle float-y ints like 5012.0
        if re.fullmatch(r"\d+(?:\.0+)?", s):
            s = s.split(".", 1)[0]
        if re.fullmatch(r"\d{3,5}", s):
            code = s.zfill(4) if len(s) < 4 else s
            return f"{code}.KL"
...
for name in selected_names:
    ...
    is_viable = (score >= float(min_score))  # align with Systematic Decision: score threshold only
...
if trade_df.empty:
    ...
    cards = [
        {
            ...
            "note": "FunnelScore ≥ MinScore (same as Systematic Decision)",
    ...
    st.dataframe(
        ...
        hide_index=True,
        column_config={
            "Viable": st.column_config.CheckboxColumn("PASS", disabled=True),
