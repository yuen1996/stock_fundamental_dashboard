...
def _load_ohlc_for_name(stock_name: str, _etag: int, *, ticker_hint: str | None = None) -> pd.DataFrame | None:
    ...
    path = None
    # use utils.name_link if available
    if name_link is not None and hasattr(name_link, "find_ohlc_path"):
        try:
            ...
            path = None
        if not path:
            # Try multiple filename candidates using both the stock name and the ticker hint.
            candidates = []
            for s in [stock_name, ticker_hint]:
                if not s:
                    continue
                base = str(s).strip()
            ...
            # Fallback to just stock_name if ticker_hint is None and above didn’t hit
            if not candidates:
    ...
    if current_price_default is None:
        ...
        if "CurrentPrice" in stock_rows_for_name.columns:
            ...
            if not cur_nonnull.empty:
                current_price_default = float(cur_nonnull.iloc[0])
except Exception:
    current_price_default = None
...
if use_manual:
    ...
    if mom_px is None:
        st.session_state.pop(DISP_KEY, None)  # show clean blank
        st.text_input(
            "Current Price",
            value="",
            placeholder="—",
        ...
        st.info(
            f"Looked in: {_resolve_ohlc_dir()} for '{stock_name}' and ticker variants. "
            "Ensure an OHLC CSV exists and has Date/Close.",
            icon="🧭",
        )
    else:
        st.session_state[DISP_KEY] = f"{float(mom_px):.4f}"  # <-- refresh the display value
        st.text_input(
            "Current Price",
            value=st.session_state[DISP_KEY],
            key=DISP_KEY,
            disabled=True,
        )
    ...
    for name in [name] if name else []:
        ...
        with st.expander(str(name), expanded=expanded_now):
            ...
            if manual_default is None:
                try:
                    manual_default = float(df.loc[_mask_name, "CurrentPrice"].dropna().iloc[0])
                except Exception:
                    manual_default = None
