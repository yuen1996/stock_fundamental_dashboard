...
# ✅ Pick from existing stocks to avoid typos
_existing_names = sorted([
    str(n) for n in df["Name"].dropna().unique()
    if str(n).strip()
])

_NEW_OPT = "➕ Create new / type manually"
_picker_options = [_NEW_OPT] + _existing_names

_default_canon = _canon_name(default_stock) if default_stock else ""
_picker_index = _picker_options.index(_default_canon) if _default_canon in _picker_options else 0

picked_name = st.selectbox(
    "Select existing stock (recommended)",
    options=_picker_options,
    index=_picker_index,
    key="stock_picker_existing",
    help="Pick from your saved list to avoid typos. Choose 'Create new' to type a new stock name.",
)

if picked_name == _NEW_OPT:
    stock_name_raw = st.text_input(
        "Stock Name",
        value=(default_stock or ""),
        placeholder="e.g. Public Bank",
        key="stock_name_new",
    )
else:
    stock_name_raw = picked_name
    st.text_input(
        "Stock Name",
        value=picked_name,
        disabled=True,
        key="stock_name_selected",
    )
