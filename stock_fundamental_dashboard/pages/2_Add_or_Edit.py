...
with st.form("stock_settings_form", clear_on_submit=False):
    ...
    # In Manual: editable; seed with last manual if present, else Momentum
    # In Momentum: read-only; show the Momentum price
    if use_manual:
...
if submit_settings:
    if not stock_name:
        ...
        val_to_store = float(cp) if use_manual else float(mom_px or 0.0)
