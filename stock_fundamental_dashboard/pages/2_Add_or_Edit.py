...
# ---- Price source / momentum helpers ----
try:
    from utils import name_link, bus  # locate OHLC file & invalidate caches
except Exception:
    # fallbacks (safe if utils pathing differs)
    name_link = None
    class _Bus:
        def etag(self, *_): return 0
        def bump(self, *_): pass
    bus = _Bus()
...
def _candidate_paths(ohlc_dir: str, stock_name: str, ticker_hint: str | None) -> list[str]:
    cands: list[str] = []
    for s in filter(None, [stock_name, ticker_hint]):
        base = str(s).strip()
        safe = re.sub(r"[^0-9A-Za-z]+", "_", base).strip("_")
        cands += [
            os.path.join(ohlc_dir, f"{base}.csv"),
            os.path.join(ohlc_dir, f"{safe}.csv"),
            os.path.join(ohlc_dir, f"{base.upper()}.csv"),
            os.path.join(ohlc_dir, f"{base.lower()}.csv"),
        ]
    if not cands:
        base = str(stock_name).strip()
        safe = re.sub(r"[^0-9A-Za-z]+", "_", base).strip("_")
        cands = [os.path.join(ohlc_dir, f"{base}.csv"), os.path.join(ohlc_dir, f"{safe}.csv")]
    return cands

@st.cache_data(show_spinner=False)
def _load_ohlc_for_name(stock_name: str, _etag: int, *, ticker_hint: str | None = None) -> pd.DataFrame | None:
    """
    Load OHLC CSV for a stock using the shared resolver in utils.name_link.
    Falls back to filename candidates if name_link can't find a path.
    Cache keyed by bus.etag('ohlc') via _etag.
    """
    # 1) Resolve the OHLC directory (shared across pages)
    try:
        ohlc_dir = name_link.resolve_ohlc_dir()
    except Exception:
        # ultra-safe fallback: keep the old local candidates logic
        candidates = [
    ...
    # 2) Try the smart matcher first (handles aliases like “Maybank” ⇄ “Malayan Banking”)
    if name_link is not None and hasattr(name_link, "find_ohlc_path"):
        try:
            path = name_link.find_ohlc_path(stock_name, ohlc_dir=ohlc_dir, ticker=ticker_hint)
        except Exception:
            path = None

    # 3) Fallback to filename guesses
    if not path:
        path = next((p for p in _candidate_paths(ohlc_dir, stock_name, ticker_hint) if os.path.exists(p)), None)
        if not path:
            return None
...
def _momentum_close_before_today(stock_name: str, stock_rows: pd.DataFrame) -> tuple[float | None, pd.Timestamp | None]:
    """Return (last close before today, asof_date) from OHLC CSVs."""

    # try to pass a ticker hint if your df has one
    ticker_hint = None
    ...
    # ⬇️ include OHLC directory mtime in the cache key so new CSVs are picked up
    try:
        ohlc_dir = name_link.resolve_ohlc_dir() if name_link is not None else ""
    except Exception:
        ohlc_dir = ""
    dir_sig = _dir_mtime_ns_safe(ohlc_dir) if ohlc_dir else 0

    # mix with bus etag (if any)
    etag_bus = int(getattr(bus, "etag", lambda *_: 0)("ohlc"))
    etag = (etag_bus or 0) ^ (dir_sig or 0)

    dfp = _load_ohlc_for_name(stock_name, etag, ticker_hint=ticker_hint)
    ...
    # Fallback to the last available row (even if it's "today") when no prior rows exist
    ...
    last = pick.iloc[-1]
    try:
        px = float(last["Close"]) if pd.notna(last["Close"]) else None
    except Exception:
        px = None
    asof = pd.to_datetime(last["Date"])
