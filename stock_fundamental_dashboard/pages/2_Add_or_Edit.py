...
def compute_banking_derivatives_annual(
    ...
    # --- IEA / AEA / NIM-den (StrictPlus) ------------------------------------
    if {"Earning Assets", "Average Earning Assets", "TP_Bank_NIM_Den"} & set(available_keys):
        # Build a tiny unprefixed row view
        iea_row = {
            "Earning Assets": row_up.get("Earning Assets"),
            "Net Loans": row_up.get("Net Loans"),
            "Gross Loans": row_up.get("Gross Loans"),
            "Loan Loss Reserve": row_up.get("Loan Loss Reserve"),
            "FVOCI Investments": row_up.get("FVOCI Investments"),
            "Amortised Cost Investments": row_up.get("Amortised Cost Investments"),
            "Reverse Repos": row_up.get("Reverse Repos"),
            "FVTPL Investments": row_up.get("FVTPL Investments"),
            "FVTPL Derivatives": row_up.get("FVTPL Derivatives"),
            "Average Earning Assets": row_up.get("Average Earning Assets"),
            "TP_Bank_NIM_Den": row_up.get("TP_Bank_NIM_Den"),
        }

        # Previous annual row (same stock, year-1)
        prev_df = df[
            (df["Name"].astype(str).str.upper() == str(stock_name).upper())
            & (df["IsQuarter"] != True)
            & (pd.to_numeric(df["Year"], errors="coerce") == int(year) - 1)
        ]
        prev_dict = prev_df.iloc[0].to_dict() if not prev_df.empty else {}
        prev_iea_row = {
            "Earning Assets": prev_dict.get("Earning Assets"),
            "Net Loans": prev_dict.get("Net Loans"),
            "Gross Loans": prev_dict.get("Gross Loans"),
            "Loan Loss Reserve": prev_dict.get("Loan Loss Reserve"),
            "FVOCI Investments": prev_dict.get("FVOCI Investments"),
            "Amortised Cost Investments": prev_dict.get("Amortised Cost Investments"),
            "Reverse Repos": prev_dict.get("Reverse Repos"),
            "FVTPL Investments": prev_dict.get("FVTPL Investments"),
            "FVTPL Derivatives": prev_dict.get("FVTPL Derivatives"),
        } if prev_dict else None

        compute_bank_iea(iea_row, prev_iea_row, mode="strictplus")

        # Write back ONLY if allowed & still blank
        if "Earning Assets" in available_keys and row_up.get("Earning Assets") is None:
            row_up["Earning Assets"] = iea_row.get("Earning Assets")
        if "Average Earning Assets" in available_keys and row_up.get("Average Earning Assets") is None:
            row_up["Average Earning Assets"] = iea_row.get("Average Earning Assets")
        if "TP_Bank_NIM_Den" in available_keys and row_up.get("TP_Bank_NIM_Den") is None:
            row_up["TP_Bank_NIM_Den"] = iea_row.get("TP_Bank_NIM_Den")
    ...
    # ── Averages: ONLY compute if the CURRENT value is provided ──────────────────
    ...
    if "Average Earning Assets" in available_keys:
        ...
        # ❗ no fallback to TP_Bank_NIM_Den when current EA is blank
...
def compute_banking_derivatives_quarterly(
    new_row: dict, *, df, stock_name: str, year: int, quarter: str, available_keys: set[str]
        ...
        # --- IEA / AEA / NIM-den (StrictPlus, quarterly) -------------------------
    if {"Q_Earning Assets", "Q_Average Earning Assets", "Q_TP_Bank_NIM_Den"} & set(available_keys):
        # Current quarter (strip Q_ to feed the helper)
        iea_row = {
            "Earning Assets": new_row.get("Q_Earning Assets"),
            "Net Loans": new_row.get("Q_Net Loans"),
            "Gross Loans": new_row.get("Q_Gross Loans"),
            "Loan Loss Reserve": new_row.get("Q_Loan Loss Reserve"),
            "FVOCI Investments": new_row.get("Q_FVOCI Investments"),
            "Amortised Cost Investments": new_row.get("Q_Amortised Cost Investments"),
            "Reverse Repos": new_row.get("Q_Reverse Repos"),
            "FVTPL Investments": new_row.get("Q_FVTPL Investments"),
            "FVTPL Derivatives": new_row.get("Q_FVTPL Derivatives"),
            "Average Earning Assets": new_row.get("Q_Average Earning Assets"),
            "TP_Bank_NIM_Den": new_row.get("Q_TP_Bank_NIM_Den"),
        }

        # Previous quarter (same logic as _prev_quarter_value)
        q = str(quarter).upper().strip()
        if q in _Q_ORDER:
            py, pq = ((int(year) - 1, "Q4") if _Q_ORDER[q] == 0 else (int(year), f"Q{_Q_ORDER[q]}"))
            prev_df = df[
                (df["Name"].astype(str).str.upper() == str(stock_name).upper())
                & (df["IsQuarter"] == True)
                & (pd.to_numeric(df["Year"], errors="coerce") == int(py))
                & (df["Quarter"].astype(str).str.upper() == pq)
            ]
            prev_dict = prev_df.iloc[0].to_dict() if not prev_df.empty else {}
        else:
        ...
        prev_iea_row = {
            "Earning Assets": prev_dict.get("Q_Earning Assets"),
            "Net Loans": prev_dict.get("Q_Net Loans"),
            "Gross Loans": prev_dict.get("Q_Gross Loans"),
            "Loan Loss Reserve": prev_dict.get("Q_Loan Loss Reserve"),
            "FVOCI Investments": prev_dict.get("Q_FVOCI Investments"),
            "Amortised Cost Investments": prev_dict.get("Q_Amortised Cost Investments"),
            "Reverse Repos": prev_dict.get("Q_Reverse Repos"),
            "FVTPL Investments": prev_dict.get("Q_FVTPL Investments"),
            "FVTPL Derivatives": prev_dict.get("Q_FVTPL Derivatives"),
        } if prev_dict else None

        compute_bank_iea(iea_row, prev_iea_row, mode="strictplus")

        # Write back (Q_ variants), only if blank & allowed
        if "Q_Earning Assets" in available_keys and new_row.get("Q_Earning Assets") is None:
            new_row["Q_Earning Assets"] = iea_row.get("Earning Assets")
        if "Q_Average Earning Assets" in available_keys and new_row.get("Q_Average Earning Assets") is None:
            new_row["Q_Average Earning Assets"] = iea_row.get("Average Earning Assets")
        if "Q_TP_Bank_NIM_Den" in available_keys and new_row.get("Q_TP_Bank_NIM_Den") is None:
            new_row["Q_TP_Bank_NIM_Den"] = iea_row.get("TP_Bank_NIM_Den")
    ...
    if "Q_Average Earning Assets" in available_keys:
        ...
        # ❗ no fallback to Q_TP_Bank_NIM_Den when current Q_EA is blank
...
# --- IEA (Earning Assets) helpers --------------------------------------------
def _nz(x):
    return 0.0 if x is None else float(x)

def compute_bank_iea(row: dict, prev_row: dict | None = None, *, mode: str = "strictplus") -> dict:
    """
    Compute a single 'Earning Assets' using component fields.
      Strict     = NetLoans + FVOCI + AmortisedCost + ReverseRepos
      StrictPlus = Strict + FVTPL Investments + FVTPL Derivatives
    NetLoans fallback = GrossLoans − LoanLossReserve if 'Net Loans' blank.
    Also fills:
      - 'Average Earning Assets' (two-point average vs prev row)
      - default NIM denominator 'TP_Bank_NIM_Den' if not provided
    Returns the SAME dict `row` (mutated) for convenience.
    """
    # Resolve Net Loans with fallback
    net_loans = row.get("Net Loans")
    if net_loans is None:
        gl  = row.get("Gross Loans")
        ecl = row.get("Loan Loss Reserve")
        if gl is not None and ecl is not None:
            net_loans = _nz(gl) - _nz(ecl)
    if net_loans is None:
        return row  # not enough to compute anything

    fvoci   = _nz(row.get("FVOCI Investments"))
    amcost  = _nz(row.get("Amortised Cost Investments"))
    rrepo   = _nz(row.get("Reverse Repos"))
    fvtpl_i = _nz(row.get("FVTPL Investments"))
    fvtpl_d = _nz(row.get("FVTPL Derivatives"))

    iea_strict = _nz(net_loans) + fvoci + amcost + rrepo
    iea = iea_strict + (fvtpl_i + fvtpl_d if str(mode).lower() == "strictplus" else 0.0)

    # Fill single IEA if blank
    if row.get("Earning Assets") is None:
        row["Earning Assets"] = iea

    # Two-point Average EA (if we have a prev row)
    if prev_row:
        prev_iea = prev_row.get("Earning Assets")
        if prev_iea is None:
            _ = compute_bank_iea(prev_row.copy(), None, mode=mode)
            prev_iea = _.get("Earning Assets")
        if prev_iea is not None and row.get("Average Earning Assets") is None:
            row["Average Earning Assets"] = (iea + float(prev_iea)) / 2.0

    # Default NIM denominator = AEA (only if override not given)
    if row.get("TP_Bank_NIM_Den") is None and row.get("Average Earning Assets") is not None:
        row["TP_Bank_NIM_Den"] = row["Average Earning Assets"]

    return row

def compute_nim_if_blank(row: dict, *, quarterly: bool = False) -> dict:
    """Convenience — compute NIM if numerator/denominator are present."""
    nim_num = row.get("TP_Bank_NIM_Num") or row.get("NII (incl Islamic)")
    nim_den = row.get("TP_Bank_NIM_Den") or row.get("Average Earning Assets")
    if nim_num is not None and nim_den:
        factor = 4.0 if quarterly else 1.0
        row.setdefault("NIM", (float(nim_num) / float(nim_den)) * 100.0 * factor)
    return row
...
def render_calc_helper(
    *, bucket: str, scope: str, df, stock_name: str, year: int, quarter: str | None = None, fallback: dict | None = None
    ...
    if bucket == "Banking":
        ...
        # Current values for other helpers
        gl_cur   = _try_float(_read("Gross Loans"))
        dep_cur  = _try_float(_read("Deposits"))

        # --- IEA (Strict & StrictPlus) displayed in helper --------------------
        nl = _try_float(_read("Net Loans"))
        if nl is None:
            gl = _try_float(_read("Gross Loans"))
            llr = _try_float(_read("Loan Loss Reserve"))
            if gl is not None and llr is not None:
                nl = gl - llr

        fvoci   = _try_float(_read("FVOCI Investments")) or 0.0
        amcost  = _try_float(_read("Amortised Cost Investments")) or 0.0
        rrepo   = _try_float(_read("Reverse Repos")) or 0.0
        fvtpl_i = _try_float(_read("FVTPL Investments")) or 0.0
        fvtpl_d = _try_float(_read("FVTPL Derivatives")) or 0.0

        iea_strict = (nl + fvoci + amcost + rrepo) if nl is not None else None
        iea_plus   = (iea_strict + fvtpl_i + fvtpl_d) if iea_strict is not None else None

        # EA shown = StrictPlus (what we compute for save/autofill)
        ea_form_val = _try_float(_read("Earning Assets"))
        ea_display  = ea_form_val if ea_form_val is not None else iea_plus

        # Prior-period lookups for averages (only if we have EA to average)
        if scope == "annual":
            ea_prev = _prev_annual_value(df, stock_name, year, "Earning Assets") if ea_display is not None else None
        else:
            ea_prev = _prev_quarter_value(df, stock_name, year, quarter, "Q_Earning Assets") if ea_display is not None else None

        avg_ea_display = _avg_two(ea_display, _try_float(ea_prev)) if ea_display is not None else None

        # Other helper metrics
        ...
        nim_den_override = _try_float(_read("TP_Bank_NIM_Den"))
        nim_den = nim_den_override or avg_ea_display
        nim = (nim_num / nim_den * 100.0) if (nim_num is not None and nim_den not in (None, 0)) else None

        # CIR and Leverage (override-aware)
        ...
        # Build helper cards (now includes IEA)
        rows.extend([
        ...
        # IEA cards (StrictPlus main + Strict for transparency)
        if ea_display is not None:
            rows.append(("Earning Assets (RM)", _fmt_money(ea_display),
                         "NetLoans + FVOCI + Amortised + ReverseRepo + FVTPL inv + FVTPL deriv"))
        if iea_strict is not None:
            rows.append(("EA (Strict, RM)", _fmt_money(iea_strict),
                         "NetLoans + FVOCI + Amortised + ReverseRepo"))

        # Averages
        if gl_cur is not None:
            gl_prev = _prev_annual_value(df, stock_name, year, "Gross Loans") if scope=="annual" else _prev_quarter_value(df, stock_name, year, quarter, "Q_Gross Loans")
            rows.append(("Average Gross Loans (RM)", _fmt_money(_avg_two(gl_cur, _try_float(gl_prev))), "Avg of current & prior"))
        if dep_cur is not None:
            dep_prev = _prev_annual_value(df, stock_name, year, "Deposits") if scope=="annual" else _prev_quarter_value(df, stock_name, year, quarter, "Q_Deposits")
            rows.append(("Average Deposits (RM)", _fmt_money(_avg_two(dep_cur, _try_float(dep_prev))), "Avg of current & prior"))
