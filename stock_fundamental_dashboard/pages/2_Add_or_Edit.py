...
def compute_banking_derivatives_annual(
    ...
    # --- Operating Income (bank definition) ---
    if "Operating Income" in available_keys:
        # base components (treat the two "expense" lines as costs)
        ii  = _try_float(row_up.get("Interest Income"))
        ie  = _abs_or_none(row_up.get("Interest Expense"))  # subtract as cost
        isl = (_try_float(row_up.get("Net Islamic Income"))
               or _try_float(row_up.get("Net income from Islamic banking")))
        ins_res = (_try_float(row_up.get("Insurance Service Result"))
                   or _try_float(row_up.get("Insurance service result")))
        ins_fin_exp = (_abs_or_none(row_up.get("Net finance expense from insurance contracts"))
                       or _abs_or_none(row_up.get("Net Finance Expense (Insurance Contracts)")))

        net_core = (ii - (ie or 0) + (isl or 0) + (ins_res or 0) - (ins_fin_exp or 0)) if ii is not None else None

        fee   = (_try_float(row_up.get("Net Fee & Commission Income"))
                or _try_float(row_up.get("Net fee & commission income"))
                or _try_float(row_up.get("Fee Income")))  # 👈 new alias

        gains = (_try_float(row_up.get("Net gains/losses on financial instruments"))
                or _try_float(row_up.get("Net Gains/Losses on Financial Instruments"))
                or _try_float(row_up.get("Trading Income")))  # 👈 new alias

        other = (_try_float(row_up.get("Other Operating Income"))
                 or _try_float(row_up.get("Other operating income")))

        op_inc = None
        if net_core is not None:
            op_inc = net_core + (fee or 0) + (gains or 0) + (other or 0)

        # set only if this field exists in the form (and keep any prior user-entered value)
        row_up["Operating Income"] = op_inc if op_inc is not None else row_up.get("Operating Income", None)
    
    # ✅ NEW: CFO margin denominator override defaults to Operating Income
    if "TP_Bank_CFO_Den" in available_keys:
        if _try_float(row_up.get("TP_Bank_CFO_Den")) is None:
            base_oi = _try_float(row_up.get("Operating Income"))
            row_up["TP_Bank_CFO_Den"] = base_oi
...
def compute_banking_derivatives_quarterly(
    ...
    # --- Operating Income (bank definition, quarterly) ---
    if "Q_Operating Income" in available_keys:
        q = "Q_"
        ii  = _try_float(new_row.get(f"{q}Interest Income"))
        ie  = _abs_or_none(new_row.get(f"{q}Interest Expense"))  # subtract as cost
        isl = (_try_float(new_row.get(f"{q}Net Islamic Income"))
               or _try_float(new_row.get(f"{q}Net income from Islamic banking")))
        ins_res = (_try_float(new_row.get(f"{q}Insurance Service Result"))
                   or _try_float(new_row.get(f"{q}Insurance service result")))
        ins_fin_exp = (_abs_or_none(new_row.get(f"{q}Net finance expense from insurance contracts"))
                       or _abs_or_none(new_row.get(f"{q}Net Finance Expense (Insurance Contracts)")))

        net_core = (ii - (ie or 0) + (isl or 0) + (ins_res or 0) - (ins_fin_exp or 0)) if ii is not None else None

        fee   = (_try_float(new_row.get(f"{q}Net Fee & Commission Income"))
                or _try_float(new_row.get(f"{q}Net fee & commission income"))
                or _try_float(new_row.get(f"{q}Fee Income")))  # 👈 new alias

        gains = (_try_float(new_row.get(f"{q}Net gains/losses on financial instruments"))
                or _try_float(new_row.get(f"{q}Net Gains/Losses on Financial Instruments"))
                or _try_float(new_row.get(f"{q}Trading Income")))  # 👈 new alias

        other = (_try_float(new_row.get(f"{q}Other Operating Income"))
                 or _try_float(new_row.get(f"{q}Other operating income")))

        op_inc = None
        if net_core is not None:
            op_inc = net_core + (fee or 0) + (gains or 0) + (other or 0)

        new_row[f"{q}Operating Income"] = op_inc if op_inc is not None else new_row.get(f"{q}Operating Income", None)

    # ✅ NEW: CFO margin denominator override defaults to Operating Income (quarterly)
    if "Q_TP_Bank_CFO_Den" in available_keys:
        if _try_float(new_row.get("Q_TP_Bank_CFO_Den")) is None:
            base_oi_q = _try_float(new_row.get("Q_Operating Income"))
            new_row["Q_TP_Bank_CFO_Den"] = base_oi_q    
