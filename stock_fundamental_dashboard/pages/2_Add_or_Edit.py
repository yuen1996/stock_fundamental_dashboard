...
# 🔁 Smart OHLC resolver + duplicate-proof widget keys
...
def _audit_log_event(
    ...
    try:
        ...
        ev = {
            "ts": datetime.now(timezone.utc).isoformat(),  # tz-aware ISO UTC
            "action": action,
            "scope": scope,
            "name": name,
            "year": year,
        ...
        # keep small; don’t explode the log
...
def _safe_key_for_price_src(s: str) -> str:
    return re.sub(r"[^A-Za-z0-9_]+", "_", str(s or "")).lower().strip("_")

def _norm_price_src(v: str, default="Momentum") -> str:
    s = str(v or "").strip().lower()
    if s in {"momentum", "auto", "automatic", "default"}: return "Momentum"
    if s in {"add/edit", "manual", "addedit", "add edit"}: return "Add/Edit"
    return default

def get_price_src_prefs() -> dict:
    """Return {safe_name -> 'Momentum'|'Add/Edit'} from app_settings.json."""
    d = _load_settings()
    m = (d.get("price_src_by_stock") or {}) if isinstance(d, dict) else {}
    out = {}
    for k, v in (m.items() if isinstance(m, dict) else []):
        out[_safe_key_for_price_src(k)] = _norm_price_src(v)
    return out

def set_price_src_pref(stock_name: str, src: str) -> None:
    """Persist per-stock price source, also mirror to session_state for immediate use."""
    prefs = _load_settings() or {}
    by = prefs.get("price_src_by_stock", {})
    by[_safe_key_for_price_src(stock_name)] = _norm_price_src(src)
    prefs["price_src_by_stock"] = by
    _save_settings(prefs)
    # live for this session (Risk/Reward and others can read these)
    ss_map = st.session_state.get("price_src_by_stock", {})
    ss_map[_safe_key_for_price_src(stock_name)] = _norm_price_src(src)
    st.session_state["price_src_by_stock"] = ss_map
    st.session_state["view_price_src_by_stock"] = ss_map   

# --- NEW: remember last manual price per stock (so toggling sources keeps your last manual)
def get_manual_price_prefs() -> dict[str, float | None]:
    d = _load_settings()
    m = (d.get("manual_price_by_stock") or {}) if isinstance(d, dict) else {}
    out: dict[str, float | None] = {}
    for k, v in (m.items() if isinstance(m, dict) else []):
        sk = _safe_key_for_price_src(k)
        try:
            fv = float(v)
            out[sk] = fv if math.isfinite(fv) else None
        except Exception:
            out[sk] = None
    return out

def set_manual_price_pref(stock_name: str, price: float | None) -> None:
    prefs = _load_settings() or {}
    by = prefs.get("manual_price_by_stock", {})
    by[_safe_key_for_price_src(stock_name)] = (None if price is None else float(price))
    prefs["manual_price_by_stock"] = by
    _save_settings(prefs)
...
# --- FS helper for cache-busting on new/changed CSVs ---
def _dir_mtime_ns_safe(path: str) -> int:
    try:
        import os
...
def _momentum_close_before_today(stock_name: str, stock_rows: pd.DataFrame) -> tuple[float | None, pd.Timestamp | None]:
    """Return (last close before today, asof_date) using utils.ohlc_resolver (handles spaces/_/case/fuzzy)."""
    try:
        dfp, _path, _how = load_ohlc(stock_name)  # tolerant: "Hup Seng" → Hup_Seng.csv, etc.
    except Exception:
        return None, None
    ...
    # Detect a date-like column (resolver sorts by it but keeps original name)
    lower = {c.lower(): c for c in dfp.columns}
    date_col = next((lower[k] for k in ("date", "timestamp", "time", "datetime") if k in lower), None)
    if not date_col:
        # try dtype-based guess, else coerce first column
        dt_cols = [c for c in dfp.columns if str(dfp[c].dtype).startswith("datetime64")]
        if dt_cols:
            date_col = dt_cols[0]
        else:
    ...
    prior = dfp[dfp[date_col] < today]
    pick = prior if not prior.empty else dfp
    if pick.empty:
        return None, None
    ...
    close_col = "Close"
    if close_col not in dfp.columns:
        # very defensive fallback
        for c in dfp.columns:
            if c.strip().lower() in {"close", "adj close", "adjusted close"}:
                close_col = c
                break

    try:
        px = float(last[close_col]) if pd.notna(last.get(close_col)) else None
    except Exception:
        px = None

    asof = pd.to_datetime(last[date_col], errors="coerce")
    return (px if (px is not None and math.isfinite(px)) else None), asof
...
def _decimals_for_unit(unit: str | None, key: str = "") -> int:
    """
    Decide how many decimals to show / store for numeric inputs.
    - %  → 2dp
    - RM → 4dp (so tiny values like 0.004 are not rounded to 0)
    - Price fields → 4dp
    - Else → 0dp
    """
    if unit == "%":
    ...
    if unit in ("RM", "RM/t", "RM/boe"):
        # was 2 before; bump to 4 so small dividends etc. are preserved
        return 4
    if "Price" in key:
        return 4
    return 0
...
def _bucket_allowed_keys(bucket: str, quarterly: bool = False) -> set[str]:
    return _allowed_keys_for_bucket(bucket, quarterly=quarterly, _cir_key=_cir_cache_key())
...
try:
    _PLACEHOLDERS_BY_BUCKET["Banking"]["Operating Income"] = "NII + non-interest income."
except Exception:
    pass
...
def _cir_cache_key() -> int:
    # participates in the cache key so toggling CIR_DISABLED_BUCKETS invalidates caches
    return hash(tuple(sorted(map(str, CIR_DISABLED_BUCKETS))))
...
@st.cache_data(show_spinner=False)
def _cats(bucket: str, quarterly: bool, _cir_key: int) -> list[dict]:
    # _cir_key participates in the cache key to invalidate when CIR settings change
    return _flatten_categories(_get_categories(bucket, quarterly))

@st.cache_data(show_spinner=False)
def _allowed_keys_for_bucket(bucket: str, quarterly: bool, _cir_key: int) -> set[str]:
    return {f["key"] for f in _cats(bucket, quarterly, _cir_key) if f.get("key")}
...
# --- Banking: Non-interest income aliases (annual + Q_ via prefix) ---
_BANK_NON_INT_INCOME_KEYS = (
    "Non-Interest Income", "Non-interest Income", "Fee & Other Income",
    "Net Fee and Commission Income", "Net Fee & Commission Income",
    "Fee Income", "Fee & Commission Income",
    "Other Operating Income", "Trading & Investment Income",
    "Trading Income", "Investment Income", "Other Income",
)

def _non_interest_income_from(row: dict, *, prefix: str = "") -> float | None:
    """
    Prefer a single consolidated line if present; else sum known subcomponents.
    """
    primary = _first_float(row, ("Non-Interest Income", "Non-interest Income", "Fee & Other Income"),
                           prefix=prefix)
...
def _set_if_available_annual(year: int, key: str, val, bucket: str):
    ...
    if val is None:
        return False
    keys = {f["key"] for f in _cats(bucket, quarterly=False, _cir_key=_cir_cache_key()) if f.get("key")}
    if key not in keys:
        return False
...
def _set_if_available_quarterly(year: int, quarter: str, key: str, val, bucket: str):
    ...
    if val is None:
        return False
    keys = {f["key"] for f in _cats(bucket, quarterly=True, _cir_key=_cir_cache_key()) if f.get("key")}
...
def compute_banking_derivatives_annual(
    ...
    # Operating Income (Total): NII + Non-interest income
    if "Operating Income" in available_keys and _try_float(row_up.get("Operating Income")) is None:
        nii = _try_float(row_up.get("NII (incl Islamic)"))
        if nii is None:
            ii  = _try_float(row_up.get("Interest Income"))
            ie  = _abs_or_none(_try_float(row_up.get("Interest Expense")))
            isl = _try_float(row_up.get("Net Islamic Income"))
            nii = (ii - ie + (isl or 0.0)) if (ii is not None and ie is not None) else None

        nonint = _non_interest_income_from(row_up, prefix="")
        if (nii is not None) or (nonint is not None):
            row_up["Operating Income"] = (nii or 0.0) + (nonint or 0.0)
...
def compute_banking_derivatives_quarterly(
    ...
    # Operating Income (Total): Q_NII + Q_Non-interest income
    if "Q_Operating Income" in available_keys and _try_float(new_row.get("Q_Operating Income")) is None:
        nii_q = _try_float(new_row.get("Q_NII (incl Islamic)"))
        if nii_q is None:
            ii_q  = _try_float(new_row.get("Q_Interest Income"))
            ie_q  = _abs_or_none(_try_float(new_row.get("Q_Interest Expense")))
            isl_q = _try_float(new_row.get("Q_Net Islamic Income"))
            nii_q = (ii_q - ie_q + (isl_q or 0.0)) if (ii_q is not None and ie_q is not None) else None

        nonint_q = _non_interest_income_from(new_row, prefix="Q_")
        if (nii_q is not None) or (nonint_q is not None):
            new_row["Q_Operating Income"] = (nii_q or 0.0) + (nonint_q or 0.0)
...
def render_calc_helper(
    *, bucket: str, scope: str, df, stock_name: str, year: int, quarter: str | None = None, fallback: dict | None = None
    ...
    if bucket == "Banking":
        ...
        # Non-interest income via aliases
        def _read_any_alias(names):
            for n in names:
                v = _try_float(_read(n))
                if v is not None:
                    return v
        ...
        nonint_primary = _read_any_alias((
            "Non-Interest Income", "Non-interest Income", "Fee & Other Income",
        ))
        if nonint_primary is None:
            parts = []
            for n in (
                "Net Fee and Commission Income", "Net Fee & Commission Income",
                "Fee Income", "Fee & Commission Income",
                ...
                if v is not None:
                    parts.append(v)
            nonint_calc = sum(parts) if parts else None
        else:
            nonint_calc = nonint_primary

        op_inc_entered = _try_float(_read("Operating Income"))
        op_inc_display = (
            op_inc_entered if op_inc_entered is not None
            else ((nii or 0.0) + (nonint_calc or 0.0) if (nii is not None or nonint_calc is not None) else None)
        )

        if op_inc_display is not None:
            rows.append(("Operating Income (RM)", _fmt_money(op_inc_display), "NII + Non-interest income"))
        ...
        if _cir_allowed(bucket):
            ...
            cir_den_raw  = _try_float(_read("TP_Bank_CIR_Den")) or _try_float(_read("Operating Income"))
            cir_den      = cir_den_raw if cir_den_raw is not None else op_inc_display
...
# --- Stock Settings (no form) ---
# You can type a new stock name, or pick an existing one to avoid typos.
_STOCK_KEY = "__addedit_stock_name_raw"
_PICK_KEY  = "__addedit_pick_existing"

# Seed the typed stock name from query params once.
if _STOCK_KEY not in st.session_state:
    st.session_state[_STOCK_KEY] = default_stock or ""

_all_names = sorted(df["Name"].dropna().astype(str).unique().tolist())
_pick_options = ["(Type / New)"] + _all_names
_typed_now = _canon_name(st.session_state.get(_STOCK_KEY, ""))

# Keep picker roughly in sync with the typed value.
_pick_index = 0
try:
    if _typed_now in _all_names:
        _pick_index = _pick_options.index(_typed_now)
except Exception:
    _pick_index = 0

def _on_pick_existing_stock():
    """Fill the Stock Name box when the user selects an existing stock."""
    v = st.session_state.get(_PICK_KEY, "(Type / New)")
    if v and v != "(Type / New)":
        st.session_state[_STOCK_KEY] = v
...
picked = st.selectbox(
    ...
    index=_pick_index,
    key=_PICK_KEY,
    on_change=_on_pick_existing_stock,
    help="Pick an existing stock to auto-fill the Stock Name field. Choose '(Type / New)' to key in a new one.",
)

stock_name_raw = st.text_input("Stock Name", key=_STOCK_KEY)
stock_name = _canon_name(stock_name_raw)

# --- IMPORTANT: when switching stocks, reset Add/Edit form widget state ---
#
# Streamlit widgets keep values in st.session_state by their keys.
# Our annual/quarterly inputs use stable keys like:
#   {FieldKey}_{Year}_annual  (+ paired "_txt")
#   {FieldKey}_{Year}_{Quarter}_q (+ paired "_txt")
# If we switch stocks without clearing these, the inputs keep showing the previous
# stock's values (because `value=` is only used on first creation per key).
_prev_form_stock = st.session_state.get("__addedit_active_stock", "")
if _prev_form_stock != stock_name:
    # Clear annual/quarter numeric + display keys, plus autofill suppression flags
    _kill_suffixes = ("_annual", "_annual_txt", "_q", "_q_txt")
    _kill_prefixes = ("__suppress_autofill_", "show_inline_clear_")
    for _k in list(st.session_state.keys()):
        if (
            _k.endswith(_kill_suffixes)
            or _k.startswith(_kill_prefixes)
            or _k in ("q_form_tokens", "price_source_radio")
        ):
            st.session_state.pop(_k, None)
    # Also clear quarter token selector so defaults reflect the newly selected stock
    st.session_state.pop("q_form_tokens", None)
    st.session_state["__addedit_active_stock"] = stock_name
...
try:
    ...
    st.caption("Fields for this bucket (annual): " + (", ".join(labels) if labels else "— none —"))
except Exception:
    pass

# ---- Current Price with source toggle (Momentum vs Manual) ----
_src_prefs = get_price_src_prefs()
saved_src = _src_prefs.get(_safe_key_for_price_src(stock_name), "Momentum")

# Compute Momentum last close
stock_rows_for_name = df[df["Name"].astype(str).str.upper() == stock_name.upper()]
mom_px, mom_asof = _momentum_close_before_today(stock_name, stock_rows_for_name)
mom_caption = f"As of {mom_asof.date()}" if mom_asof is not None else "No OHLC found — update on Momentum page"

# Manual prefill from last saved manual (if any)
current_price_default = None
try:
    # Prefer the last manual price we remembered in app_settings
    _man_map = get_manual_price_prefs()
    current_price_default = _man_map.get(_safe_key_for_price_src(stock_name))
    if current_price_default is None:
        # Fallback to any saved CurrentPrice in the rows (older behavior)
        if "CurrentPrice" in stock_rows_for_name.columns:
            cur_nonnull = stock_rows_for_name["CurrentPrice"].dropna()
            if not cur_nonnull.empty:
                current_price_default = float(cur_nonnull.iloc[-1])  # use latest
...
src_choice = st.radio(
    ...
    ["Momentum (last close)", "Manual (key in)"],
    index=(0 if saved_src == "Momentum" else 1),
    horizontal=True,
    key="price_source_radio",
    help="Pick Momentum to link the price to your OHLC CSVs; pick Manual to store your own price.",
)

# --- PRICE WIDGETS (namespaced per stock) ---
safe = _safe_key_for_price_src(stock_name)
MAN_KEY  = f"cur_price_stock_num__{safe}"
DISP_KEY = f"cur_price_stock_num_display__{safe}"

# reset if source OR stock changed (also clear any legacy keys)
_prev_src  = st.session_state.get("__prev_price_source_radio")
_prev_name = st.session_state.get("__prev_stock_name")
if (_prev_src != src_choice) or (_prev_name != stock_name):
    for k in ("cur_price_stock_num", "cur_price_stock_num_display", MAN_KEY, DISP_KEY):
        st.session_state.pop(k, None)
st.session_state["__prev_price_source_radio"] = src_choice
st.session_state["__prev_stock_name"] = stock_name

use_manual = src_choice.startswith("Manual")

if use_manual:
    # seed manual with last saved manual, else fallback to Momentum
    seed = current_price_default if current_price_default is not None else mom_px
    st.number_input(
        "Current Price",
    ...
    st.caption("Source: Add/Edit (manual)")
else:
    # Momentum mode: force the session value to the latest mom_px
    if mom_px is None:
        st.number_input(
            "Current Price",
            value=0.0,
            ...
            format="%.4f",
            disabled=True,
            key=DISP_KEY + "_numfallback",
        )
        st.caption("Source: Momentum (last close) • No OHLC found — update on Momentum page")
    else:
        st.session_state[DISP_KEY] = f"{float(mom_px):.4f}"
        st.text_input("Current Price", key=DISP_KEY, disabled=True)
        st.caption(f"Source: Momentum (last close) • {mom_caption}")


# Save action (plain button so radio toggles rerun instantly)
save_settings_clicked = st.button("💾 Save stock settings", use_container_width=True)

if save_settings_clicked:
    if not stock_name:
        st.error("Please enter a Stock Name first.")
    else:
        # read the radio + current price field
        _use_manual = st.session_state.get("price_source_radio", "").startswith("Manual")
        _safe = _safe_key_for_price_src(stock_name)
        MAN_KEY = f"cur_price_stock_num__{_safe}"
        # prefer namespaced key; fall back to legacy key if present
        _entered = st.session_state.get(MAN_KEY, st.session_state.get("cur_price_stock_num", None))

        set_price_src_pref(stock_name, ("Add/Edit" if _use_manual else "Momentum"))

        # ✅ Always snapshot the visible current price into CurrentPrice
        if _use_manual:
            val_to_store = float(_entered or 0.0)
            # remember last manual for better prefills later
            set_manual_price_pref(stock_name, val_to_store)
        else:
            # momentum mode -> snapshot last close if available, else leave as NA
            val_to_store = float(mom_px) if mom_px is not None else pd.NA
        ...
        if mask.any():
            ...
            df.loc[mask, "CurrentPrice"] = val_to_store
            if ind_canon:
            ...
            # create placeholder row if none exist yet (meta timestamp only)
            base_row = {
                "Name": name_canon,
                "Industry": ind_canon,
        ...
        st.session_state["qeb_pick"] = name_canon
        st.session_state["__open_qe_for"] = name_canon
        ...
        st.session_state.pop("cur_price_stock_num", None)  # NEW: clear the numeric widget too
        DISP_KEY = f"cur_price_stock_num_display__{_safe}"
        st.session_state.pop(MAN_KEY, None)
        st.session_state.pop(DISP_KEY, None)
...
# ---------------- Bulk update: Current Prices from Momentum ----------------
st.markdown(section(
    "⚡ Bulk Price Update",
    "One click to refresh **CurrentPrice** for your stocks using the latest Momentum OHLC (last close). "
    "Results will show in a popup table."
), unsafe_allow_html=True)

# Options (safe defaults)
cbu1, cbu2, cbu3 = st.columns([1.3, 1, 1])
with cbu1:
    bulk_only_momentum_pref = st.checkbox(
        "Only update stocks linked to Momentum",
        value=True,
        help="If ON, stocks set to Manual (Add/Edit) will NOT be overwritten.",
        key="bulk_only_momentum_pref",
    )
with cbu2:
    bulk_overwrite_manual = st.checkbox(
        "Overwrite Manual prices",
        value=False,
        help="If ON, this will overwrite Manual (Add/Edit) current prices using Momentum too.",
        key="bulk_overwrite_manual",
    )
with cbu3:
    bulk_set_missing_pref = st.checkbox(
        "Set missing preference → Momentum",
        value=True,
        help="If a stock has no saved price source, treat it as Momentum-linked.",
        key="bulk_set_missing_pref",
    )

def _bulk_update_current_prices_from_momentum(
    df_in: pd.DataFrame,
    *,
    only_momentum_pref: bool = True,
    overwrite_manual: bool = False,
    set_missing_pref_to_momentum: bool = True,
) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Update df['CurrentPrice'] for stocks using Momentum last close. Returns (df_updated, results_df)."""
    df = df_in.copy()
    ...
    prefs = get_price_src_prefs()
    now_iso = _now_iso()

    # Collect unique canonical names
    raw_names = []
    try:
        raw_names = df["Name"].dropna().astype(str).tolist()
    except Exception:
        raw_names = []
    names = sorted({ _canon_name(n) for n in raw_names if str(n).strip() })

    rows = []
    for name_canon in names:
        safe = _safe_key_for_price_src(name_canon)
        pref = prefs.get(safe)
        if pref is None and set_missing_pref_to_momentum:
            pref = "Momentum"
        pref = _norm_price_src(pref or "Momentum")

        if only_momentum_pref and (pref != "Momentum") and (not overwrite_manual):
            # Skip manual stocks unless overwrite is enabled
            rows.append({
                "Name": name_canon,
                "Pref": pref,
                "OldCurrentPrice": _get_last_nonnull_price(df, name_canon),
                "NewCurrentPrice": _get_last_nonnull_price(df, name_canon),
                "AsOf": pd.NaT,
                "Status": "SKIPPED",
                "Note": "Manual price source",
            })
            continue

        # Compute momentum last close
        stock_rows = df[df["Name"].astype(str) == name_canon]
        mom_px, mom_asof = _momentum_close_before_today(name_canon, stock_rows)

        old_px = _get_last_nonnull_price(df, name_canon)

        if mom_px is None:
            # Don't destroy existing price; keep old
            rows.append({
                "Name": name_canon,
                "Pref": pref,
                "OldCurrentPrice": old_px,
                "NewCurrentPrice": old_px,
                "AsOf": mom_asof,
                "Status": "NO_OHLC",
                "Note": "No OHLC found (update Momentum page / check CSV name)",
            })
            continue
        ...
        mask = df["Name"].astype(str) == name_canon
        if mask.any():
            df.loc[mask, "CurrentPrice"] = new_px
            df.loc[mask, "MetaModified"] = now_iso
        else:
            # Create a placeholder meta row if missing
            base_row = {
                "Name": name_canon,
                "Industry": pd.NA,
                "IndustryBucket": pd.NA,
                "IsQuarter": False,
                "Quarter": "",
                "Year": pd.NA,
                "CurrentPrice": new_px,
    ...
    if not res.empty:
        # Pretty display
        try:
            res["AsOf"] = pd.to_datetime(res["AsOf"], errors="coerce").dt.date
        except Exception:
            pass
    return df, res

def _get_last_nonnull_price(df: pd.DataFrame, name_canon: str) -> float | None:
    try:
        m = df["Name"].astype(str) == name_canon
        if "CurrentPrice" in df.columns:
            s = df.loc[m, "CurrentPrice"].dropna()
            if not s.empty:
                v = float(s.iloc[-1])
                return v if math.isfinite(v) else None
    except Exception:
...
# Button row
cbtn1, cbtn2 = st.columns([1.25, 1])
with cbtn1:
    do_bulk = st.button("⚡ Update ALL Current Prices (from Momentum)", use_container_width=True)
with cbtn2:
    st.caption("Tip: This updates **CurrentPrice** snapshot used across pages.")

# Popup / dialog renderer (supports old Streamlit too)
def _render_bulk_price_results(res_df: pd.DataFrame):
    if res_df is None or res_df.empty:
        st.info("No stocks found to update.")
        return
    # Summary
    try:
        updated = int((res_df["Status"] == "UPDATED").sum())
        skipped = int((res_df["Status"] == "SKIPPED").sum())
        noohlc  = int((res_df["Status"] == "NO_OHLC").sum())
        st.success(f"Done. Updated: {updated} • Skipped: {skipped} • No OHLC: {noohlc}")
    except Exception:
        st.success("Done.")
    st.dataframe(res_df, use_container_width=True, hide_index=True)

def _close_bulk_dialog():
    st.session_state["_show_bulk_price_dialog"] = False
    st.session_state.pop("_bulk_price_results", None)
    _safe_rerun()

if do_bulk:
    with st.spinner("Updating current prices from Momentum…"):
        df0 = get_df()
        df_updated, res_df = _bulk_update_current_prices_from_momentum(
            df0,
            only_momentum_pref=bool(st.session_state.get("bulk_only_momentum_pref", True)),
            overwrite_manual=bool(st.session_state.get("bulk_overwrite_manual", False)),
            set_missing_pref_to_momentum=bool(st.session_state.get("bulk_set_missing_pref", True)),
        )
        save_df(df_updated)
        st.session_state["_bulk_price_results"] = res_df
        st.session_state["_show_bulk_price_dialog"] = True

# Show results (dialog if supported)
if st.session_state.get("_show_bulk_price_dialog") and st.session_state.get("_bulk_price_results") is not None:
    res_df = st.session_state.get("_bulk_price_results")

    if hasattr(st, "dialog"):
        @st.dialog("✅ Bulk price update results")
        def _bulk_price_dialog():
            _render_bulk_price_results(res_df)
            if st.button("Close", use_container_width=True):
                _close_bulk_dialog()
        _bulk_price_dialog()
    else:
        # Fallback: expandable 'popup'
        with st.expander("✅ Bulk price update results (close when done)", expanded=True):
            _render_bulk_price_results(res_df)
            if st.button("Close", use_container_width=True):
                _close_bulk_dialog()
...
if stock_name:
    ...
    with tabs_top[0]:
        ...
        _safe_stock = _safe_key_for_price_src(stock_name)
        years = st.multiselect(
            "Years to edit/add (Annual)",
            options=[y for y in range(2000, 2036)],
            default=years_for_stock or [2023],
            key=f"annual_years__{_safe_stock}",
        )

        cat_dict = _get_categories(bucket, quarterly=False)
        flat_annual = _cats(bucket, quarterly=False, _cir_key=_cir_cache_key())
        ...
        if st.button("💾 Save All Annual Changes"):
            ...
            for year, inputs_for_year in annual_data.items():
                ...
                diff_allowlist = set(inputs_for_year.keys()) | _allowed_meta_fields("annual")
                changes = _diff_dict(before, row_up, allowed=diff_allowlist)
    ...
    with tabs_top[1]:
        ...
        sel_tokens = st.multiselect(
            ...
            key=f"q_form_tokens__{_safe_stock}",
        )
        ...
        flat_q = _cats(bucket, quarterly=True, _cir_key=_cir_cache_key())
        ...
        if st.button("💾 Save ALL selected quarters", key="save_all_selected_quarters"):
            ...
            for (y, q) in parsed:
                ...
                diff_allowlist_q = set(available_q_keys) | _allowed_meta_fields("quarterly")
                changes = _diff_dict(before, new_row, allowed=diff_allowlist_q)
...
if all_rows.empty:
    ...
    for name in [name] if name else []:
        ...
        with st.expander(str(name), expanded=expanded_now):
            ...
            # Quick current price + source (Momentum vs Manual)
            _mask_name = df["Name"].astype(str).str.upper() == str(name).strip().upper()
            
            # NEW: prefer remembered manual price for the seed
            _man_map_qe = get_manual_price_prefs()
            manual_default = _man_map_qe.get(_safe_key_for_price_src(name))
            if manual_default is None:
                try:
                    manual_default = float(df.loc[_mask_name, "CurrentPrice"].dropna().iloc[-1])
                except Exception:
                    manual_default = None

            # Read saved pref (default Momentum) + get Momentum last close
            pref_map = get_price_src_prefs()
            saved_src = pref_map.get(_safe_key_for_price_src(name), "Momentum")

            mom_px, mom_asof = _momentum_close_before_today(name, df[_mask_name])
            mom_note = f"As of {mom_asof.date()}" if mom_asof is not None else "No OHLC found — update on Momentum page"

            # UI: pick source
            src_choice_qe = st.radio(
                "Current price source",
                ["Momentum (last close)", "Manual (key in)"],
                index=(0 if saved_src == "Momentum" else 1),
                horizontal=True,
                key=f"qe_price_src_{name}",
                help="Pick Momentum to link price to your OHLC CSVs; pick Manual to store your own price.",
            )
            use_manual_qe = src_choice_qe.startswith("Manual")

            if use_manual_qe:
                # Prefill manual from remembered manual; else fall back to Momentum
                seed = manual_default if manual_default is not None else mom_px
                ...
                st.caption("Source: Add/Edit (manual)")
                val_to_store_qe = float(cur_price_edit or 0.0)
            else:
                # Show Momentum price (read-only)
                st.number_input(
                    "Current Price (Momentum)",
                    value=float(mom_px or 0.0),
                    step=0.0001,
                    format="%.4f",
                    disabled=True,
                )
                st.caption(f"Source: Momentum (last close) • {mom_note}")
                # Keep linked semantics by default (don’t snapshot Momentum)
                val_to_store_qe = float(mom_px) if mom_px is not None else pd.NA

            if st.button("💾 Save current price", key=unique_key("save_cur_price", name)):
                # Persist preference + price value (NA when Momentum so it stays linked)
                set_price_src_pref(name, ("Add/Edit" if use_manual_qe else "Momentum"))
                if use_manual_qe:
                    set_manual_price_pref(name, val_to_store_qe)  # remember last manual

                if "CurrentPrice" not in df.columns:
                    df["CurrentPrice"] = pd.NA
                df.loc[_mask_name, "CurrentPrice"] = val_to_store_qe
                df.loc[_mask_name, "MetaModified"] = _now_iso()

                save_df(df)
                st.success("Saved. Quick Edit synced.")
                # Clean up widget state so it reseeds next run
                st.session_state.pop(f"cur_price_quick_{name}", None)
                st.session_state.pop(f"cur_price_quick_preview_{name}", None)
                _safe_rerun()
            ...
            with tabs[0]:
                ...
                if submit_a:
                    ...
                    for _, er in keep.iterrows():
                        ...
                        # ⇩ NEW: also compute banking-specific deriveds for Annual Quick Edit
                        if bucket_default_this == "Banking":
                            compute_banking_derivatives_annual(
                                row_up,
                                df=df,
                                stock_name=name,
                                year=y,
                                available_keys=allowed_a_keys,  # use the same allowlist you're editing
                            )
            ...
            try:
                ...
                if incoming is None or incoming.empty:
                    ...
                    if not cleaned_rows:
                        ...
                        # Recompute derived fields so downstream pages are consistent immediately
                        bucket_for_stock = _infer_bucket_for_stock(df, stock_pick, fallback="General")
                        rows_out: list[dict] = []
                        for _, rr in incoming_df.iterrows():
                            ...
                            try:
                                ...
                                y = 0
                            q = str(row.get("Quarter") or "")
                            if _is_q:
                                compute_common_derivatives_inplace(row, bucket=bucket_for_stock, prefix="Q_")
                                compute_banking_derivatives_quarterly(
                                    row,
                                    df=df,
                                    stock_name=stock_pick,
                                    year=y,
                                    quarter=q,
                                    available_keys=set(row.keys()),
                                )
                            else:
                                compute_common_derivatives_inplace(row, bucket=bucket_for_stock, prefix="")
                                compute_banking_derivatives_annual(
                                    row,
                                    df=df,
                                    stock_name=stock_pick,
                        ...
                        incoming_df = pd.DataFrame(rows_out)
