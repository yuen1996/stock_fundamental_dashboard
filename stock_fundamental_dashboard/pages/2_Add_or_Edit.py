...
# --- Reuse Momentum folder & resolve the right CSV path  ---
def _resolve_ohlc_dir() -> str:
    ...
    for d in candidates:
        if os.path.isdir(d):
            return d
    d = os.path.abspath(os.path.join(_PARENT, "data", "ohlc"))
...
def _load_ohlc_for_name(stock_name: str, _etag: int, *, ticker_hint: str | None = None) -> pd.DataFrame | None:
    ...
    # fallback to filename candidates regardless of name_link presence
    if not path:
