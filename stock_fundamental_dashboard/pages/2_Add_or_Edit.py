...
def _diff_dict(before: dict | None, after: dict | None, *, allowed: set[str] | None = None) -> dict:
    ...
    for k in allowed:
        ...
        b = _normalize_nullish(b)
        a = _normalize_nullish(a)
        if b != a:
            changes[k] = [b, a]
    return changes

def _normalize_nullish(v):
    try:
        if pd.isna(v):
            return None
    except Exception:
...
def _set_if_available_annual(year: int, key: str, val, bucket: str):
    ...
    prev = st.session_state.get(ssk, None)
    st.session_state[ssk] = float(val)

    # NEW: mirror into the visible text widget so the form shows it
    ...
    return prev != st.session_state[ssk]
...
with c1:
    new_fd = st.number_input(
        ...
        value=(cur_fd if cur_fd is not None else 0.00),
        step=0.10, format="%.2f",
        help="Global FD / risk-free rate."
    )
with c2:
    new_epf = st.number_input(
        "EPF rate (%)",
        value=(cur_epf if cur_epf is not None else 0.00),
        step=0.10, format="%.2f",
        help="Global EPF rate."
    )
...
if submit_settings:
    if not stock_name:
        ...
        if mask.any():
            ...
            df.loc[mask, "CurrentPrice"] = float(cp)
            if ind_canon:
        ...
        for i, year in enumerate(years):
            with tab_annual[i]:
                ...
                if bucket == "Banking":
                    ...
                    before_temp = {k: _normalize_nullish(v) for k, v in temp.items()}
                    ...
                    for k, v in temp.items():
                        new_v = _normalize_nullish(v)
                        if new_v is None:
                            continue
                        if before_temp.get(k) == new_v:
                            continue
                        _set_if_available_annual(year, k, new_v, bucket)
        ...
        for i, (y, q) in enumerate(parsed):
            with tabs_q[i]:
                ...
                if bucket == "Banking":
                    ...
                    before_tmp = {k: _normalize_nullish(v) for k, v in tmp.items()}
                    ...
                    for k, v in tmp.items():
                        ...
                        if new_v is None:
                            continue
                        if before_tmp.get(k) == new_v:
                            continue
                        _set_if_available_quarterly(int(y), q, k, new_v, bucket)
