...
# ⬅️ Stock name OUTSIDE the form so Momentum can react immediately
stock_name_raw = st.text_input("Stock Name", value=default_stock, key="stock_name_input")
stock_name = _canon_name(stock_name_raw)

with st.form("stock_settings_form", clear_on_submit=False):
    # everything else in your form stays the same (Industry/Bucket/etc)
    ...
    if use_manual:
        # show manual editor (prefill from last saved manual if any)
        ...
        st.number_input(
            ...
            disabled=True,
            key="cur_price_stock_preview",
        )
        st.caption(f"Source: Momentum (last close) • {mom_caption}")
        # keep cp in scope for save-path (we'll write Momentum px if you want a snapshot)
        cp = float(mom_px) if mom_px is not None else None
        ...
        with st.expander(str(name), expanded=expanded_now):
            ...
            if use_manual_qe:
                ...
                st.number_input(
                    ...
                    format="%.4f",
                    disabled=True,
                    key=f"cur_price_quick_preview_{name}",
                )
