...
# --- FIELD DEFINITIONS (these are the ONLY data-entry fields we allow in quick-edit) ---
...
# ---------- Annual & Quarterly Forms (original section kept) ----------
if stock_name:
    ...
    for name in sorted(all_rows["Name"].dropna().unique()):
        ...
        with st.expander(name, expanded=False):
            ...
            with tabs[0]:
                ...
                # If no annual rows yet, build a template frame so user can add rows
                if av.empty:
                    ...
                    av.loc[:, "Name"] = name
                    av.loc[:, "Industry"] = ""         # editable
                    av.loc[:, "Year"] = pd.Series(dtype="Int64")
                    av.loc[:, "IsQuarter"] = False
                    av.loc[:, "Quarter"] = ""

                    # Insert RowKey/Delete as scalars
                    av.insert(0, "RowKey", "")
                ...
                allowed_a = ["RowKey", "Delete", "Name", "Industry", "Year"] + \
                            [c for c in av.columns if c in ANNUAL_ALLOWED_BASE and c not in {"IsQuarter", "Quarter"}]
                ...
                edited_a = st.data_editor(
                    ...
                    height=360,
                    hide_index=True,
                    num_rows="dynamic",  # allow + Add row
                ...
                if st.button(f"💾 Save Annual for {name}", key=f"qeb_save_a_{name}"):
                    ...
                    for _, er in keep.iterrows():
                        ...
                        if not er.get("RowKey"):  # new row
                            row_up = {"Name": name,
                                      "Industry": er.get("Industry", ""),
                                      "Year": y,
                                      "IsQuarter": False,
                                      "Quarter": ""}
                            # push only allowed base fields
            ...
            with tabs[1]:
                ...
                if qv.empty:
                    ...
                    qv.loc[:, "Name"] = name
                    qv.loc[:, "Industry"] = ""          # editable
                    qv.loc[:, "Year"] = pd.Series(dtype="Int64")
                    qv.loc[:, "Quarter"] = "—"          # placeholder allowed by Selectbox options
                ...
                # Keep ONLY base-entry columns (hide ratios)
                allowed_q = ["RowKey", "Delete", "Name", "Industry", "Year", "Quarter"] + \
                            [c for c in qv.columns if c in QUARTERLY_ALLOWED_BASE and c not in {"IsQuarter"}]
                qv_display = qv[[c for c in allowed_q if c in qv.columns]].copy()
                ...
                if st.button(f"💾 Save Quarterly for {name}", key=f"qeb_save_q_{name}"):
                    del_keys = set(edited_q.loc[edited_q.get("Delete", False) == True, "RowKey"].tolist()) if not edited_q.empty else set()
                    keep = edited_q[edited_q.get("Delete", False) != True].copy() if not edited_q.empty else edited_q
                    ...
                    for _, er in keep.iterrows():
                        ...
                        y = int(er["Year"])
                        qt = er.get("Quarter", "—")
                        if qt == "—":  # ignore placeholder
                            continue

                        if not er.get("RowKey"):  # new row -> RowKey will appear after save/rerun
                            row_uq = {"Name": name,
                                     "Industry": er.get("Industry", ""),
                                     "Year": y,
                                     "IsQuarter": True,
                            ...
                            for c in keep.columns:
                                if c in ("RowKey", "Delete"):
                                    continue
                                if c in QUARTERLY_ALLOWED_BASE and c in df.columns:
                                    df.loc[mask, c] = er.get(c, None)
