...
with st.form("stock_settings_form", clear_on_submit=False):
    ...
    # 2.5) Manual prefill from last saved manual (if any)
    ...
    # 3) UI (always render manual input so you can type immediately)
    src_choice = st.radio(
    ...
    use_manual = (src_choice.endswith("key in"))

    # Manual editor (always visible; only saved if 'Manual' selected)
    cp_manual = calc_number_input(
        "Current Price (manual)",
        value=(current_price_default if current_price_default not in (None, "") else None),
        key="cur_price_stock",
        decimals=4,
        placeholder="e.g. 0.3450 (leave blank to clear)",
    )
    st.caption("If 'Manual (key in)' is selected, this value will be saved.")

    # Momentum preview (read-only)
    st.number_input(
    ...
    st.caption(f"Momentum as of: {mom_caption}")

    # Value chosen for save logic (do NOT snapshot momentum into CSV)
    cp = cp_manual if use_manual else None
...
if submit_settings:
    if not stock_name:
        ...
        set_price_src_pref(stock_name, ("Add/Edit" if use_manual else "Momentum"))
        val_to_store = (float(cp) if (cp is not None and use_manual) else pd.NA)
...
if all_rows.empty:
    ...
    for name in [name] if name else []:
        ...
        with st.expander(str(name), expanded=expanded_now):
            ...
            use_manual_qe = src_choice_qe.endswith("key in")

            if use_manual_qe:
                # Prefill manual from last saved CurrentPrice (if any)
                manual_default = None
                try:
                    if "CurrentPrice" in df.columns and df.loc[_mask_name, "CurrentPrice"].dropna().size:
                ...
                cur_price_edit = st.number_input(
                    "Current Price (manual)",
                    value=float(manual_default or 0.0),
                    step=0.0001,
                    format="%.4f",
                ...
                # Show Momentum price (read-only) — no key so value updates on rerun
                st.number_input(
                    "Current Price (Momentum)",
                    value=float(mom_px or 0.0),
                    step=0.0001,
                ...
                # Pure-link semantics: don't snapshot momentum into CSV
