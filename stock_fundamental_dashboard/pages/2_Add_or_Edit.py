    ...
    with tabs_top[0]:
        ...
        for i, year in enumerate(years):
            with tab_annual[i]:
                ...
                # Clear (None/blank) instead of forcing 0s
                if st.button("Clear all fields (blank)", key=f"reset_{year}_annual"):
                    for f in flat_annual:
                        k = f.get("key", "")
                        if not k:
                            continue
                        st.session_state[f"{k}_{year}_annual"] = None
                        txtk = f"{k}_{year}_annual_txt"
                        if txtk in st.session_state:
                            st.session_state[txtk] = ""
                    _safe_rerun()
    ...
    with tabs_top[1]:
        ...
        for i, (y, q) in enumerate(parsed):
            with tabs_q[i]:
                ...
                # Clear (None/blank) instead of forcing 0s
                if st.button(f"Clear all fields (blank)", key=f"reset_{y}_{q}_q"):
                    for f in flat_q:
                        k = f.get("key", "")
                        if not k:
                            continue
