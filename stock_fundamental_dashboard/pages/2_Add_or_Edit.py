...
with st.form("stock_settings_form", clear_on_submit=False):
    ...
    # ✅ One field only: label stays 'Current Price (Momentum)'
    #    - In Manual: editable (different key so Streamlit reseeds correctly)
    #    - In Momentum: read-only, shows the momentum price
    if use_manual:
        cp = st.number_input(
            "Current Price (Momentum)",
            value=float(current_price_default or 0.0),
        ...
        cp = st.number_input(
            ...
            step=0.0001, format="%.4f",
            key="cur_price_stock_num_momentum",  # different key so the value updates when toggling
            disabled=True,
        )
