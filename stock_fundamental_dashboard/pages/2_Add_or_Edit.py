...
# --- Banking TP alias map: friendly UI key -> (canonical override key, base source key)
# If you use slightly different base names in your config, adjust the strings on the right.
_BANK_TP_ALIASES = {
    "TP (Bank) – NIM Numerator: Net Interest Income incl. Islamic (RM)": ("TP_Bank_NIM_Num", "NII (incl Islamic)"),
    "TP (Bank) – NIM Denominator: Average Earning Assets (RM)": ("TP_Bank_NIM_Den", "Average Earning Assets"),
    "TP (Bank) – CIR Numerator: Operating Expenses (RM)": ("TP_Bank_CIR_Num", "Operating Expenses"),
    "TP (Bank) – CIR Denominator: Operating Income (RM)": ("TP_Bank_CIR_Den", "Operating Income"),
    "TP (Bank) – Leverage Numerator: Total Assets (RM)": ("TP_Bank_Lev_Num", "Total Assets"),
    "TP (Bank) – Leverage Denominator: Equity (RM)": ("TP_Bank_Lev_Den", "Equity"),
}
...
def _link_bank_tp_overrides_in_state(*, scope: str, year: int, quarter: str | None = None):
    """
    Keep friendly TP fields and canonical overrides in sync and, if both are blank,
    auto-fill them from their base source fields (Operating Income, Total Assets, etc.).
    """
    ...
    for friendly, (canon, src) in _BANK_TP_ALIASES.items():
        fk = k(friendly)
        ck = k(canon)
        sk = k(src)

        fv = _try_float(st.session_state.get(fk))
        cv = _try_float(st.session_state.get(ck))
        sv = _try_float(st.session_state.get(sk))

        # If user typed friendly but canonical is empty → copy to canonical
        if fv is not None and cv is None:
            st.session_state[ck] = fv

        # If canonical has a value but friendly is empty → mirror into friendly
        if cv is not None and fv is None:
            st.session_state[fk] = cv

        # If both friendly and canonical empty but base exists → auto-fill both from base
        if fv is None and cv is None and sv is not None:
            st.session_state[fk] = sv
...
with st.form("stock_settings_form", clear_on_submit=False):
    ...
    # Current price default for this stock
    ...
    with tabs_top[0]:
        ...
        if st.button("💾 Save All Annual Changes"):
            ...
            for year, inputs_for_year in annual_data.items():
                ...
                # Map friendly TP fields to canonical; fallback from base if canonical is still blank
                for friendly, (canon, src) in _BANK_TP_ALIASES.items():
                    fv = _try_float(row_up.get(friendly))
                    cv = _try_float(row_up.get(canon))
                    sv = _try_float(row_up.get(src))
                    if cv is None:
                        row_up[canon] = fv if fv is not None else sv
                    row_up.pop(friendly, None)  # don't persist friendly UI column
    ...
    with tabs_top[1]:
        ...
        if st.button("💾 Save ALL selected quarters", key="save_all_selected_quarters"):
            ...
            for (y, q) in parsed:
                ...
                # Map friendly TP fields to canonical; fallback from base if canonical is still blank
                for friendly, (canon, src) in _BANK_TP_ALIASES.items():
                    fv = _try_float(new_row.get(friendly))
                    cv = _try_float(new_row.get(canon))
                    sv = _try_float(new_row.get(src))
                    if cv is None:
                        new_row[canon] = fv if fv is not None else sv
                    new_row.pop(friendly, None)  # don't persist friendly UI column
