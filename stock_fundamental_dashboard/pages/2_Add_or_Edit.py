...
if df is None or df.empty:
    df = pd.DataFrame(columns=["Name", "Industry", "Year", "IsQuarter", "Quarter"] +
        [f[1] for f in INCOME_STATEMENT_FIELDS] +
        [f[1] for f in BALANCE_SHEET_FIELDS] +
        [f[1] for f in OTHER_DATA_FIELDS] +
        [f[1] for f in QUARTERLY_FIELDS]
    )

stock_name = st.text_input("Stock Name")
industry = st.text_input("Industry")
if not stock_name: st.info("Enter a stock name to begin.")

# --- ANNUAL DATA ---
st.subheader("Annual Financial Data")
years_for_stock = sorted(df[(df["Name"] == stock_name) & (df["IsQuarter"] != True)]["Year"].unique()) if stock_name else []
years = st.multiselect("Years to edit/add (Annual)", options=[y for y in range(2000, 2031)], default=years_for_stock or [2023])
tab_annual = st.tabs([f"Year {y}" for y in years])
...
for i, year in enumerate(years):
    with tab_annual[i]:
        ...
        for label, key in INCOME_STATEMENT_FIELDS:
            year_data[key] = st.number_input(label, value=float(prefill.get(key, 0.0)), key=f"{key}_{year}_annual", step=1.0, format="%.4f")
        # Balance Sheet
        ...
        for label, key in OTHER_DATA_FIELDS:
            year_data[key] = st.number_input(label, value=float(prefill.get(key, 0.0)), key=f"{key}_{year}_annual", step=0.0001, format="%.4f")
        annual_data[year] = year_data

        # --- DELETE BUTTON FOR YEAR ---
        if row.shape[0]:
            if st.button(f"Delete Year {year}", key=f"del_year_{year}"):
...
quarters_in_df = df[(df["Name"] == stock_name) & (df["IsQuarter"] == True)][["Year", "Quarter"]].drop_duplicates()
quarter_choices = []
for y in range(2000, 2032):
    for q in all_quarters:
        quarter_choices.append((y, q))

quarters_to_edit = st.multiselect(
    "Quarters to edit/add", 
    options=quarter_choices, 
    format_func=lambda x: f"{x[1]} {x[0]}", 
    default=[(row["Year"], row["Quarter"]) for idx, row in quarters_in_df.iterrows()] or [(2023, "Q4")]
)
tab_quarters = st.tabs([f"{q[1]} {q[0]}" for q in quarters_to_edit])
...
for i, (year, quarter) in enumerate(quarters_to_edit):
    with tab_quarters[i]:
        ...
        for label, key in QUARTERLY_FIELDS[:7]:
            q_data[key] = st.number_input(label, value=float(prefill.get(key, 0.0)), key=f"{key}_{year}_{quarter}_q", step=1.0, format="%.4f")
        # Balance Sheet
        ...
        for label, key in QUARTERLY_FIELDS[21:]:
            q_data[key] = st.number_input(label, value=float(prefill.get(key, 0.0)), key=f"{key}_{year}_{quarter}_q", step=0.0001, format="%.4f")
        quarterly_data[(year, quarter)] = q_data

        # --- DELETE BUTTON FOR QUARTER ---
        if row.shape[0]:
            if st.button(f"Delete {quarter} {year}", key=f"del_{year}_{quarter}"):
