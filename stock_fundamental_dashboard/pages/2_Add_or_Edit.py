...
.sec{
  ...
  box-shadow:var(--shadow); padding:.65rem .9rem; margin:1rem 0 .6rem 0;
...
div[data-testid="stDataFrame"] table, div[data-testid="stDataEditor"] table{ border-collapse:separate !important; border-spacing:0; }
div[data-testid="stDataFrame"] table tbody tr:hover td, div[data-testid="stDataEditor"] table tbody tr:hover td{ background:#f8fafc !important; }
...
/* Optional: hide built-in page list in sidebar
...
if stock_name:
    ...
    with tabs_top[1]:
        ...
        for label, key in QUARTERLY_FIELDS[20:]:
                wkey = f"{key}_{sel_year}_{sel_quarter}_q"
                val = st.session_state.get(wkey, float(prefill_q.get(key, 0.0) or 0.0))
                comma_number_input(label, value=val, key=wkey, decimals=4)

        c1, c2, _ = st.columns([1, 1, 2])
        with c1:
            if st.button("💾 Save this quarter", key=f"save_{sel_year}_{sel_quarter}_q"):
                new_row = {"Name": stock_name, "Industry": industry, "Year": int(sel_year), "IsQuarter": True, "Quarter": sel_quarter}
