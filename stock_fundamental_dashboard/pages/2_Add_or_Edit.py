...
# --- Current Price store (decoupled from financial saves) ---
_PRICES_FILE = os.path.join(_GRANDP, "data", "current_prices.json")
...
def _save_prices(d: dict) -> None:
    os.makedirs(os.path.dirname(_PRICES_FILE), exist_ok=True)
    with open(_PRICES_FILE, "w", encoding="utf-8") as f:
        json.dump(d, f, indent=2, ensure_ascii=False)

def get_price(name: str) -> float | None:
    try:
        v = _load_prices().get(_canon_name(name))
        return float(v) if v is not None else None
    except Exception:
        return None

def set_price(name: str, v: float | None) -> None:
    d = _load_prices()
    if v in (None, "", "—"):
        d.pop(_canon_name(name), None)
    else:
        d[_canon_name(name)] = float(v)
    _save_prices(d)
    # small etag bump so other pages see it
    _bump_data_etag()
...
_PLACEHOLDERS_BASE = {
    ...
    "CostOfSales": "Direct costs (COGS). Enter as a positive number.",
    "Gross Profit": "Revenue − COGS.",
    "Operating Profit": "EBIT; excludes finance costs & tax.",
    "EBITDA": "EBIT + depreciation + amortisation.",
    "DepPPE": "PPE depreciation (absolute, +ve).",
    "DepROU": "ROU / lease depreciation (absolute, +ve).",
    "DepInvProp": "Investment property depreciation (absolute, +ve).",
    "DepAmort": "Total depreciation & amortisation (incl. PPE/ROU/Inv Prop).",
    "Three Fees": "Selling + Administrative + Interest expense (absolute).",
    "Interest Expense": "Finance costs (absolute, +ve).",
    "Net Profit": "Profit after tax & minorities.",
    "Total Borrowings": "Gross debt outstanding.",
    "Cash & Cash Equivalents": "Cash and near-cash balances.",
    "Units Outstanding": "Shares/units outstanding (avg or current).",
    "Equity": "Shareholders’ equity / net assets.",
    "Capex": "Capital expenditures (cash basis).",
    ...
    "NII (incl Islamic)": "Interest income − interest expense + net Islamic.",
    "Earning Assets": "Interest-earning assets.",
    "TP_Bank_NIM_Num": "Override NIM numerator (defaults to ‘NII (incl Islamic)’).",
    "TP_Bank_NIM_Den": "Override NIM denominator (defaults to ‘Average Earning Assets’).",
    "NIM": "Net Interest Margin (% of average earning assets).",

    # (If you surface these in your forms later, these hints are ready)
    "TP_Bank_CIR_Num": "Override CIR numerator: Operating Expenses.",
    "TP_Bank_CIR_Den": "Override CIR denominator: Operating Income.",
    "TP_Bank_Lev_Num": "Override Leverage numerator: Total Assets.",
    "TP_Bank_Lev_Den": "Override Leverage denominator: Equity.",
}

# Extra hints for friendly TP (Bank) labels used in config forms
_PLACEHOLDERS_BASE.update({
    "TP (Bank) – NIM Numerator: Net Interest Income incl. Islamic (RM)": "Optional override; if blank, engine uses ‘NII (incl Islamic)’.",
    "TP (Bank) – NIM Denominator: Average Earning Assets (RM)": "Optional override; if blank, engine uses ‘Average Earning Assets’.",
    "TP (Bank) – CIR Numerator: Operating Expenses (RM)": "Optional override; if blank, uses ‘Operating Expenses’.",
    "TP (Bank) – CIR Denominator: Operating Income (RM)": "Optional override; if blank, uses ‘Operating Income’.",
    "TP (Bank) – Leverage Numerator: Total Assets (RM)": "Optional override; if blank, uses ‘Total Assets’.",
    "TP (Bank) – Leverage Denominator: Equity (RM)": "Optional override; if blank, uses ‘Equity’.",
})

# Map friendly TP (Bank) labels used in config → canonical internal keys
_BANK_TP_ALIASES = {
    "TP (Bank) – NIM Numerator: Net Interest Income incl. Islamic (RM)": "TP_Bank_NIM_Num",
    "TP (Bank) – NIM Denominator: Average Earning Assets (RM)": "TP_Bank_NIM_Den",
    "TP (Bank) – CIR Numerator: Operating Expenses (RM)": "TP_Bank_CIR_Num",
    "TP (Bank) – CIR Denominator: Operating Income (RM)": "TP_Bank_CIR_Den",
    "TP (Bank) – Leverage Numerator: Total Assets (RM)": "TP_Bank_Lev_Num",
    "TP (Bank) – Leverage Denominator: Equity (RM)": "TP_Bank_Lev_Den",
}
...
with st.form("stock_settings_form", clear_on_submit=False):
    ...
    # Current price default (prices store first, then legacy df fallbacks)
    current_price_default = None
    if stock_name:
        current_price_default = get_price(stock_name)
    ...
    with tabs_top[0]:
        ...
        for i, year in enumerate(years):
            with tab_annual[i]:
                ...
                for f in flat_annual:
                    ...
                    # Keep friendly TP fields and canonical in sync (Annual)
                    for friendly, canon in _BANK_TP_ALIASES.items():
                        fk = f"{friendly}_{year}_annual"
                        ck = f"{canon}_{year}_annual"
                        if fk in st.session_state and st.session_state.get(fk) is not None and st.session_state.get(ck) is None:
                            st.session_state[ck] = _try_float(st.session_state[fk])
                        if ck in st.session_state and st.session_state.get(ck) is not None and st.session_state.get(fk) is None:
                            st.session_state[fk] = st.session_state[ck]

                # compute derived (banking + common)
                if bucket == "Banking":
        ...
        if st.button("💾 Save All Annual Changes"):
            ...
            for year, inputs_for_year in annual_data.items():
                ...
                # Map friendly TP labels → canonical keys before compute
                for friendly, canon in _BANK_TP_ALIASES.items():
                    if friendly in row_up and row_up.get(canon) is None:
                        row_up[canon] = row_up.pop(friendly)           
    ...
    with tabs_top[1]:
        ...
        for i, (y, q) in enumerate(parsed):
            with tabs_q[i]:
                ...
                # Keep friendly TP fields and canonical in sync (Quarterly)
                for friendly, canon in _BANK_TP_ALIASES.items():
                    fk = f"{friendly}_{y}_{q}_q"
                    ck = f"{canon}_{y}_{q}_q"
                    if fk in st.session_state and st.session_state.get(fk) is not None and st.session_state.get(ck) is None:
                        st.session_state[ck] = _try_float(st.session_state[fk])
                    if ck in st.session_state and st.session_state.get(ck) is not None and st.session_state.get(fk) is None:
                        st.session_state[fk] = st.session_state[ck]
        ...
        if st.button("💾 Save ALL selected quarters", key="save_all_selected_quarters"):
            ...
            for (y, q) in parsed:
                ...
                # Map friendly TP labels → canonical keys before compute
                for friendly, canon in _BANK_TP_ALIASES.items():
                    if friendly in new_row and new_row.get(canon) is None:
                        new_row[canon] = new_row.pop(friendly)
