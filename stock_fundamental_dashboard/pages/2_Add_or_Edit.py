...
# --- Keep ONLY meta fields inside the form (no price widgets here) ---
with st.form("stock_settings_form", clear_on_submit=False):
    stock_name_raw = st.text_input("Stock Name", value=default_stock, key="stock_name_inp")
    ...
    with c1:
        ...
        industry = st.text_input("Industry (free text)", value=industry_text_prefill, key="industry_inp")
    with c2:
        bucket_prefill = (
...
# --- OUTSIDE the form so radio clicks re-run immediately ---
stock_name = _canon_name(st.session_state.get("stock_name_inp", ""))
industry   = st.session_state.get("industry_inp", "")
bucket     = st.session_state.get("bucket_sel", "General")

# Read saved pref + compute Momentum price for the currently typed name
...
# Prefill manual from last saved manual (if any)
...
use_manual = st.session_state.get("price_source_radio", "").startswith("Manual")
...
if use_manual:
    ...
    if mom_px is None:
        st.text_input("Current Price", value="", placeholder="—",
                      key="cur_price_stock_num_display", disabled=True)
...
# --- Save handler (now reads session_state values) ---
if submit_settings:
    if not stock_name:
        ...
        # Keep Momentum linked by NOT snapshotting a number
        if _use_manual:
...
if all_rows.empty:
    ...
    for name in [name] if name else []:
        ...
        with st.expander(str(name), expanded=expanded_now):
            ...
            if use_manual_qe:
                # Prefill manual from last saved value; else fall back to Momentum
                try:
                ...
                st.caption("Source: Add/Edit (manual)")
                val_to_store_qe = (float(cur_price_edit) if cur_price_edit is not None else pd.NA)
            else:
