...
with st.form("stock_settings_form", clear_on_submit=False):
    ...
    # ---- Current Price (always keep manual open) ----
    ...
    c_mom, c_manual = st.columns([1, 1])

    with c_mom:
        st.number_input(
            "Current Price (Momentum — read-only)",
            value=float(mom_px or 0.0),
            step=0.0001,
            format="%.4f",
            disabled=True,
            key=f"cur_price_mom_preview_{stock_name}",
        )
        st.caption(f"Source: Momentum (last close) • {mom_caption}")

    with c_manual:
        cp_manual = calc_number_input(
            "Current Price (Manual — always editable)",
            value=(current_price_default if current_price_default not in (None, "") else None),
            key="cur_price_stock",
            decimals=4,
            placeholder="leave blank to clear",
        )
        st.caption("Source: Add/Edit (manual). This stays open even if you choose Momentum below.")

    # 3) Decide which price to USE (manual stays open either way)
    src_choice = st.radio(
        "Which price should be used?",
        ["Momentum (link to OHLC)", "Manual (use value typed)"],
        index=(0 if saved_src == "Momentum" else 1),
        horizontal=True,
        key="price_source_radio",
        help="Momentum keeps price linked to your OHLC CSVs. Manual saves the value you typed to CurrentPrice.",
    )
...
if submit_settings:
    if not stock_name:
        ...
        val_to_store = (float(cp_manual) if (use_manual and cp_manual is not None) else pd.NA)
...
if all_rows.empty:
    ...
    for name in [name] if name else []:
        ...
        with st.expander(str(name), expanded=expanded_now):
            ...
            # Quick current price: keep manual input open (flexible)
            _mask_name = df["Name"].astype(str).str.upper() == str(name).strip().upper()

            # Read saved pref (default Momentum) + Momentum preview
            ...
            with c_mom:
                st.number_input(
                    ...
                    format="%.4f",
                    disabled=True,
                    key=f"cur_price_mom_preview_{name}",
                )
            ...
            with c_manual:
                manual_input = calc_number_input(
                    "Current Price (Manual — always editable)",
                    value=(manual_default if manual_default not in (None, "") else None),
            ...
            src_choice_qe = st.radio(
                ...
                help="Momentum keeps price linked to OHLC; Manual saves the value you typed to CurrentPrice.",
            )
