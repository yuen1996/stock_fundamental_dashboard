...
with st.form("stock_settings_form", clear_on_submit=False):
    ...
    # 3) UI (ALWAYS render both fields so it works inside forms)
    src_choice = st.radio(
...
if submit_settings:
    if not stock_name:
        ...
        _manual_val = st.session_state.get("cur_price_stock_num_manual", None)

        set_price_src_pref(stock_name, ("Add/Edit" if _use_manual else "Momentum"))
        val_to_store = float(_manual_val) if (_use_manual and _manual_val is not None) else float(mom_px or 0.0)
