...
def _df_to_json_bytes(df: pd.DataFrame) -> bytes:
    """Serialize the whole dataset to a JSON payload."""
    safe = df.copy()
    # keep None for missing, convert numpy NaN to None
...
def _json_to_df(obj) -> pd.DataFrame:
    """Accepts either {'rows':[...]} or a raw list of rows."""
    rows = obj.get("rows") if isinstance(obj, dict) else obj
    if not isinstance(rows, list):
        raise ValueError("Invalid JSON: expected {'rows': [...]} or a list of rows")
    return pd.DataFrame(rows)


def _normalize_row_for_upsert(r: dict) -> dict:
    """Make incoming row compatible with our schema and conventions."""
    out = dict(r)
    out["Name"] = _canon_name(out.get("Name", ""))
    out["Industry"] = _canon_industry(out.get("Industry", ""))
    out["IndustryBucket"] = _canonical_bucket(out.get("IndustryBucket", ""))
    # Normalise flags/keys
    out["IsQuarter"] = bool(out.get("IsQuarter", False))
    q = str(out.get("Quarter", "") or "").strip().upper()
    out["Quarter"] = (q if out["IsQuarter"] else "")
    # Year -> Int or None
    try:
        out["Year"] = int(out["Year"]) if out.get("Year") not in (None, "", "—") else None
    except Exception:
        out["Year"] = None
    return out


def _merge_upsert(existing: pd.DataFrame, incoming: pd.DataFrame) -> tuple[pd.DataFrame, int, int]:
    """
    Upsert rows from 'incoming' into 'existing' by (Name, IsQuarter, Year, Quarter).
    Only columns present in incoming rows are written.
    Returns: (merged_df, creates, updates)
    """
    df = existing.copy()
    creates = updates = 0

    # Ensure core columns exist
    core = ["Name","Industry","IndustryBucket","IsQuarter","Year","Quarter","LastModified","MetaModified"]
    for c in core:
        if c not in df.columns:
            df[c] = pd.NA

    # Iterate incoming records
    for _, raw in incoming.iterrows():
        row = _normalize_row_for_upsert(raw.to_dict())

        # Build match condition
        cond = (
            (df["Name"].astype(str) == row["Name"])
            & (df["IsQuarter"].astype(bool) == row["IsQuarter"])
        )
        if row["IsQuarter"]:
            cond &= (pd.to_numeric(df["Year"], errors="coerce") == (row["Year"] or -1))
            cond &= (df["Quarter"].astype(str).str.upper() == (row["Quarter"] or ""))
        else:
            cond &= (pd.to_numeric(df["Year"], errors="coerce") == (row["Year"] or -1))

        incoming_keys = set(k for k in row.keys() if k not in {"LastModified", "MetaModified"})
        ...
        # Compute allowed keys for diff: meta + everything the row carries
        allowed = incoming_keys | _allowed_meta_fields("quarterly" if row["IsQuarter"] else "annual")
        changes = _diff_dict(before, row, allowed=allowed)

        if cond.any():
            if changes:
                row["LastModified"] = _now_iso()
                # Only update the columns that are present in the incoming row (+ core)
                to_write = {k: row[k] for k in (incoming_keys | {"LastModified","Industry","IndustryBucket","Name","IsQuarter","Year","Quarter"}) if k in row}
                df.loc[cond, to_write.keys()] = list(to_write.values())
                updates += 1
                _audit_log_event("update", name=row["Name"],
                                 scope=("quarterly" if row["IsQuarter"] else "annual"),
                                 year=row["Year"], quarter=(row["Quarter"] or None),
                                 changes=changes, before=before, after=df.loc[cond].iloc[0].to_dict(),
                                 source="json_import")
        else:
            row["LastModified"] = _now_iso()
            df = pd.concat([df, pd.DataFrame([row])], ignore_index=True)
            creates += 1
            _audit_log_event("create", name=row["Name"],
                             scope=("quarterly" if row["IsQuarter"] else "annual"),
                             year=row["Year"], quarter=(row["Quarter"] or None),
                             changes={k: [None, row.get(k)] for k in incoming_keys},
                             before={}, after=row, source="json_import")

    return df, creates, updates
...
st.markdown(
    section("📦 Manual backup (JSON)", "Download a full JSON backup or import one"),
    unsafe_allow_html=True
)
...
with c_dl:
    # Full backup of current dataset (already cleaned above)
    try:
        b = _df_to_json_bytes(df)
        fname = f"stocks_data_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        st.download_button(
        ...
        st.error(f"Could not prepare backup: {e}")

with c_ul:
    up = st.file_uploader(
        "Upload JSON (backup or partial)",
        type=["json"],
    ...
    if up is not None:
        try:
            ...
            inc_df = _json_to_df(payload)
            st.caption(f"Detected **{len(inc_df)}** row(s) in JSON.")
            mode = st.radio(
                "Import mode",
                ["Merge / Upsert", "Replace everything"],
                horizontal=True,
                key="json_imp_mode"
            )
            if st.button("Import now", use_container_width=True):
                if mode.startswith("Replace"):
                    ...
                    new_all = _json_to_df(payload)
                    # Minimal normalisation for schema consistency
                    new_all = new_all.apply(
                        lambda r: pd.Series(_normalize_row_for_upsert(r.to_dict())),
                        axis=1
                    )
                    save_df(new_all)
                    st.success(f"Replaced dataset with {len(new_all)} row(s).")
                else:
                    merged, creates, updates = _merge_upsert(df, inc_df)
                    save_df(merged)
                    st.success(f"Imported JSON: {creates} created, {updates} updated.")
                _safe_rerun()
        except Exception as e:
