...
if stock_name:
    ...
    # -------- Single-quarter editor (dropdowns; flexible) --------
    ...
    with cb:
        sel_quarter = st.selectbox("Quarter", options=["Q1", "Q2", "Q3", "Q4"], index=3, key="q_form_quarter")
    ...
    # Save all changes (annual only here; quarter saved via individual button above)
...
if all_rows.empty:
    ...
    for name in sorted(all_rows["Name"].dropna().unique()):
        ...
        with st.expander(name, expanded=False):
            ...
            # ----------------- Quarterly Tab (robust: prefills + stable dtypes + live RowKey) -----------------
            with tabs[1]:
                ...
                if qv.empty:
                    # template with required columns so the editor has everything it needs
                ...
                # Build display columns (hide ratios)
                ...
                # Create the display frame and **enforce dtypes BEFORE the editor**
                qv_display = pd.DataFrame(columns=[c for c in allowed_q if c not in ("RowKey", "Delete")])
                for c in qv_display.columns:
                    if c in qv.columns:
                        qv_display[c] = qv[c]

                # live defaults and stable dtypes
                qv_display.insert(0, "RowKey", "")
                qv_display.insert(1, "Delete", False)

                # String-like (TextColumn)
                for col in ["RowKey", "Name", "Industry", "Quarter"]:
                    if col in qv_display.columns:
                        qv_display[col] = qv_display[col].astype("string")

                # Year numeric (nullable int) and Delete boolean
                if "Year" in qv_display.columns:
                    qv_display["Year"] = pd.to_numeric(qv_display["Year"], errors="coerce").astype("Int64")
                qv_display["Delete"] = qv_display["Delete"].fillna(False).astype(bool)

                # Always prefill Name/Industry for any blank rows
                if "Name" in qv_display.columns:
                    qv_display["Name"] = name
                if "Industry" in qv_display.columns:
                    ind = qv_display["Industry"].astype("string").str.strip()
                    ind = ind.replace({"None": "", "none": "", "NaN": "", "nan": ""})
                    qv_display["Industry"] = ind.where(ind != "", industry_default_this)

                # Session buffer so +Add row gets normalized and keeps values across reruns
                state_key = f"qeb_quarter_{name}_df"

                def _normalize_quarter(df_work: pd.DataFrame) -> pd.DataFrame:
                    if df_work is None or df_work.empty:
                        return df_work
                    out = df_work.copy()

                    # enforce types again (user may have added a new row)
                    out["Name"] = name
                    out["Industry"] = (
                    ...
                    out["Quarter"] = (
                        out["Quarter"].astype("string").str.strip().str.upper()
                        .where(lambda s: s.isin(quarters[1:]), "—")
                    )
                    out["Year"] = pd.to_numeric(out["Year"], errors="coerce").astype("Int64")
                    if "Delete" in out:
                        out["Delete"] = out["Delete"].fillna(False).astype(bool)
                    else:
                        out.insert(1, "Delete", False)

                    # live RowKey when Year & Quarter valid
                    def _mk(r):
                        y = r.get("Year")
                        q = r.get("Quarter")
                        return f"{name}|{int(y)}|{q}|Q" if pd.notna(y) and q in {"Q1","Q2","Q3","Q4"} else ""
                    out["RowKey"] = out.apply(_mk, axis=1).astype("string")
                    return out

                # initialize / refresh buffer
                if state_key not in st.session_state:
                    st.session_state[state_key] = _normalize_quarter(qv_display)
                else:
                    st.session_state[state_key] = _normalize_quarter(st.session_state[state_key])
                ...
                # persist normalized result for the next run (keeps RowKey + prefills visible)
                st.session_state[state_key] = _normalize_quarter(edited_q)

                # Save
                if st.button(f"💾 Save Quarterly for {name}", key=f"qeb_save_q_{name}"):
                    buf = _normalize_quarter(st.session_state[state_key])
                    if buf is None or buf.empty:
                        st.info("Nothing to save.")
                    else:
                        valid = buf[(buf["Year"].notna()) & (buf["Quarter"].isin(["Q1","Q2","Q3","Q4"]))]

                        to_del = valid[valid.get("Delete", False) == True]
                        to_up  = valid[valid.get("Delete", False) != True]

                        if to_del.empty and to_up.empty:
                            st.info("Nothing to save.")
                        else:
                            # deletions
                            for _, r in to_del.iterrows():
                                mask = (df["Name"] == name) & (df["IsQuarter"] == True) & \
                                       (df["Year"] == int(r["Year"])) & (df["Quarter"] == str(r["Quarter"]))
                                df.drop(df[mask].index, inplace=True)
