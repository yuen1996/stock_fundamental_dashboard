...
def _strip_three_fees(cats: dict, bucket: str, quarterly: bool = False) -> dict:
    """
    Remove the derived 'Three Fees' field from the UI categories so it can't be edited directly.
    Works for both annual ('Three Fees') and quarterly ('Q_Three Fees') keys.
    """
    if not cats:
        return cats or {}

    drop = {"Three Fees", "Q_Three Fees"}
    out = {}
    for sec, items in (cats or {}).items():
        cleaned = []
        for f in (items or []):
...
def compute_banking_derivatives_annual(
    ...
    if nl_target and _try_float(row_up.get(nl_target)) is None:
        ...
        # Earning Assets (Strict+): NL + FVOCI + Amort + Placements + Reverse Repo (exclude FVTPL)
...
def render_calc_helper(
    *, bucket: str, scope: str, df, stock_name: str, year: int, quarter: str | None = None, fallback: dict | None = None
    ...
    # Don’t show the ‘Three Fees’ helper card (still computed for save-time logic)
    if SHOW_THREE_FEES_HELPER and three_fees is not None:
