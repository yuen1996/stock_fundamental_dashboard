...
def compute_banking_derivatives_annual(
    ...
    # ───────────────────────────────────────────────────────────────────────────
    # Net Loans fallback: write into whichever NL key exists in your config
    ...
    # Earning Assets (Strict [+ interest-bearing FVTPL when provided])
    if "Earning Assets" in available_keys and _try_float(row_up.get("Earning Assets")) is None:
        ...
        fvtpl = _first_float(row_up, _BANK_ALIASES["FVTPL"])  # interest-bearing only

        parts = [v for v in (nl, fvoci, amort, rrepo) if v is not None]
        if parts:
            if fvtpl is not None:
                parts.append(fvtpl)
            row_up["Earning Assets"] = sum(parts)
    # ───────────────────────────────────────────────────────────────────────────
...
def compute_banking_derivatives_quarterly(
    ...
    # Earning Assets (Quarterly): Strict (+ interest-bearing FVTPL when provided)
    if "Q_Earning Assets" in available_keys and _try_float(new_row.get("Q_Earning Assets")) is None:
        ...
        fvtpl = _first_float(new_row, _BANK_ALIASES["FVTPL"], prefix="Q_")  # interest-bearing only
        parts = [v for v in (nl, fvoci, amort, rrepo) if v is not None]
        if parts:
            if fvtpl is not None:
                parts.append(fvtpl)
            new_row["Q_Earning Assets"] = sum(parts)
    # ───────────────────────────────────────────────────────────────────────────
