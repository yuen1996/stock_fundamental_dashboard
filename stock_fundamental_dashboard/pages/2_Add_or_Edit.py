...
# ---- Which fields are derived at save time (so diffs should include them) ----
_DERIVED_COMMON = {"Gross Profit", "DepAmort", "EBITDA", "Three Fees"}
_DERIVED_BY_BUCKET = {
    "Utilities": {"Capex to Revenue"},
    "Telco": {"Capex to Revenue"},
    "Tech": {"R&D Intensity"},
    "Property": {"Net Debt"},
    "REITs": {"Avg Cost of Debt"},
}
_BANK_DERIVED_ANNUAL = {
    "Loan-to-Deposit Ratio",
    "CASA Ratio", "CASA (Core, %)",
    "Net Loans", "Earning Assets",
    "Average Gross Loans", "Average Deposits", "Average Earning Assets",
    "NIM",
}
...
def compute_banking_derivatives_annual(
    ...
    # Net Loans fallback: if blank, derive as Gross Loans − Loan Loss Reserve
    if "Net Loans" in available_keys and _try_float(row_up.get("Net Loans")) is None:
        gl  = _try_float(row_up.get("Gross Loans"))
        llr = _abs_or_none(_first_float(row_up, _LLR_KEYS))
        if gl is not None:
            row_up["Net Loans"] = gl - (llr or 0.0)

    # Earning Assets: Strict (+ interest-bearing FVTPL when provided) — only if blank
    if "Earning Assets" in available_keys and _try_float(row_up.get("Earning Assets")) is None:
        parts = []
        for k in ("Net Loans", "FVOCI Investments", "Amortised Cost Investments", "Reverse Repos"):
            v = _try_float(row_up.get(k))
            if v is not None:
                parts.append(v)
        fvtpl_ib = _try_float(row_up.get("FVTPL Investments"))  # interest-bearing only, per help
        if fvtpl_ib is not None:
            parts.append(fvtpl_ib)
        row_up["Earning Assets"] = sum(parts) if parts else None

    
    # ── Averages: ONLY compute if the CURRENT value is provided ──────────────────

    # Net Loans fallback (Quarterly): if blank, derive as Gross − LLR
    if "Q_Net Loans" in available_keys and _try_float(new_row.get("Q_Net Loans")) is None:
        gl  = _try_float(new_row.get("Q_Gross Loans"))
        llr = _abs_or_none(_first_float(new_row, _LLR_KEYS, prefix="Q_"))
        if gl is not None:
            new_row["Q_Net Loans"] = gl - (llr or 0.0)

    # Earning Assets (Quarterly): Strict (+ interest-bearing FVTPL when provided) — only if blank
    if "Q_Earning Assets" in available_keys and _try_float(new_row.get("Q_Earning Assets")) is None:
        parts = []
        for k in ("Q_Net Loans", "Q_FVOCI Investments", "Q_Amortised Cost Investments", "Q_Reverse Repos"):
            v = _try_float(new_row.get(k))
            if v is not None:
                parts.append(v)
        fvtpl_ib = _try_float(new_row.get("Q_FVTPL Investments"))
        if fvtpl_ib is not None:
            parts.append(fvtpl_ib)
...
def render_calc_helper(
    *, bucket: str, scope: str, df, stock_name: str, year: int, quarter: str | None = None, fallback: dict | None = None
    ...
    if bucket == "Banking":
        ...
        # Earning Assets display (Strict [+ FVTPL if present])
        fvoci  = _try_float(_read("FVOCI Investments"))
        amort  = _try_float(_read("Amortised Cost Investments"))
        rrepo  = _try_float(_read("Reverse Repos"))
        fvtpl  = _try_float(_read("FVTPL Investments"))
        parts  = [v for v in (nl_display, fvoci, amort, rrepo) if v is not None]
        parts_plus = parts + ([fvtpl] if fvtpl is not None else [])
        ea_calc = sum(parts_plus) if parts_plus else None
        if ea_calc is not None:
            note = "Net Loans + FVOCI + Amortised Cost + Reverse Repos" + (" + FVTPL" if fvtpl is not None else "")
    ...
    with tabs_top[0]:
        ...
        if st.button("💾 Save All Annual Changes"):
            ...
            for year in years:
                ...
                # 1) Pull latest typed values from session_state (not from annual_data)
                for k in available_keys:
                    ss_key = f"{k}_{year}_annual"
                    row_up[k] = _try_float(st.session_state.get(ss_key))

                # 2) Recompute deriveds just before save (hard override)
                ...
                # 3) Ensure columns exist (after compute)
                for c in row_up.keys():
                    if c not in df.columns:
                        df[c] = pd.NA

                # 4) Upsert + audit; include DERIVED keys in diff
                cond = (
                    (df["Name"] == stock_name)
                ...
                allowed_for_diff = (
                    set(available_keys)
                    | _derived_keys_for(bucket, scope="annual")
                    | _allowed_meta_fields("annual")
                )
                changes = _diff_dict(before, row_up, allowed=allowed_for_diff)
    ...
    with tabs_top[1]:
        ...
        if st.button("💾 Save ALL selected quarters", key="save_all_selected_quarters"):
            ...
            for (y, q) in parsed:
                ...
                allowed_q_for_diff = (
                    set(available_q_keys)
                    | _derived_keys_for(bucket, scope="quarterly")
                    | _allowed_meta_fields("quarterly")
                )
                changes = _diff_dict(before, new_row, allowed=allowed_q_for_diff)
            ...
            with tabs[0]:
                ...
                if submit_a:
                    ...
                    for _, er in keep.iterrows():
                        ...
                        # Recompute banking-derived fields too (Annual)
                        compute_banking_derivatives_annual(
                            row_up,
                            df=df,
                            stock_name=name,
                            year=y,
                            available_keys=allowed_a_keys,
                        )
                        ...
                        allowed = (
                            set(allowed_a_keys)
                            | _derived_keys_for(bucket_default_this, "annual")
                            | _allowed_meta_fields("annual")
                        )
            ...
            with tabs[1]:
                ...
                if submit_q:
                    ...
                    for _, r in valid.iterrows():
                        ...
                        allowed = (
                            set(allowed_q_keys)
                            | _derived_keys_for(bucket_default_this, "quarterly")
                            | _allowed_meta_fields("quarterly")
                        )
