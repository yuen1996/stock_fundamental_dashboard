    ...
    for name in sorted(all_rows["Name"].dropna().unique()):
        ...
        with st.expander(name, expanded=False):
            ...
            with tabs[0]:
                ...
                # Show editor (annual)
                annual_col_order = ["RowKey", "Delete", "Name", "Industry", "Year"] + [c for c in av.columns if c not in ["RowKey", "Delete", "Name", "Industry", "Year"]]
            ...
            with tabs[1]:
                ...
                if qv.empty:
                    ...
                    # make sure Quarter is a clean string in options (or placeholder)
                ...
                # We don't need to show IsQuarter in the editor
                qv_display = qv.drop(columns=[c for c in ["IsQuarter"] if c in qv.columns])

                # Ensure key columns appear first
                quarter_col_order = ["RowKey", "Delete", "Name", "Industry", "Year", "Quarter"] + \
                                    [c for c in qv_display.columns if c not in ["RowKey", "Delete", "Name", "Industry", "Year", "Quarter"]]
                ...
                edited_q = st.data_editor(
                    ...
                    hide_index=True,
                    num_rows="dynamic",  # allow + Add row
                    column_order=quarter_col_order,
                    column_config={
                        "RowKey": st.column_config.TextColumn("RowKey", help="Internal key", disabled=True, width="small"),
