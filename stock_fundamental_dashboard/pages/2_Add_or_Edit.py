        ...
        with st.expander(name, expanded=False):
            ...
            with tabs[0]:
                ...
                if av.empty:
                    ...
                    # Insert RowKey/Delete as scalars (length 0 ok)
                    av.insert(0, "RowKey", "")
            ...
            with tabs[1]:
                ...
                if qv.empty:
                    ...
                    qv.loc[:, "Quarter"] = ""   # will pick from selectbox
                ...
                if st.button(f"💾 Save Quarterly for {name}", key=f"qeb_save_q_{name}"):
                    ...
                    # upserts
                    for _, er in keep.iterrows():
                        if pd.isna(er.get("Year")) or pd.isna(er.get("Quarter")) or not er.get("Quarter"):
                            continue
                        y = int(er["Year"])
                        qt = str(er["Quarter"])
                        if not er.get("RowKey"):  # new row
