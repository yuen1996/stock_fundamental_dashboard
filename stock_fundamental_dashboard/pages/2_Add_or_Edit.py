...
with tabs[1]:
    ...
    # keep latest edits in state (recompute RowKey & autofill on each rerun)
    ...
    if st.button(f"💾 Save Quarterly for {name}", key=f"qeb_save_q_{name}"):
        ...
        if buf is None:
            ...
            # rows the user wants to delete (only if Year+Quarter valid)
            valid_mask = buf["Year"].notna() & buf["Quarter"].isin(["Q1", "Q2", "Q3", "Q4"])
            to_delete = buf[(buf.get("Delete", False) == True) & valid_mask]
            to_upsert = buf[(buf.get("Delete", False) != True) & valid_mask]
