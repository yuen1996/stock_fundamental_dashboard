...
def _load_ohlc_for_name(stock_name: str, _etag: int, *, ticker_hint: str | None = None) -> pd.DataFrame | None:
    ...
    if not path:
        ...
        if not os.path.exists(path):
            # last fallback: sanitized
            safe = re.sub(r"[^0-9A-Za-z]+", "_", str(stock_name)).strip("_")
...
def _momentum_close_before_today(stock_name: str, stock_rows: pd.DataFrame) -> tuple[float | None, pd.Timestamp | None]:
    ...
    return (px if (px is not None and math.isfinite(px)) else None), pd.to_datetime(last["Date"])   
