...
if all_rows.empty:
    ...
    for name in sorted(all_rows["Name"].dropna().unique()):
       ...
       .copy()
            ...
            if submit_q:
                # persist buffer (drop UI-only fields)
                ...
                if buf_save is None or buf_save.empty:
                    st.info("Nothing to save.")
                else:
                    valid = buf_save[(buf_save["Year"].notna()) & (buf_save["Quarter"].isin(["Q1","Q2","Q3","Q4"]))]

                    # (Optional) if you later add real delete tracking to the buffer, handle it here
                    to_up = valid
