...
# --- FIX: clear stale price widget state when switching source ---
_prev = st.session_state.get("__prev_price_source_radio")
if _prev != src_choice:
    # let the next widget's value= take effect (Momentum or Manual)
    st.session_state.pop("cur_price_stock_num", None)
    st.session_state.pop("cur_price_stock_num_display", None)  # safe to clear as well
...
if use_manual:
    ...
    st.number_input(
        ...
        key="cur_price_stock_num",
    )
    st.caption("Source: Add/Edit (manual)")
else:
    # Momentum mode: show read-only text field with a DIFFERENT key
    if mom_px is None:
        st.text_input(
            "Current Price",
            value="",
            placeholder="—",
            key="cur_price_stock_num_display",  # <-- different key from manual
            disabled=True,
        )
        st.caption("Source: Momentum (last close) • No OHLC found — update on Momentum page")
        # Optional: more detail about where we looked
        st.info(
...
if save_settings_clicked:
    if not stock_name:
        ...
        _entered = st.session_state.get("cur_price_stock_num", None)
