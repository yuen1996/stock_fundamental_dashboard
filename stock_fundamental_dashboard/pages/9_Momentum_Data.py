...
def _parse_time_series(s: pd.Series) -> pd.Series:
    ...
    try:
        ...
        pass
    # Try dayfirst (TradingView often DD/MM/YYYY)
...
def _normalize_csv_to_ohlc(raw: pd.DataFrame) -> tuple[pd.DataFrame, dict]:
    ...
    Returns (normalized_df, diag)
      normalized_df columns: Date, Open, High, Low, Close[, Volume]
      diag: diagnostics dict for UX
    """
    raw_rows = len(raw)
        ...
        with st.expander(name, expanded=False):
            ...
            if up is not None:
                try:
                    ...
                    # Refresh so metrics update immediately
                    try:
                        st.rerun()
                    except Exception:
                        st.experimental_rerun()
