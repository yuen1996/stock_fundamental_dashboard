...
# --- Canonical OHLC write location (now user-writable by default) ---
...
# Force a safe default that always works (home folder).
# If you *want* to override later, export SFD_OHLC_DIR / OHLC_DIR / ST_OHLC_DIR in the service env.
os.environ.setdefault("SFD_OHLC_DIR", _alt_ohlc_dir())
os.environ.setdefault("OHLC_DIR", os.environ["SFD_OHLC_DIR"])

def _resolve_ohlc_dir() -> str:
    """
    Prefer ENV overrides first, then project paths, then home fallback.
    Recognizes the same env names used by utils/ohlc_resolver.py.
    """
    for env_name in ("SFD_OHLC_DIR", "OHLC_DIR", "ST_OHLC_DIR"):
    ...
    candidates = [
        os.path.join(_PARENT, "data", "ohlc"),      # <project>/data/ohlc
        ...
        _alt_ohlc_dir(),                            # home fallback (always exists)
    ]
    for d in candidates:
        try:
            if os.path.isdir(d):
...
def _prefs_paths() -> list[str]:
    """
    We keep both locations IN SYNC to avoid 'saved but not seen' on rerun.
    """
    paths: list[str] = []
    # 1) Home
    try:
        p = os.path.join(_home_state_dir(), "momentum_yf_prefs.json")
        paths.append(p)
    except Exception:
        pass
    # 2) Project-local (near your CSVs): <project>/data/momentum_yf_prefs.json
    try:
        proj_dir = os.path.abspath(os.path.join(OHLC_DIR, ".."))  # parent of data/ohlc
...
def _save_prefs(prefs: dict) -> str | None:
    """
    Save to BOTH locations so the next rerun reads the same mapping:
      1) ~/.app_state/momentum_yf_prefs.json
      2) <parent_of_OHLC_DIR>/momentum_yf_prefs.json
    """
    paths = [
        os.path.join(_home_state_dir(), "momentum_yf_prefs.json"),
        os.path.join(os.path.dirname(OHLC_DIR), "momentum_yf_prefs.json"),
    ]
    last_used = None
    for p in paths:
        try:
            ...
            continue
    if last_used:
        globals()["_ACTIVE_PREFS_PATH"] = last_used
    return last_used
...
def _save_ohlc(name: str, df: pd.DataFrame) -> str:
    ...
    and switch the session to use that dir (so subsequent reads work).
    """
    global OHLC_DIR
    ...
    try:
        ...
        OHLC_DIR = alt_dir  # switch for this session so following ops stay consistent
        alt = os.path.join(alt_dir, f"{_safe_name(name)}.csv")
        try:
...
def _fetch_yahoo_ohlc(ticker: str, start_d: date, end_d: date) -> pd.DataFrame:
    ...
    df = None
    # Try download() first (often more robust)
    try:
        df = yf.download(
            sym,
        ...
        with col_y5:
            ...
            if st.button("💾 Save default", key=unique_key("yf_save_default", name), use_container_width=True):
                ...
                if not t:
                    ...
                    # refresh cached prefs so prefill updates immediately on this render
                    prefs.update(_load_prefs())
                    if used_path:
                        st.success(f"Saved: {name} → {t}\nPrefs synced at: {used_path}")
                    else:
                        st.error("Failed to save preferences to any location.")
