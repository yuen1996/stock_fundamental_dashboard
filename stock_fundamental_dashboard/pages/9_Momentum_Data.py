...
# New: dual-location prefs with fallback and visible errors

_ACTIVE_PREFS_PATH = None  # for UI/debug

def _prefs_paths() -> list[str]:
    paths: list[str] = []
    # 1) Home (original location)
    try:
...
def _load_prefs() -> dict:
    ...
    for p in _prefs_paths():
        try:
            ...
            continue
    # if nothing exists yet, remember a first choice path for UI display
    paths = _prefs_paths()
