...
def _fetch_yahoo_ohlc(ticker: str, start_d: date, end_d: date) -> pd.DataFrame:
    """
    Fetch daily OHLC from Yahoo Finance via yfinance and normalize columns.
    ...
    df = yf.download(
        ...
        end=(end_d + timedelta(days=1)).isoformat(),  # inclusive end-date
        interval="1d",
        progress=False,
        auto_adjust=False,
        threads=True,
    ...
    for c in ["Open","High","Low","Close","Volume"]:
        if c in df.columns:
            df[c] = pd.to_numeric(df[c], errors="coerce")
    df = df[~(df["Date"].isna() | df["Close"].isna())]
