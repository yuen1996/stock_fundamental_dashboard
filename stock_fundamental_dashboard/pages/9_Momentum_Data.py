...
# Canonical OHLC write location shared across all pages
# (Dashboard, Add/Edit, View Stock, etc. all read from this.)
OHLC_DIR = str(default_ohlc_dir())
...
def _load_ohlc_with_path(name: str) -> tuple[pd.DataFrame | None, str | None]:
    ...
    try:
        ...
        if dfp is not None and not dfp.empty:
            return dfp.reset_index(drop=True), str(path)
    except Exception:
        pass
