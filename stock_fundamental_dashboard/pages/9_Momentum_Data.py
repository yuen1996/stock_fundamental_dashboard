...
OHLC_DIR = os.path.join(_PARENT, "data", "ohlc")  # unify with other pages

_VERSION_FILE = os.path.join(_GRANDP, ".data.version")  # (keep if you want; not used)
def _bump_data_etag():
    # publish “ohlc changed” so other pages invalidate immediately
    try:
        bus.bump("ohlc")
    except Exception:
        pass
