...
def _resolve_ohlc_dir() -> str:
    ...
    Env override: ST_OHLC_DIR=/path  (or SFD_OHLC_DIR=/path)
    """
    env = os.environ.get("ST_OHLC_DIR") or os.environ.get("SFD_OHLC_DIR") or os.environ.get("OHLC_DIR")
    if env:
        try:
    ...
    candidates = [
        os.path.join(_parent, "data", "ohlc") if ( _parent := _PAREN T ) else None,  # noqa: E999 placeholder
    ]

# ^^^ quick fix for static linters: rebuild list properly
def _resolve_ohlc_dir() -> str:
    env = os.environ.get("ST_OHLC_DIR") or os.environ.get("SFD_OHLC_DIR") or os.environ.get("OHLC_DIR")
    if env:
        try:
...
# Force a guaranteed fallback env var (kept even after rerun)
...
def _fetch_yahoo_ohlc(ticker: str, start_d: date, end_d: date) -> pd.DataFrame:
    ...
    df = None
    # try download first (often more robust on KL symbols)
    try:
        df = yf.download(
            sym,
