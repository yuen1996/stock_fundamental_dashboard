...
from utils.ohlc_resolver import load_ohlc, unique_key, default_ohlc_dir, read_ohlc_csv
...
def _load_ohlc_with_path(name: str) -> tuple[pd.DataFrame | None, str | None]:
    """
    Prefer the per-name canonical path FIRST to avoid name collisions
    (e.g., SLP vs SKP). Only fall back to the resolver if there is no
    canonical file yet.
    """
    # 1) Canonical path per UI name
    p = _ohlc_path_canonical(name)
    if os.path.exists(p):
        try:
            df = read_ohlc_csv(Path(p))
            if not df.empty:
                return df, p
        except Exception:
    ...
    # 2) Resolver fallback (legacy file names, code-based files, etc.)
    try:
