...
def _save_prefs(prefs: dict) -> str | None:
    """
    IMPORTANT: write to ALL valid paths so whichever one is loaded next run, it matches.
    Return the first successful path (for display).
    """
    first_ok = None
    for p in _prefs_paths():
        try:
...
def _fetch_yahoo_ohlc(ticker: str, start_d: date, end_d: date) -> pd.DataFrame:
    ...
    df = None
    # try download first (more robust on some symbols)
    try:
        df = yf.download(
            sym,
        ...
        _YF_LAST_ERR = f"download({sym}) failed: {e}\n{traceback.format_exc(limit=1)}"
    ...
    if df is None or df.empty:
        try:
            ...
            _YF_LAST_ERR = f"history({sym}) failed: {e}\n{traceback.format_exc(limit=1)}"
            df = None
