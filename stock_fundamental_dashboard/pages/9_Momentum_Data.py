...
# Dual-location prefs with fallback and visible path
...
def _save_prefs(prefs: dict) -> str | None:
    """Try both locations; return the path used or None if all failed."""
    for p in _prefs_paths():
        try:
...
def _save_ohlc(name: str, df: pd.DataFrame) -> str:
    ...
    Use atomic replace so partial writes never leave a broken file.
    """
    _ensure_dir()
...
# Page content
# ------------------------------------------------------------------
# Writeability banner (helps when the 10th stock fails silently)
...
with st.expander("📂 Storage locations & health", expanded=True):
    ...
    st.caption(f"Saved tickers count: {len((prefs_now or {}).get('map', {}))}")
...
for name in stocks:
    with st.expander(name, expanded=False):
        ...
        # --- Quick write test (helps when the 10th stock refuses to save) ---
        ...
        # Small convenience: if SKP appears, prefill 7155.KL when there is no saved
            ...
            if st.button("💾 Save default", key=unique_key("yf_save_default", name), use_container_width=True):
                ...
                if not t:
                    ...
                    if used_path:
                        ...
                        st.error(
                            "Failed to save preferences (no writeable location). "
                            "Run the app with write permissions or create "
                            "`data/momentum_yf_prefs.json` manually."
                        )
